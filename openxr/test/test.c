...
static XrPosef identity_pose = {.orientation = {.x = 0.f, .y = 0.f, .z = 0.f, .w = 1.f}, .position = {.x = 0, .y = 0, .z = 0}};
...
int main(int argc, char** argv) {
    ...
    *app                         = (struct ApplicationState) {
    ...
    // --- Create XrSystem
    XrSystemGetInfo system_get_info = {.type = XR_TYPE_SYSTEM_GET_INFO, .formFactor = app->oxr.form_factor, .next = NULL};
    ...
    // support
    {
        ...
        struct hand_tracking_t* ht_ext = (struct hand_tracking_t*) get_ext(app, XR_EXT_HAND_TRACKING_EXTENSION_NAME);

        XrSystemHandTrackingPropertiesEXT ht = {
            .type = XR_TYPE_SYSTEM_HAND_TRACKING_PROPERTIES_EXT, .next = system_props.next, .supportsHandTracking = 1};
    ...
    result = xrEnumerateViewConfigurationViews(app->oxr.instance, app->oxr.system_id, app->oxr.view_type, 0,
                                               &app->oxr.view_count, NULL);
    ...
    result = xrEnumerateViewConfigurationViews(app->oxr.instance, app->oxr.system_id, app->oxr.view_type, app->oxr.view_count,
    ...
    result = opengl_ext->xrGetOpenGLGraphicsRequirementsKHR(app->oxr.instance, app->oxr.system_id, &opengl_reqs);
    ...
    result =
        xrEnumerateEnvironmentBlendModes(app->oxr.instance, app->oxr.system_id, app->oxr.view_type, 0, &blend_mode_count, NULL);
    ...
    result = xrEnumerateEnvironmentBlendModes(app->oxr.instance, app->oxr.system_id, app->oxr.view_type, blend_mode_count,
    ...
    if (!init_sdl_window(&graphics_binding_gl.xDisplay, &graphics_binding_gl.glxDrawable, &graphics_binding_gl.glxContext,
    ...
    XrSessionCreateInfo session_create_info = {
        .type = XR_TYPE_SESSION_CREATE_INFO, .next = &graphics_binding_gl, .systemId = app->oxr.system_id};
    ...
    XrReferenceSpaceCreateInfo play_space_create_info = {.type                 = XR_TYPE_REFERENCE_SPACE_CREATE_INFO,
                                                         .next                 = NULL,
                                                         .referenceSpaceType   = app->oxr.play_space_type,
    ...
    result = xrEnumerateSwapchainFormats(app->oxr.session, swapchain_format_count, &swapchain_format_count, swapchain_formats);
    ...
    int64_t color_format = get_swapchain_format(app->oxr.instance, app->oxr.session, GL_SRGB8_ALPHA8_EXT, true);

    int64_t quad_format = get_swapchain_format(app->oxr.instance, app->oxr.session, GL_RGBA8_EXT, true);

    XrSwapchainUsageFlags color_flags = XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
    if (!create_swapchain_from_views(app->oxr.instance, app->oxr.session, &app->vr_swapchains[SWAPCHAIN_PROJECTION],
                                     app->oxr.view_count, color_format, app->oxr.viewconfig_views, color_flags))
        return 1;

    if (!create_one_swapchain(app->oxr.instance, app->oxr.session, &app->quad_layer.swapchain, quad_format, 1,
                              app->quad_layer.pixel_width, app->quad_layer.pixel_height, color_flags))
        return 1;

    // Do not allocate these every frame to save some resources
    app->oxr.views = (XrView*) malloc(sizeof(XrView) * app->oxr.view_count);
    app->oxr.projection_views =
        (XrCompositionLayerProjectionView*) malloc(sizeof(XrCompositionLayerProjectionView) * app->oxr.view_count);
    for (uint32_t i = 0; i < app->oxr.view_count; i++) {
        ...
        app->oxr.projection_views[i].subImage.swapchain          = app->vr_swapchains[SWAPCHAIN_PROJECTION].swapchains[i];
    ...
    struct refresh_rate_t* refresh_rate_ext = (struct refresh_rate_t*) get_ext(app, XR_FB_DISPLAY_REFRESH_RATE_EXTENSION_NAME);
    // get info from fb_refresh_rate
    if (refresh_rate_ext->base.supported) {
        uint32_t refresh_rate_count;
        result = refresh_rate_ext->xrEnumerateDisplayRefreshRatesFB(app->oxr.session, 0, &refresh_rate_count, NULL);
        ...
        if (refresh_rate_count > 0) {
            ...
            result               = refresh_rate_ext->xrEnumerateDisplayRefreshRatesFB(app->oxr.session, refresh_rate_count,
                                                                                      &refresh_rate_count, refresh_rates);
            ...
            result = refresh_rate_ext->xrRequestDisplayRefreshRateFB(app->oxr.session, refresh_rates[refresh_rate_count - 1]);
    ...
    struct hand_tracking_t* hand_tracking_ext = (struct hand_tracking_t*) get_ext(app, XR_EXT_HAND_TRACKING_EXTENSION_NAME);
    ...
    if (init_gl(app->oxr.view_count, app->vr_swapchains[SWAPCHAIN_PROJECTION].swapchain_lengths, &app->gl_renderer) != 0) {
    ...
    while (!quit_renderloop && frame_count < 300) {
                ...
                case XR_SESSION_STATE_READY: {
                    ...
                    if (!session_running) {
                        XrSessionBeginInfo session_begin_info = {.type                         = XR_TYPE_SESSION_BEGIN_INFO,
                                                                 .next                         = NULL,
                                                                 .primaryViewConfigurationType = app->oxr.view_type};
        ...
        for (int i = 0; i < HAND_COUNT; i++) {
            if (hand_tracking_ext->system_supported) {
                get_hand_tracking(app->oxr.instance, app->oxr.play_space, app->oxr.frameState.predictedDisplayTime, false,
                                  hand_tracking_ext, i);
            }
        }
        frame_count++;

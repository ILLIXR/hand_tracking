...
void ILLIXR::hand_tracking_publisher::_p_one_iteration() {
    ...
    detections_.emplace(out_type, data_format::ht::ht_detection{end_time - start_time, output_frame.left_palm, output_frame.right_palm,
    ...
    if (detections_.size() == frame_count_) {
        ...
        if (current_raw_.pose_valid) {
            if (current_raw_.eye_count == 1) {
                ...
                current_pose_ = static_cast<data_format::pose_data>(current_raw_.poses.at(data_format::units::non_primary(current_raw_.primary)));
            }
        } else {
        ...
        auto current_position = data_format::ht::position(hp, current_pose_.unit, std::chrono::system_clock::now().time_since_epoch().count());
        ...
        if (last_position_.valid) {
            velocity[data_format::ht::LEFT_HAND] = data_format::ht::velocity(current_position.points[data_format::ht::LEFT_HAND],
                                                                             last_position_.points[data_format::ht::LEFT_HAND],
            ...
            velocity[data_format::ht::RIGHT_HAND] = data_format::ht::velocity(current_position.points[data_format::ht::RIGHT_HAND],
                                                                              last_position_.points[data_format::ht::RIGHT_HAND],
                                                                              static_cast<float>(current_position.time - last_position_.time) * NANO);
        }
...
void ILLIXR::hand_tracking_publisher::calculate_proper_position(std::map<data_format::ht::hand, data_format::ht::hand_points>& hp) {
    ...
    for (auto& item : detections_)
        data_format::denormalize(item.second, static_cast<float>(img_size_x_), static_cast<float>(img_size_y_), data_format::units::PIXEL);

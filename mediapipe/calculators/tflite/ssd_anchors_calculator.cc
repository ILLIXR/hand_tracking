...
namespace {
    ...
    float CalculateScale(float min_scale, float max_scale, int stride_index, int num_strides) {
        if (num_strides == 1) {
            ...
            return min_scale + (max_scale - min_scale) * (float) stride_index / ((float) num_strides - 1.0f);
        }
    }
    ...
    FeatureMapDim GetFeatureMapDimensions(const SsdAnchorsCalculatorOptions& options, int index) {
        ...
        if (options.feature_map_height_size()) {
            ...
            feature_map_dims.height = std::ceil((float) options.input_size_height() / (float) stride);
            feature_map_dims.width  = std::ceil((float) options.input_size_width() / (float) stride);
        }
    ...
    std::pair<float, float> GetMultiScaleAnchorOffset(const SsdAnchorsCalculatorOptions& options, const float stride,
                                                      const int level) {
        std::pair<float, float> result(0., 0.);
        int                     denominator = (int) std::pow(2, level);
        if (options.input_size_height() % denominator == 0 || options.input_size_height() == 1) {
            result.first = stride / 2.0f;
        }
        if (options.input_size_width() % denominator == 0 || options.input_size_width() == 1) {
    ...
    Anchor CalculateAnchorBox(const int y_center, const int x_center, const float scale, const float aspect_ratio,
                              const std::pair<float, float> base_anchor_size,
                              // y-height first
                              const std::pair<float, float> anchor_stride, const std::pair<float, float> anchor_offset) {
        Anchor result;
...
public:
    ...
    absl::Status Open(CalculatorContext* cc) override {
        ...
        const SsdAnchorsCalculatorOptions& options = cc->Options<SsdAnchorsCalculatorOptions>();
        ...
        if (!options.fixed_anchors().empty()) {
            ...
            if (options.has_input_size_height() || options.has_input_size_width() || options.has_min_scale() ||
                options.has_max_scale() || options.has_num_layers() || options.multiscale_anchor_generation()) {
                return absl::InvalidArgumentError("Fixed anchors are provided, but fields are set for generating "
                                                  "anchors. When fixed anchors are set, fields for generating "
                                                  "anchors must not be set.");
            }
            anchors->assign(options.fixed_anchors().begin(), options.fixed_anchors().end());
...
private:
    static absl::Status GenerateAnchors(std::vector<Anchor>* anchors, const SsdAnchorsCalculatorOptions& options);

    static absl::Status GenerateMultiScaleAnchors(std::vector<Anchor>* anchors, const SsdAnchorsCalculatorOptions& options);
};
...
absl::Status SsdAnchorsCalculator::GenerateMultiScaleAnchors(std::vector<Anchor>*               anchors,
    ...
    for (int i = options.min_level(); i <= options.max_level(); ++i) {
        ...
        for (int i = 0; i < options.scales_per_octave(); ++i) {
            current_anchor_info.scales.push_back((float) std::pow(2.0, (double) i / (double) options.scales_per_octave()));
        }

        // anchor stride
        float anchor_stride               = (float) std::pow(2.0, i);
        current_anchor_info.anchor_stride = std::make_pair(anchor_stride, anchor_stride);

        // base_anchor_size
        current_anchor_info.base_anchor_size =
            std::make_pair(anchor_stride * options.anchor_scale(), anchor_stride * options.anchor_scale());
                ...
                for (unsigned int j = 0; j < anchor_infos[i].aspect_ratios.size(); ++j) {
                    for (unsigned int k = 0; k < anchor_infos[i].scales.size(); ++k) {
                        Anchor anchor = CalculateAnchorBox(
                            /*y_center=*/y, /*x_center=*/x, anchor_infos[i].scales[k], anchor_infos[i].aspect_ratios[j],
                            anchor_infos[i].base_anchor_size,
                            /*anchor_stride=*/anchor_infos[i].anchor_stride,
                            /*anchor_offset=*/
                            GetMultiScaleAnchorOffset(options, anchor_infos[i].anchor_stride.first, anchor_infos[i].level));
                        if (options.normalize_coordinates()) {
                            NormalizeAnchor(options.input_size_height(), options.input_size_width(), &anchor);
                        }
...
absl::Status SsdAnchorsCalculator::GenerateAnchors(std::vector<Anchor>* anchors, const SsdAnchorsCalculatorOptions& options) {
    // Verify the options.
    if (!options.feature_map_height_size() && !options.strides_size()) {
        return absl::InvalidArgumentError("Both feature map shape and strides are missing. Must provide either "
                                          "one.");
    }
    ...
    if (options.feature_map_height_size()) {
        ...
        ABSL_CHECK_EQ(options.feature_map_height_size(), options.feature_map_width_size());
    } else {
    ...
    while (layer_id < options.num_layers()) {
        ...
        while (last_same_stride_layer < options.strides_size() &&
            ...
            const float scale =
                CalculateScale(options.min_scale(), options.max_scale(), last_same_stride_layer, options.strides_size());
            if (last_same_stride_layer == 0 && options.reduce_boxes_in_lowest_layer()) {
                ...
                for (int aspect_ratio_id = 0; aspect_ratio_id < options.aspect_ratios_size(); ++aspect_ratio_id) {
                    aspect_ratios.push_back(options.aspect_ratios(aspect_ratio_id));
                    scales.push_back(scale);
                }
                if (options.interpolated_scale_aspect_ratio() > 0.0) {
                    const float scale_next = last_same_stride_layer == options.strides_size() - 1
                        ? 1.0f
                        : CalculateScale(options.min_scale(), options.max_scale(), last_same_stride_layer + 1,
        ...
        if (options.feature_map_height_size()) {
            ...
            feature_map_height = std::ceil((float) options.input_size_height() / (float) stride);
        ...
        for (int y = 0; y < feature_map_height; ++y) {
            for (int x = 0; x < feature_map_width; ++x) {
                for (int anchor_id = 0; anchor_id < (int) anchor_height.size(); ++anchor_id) {
                    ...
                    const float x_center = ((float) x + options.anchor_offset_x()) / (float) feature_map_width;
                    const float y_center = ((float) y + options.anchor_offset_y()) / (float) feature_map_height;

...
public:
    using TfLiteModelPtr       = std::unique_ptr<tflite::FlatBufferModel, std::function<void(tflite::FlatBufferModel*)>>;
    ...
    static absl::Status GetContract(CalculatorContract* cc) {
        ...
        if (cc->InputSidePackets().HasTag(kModelFDTag)) {
            cc->InputSidePackets().Tag(kModelFDTag).Set<std::tuple<int, size_t, size_t>>();
        }

        if (cc->InputSidePackets().HasTag(kModelSpanTag)) {
            cc->InputSidePackets().Tag(kModelSpanTag).Set<absl::Span<const uint8_t>>();
        }

        RET_CHECK(cc->OutputSidePackets().HasTag(kModelTag) ^ cc->OutputSidePackets().HasTag(kSharedModelTag));
    ...
    absl::Status Open(CalculatorContext* cc) override {
        ...
        if (cc->InputSidePackets().HasTag(kModelBlobTag)) {
            ...
            model                         = tflite::FlatBufferModel::BuildFromBuffer(model_blob.data(), model_blob.size());
        }

        if (cc->InputSidePackets().HasTag(kModelSpanTag)) {
            model_packet                                = cc->InputSidePackets().Tag(kModelSpanTag);
            const absl::Span<const uint8_t>& model_view = model_packet.Get<absl::Span<const uint8_t>>();
            model =
                tflite::FlatBufferModel::BuildFromBuffer(reinterpret_cast<const char*>(model_view.data()), model_view.size());
        }

        if (cc->InputSidePackets().HasTag(kModelFDTag)) {
#if defined(ABSL_HAVE_MMAP) && !TFLITE_WITH_STABLE_ABI
            model_packet                 = cc->InputSidePackets().Tag(kModelFDTag);
            const auto& model_fd         = model_packet.Get<std::tuple<int, size_t, size_t>>();
            auto        model_allocation = std::make_unique<tflite::MMAPAllocation>(
                std::get<0>(model_fd), std::get<1>(model_fd), std::get<2>(model_fd), tflite::DefaultErrorReporter());
            model = tflite::FlatBufferModel::BuildFromAllocation(std::move(model_allocation), tflite::DefaultErrorReporter());
#else
            return absl::FailedPreconditionError("Loading by file descriptor is not supported on this platform.");
#endif
        }

        RET_CHECK(model) << "Failed to load TfLite model.";

        TfLiteModelPtr output_model = TfLiteModelPtr(model.release(), [model_packet](tflite::FlatBufferModel* model) {
        ...
        if (cc->OutputSidePackets().HasTag(kModelTag)) {
            cc->OutputSidePackets().Tag(kModelTag).Set(MakePacket<TfLiteModelPtr>(std::move(output_model)));
        } else {
            cc->OutputSidePackets().Tag(kSharedModelTag).Set(MakePacket<SharedTfLiteModelPtr>(std::move(output_model)));
        }

...
namespace api2 {
    ...
    namespace {
        ...
        void ConvertRawValuesToAnchors(const float* raw_anchors, int num_boxes, std::vector<Anchor>* anchors) {
            anchors->clear();
        ...
        void ConvertAnchorsToRawValues(const std::vector<Anchor>& anchors, int num_boxes, float* raw_anchors) {
        ...
        absl::Status CheckCustomTensorMapping(const TensorsToDetectionsCalculatorOptions::TensorMapping& tensor_mapping) {
            RET_CHECK(tensor_mapping.has_detections_tensor_index() && tensor_mapping.has_scores_tensor_index());
            int bitmap = 0;
            bitmap |= 1 << tensor_mapping.detections_tensor_index();
            bitmap |= 1 << tensor_mapping.scores_tensor_index();
            if (!tensor_mapping.has_num_detections_tensor_index() && !tensor_mapping.has_classes_tensor_index() &&
    ...
    public:
        ...
        static constexpr SideInput<std::vector<Anchor>>::Optional kInAnchors{"ANCHORS"};
        static constexpr SideInput<std::vector<int>>::Optional    kSideInIgnoreClasses{"IGNORE_CLASSES"};
    ...
    private:
        absl::Status ProcessCPU(CalculatorContext* cc, std::vector<Detection>* output_detections);
        absl::Status ProcessGPU(CalculatorContext* cc, std::vector<Detection>* output_detections);

        absl::Status LoadOptions(CalculatorContext* cc);
        absl::Status GpuInit(CalculatorContext* cc);
        absl::Status DecodeBoxes(const float* raw_boxes, const std::vector<Anchor>& anchors, std::vector<float>* boxes);
        absl::Status ConvertToDetections(const float* detection_boxes, const float* detection_scores,
                                         const int* detection_classes, std::vector<Detection>* output_detections);
        Detection    ConvertToDetection(float box_ymin, float box_xmin, float box_ymax, float box_xmax,
                                        absl::Span<const float> scores, absl::Span<const int> class_ids, bool flip_vertically);
    ...
    absl::Status TensorsToDetectionsCalculator::UpdateContract(CalculatorContract* cc) {
        UNUSED(cc);
        if (CanUseGpu()) {
#ifndef MEDIAPIPE_DISABLE_GL_COMPUTE
            MP_RETURN_IF_ERROR(mediapipe::GlCalculatorHelper::UpdateContract(cc, /*request_gpu_as_optional=*/true));
#elif MEDIAPIPE_METAL_ENABLED
    ...
    absl::Status TensorsToDetectionsCalculator::Process(CalculatorContext* cc) {
        ...
        if (!scores_tensor_index_is_set_) {
            if (num_input_tensors == 2 || num_input_tensors == kNumInputTensorsWithAnchors) {
    ...
    absl::Status TensorsToDetectionsCalculator::ProcessCPU(CalculatorContext* cc, std::vector<Detection>* output_detections) {
        const auto& input_tensors = *kInTensors(cc);

        if (input_tensors.size() == 2 || input_tensors.size() == kNumInputTensorsWithAnchors) {
            ...
            auto raw_box_tensor = &input_tensors[tensor_mapping_.detections_tensor_index()];
            ...
            if (raw_box_tensor->shape().dims.size() == 3) {
                ...
                return absl::InvalidArgumentError("The dimensions of box Tensor must be 3 or 4.");
            }
            auto raw_score_tensor = &input_tensors[tensor_mapping_.scores_tensor_index()];
            if (raw_score_tensor->shape().dims.size() == 3) {
                ...
                return absl::InvalidArgumentError("The dimensions of score Tensor must be 3 or 4.");
            }
            ...
            if (!anchors_init_) {
                if (input_tensors.size() == kNumInputTensorsWithAnchors) {
                    auto anchor_tensor = &input_tensors[tensor_mapping_.anchors_tensor_index()];
                    ...
                    if (IsClassIndexAllowed(score_idx)) {
                        ...
                        if (options_.sigmoid_score()) {
                            if (options_.has_score_clipping_thresh()) {
                                score = score < -options_.score_clipping_thresh() ? -options_.score_clipping_thresh() : score;
                                score = score > options_.score_clipping_thresh() ? options_.score_clipping_thresh() : score;
                            }
            ...
            auto num_boxes_tensor = &input_tensors[tensor_mapping_.num_detections_tensor_index()];
            RET_CHECK_EQ(num_boxes_tensor->shape().dims.size(), 1);
            RET_CHECK_EQ(num_boxes_tensor->shape().dims[0], 1);

            auto detection_boxes_tensor = &input_tensors[tensor_mapping_.detections_tensor_index()];
            ...
            auto detection_classes_tensor = &input_tensors[tensor_mapping_.classes_tensor_index()];
            ...
            auto detection_scores_tensor = &input_tensors[tensor_mapping_.scores_tensor_index()];
            ...
            MP_RETURN_IF_ERROR(
                ConvertToDetections(detection_boxes, detection_scores, detection_classes.data(), output_detections));
        }
        return absl::OkStatus();
    }

    absl::Status TensorsToDetectionsCalculator::ProcessGPU(CalculatorContext* cc, std::vector<Detection>* output_detections) {
        ...
        MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([this, &input_tensors, &cc, &output_detections]() -> absl::Status {
            if (!anchors_init_) {
                if (input_tensors.size() == kNumInputTensorsWithAnchors) {
                    auto read_view = input_tensors[tensor_mapping_.anchors_tensor_index()].GetOpenGlBufferReadView();
            ...
            // the reading buffers' views.
            {
                ...
                auto decoded_boxes_view = decoded_boxes_buffer_->GetOpenGlBufferWriteView();
                glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, decoded_boxes_view.name());
                auto input0_view = input_tensors[tensor_mapping_.detections_tensor_index()].GetOpenGlBufferReadView();
                ...
                auto input1_view = input_tensors[tensor_mapping_.scores_tensor_index()].GetOpenGlBufferReadView();
        ...
        MP_RETURN_IF_ERROR(ConvertToDetections(boxes, detection_scores.data(), detection_classes.data(), output_detections));
#elif MEDIAPIPE_METAL_ENABLED
        if (!anchors_init_) {
            if (input_tensors.size() == kNumInputTensorsWithAnchors) {
                ...
                auto src_buffer =
                    MtlBufferView::GetReadView(input_tensors[tensor_mapping_.anchors_tensor_index()], command_buffer);
                auto                      dest_buffer  = MtlBufferView::GetWriteView(*raw_anchors_buffer_, command_buffer);
                ...
                [blit_command copyFromBuffer:src_buffer.buffer()
                           ...
                           destinationOffset:0
                                        size:input_tensors[tensor_mapping_.anchors_tensor_index()].bytes()];
                [blit_command endEncoding];
                [command_buffer commit];
            } else if (!kInAnchors(cc).IsEmpty()) {
                const auto& anchors          = *kInAnchors(cc);
                auto        raw_anchors_view = raw_anchors_buffer_->GetCpuWriteView();
                ConvertAnchorsToRawValues(anchors, num_boxes_, raw_anchors_view.buffer<float>());
            } else {
                return absl::UnavailableError("No anchor data available.");
            }
            anchors_init_ = true;
        ...
        {
            auto scored_boxes_view  = MtlBufferView::GetWriteView(*scored_boxes_buffer_, command_buffer);
            auto decoded_boxes_view = MtlBufferView::GetWriteView(*decoded_boxes_buffer_, command_buffer);
            [command_encoder setBuffer:decoded_boxes_view.buffer() offset:0 atIndex:0];
            auto input0_view =
                MtlBufferView::GetReadView(input_tensors[tensor_mapping_.detections_tensor_index()], command_buffer);
            [command_encoder setBuffer:input0_view.buffer() offset:0 atIndex:1];
            auto raw_anchors_view = MtlBufferView::GetReadView(*raw_anchors_buffer_, command_buffer);
            ...
            [command_encoder dispatchThreadgroups:decode_threadgroups threadsPerThreadgroup:decode_threads_per_group];
            ...
            auto input1_view = MtlBufferView::GetReadView(input_tensors[tensor_mapping_.scores_tensor_index()], command_buffer);
            ...
            [command_encoder dispatchThreadgroups:score_threadgroups threadsPerThreadgroup:score_threads_per_group];
    ...
    absl::Status TensorsToDetectionsCalculator::LoadOptions(CalculatorContext* cc) {
        ...
        ABSL_CHECK_NE(options_.max_results(), 0) << "The maximum number of the top-scored detection results must be "
                                                    "non-zero.";
        ...
        ABSL_CHECK_EQ(options_.num_keypoints() * options_.num_values_per_keypoint() + kNumCoordsPerBox, num_coords_);
        ...
        if (options_.has_box_boundaries_indices()) {
            box_indices_ = {options_.box_boundaries_indices().ymin(), options_.box_boundaries_indices().xmin(),
                            options_.box_boundaries_indices().ymax(), options_.box_boundaries_indices().xmax()};
            int bitmap   = 0;
    ...
    absl::Status TensorsToDetectionsCalculator::DecodeBoxes(const float* raw_boxes, const std::vector<Anchor>& anchors,
        ...
        for (int i = 0; i < num_boxes_; ++i) {
            ...
            if (options_.num_keypoints()) {
                for (int k = 0; k < options_.num_keypoints(); ++k) {
                    const int offset =
                        i * num_coords_ + options_.keypoint_coord_offset() + k * options_.num_values_per_keypoint();
    ...
    absl::Status TensorsToDetectionsCalculator::ConvertToDetections(const float* detection_boxes, const float* detection_scores,
        ...
        for (int i = 0; i < num_boxes_ * classes_per_detection_; i += classes_per_detection_) {
            ...
            Detection detection  = ConvertToDetection(
                ...
                absl::MakeConstSpan(detection_classes + i, classes_per_detection_), options_.flip_vertically());
            ...
            if (options_.num_keypoints() > 0) {
                ...
                for (int kp_id = 0; kp_id < options_.num_keypoints() * options_.num_values_per_keypoint();
                    ...
                    keypoint->set_y(options_.flip_vertically() ? 1.f - detection_boxes[keypoint_index + 1]
    ...
    Detection TensorsToDetectionsCalculator::ConvertToDetection(float box_ymin, float box_xmin, float box_ymax, float box_xmax,
        ...
        for (int i = 0; i < (int) scores.size(); ++i) {
            ...
            if (options_.has_min_score_thresh() && scores[i] < options_.min_score_thresh()) {
                continue;
            }
    ...
    absl::Status TensorsToDetectionsCalculator::GpuInit(CalculatorContext* cc) {
        ...
        MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([this, output_format_flag]() -> absl::Status {
  ...
  if (num_keypoints > int(0)){
    for (int k = 0; k < num_keypoints; ++k) {
      ...
      boxes.data[kp_offset + int(1)] = kp_y / scale.y * anchor_h + anchor_yc;
                ...
                output_format_flag, options_.apply_exponential_on_box_size() ? 1 : 0, options_.box_coord_offset(),
                options_.num_keypoints(), options_.keypoint_coord_offset(), options_.num_values_per_keypoint());
            ...
            decoded_boxes_buffer_ =
                absl::make_unique<Tensor>(Tensor::ElementType::kFloat32, Tensor::Shape{1, num_boxes_ * num_coords_});
            raw_anchors_buffer_ =
                absl::make_unique<Tensor>(Tensor::ElementType::kFloat32, Tensor::Shape{1, num_boxes_ * kNumCoordsPerBox});
            // Parameters
            glUseProgram(decode_program_);
            glUniform4f(0, options_.x_scale(), options_.y_scale(), options_.w_scale(), options_.h_scale());
...
void main() {
  ...
  if (s_idx == uint(0)) {
    ...
    scored_boxes.data[g_idx * uint(2) + uint(1)] = max_class;
                ...
                num_classes_, options_.sigmoid_score() ? 1 : 0, options_.has_score_clipping_thresh() ? 1 : 0,
                options_.has_score_clipping_thresh() ? options_.score_clipping_thresh() : 0, !IsClassIndexAllowed(0));
            ...
            if (!gpu_has_enough_work_groups_) {
                return absl::FailedPreconditionError(
                    absl::StrFormat("Hardware limitation: Processing will be done on CPU, because "
            ...
            if (class_index_set_.is_allowlist) {
                ABSL_CHECK_EQ(class_index_set_.values.size(), IsClassIndexAllowed(0) ? num_classes_ : num_classes_ - 1)
            ...
            // Outputs
            scored_boxes_buffer_ = absl::make_unique<Tensor>(Tensor::ElementType::kFloat32, Tensor::Shape{1, num_boxes_ * 2});
...
kernel void decodeKernel(
  ...
  float4 scale = float4(($0),($1),($2),($3));
            ...
            options_.x_scale(), options_.y_scale(), options_.w_scale(), options_.h_scale());
        decode_src += R"(
  uint g_idx = gid.x;
  uint box_offset = g_idx * num_coords + uint(box_coord_offset);
  uint anchor_offset = g_idx * uint(4);  // check kNumCoordsPerBox
  ...
  if (num_keypoints > int(0)){
    for (int k = 0; k < num_keypoints; ++k) {
      ...
      boxes[kp_offset + int(1)] = kp_y / scale.y * anchor_h + anchor_yc;
        ...
        {
            // Shader program
            NSString*      library_source = [NSString stringWithUTF8String:decode_src.c_str()];
            NSError*       error          = nil;
            id<MTLLibrary> library        = [device newLibraryWithSource:library_source options:nullptr error:&error];
            RET_CHECK(library != nil) << "Couldn't create shader library " << [[error localizedDescription] UTF8String];
            ...
            decode_program_ = [device newComputePipelineStateWithFunction:kernel_func error:&error];
            RET_CHECK(decode_program_ != nil) << "Couldn't create pipeline state " << [[error localizedDescription] UTF8String];
            // Outputs
            decoded_boxes_buffer_ =
                absl::make_unique<Tensor>(Tensor::ElementType::kFloat32, Tensor::Shape{1, num_boxes_ * num_coords_});
            // Inputs
  ...
  if (s_idx == uint(0)) {
    ...
    scored_boxes[g_idx * uint(2) + uint(1)] = max_class;
        ...
        if (class_index_set_.is_allowlist) {
            ...
            ABSL_CHECK_EQ(class_index_set_.values.size(), IsClassIndexAllowed(0) ? 0 : 1) << "Only ignore class 0 is allowed";
        }

        {
            // Shader program
            NSString*      library_source = [NSString stringWithUTF8String:score_src.c_str()];
            NSError*       error          = nil;
            id<MTLLibrary> library        = [device newLibraryWithSource:library_source options:nullptr error:&error];
            RET_CHECK(library != nil) << "Couldn't create shader library " << [[error localizedDescription] UTF8String];
            id<MTLFunction> kernel_func = nil;
            ...
            RET_CHECK(kernel_func != nil) << "Couldn't create kernel function.";
            score_program_ = [device newComputePipelineStateWithFunction:kernel_func error:&error];
            RET_CHECK(score_program_ != nil) << "Couldn't create pipeline state " << [[error localizedDescription] UTF8String];
            // Outputs
            scored_boxes_buffer_ = absl::make_unique<Tensor>(Tensor::ElementType::kFloat32, Tensor::Shape{1, num_boxes_ * 2});
            // # filter classes supported is hardware dependent.

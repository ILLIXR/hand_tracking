...
namespace {
    ...
    bool TfLiteTensorSpecEqual(const TfLiteTensor& a, const TfLiteTensor& b) {
        return a.type == b.type && TfLiteIntArrayEqual(a.dims, b.dims) && a.params.scale == b.params.scale &&
            a.params.zero_point == b.params.zero_point && a.allocation_type == b.allocation_type && a.bytes == b.bytes;
    }

    absl::flat_hash_map<std::string, int> CreateNameToIndexMap(const std::vector<std::string>& names) {
...
absl::Status InferenceFeedbackManager::Init(const InferenceCalculatorOptions::InputOutputConfig& io_config,
                                            const InputOutputTensorNames&                        input_output_tensor_names,
                                            tflite::Interpreter*                                 interpreter) {
    interpreter_ = interpreter;
    MP_ASSIGN_OR_RETURN(feedback_tensor_indices_links_,
                        ConvertSignatureTensorNamesToModelIndices(io_config, input_output_tensor_names));

    for (const auto& link : feedback_tensor_indices_links_) {
        const auto [output_unused_iter, output_was_inserted] = feedback_output_indices_.insert(link.from_idx);
        RET_CHECK(output_was_inserted) << "Feedback output tensors must be unique.";
        TfLiteTensor* from_tensor = interpreter_->tensor(interpreter->outputs()[link.from_idx]);
        RET_CHECK(!util::tflite::IsDynamicTensor(*from_tensor)) << "Feedback output tensors must not be dynamic.";
        const auto [input_unused_iter, input_was_inserted] = feedback_input_indices_.insert(link.to_idx);
        RET_CHECK(input_was_inserted) << "Feedback input tensors must be unique.";
        TfLiteTensor* to_tensor = interpreter_->tensor(interpreter->inputs()[link.to_idx]);
        RET_CHECK(!util::tflite::IsDynamicTensor(*to_tensor)) << "Feedback input tensors must not be dynamic.";
        RET_CHECK(TfLiteTensorSpecEqual(*from_tensor, *to_tensor)) << "Feedback tensors must have the same spec.";
...
void InferenceFeedbackManager::SwapFeedbackTensors() {
    for (const auto& link : feedback_tensor_indices_links_) {
        TfLiteTensor* from_tensor = interpreter_->tensor(interpreter_->outputs()[link.from_idx]);
        TfLiteTensor* to_tensor   = interpreter_->tensor(interpreter_->inputs()[link.to_idx]);
        {
            using std::swap;
...
InferenceFeedbackManager::ConvertSignatureTensorNamesToModelIndices(
    ...
    if (input_output_tensor_names_map.empty() || input_output_tensor_names_map.size() > 1) {
        // Fail gracefully by returning an empty TensorFeedbackIndicesLink list if
        // SignatureDef is not available or not supported.
        ABSL_LOG(WARNING) << "Feedback manager requires a model with a single signature "
    ...
    const auto& input_output_tensor_names = input_output_tensor_names_map.begin()->second;

    const auto input_name_to_index_map  = CreateNameToIndexMap(input_output_tensor_names.input_tensor_names);
    const auto output_name_to_index_map = CreateNameToIndexMap(input_output_tensor_names.output_tensor_names);
    ...
    for (const auto& link : io_config.feedback_tensor_links()) {
        RET_CHECK(!input_output_mapping_tensor_names.contains(link.from_output_tensor_name()))
            << absl::StrFormat("Feedback output tensor [%s] cannot be used for input/output "
                               "mapping. Input/output mapping tensor names: [%s]",
                               link.from_output_tensor_name(), absl::StrJoin(input_output_mapping_tensor_names, ", "));
        RET_CHECK(!input_output_mapping_tensor_names.contains(link.to_input_tensor_name()))
            << absl::StrFormat("Feedback input tensor [%s] cannot be used for input/output "
                               "mapping. Input/output mapping tensor names: [%s]",
                               link.to_input_tensor_name(), absl::StrJoin(input_output_mapping_tensor_names, ", "));
        TensorFeedbackIndicesLink indices_link;
        auto                      from_it = output_name_to_index_map.find(link.from_output_tensor_name());
        ...
        RET_CHECK(to_it != input_name_to_index_map.end()) << "Input tensor name not found: " << link.to_input_tensor_name();
...
absl::StatusOr<int> InferenceFeedbackManager::MapInputTensorToModelIndex(int input_idx) const {
    RET_CHECK(input_idx >= 0 && (size_t) input_idx <= input_tensor_to_model_indices_.size())

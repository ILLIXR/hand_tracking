...
namespace api2 {
    ...
    class InferenceCalculatorSelectorImpl : public SubgraphImpl<InferenceCalculatorSelector, InferenceCalculatorSelectorImpl> {
    public:
        absl::StatusOr<CalculatorGraphConfig> GetConfig(const CalculatorGraphConfig::Node& subgraph_node) override {
            const auto&                    options = Subgraph::GetOptions<mediapipe::InferenceCalculatorOptions>(subgraph_node);
            std::vector<absl::string_view> impls;

            const bool should_use_gpu = !options.has_delegate() || // Use GPU delegate if not specified
            ...
            if (should_use_gpu) {
                ...
                const bool prefer_gl_advanced = options.delegate().gpu().use_advanced_gpu_api() &&
            ...
            for (const auto& suffix : impls) {
                ...
                VLOG(1) << "Using " << suffix << " for InferenceCalculator with "
                        << (options.has_model_path() ? "model " + options.model_path()
                                                     : "output_stream " +
                                    (subgraph_node.output_stream_size() > 0 ? subgraph_node.output_stream(0) : "<none>"));
    ...
    absl::StatusOr<Packet<TfLiteModelPtr>> InferenceCalculator::GetModelAsPacket(CalculatorContext* cc) {
        const auto& options = cc->Options<mediapipe::InferenceCalculatorOptions>();
        if (!options.model_path().empty()) {
            return TfLiteModelLoader::LoadFromPath(options.model_path(), options.try_mmap_model());
        }
    ...
    absl::StatusOr<Packet<tflite::OpResolver>> InferenceCalculator::GetOpResolverAsPacket(CalculatorContext* cc) {
        ...
        return PacketAdopting<tflite::OpResolver>(
            std::make_unique<tflite::ops::builtin::BuiltinOpResolverWithoutDefaultDelegates>());
    }

    void InferenceCalculator::WarnFeedbackTensorsUnsupported(CalculatorContract* cc) {
        const auto& options = cc->Options<mediapipe::InferenceCalculatorOptions>();
        if (options.has_input_output_config() && !options.input_output_config().feedback_tensor_links().empty()) {
            ABSL_LOG(WARNING) << "Feedback tensor support is only available for CPU and "

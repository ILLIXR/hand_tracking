...
namespace {
    ...
    GenerateAndValidateTensorList(const InferenceCalculatorOptions::InputOutputConfig::TensorIndicesMap& tensor_indices_list) {
        absl::flat_hash_set<int> indices_set;
        std::vector<int>         result;
        for (const int index : tensor_indices_list.model_tensor_indices()) {
            RET_CHECK(indices_set.insert(index).second) << "Indices in TensorIndicesMap are not unique.";
    ...
    absl::StatusOr<absl::flat_hash_map<std::string, int>> CreateNameToIndexMap(const std::vector<std::string>& names) {
        ...
        for (int i = 0; i < (int) names.size(); ++i) {
            ...
            RET_CHECK(was_inserted) << "Duplicate tensor names found in model signatures: " << absl::StrJoin(names, ", ");
        }
    ...
    static absl::StatusOr<std::vector<int>>
    MapTensorNamesToIndices(const std::vector<std::string>&                                      signature_tensor_names,
        ...
        MP_ASSIGN_OR_RETURN(const auto input_name_to_index_map, CreateNameToIndexMap(signature_tensor_names));
        for (const auto& tensor_name : config_tensor_names.tensor_names()) {
            ...
            RET_CHECK(it != input_name_to_index_map.end())
                ...
                << " not found in model signatures. Model tensor names: " << absl::StrJoin(signature_tensor_names, ", ");
            result.push_back(it->second);
        }
        RET_CHECK(!ContainsDuplicates(result))
            << "Duplicate tensor names found in TensorNamesMap: " << absl::StrJoin(config_tensor_names.tensor_names(), ", ");
        return result;
    };
    ...
    absl::Status
    ExcludeFeedbackTensorsFromRemappingIndicesVector(const InferenceCalculatorOptions::InputOutputConfig& io_config,
        ...
        for (int i = 0; i < (int) remapping_tensor_indices.size(); ++i) {
            ...
            remapping_tensor_indices[i] = indices_translation[remapping_tensor_indices[i]];
        }
...
InferenceIoMapper::GetInputOutputTensorNamesFromInterpreter(const tflite::Interpreter& interpreter) {
    auto input_output_tensor_names = TfLiteSignatureReader::GetInputOutputTensorNamesFromAllTfliteSignatures(interpreter);
    if (!input_output_tensor_names.ok()) {
        // TODO b/336260063 - remove this warning once the bug is fixed.
        ABSL_LOG_FIRST_N(WARNING, 1) << "Unable to extract TfLite model's tensor names from "
...
// static
absl::StatusOr<InputOutputTensorNames>
InferenceIoMapper::GetInputOutputTensorNamesFromModel(const tflite::FlatBufferModel& flatbuffer,
    ...
    if (interpreter_builder(&interpreter) != kTfLiteOk || !interpreter) {
        ABSL_LOG_EVERY_N(WARNING, 1) << "Extracting input output tensor names from TfliteSignature failed: "
...
absl::Status InferenceIoMapper::UpdateIoMap(const InferenceCalculatorOptions::InputOutputConfig& io_config,
                                            const InputOutputTensorNames&                        input_output_tensor_names) {
    num_feedback_tensors_ = io_config.feedback_tensor_links().size();

    if ((io_config.has_input_tensor_indices_map() || io_config.has_output_tensor_indices_map()) && num_feedback_tensors_ > 0) {
        // TODO b/336767692 - remove this check once indices-based feedback
        // tensors are supported.
        return absl::FailedPreconditionError("Feedback tensors are not supported with tensor index-based I/O "
                                             "mapping.");
    }
    ...
    if (io_config.has_input_tensor_indices_map()) {
        input_tensor_indices_.reserve(io_config.input_tensor_indices_map().model_tensor_indices().size());
        MP_ASSIGN_OR_RETURN(input_tensor_indices_, GenerateAndValidateTensorList(io_config.input_tensor_indices_map()));
    }

    if (io_config.has_output_tensor_indices_map()) {
        output_tensor_indices_.reserve(io_config.output_tensor_indices_map().model_tensor_indices().size());
        MP_ASSIGN_OR_RETURN(output_tensor_indices_, GenerateAndValidateTensorList(io_config.output_tensor_indices_map()));
    }

    if (!io_config.has_input_tensor_names_map() && !io_config.has_output_tensor_names_map()) {
        // No tensor name mapping is provided.
        return absl::OkStatus();
    }

    if (input_output_tensor_names.empty()) {
        return absl::FailedPreconditionError("Tensor name-based mapping requires a model with one signature.");
    }

    if (input_output_tensor_names.size() > 1) {
        return absl::FailedPreconditionError("Tensor name-based mapping is not supported with multi-signature "
                                             "models.");
    }

    // Use tensor names of default signature.
    const auto input_output_tensor_names_default_signature = input_output_tensor_names.begin()->second;

    if (io_config.has_input_tensor_names_map()) {
        // Read number of model inputs directly from the signature.
        const int num_model_input_tensors = (int) input_output_tensor_names_default_signature.input_tensor_names.size();
        input_tensor_indices_.reserve(io_config.input_tensor_names_map().tensor_names().size());
        MP_ASSIGN_OR_RETURN(input_tensor_indices_,
                            MapTensorNamesToIndices(input_output_tensor_names_default_signature.input_tensor_names,
        ...
        if (num_feedback_tensors_ > 0) {
            MP_RETURN_IF_ERROR(ExcludeFeedbackTensorsFromRemappingIndicesVector(
                io_config, input_output_tensor_names_default_signature.input_tensor_names, input_tensor_indices_));
        }
        // Feedback tensors are excluded from the input_tensor_indices_.
        RET_CHECK_EQ(input_tensor_indices_.size() + num_feedback_tensors_, (size_t) num_model_input_tensors)
            << "Unexpected number of input tensors.";
    }

    if (io_config.has_output_tensor_names_map()) {
        const int num_model_output_tensors = (int) input_output_tensor_names_default_signature.output_tensor_names.size();
        output_tensor_indices_.reserve(num_model_output_tensors);
        MP_ASSIGN_OR_RETURN(output_tensor_indices_,
                            MapTensorNamesToIndices(input_output_tensor_names_default_signature.output_tensor_names,
                                                    io_config.output_tensor_names_map()));
        if (num_feedback_tensors_ > 0) {
            MP_RETURN_IF_ERROR(ExcludeFeedbackTensorsFromRemappingIndicesVector(
                io_config, input_output_tensor_names_default_signature.output_tensor_names, output_tensor_indices_));
        }
        // Feedback tensors are excluded from the output_tensor_indices_.
        RET_CHECK_EQ(output_tensor_indices_.size() + num_feedback_tensors_, (size_t) num_model_output_tensors)
            << "Unexpected number of output tensors.";
    }
    return absl::OkStatus();
}

absl::StatusOr<TensorSpan> InferenceIoMapper::RemapInputTensors(const TensorSpan& unmapped_tensors) {
    if (input_tensor_indices_.empty()) {
        return unmapped_tensors;
    }
    RET_CHECK_EQ((size_t) unmapped_tensors.size(), input_tensor_indices_.size()) << "Unexpected number of input tensors.";
    ...
    for (int i = 0; i < unmapped_tensors.size(); ++i) {
        ...
        RET_CHECK(index < unmapped_tensors.size()) << "Index " << index << " out of range"
...
absl::StatusOr<std::vector<Tensor>> InferenceIoMapper::RemapOutputTensors(std::vector<Tensor>&& unmapped_tensors) {
    if (output_tensor_indices_.empty()) {
        return std::move(unmapped_tensors);
    }
    RET_CHECK_EQ(unmapped_tensors.size(), output_tensor_indices_.size()) << "Unexpected number of output tensors.";
    ...
    for (int i = 0; i < (int) unmapped_tensors.size(); ++i) {
        ...
        RET_CHECK(index < (int) unmapped_tensors.size()) << "Index " << index << " out of range"
                                                         << ". Size of TensorIndicesMap: " << unmapped_tensors.size() << ".";

...
namespace {
    ...
    public:
        absl::Status Init(CalculatorContext* cc, bool input_starts_at_bottom, BorderMode border_mode) {
            ...
            return gl_helper_.RunInGlContext([this, input_starts_at_bottom, border_mode]() -> absl::Status {
                use_custom_zero_border_ =
                    border_mode == BorderMode::kZero && !IsGlClampToBorderSupported(gl_helper_.GetGlContext());
          ...
          #define INPUT_STARTS_AT_BOTTOM
                ...
                const std::string extract_sub_rect_vertex_src =
                    absl::StrCat(mediapipe::kMediaPipeVertexShaderPreamble, starts_at_bottom_def, kExtractSubRectVertexShader);
          ...
          #define CUSTOM_ZERO_BORDER_MODE
                ...
                const std::string extract_sub_rect_frag_src = absl::StrCat(
                    mediapipe::kMediaPipeFragmentShaderPreamble, custom_zero_border_mode_def, kExtractSubRectFragBody);
                mediapipe::GlhCreateProgram(extract_sub_rect_vertex_src.c_str(), extract_sub_rect_frag_src.c_str(),
                                            kNumAttributes, &attr_name[0], attr_location, &program_);
                ...
                glBufferData(GL_ARRAY_BUFFER, sizeof(mediapipe::kBasicSquareVertices), mediapipe::kBasicSquareVertices,
                             GL_STATIC_DRAW);

                // vbo 1
                glBindBuffer(GL_ARRAY_BUFFER, vbo_[1]);
                glBufferData(GL_ARRAY_BUFFER, sizeof(mediapipe::kBasicTextureVertices), mediapipe::kBasicTextureVertices,
                             GL_STATIC_DRAW);
        ...
        absl::Status Convert(const mediapipe::Image& input, const RotatedRect& roi, float range_min, float range_max,
            ...
            RET_CHECK_EQ(tensor_buffer_offset, 0) << "The non-zero tensor_buffer_offset input is not supported yet.";
            const auto& output_shape = output_tensor.shape();
            MP_RETURN_IF_ERROR(ValidateTensorShape(output_shape));

            MP_RETURN_IF_ERROR(gl_helper_.RunInGlContext(
                [this, &output_tensor, &input, &roi, &output_shape, range_min, range_max]() -> absl::Status {
        ...
        absl::Status ExtractSubRect(const mediapipe::GlTexture& texture, const RotatedRect& sub_rect, bool flip_horizontally,
                                    float alpha, float beta, const Tensor::Shape& output_shape,
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, output->name(), 0);
            ...
            if (gl_context->GetGlVersion() == mediapipe::GlVersion::kGLES2) {
                GetTransposedRotatedSubRectToRectTransformMatrix(sub_rect, texture.width(), texture.height(), flip_horizontally,
                                                                 &transform_mat);
                glUniformMatrix4fv(matrix_id_, 1, GL_FALSE, transform_mat.data());
            } else {
                GetRotatedSubRectToRectTransformMatrix(sub_rect, texture.width(), texture.height(), flip_horizontally,
                                                       &transform_mat);
    ...
    private:
        absl::Status ValidateTensorShape(const Tensor::Shape& output_shape) {
            ...
            RET_CHECK_EQ(output_shape.dims[0], 1) << "Handling batch dimension not equal to 1 is not implemented in this "
                                                     "converter.";

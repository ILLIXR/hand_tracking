...
namespace api2 {
    ...
    class InferenceCalculatorCpuImpl : public InferenceCalculatorNodeImpl<InferenceCalculatorCpu, InferenceCalculatorCpuImpl> {
    public:
    ...
    private:
        absl::StatusOr<std::unique_ptr<InferenceRunner>> CreateInferenceRunner(CalculatorContext* cc);
    ...
    absl::Status InferenceCalculatorCpuImpl::UpdateContract(CalculatorContract* cc) {
    ...
    absl::Status InferenceCalculatorCpuImpl::Open(CalculatorContext* cc) {
        ...
        return InferenceCalculatorNodeImpl::UpdateIoMapping(cc, inference_runner_->GetInputOutputTensorNames());
    }

    absl::StatusOr<std::vector<Tensor>> InferenceCalculatorCpuImpl::Process(CalculatorContext* cc,
    ...
    absl::StatusOr<std::unique_ptr<InferenceRunner>> InferenceCalculatorCpuImpl::CreateInferenceRunner(CalculatorContext* cc) {
        ...
        const auto& options                 = cc->Options<mediapipe::InferenceCalculatorOptions>();
        const int   interpreter_num_threads = cc->Options<mediapipe::InferenceCalculatorOptions>().cpu_num_thread();
        MP_ASSIGN_OR_RETURN(TfLiteDelegatePtr delegate, MaybeCreateDelegate(cc));
        return CreateInferenceInterpreterDelegateRunner(std::move(model_packet), std::move(op_resolver_packet),
                                                        std::move(delegate), interpreter_num_threads,
                                                        &options.input_output_config());
    }

    absl::StatusOr<TfLiteDelegatePtr> InferenceCalculatorCpuImpl::MaybeCreateDelegate(CalculatorContext* cc) {
        ...
        if (!kDelegate(cc).IsEmpty()) {
            ...
            RET_CHECK(input_side_packet_delegate.has_tflite() || input_side_packet_delegate.has_xnnpack() ||
        ...
        const bool opts_has_delegate = calculator_opts.has_delegate() || !kDelegate(cc).IsEmpty();
...
#if defined(MEDIAPIPE_ANDROID)
        const bool nnapi_requested = opts_has_delegate ? opts_delegate.has_nnapi() : calculator_opts.use_nnapi();
        if (nnapi_requested) {
            ...
            options.cache_dir        = nnapi.has_cache_dir() ? nnapi.cache_dir().c_str() : nullptr;
            options.model_token      = nnapi.has_model_token() ? nnapi.model_token().c_str() : nullptr;
            options.accelerator_name = nnapi.has_accelerator_name() ? nnapi.accelerator_name().c_str() : nullptr;
            return TfLiteDelegatePtr(new tflite::StatefulNnApiDelegate(options), [](TfLiteDelegate*) { });
        }

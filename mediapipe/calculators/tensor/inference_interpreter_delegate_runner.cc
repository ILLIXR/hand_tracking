...
namespace {
    ...
    void CopyTensorBufferToInterpreter(const Tensor& input_tensor, Interpreter* interpreter, int input_tensor_index) {
        ...
        T*   local_tensor_buffer = interpreter->typed_input_tensor<T>(input_tensor_index);
    ...
    void CopyTensorBufferToInterpreter<char>(const Tensor& input_tensor, Interpreter* interpreter, int input_tensor_index) {
        const char*           input_tensor_buffer = input_tensor.GetCpuReadView().buffer<char>();
        tflite::DynamicBuffer dynamic_buffer;
        dynamic_buffer.AddString(input_tensor_buffer, input_tensor.shape().num_elements());
        dynamic_buffer.WriteToTensorAsVector(interpreter->tensor(interpreter->inputs()[input_tensor_index]));
    }
    ...
    void CopyTensorBufferFromInterpreter(Interpreter* interpreter, int output_tensor_index, Tensor* output_tensor) {
        ...
        T*   local_tensor_buffer  = interpreter->typed_output_tensor<T>(output_tensor_index);
...
public:
    InferenceInterpreterDelegateRunner(api2::Packet<TfLiteModelPtr> model, std::unique_ptr<Interpreter> interpreter,
                                       TfLiteDelegatePtr delegate, InputOutputTensorNames&& input_output_tensor_names,
    ...
    absl::StatusOr<std::vector<Tensor>> Run(CalculatorContext* cc, const TensorSpan& tensor_span) override;
...
absl::StatusOr<std::vector<Tensor>> InferenceInterpreterDelegateRunner::Run(CalculatorContext* cc,
                                                                            const TensorSpan&  tensor_span) {
    UNUSED(cc);
    const int num_feedback_tensors = feedback_manager_ ? feedback_manager_->GetNumberOfFeedbackTensors() : 0;

    RET_CHECK_EQ((size_t) (tensor_span.size() + num_feedback_tensors), interpreter_->inputs().size());
    ...
    for (int i = 0; i < tensor_span.size(); ++i) {
        ...
        if (feedback_manager_) {
            ...
            // model index.
            MP_ASSIGN_OR_RETURN(input_model_index, feedback_manager_->MapInputTensorToModelIndex(i));
        } else {
            input_model_index = i;
        }
        const Tensor& input_tensor = tensor_span[i];
        if (input_tensor.shape().is_dynamic) {
            const TfLiteTensor* interpreter_tensor = interpreter_->tensor(interpreter_->inputs()[input_model_index]);
            // TODO: Can avoid copying even these <= 4 values in the future.
            std::vector<int> interpreter_dims{interpreter_tensor->dims->data,
                                              interpreter_tensor->dims->data + interpreter_tensor->dims->size};
            if (interpreter_dims != input_tensor.shape().dims) {
                interpreter_->ResizeInputTensorStrict(input_model_index, input_tensor.shape().dims);
    ...
    for (int i = 0; i < tensor_span.size(); ++i) {
        ...
        const TfLiteType input_tensor_type = interpreter_->tensor(interpreter_->inputs()[input_model_index])->type;
        const Tensor&    input_tensor      = tensor_span[i];
        switch (input_tensor_type) {
        case TfLiteType::kTfLiteFloat16:
        case TfLiteType::kTfLiteFloat32: {
            CopyTensorBufferToInterpreter<float>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        case TfLiteType::kTfLiteUInt8: {
            CopyTensorBufferToInterpreter<uint8_t>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        case TfLiteType::kTfLiteInt8: {
            CopyTensorBufferToInterpreter<int8_t>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        case TfLiteType::kTfLiteInt32: {
            CopyTensorBufferToInterpreter<int32_t>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        case TfLiteType::kTfLiteString: {
            CopyTensorBufferToInterpreter<char>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        case TfLiteType::kTfLiteBool: {
            CopyTensorBufferToInterpreter<bool>(input_tensor, interpreter_.get(), input_model_index);
            break;
        }
        default:
            return absl::InvalidArgumentError(absl::StrCat("Unsupported input tensor type:", input_tensor_type));
        }
    }
    ...
    for (int i = 0; i < (int) tensor_indexes.size(); ++i) {
        if (feedback_manager_ && feedback_manager_->IsFeedbackOutputTensorAtIndex(i)) {
            // Exclude feedback tensors from InferenceRunner output.
            continue;
        }
        TfLiteTensor* tensor = interpreter_->tensor(tensor_indexes[i]);
        Tensor::Shape shape{std::vector<int>{tensor->dims->data, tensor->dims->data + tensor->dims->size}};
        switch (tensor->type) {
        case TfLiteType::kTfLiteFloat16:
        case TfLiteType::kTfLiteFloat32:
            output_tensors.emplace_back(Tensor::ElementType::kFloat32, shape);
            CopyTensorBufferFromInterpreter<float>(interpreter_.get(), i, &output_tensors.back());
            break;
        case TfLiteType::kTfLiteUInt8:
            output_tensors.emplace_back(Tensor::ElementType::kUInt8, shape,
                                        Tensor::QuantizationParameters{tensor->params.scale, tensor->params.zero_point});
            CopyTensorBufferFromInterpreter<uint8_t>(interpreter_.get(), i, &output_tensors.back());
            break;
        case TfLiteType::kTfLiteInt8:
            output_tensors.emplace_back(Tensor::ElementType::kInt8, shape,
                                        Tensor::QuantizationParameters{tensor->params.scale, tensor->params.zero_point});
            CopyTensorBufferFromInterpreter<int8_t>(interpreter_.get(), i, &output_tensors.back());
            break;
        case TfLiteType::kTfLiteInt32:
            output_tensors.emplace_back(Tensor::ElementType::kInt32, shape);
            CopyTensorBufferFromInterpreter<int32_t>(interpreter_.get(), i, &output_tensors.back());
            break;
        case TfLiteType::kTfLiteBool:
            output_tensors.emplace_back(Tensor::ElementType::kBool, shape, Tensor::QuantizationParameters{1.0f, 0});
            CopyTensorBufferFromInterpreter<bool>(interpreter_.get(), i, &output_tensors.back());
            break;
        case TfLiteType::kTfLiteString:
            // No current use-case for copying TfLiteTensors with string type to
            // MediaPipe Tensors.
        default:
            return absl::InvalidArgumentError(absl::StrCat("Unsupported output tensor type:", TfLiteTypeGetName(tensor->type)));
        }
    }
    if (feedback_manager_) {
...
CreateInferenceInterpreterDelegateRunner(api2::Packet<TfLiteModelPtr> model, api2::Packet<tflite::OpResolver> op_resolver,
                                         TfLiteDelegatePtr delegate, int interpreter_num_threads,
    ...
    MP_ASSIGN_OR_RETURN(auto input_output_tensor_names,
                        InferenceIoMapper::GetInputOutputTensorNamesFromInterpreter(*interpreter));
    ...
    if (input_output_config) {
        ...
        MP_RETURN_IF_ERROR(
            inference_feedback_manager->Init(*input_output_config, input_output_tensor_names, interpreter.get()));
    }
    return std::make_unique<InferenceInterpreterDelegateRunner>(std::move(model), std::move(interpreter), std::move(delegate),

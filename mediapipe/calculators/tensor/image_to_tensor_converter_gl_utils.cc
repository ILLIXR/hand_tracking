...
namespace {
    ...
    public:
        GlTexParameterfvOverride(GLenum name, std::array<float, kNumValues> old_values)
            : name_(name)
            , old_values_(std::move(old_values)) { }
...
template<int kNumValues>
std::unique_ptr<GlOverride> OverrideGlTexParameterfv(GLenum name, std::array<GLfloat, kNumValues> values) {
    ...
    if (values != old_values) {
        ...
        return {absl::make_unique<GlTexParameterfvOverride<kNumValues>>(name, std::move(old_values))};
    }
    return {absl::make_unique<GlNoOpOverride>()};
}

template std::unique_ptr<GlOverride> OverrideGlTexParameterfv<4>(GLenum name, std::array<GLfloat, 4> values);

bool IsGlClampToBorderSupported(const mediapipe::GlContext& gl_context) {
    return gl_context.gl_major_version() > 3 || (gl_context.gl_major_version() == 3 && gl_context.gl_minor_version() >= 2);
}

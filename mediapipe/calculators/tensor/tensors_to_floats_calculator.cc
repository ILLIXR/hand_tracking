...
namespace api2 {
    ...
    public:
        ...
        MEDIAPIPE_NODE_INTERFACE(TensorsToFloatsCalculator, kInTensors, kOutFloat, kOutFloats);
    ...
    absl::Status TensorsToFloatsCalculator::Process(CalculatorContext* cc) {
        ...
        auto output_floats = absl::make_unique<std::vector<float>>(raw_floats, raw_floats + num_values);

        switch (options_.activation()) {
        case TensorsToFloatsCalculatorOptions::SIGMOID:
            std::transform(output_floats->begin(), output_floats->end(), output_floats->begin(), Sigmoid);
            break;

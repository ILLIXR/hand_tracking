...
namespace api2 {
    namespace {
        ...
        void SetClassificationLabel(const LabelMapItem label_map_item, Classification* classification) {
    ...
    public:
        ...
        static constexpr Output<ClassificationList> kOutClassificationList{"CLASSIFICATIONS"};
    ...
    private:
        ...
        const proto_ns::Map<int64_t, LabelMapItem>& GetLabelMap(CalculatorContext* cc);
    };
    ...
    absl::Status TensorsToClassificationCalculator::Open(CalculatorContext* cc) {
        ...
        if (options.has_label_map_path()) {
            ...
            MP_ASSIGN_OR_RETURN(string_path, PathToResourceAsFile(options.label_map_path()));
            std::string label_map_string;
            MP_RETURN_IF_ERROR(mediapipe::GetResourceContents(string_path, &label_map_string));
            ...
            for (int i = 0; i < options.label_map().entries_size(); ++i) {
                ...
                RET_CHECK(!local_label_map_.contains(entry.id())) << "Duplicate id found: " << entry.id();
                LabelMapItem item;
        ...
        if (is_binary_classification_) {
            RET_CHECK(options.allow_classes().empty() && options.ignore_classes().empty());
        }
    ...
    absl::Status TensorsToClassificationCalculator::Process(CalculatorContext* cc) {
        ...
        if (top_k_ > 0) {
            int desired_size = std::min(classification_list->classification_size(), top_k_);
            std::partial_sort(raw_classification_list->begin(), raw_classification_list->begin() + desired_size,
                              raw_classification_list->end(), [](const Classification a, const Classification b) {
                                  return a.score() > b.score();
                              });

            if (desired_size >= top_k_) {
                // Resizes the underlying list to have only top_k_ classifications.
                raw_classification_list->DeleteSubrange(top_k_, raw_classification_list->size() - top_k_);
            }
    ...
    const proto_ns::Map<int64_t, LabelMapItem>& TensorsToClassificationCalculator::GetLabelMap(CalculatorContext* cc) {
        return !local_label_map_.empty() ? local_label_map_
                                         : cc->Options<TensorsToClassificationCalculatorOptions>().label_items();
    }

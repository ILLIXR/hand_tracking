...
namespace api2 {
    ...
    namespace {
        ...
        float ApplyActivation(::mediapipe::TensorsToLandmarksCalculatorOptions::Activation activation, float value) {
            switch (activation) {
    ...
    public:
        ...
        static constexpr Input<bool>::SideFallback::Optional      kFlipHorizontally{"FLIP_HORIZONTALLY"};
        static constexpr Input<bool>::SideFallback::Optional      kFlipVertically{"FLIP_VERTICALLY"};
        static constexpr Output<LandmarkList>::Optional           kOutLandmarkList{"LANDMARKS"};
        static constexpr Output<NormalizedLandmarkList>::Optional kOutNormalizedLandmarkList{"NORM_LANDMARKS"};
    ...
    absl::Status TensorsToLandmarksCalculator::Open(CalculatorContext* cc) {
        ...
        if (kOutNormalizedLandmarkList(cc).IsConnected()) {
            RET_CHECK(options_.has_input_image_height() && options_.has_input_image_width())
                << "Must provide input width/height for getting normalized landmarks.";
        }
        if (kOutLandmarkList(cc).IsConnected() &&
            (options_.flip_horizontally() || options_.flip_vertically() || kFlipHorizontally(cc).IsConnected() ||
    ...
    absl::Status TensorsToLandmarksCalculator::Process(CalculatorContext* cc) {
        ...
        bool flip_horizontally = kFlipHorizontally(cc).GetOr(options_.flip_horizontally());
        ...
        for (int ld = 0; ld < num_landmarks_; ++ld) {
            ...
            if (num_dimensions > 1) {
                if (flip_vertically) {
                    landmark->set_y(options_.input_image_height() - raw_landmarks[offset + 1]);
                } else {
            ...
            if (num_dimensions > 3) {
                landmark->set_visibility(ApplyActivation(options_.visibility_activation(), raw_landmarks[offset + 3]));
            }
            if (num_dimensions > 4) {
                landmark->set_presence(ApplyActivation(options_.presence_activation(), raw_landmarks[offset + 4]));
            }
        }
        ...
        if (kOutNormalizedLandmarkList(cc).IsConnected()) {
            ...
            for (int i = 0; i < output_landmarks.landmark_size(); ++i) {
                ...
                norm_landmark->set_z(landmark.z() / options_.input_image_width() / options_.normalize_z());

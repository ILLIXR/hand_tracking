...
namespace {
    ...
    public:
        ImageToTensorOpenCvConverter(BorderMode border_mode, Tensor::ElementType tensor_type, cv::InterpolationFlags flags)
        ...
        absl::Status Convert(const mediapipe::Image& input, const RotatedRect& roi, float range_min, float range_max,
            ...
            const bool is_supported_format = input.image_format() == mediapipe::ImageFormat::SRGB ||
                input.image_format() == mediapipe::ImageFormat::SRGBA || input.image_format() == mediapipe::ImageFormat::GRAY8;
            if (!is_supported_format) {
                return absl::InvalidArgumentError(
                    absl::StrCat("Unsupported format: ", static_cast<uint32_t>(input.image_format())));
            }

            RET_CHECK_GE(tensor_buffer_offset, 0) << "The input tensor_buffer_offset needs to be non-negative.";
            ...
            const int num_elements_per_img = output_height * output_width * output_channels;
            auto      buffer_view          = output_tensor.GetCpuWriteView();
            cv::Mat   dst;
            const int dst_data_type = output_channels == 1 ? mat_gray_type_ : mat_type_;
            switch (tensor_type_) {
            case Tensor::ElementType::kInt8:
                RET_CHECK_GE((size_t) output_shape.num_elements(), tensor_buffer_offset / sizeof(int8_t) + num_elements_per_img)
                ...
                dst = cv::Mat(output_height, output_width, dst_data_type,
                              buffer_view.buffer<int8_t>() + tensor_buffer_offset / sizeof(int8_t));
                break;
            case Tensor::ElementType::kFloat32:
                RET_CHECK_GE((size_t) output_shape.num_elements(), tensor_buffer_offset / sizeof(float) + num_elements_per_img)
                    << "The buffer offset + the input image size is larger than the "
                       "allocated tensor buffer.";
                dst = cv::Mat(output_height, output_width, dst_data_type,
                ...
                break;
            case Tensor::ElementType::kUInt8:
                RET_CHECK_GE((size_t) output_shape.num_elements(),
                             tensor_buffer_offset / sizeof(uint8_t) + num_elements_per_img)
                    << "The buffer offset + the input image size is larger than the "
                       "allocated tensor buffer.";
                dst = cv::Mat(output_height, output_width, dst_data_type,
                              buffer_view.buffer<uint8_t>() + tensor_buffer_offset / sizeof(uint8_t));
                break;
            default:
                return absl::InvalidArgumentError(absl::StrCat("Unsupported tensor type: ", tensor_type_));
            }
    ...
    float dst_corners[8] = {0.0f,      dst_height,
            ...
            cv::Mat transformed;
            cv::warpPerspective(*src, transformed, projection_matrix, cv::Size((int) dst_width, (int) dst_height),
                                /*flags=*/flags_,
            ...
            transformed.convertTo(dst, dst_data_type, transform.scale, transform.offset);
    ...
    private:
        absl::Status ValidateTensorShape(const Tensor::Shape& output_shape) {
            ...
            RET_CHECK_GE(output_shape.dims[0], 1) << "The batch dimension needs to be equal or larger than 1.";
...
absl::StatusOr<std::unique_ptr<ImageToTensorConverter>> CreateOpenCvConverter(CalculatorContext* cc, BorderMode border_mode,
    ...
    UNUSED(cc);
    if (tensor_type != Tensor::ElementType::kInt8 && tensor_type != Tensor::ElementType::kFloat32 &&
        tensor_type != Tensor::ElementType::kUInt8) {
        return absl::InvalidArgumentError(absl::StrCat("Tensor type is currently not supported by "
                                                       "ImageToTensorOpenCvConverter, type: ",
                                                       tensor_type));
    }
    return std::make_unique<ImageToTensorOpenCvConverter>(border_mode, tensor_type, flags);
}

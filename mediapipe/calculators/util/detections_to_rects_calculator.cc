...
namespace {
    ...
    absl::Status NormRectFromKeyPoints(const LocationData& location_data, NormalizedRect* rect) {
        RET_CHECK_GT(location_data.relative_keypoints_size(), 1) << "2 or more key points required to calculate a rect.";
...
absl::Status DetectionsToRectsCalculator::DetectionToRect(const Detection& detection, const DetectionSpec& detection_spec,
                                                          Rect* rect) {
    ...
    case mediapipe::DetectionsToRectsCalculatorOptions_ConversionMode_USE_KEYPOINTS: {
        RET_CHECK(detection_spec.image_size.has_value()) << "Rect with absolute coordinates calculation requires image size.";
...
absl::Status DetectionsToRectsCalculator::DetectionToNormalizedRect(const Detection&     detection,
                                                                    const DetectionSpec& detection_spec, NormalizedRect* rect) {
...
absl::Status DetectionsToRectsCalculator::GetContract(CalculatorContract* cc) {
    RET_CHECK(cc->Inputs().HasTag(kDetectionTag) ^ cc->Inputs().HasTag(kDetectionsTag))
        << "Exactly one of DETECTION or DETECTIONS input stream should be "
           "provided.";
    RET_CHECK_EQ((cc->Outputs().HasTag(kNormRectTag) ? 1 : 0) + (cc->Outputs().HasTag(kRectTag) ? 1 : 0) +
                     (cc->Outputs().HasTag(kNormRectsTag) ? 1 : 0) + (cc->Outputs().HasTag(kRectsTag) ? 1 : 0),
                 1)
...
absl::Status DetectionsToRectsCalculator::Open(CalculatorContext* cc) {
    ...
    if (options_.has_rotation_vector_start_keypoint_index()) {
        ...
        RET_CHECK(options_.has_rotation_vector_target_angle() ^ options_.has_rotation_vector_target_angle_degrees());
        ...
        if (options_.has_rotation_vector_target_angle()) {
            ...
            target_angle_ = M_PI * options_.rotation_vector_target_angle_degrees() / 180.f;
        }
    ...
    output_zero_rect_for_empty_detections_ = options_.output_zero_rect_for_empty_detections();

    return absl::OkStatus();
}

absl::Status DetectionsToRectsCalculator::Process(CalculatorContext* cc) {
    if (cc->Inputs().HasTag(kDetectionTag) && cc->Inputs().Tag(kDetectionTag).IsEmpty()) {
        return absl::OkStatus();
    }
    ...
    if (cc->Inputs().HasTag(kDetectionsTag)) {
        ...
        if (detections.empty()) {
            if (output_zero_rect_for_empty_detections_) {
                if (cc->Outputs().HasTag(kRectTag)) {
                    cc->Outputs().Tag(kRectTag).AddPacket(MakePacket<Rect>().At(cc->InputTimestamp()));
                }
                if (cc->Outputs().HasTag(kNormRectTag)) {
                    cc->Outputs().Tag(kNormRectTag).AddPacket(MakePacket<NormalizedRect>().At(cc->InputTimestamp()));
                }
                if (cc->Outputs().HasTag(kNormRectsTag)) {
                    auto rect_vector = absl::make_unique<std::vector<NormalizedRect>>();
                    rect_vector->emplace_back(NormalizedRect());
                    cc->Outputs().Tag(kNormRectsTag).Add(rect_vector.release(), cc->InputTimestamp());
                }
            }
            return absl::OkStatus();
        }
    ...
    if (cc->Outputs().HasTag(kRectTag)) {
        ...
        MP_RETURN_IF_ERROR(DetectionToRect(detections[0], detection_spec, output_rect.get()));
        if (rotate_) {
            float rotation;
            MP_RETURN_IF_ERROR(ComputeRotation(detections[0], detection_spec, &rotation));
            output_rect->set_rotation(rotation);
        }
        cc->Outputs().Tag(kRectTag).Add(output_rect.release(), cc->InputTimestamp());
    }
    if (cc->Outputs().HasTag(kNormRectTag)) {
        auto output_rect = absl::make_unique<NormalizedRect>();
        MP_RETURN_IF_ERROR(DetectionToNormalizedRect(detections[0], detection_spec, output_rect.get()));
        if (rotate_) {
            float rotation;
            MP_RETURN_IF_ERROR(ComputeRotation(detections[0], detection_spec, &rotation));
            output_rect->set_rotation(rotation);
    ...
    if (cc->Outputs().HasTag(kRectsTag)) {
        ...
        for (int i = 0; i < (int) detections.size(); ++i) {
            MP_RETURN_IF_ERROR(DetectionToRect(detections[i], detection_spec, &(output_rects->at(i))));
            if (rotate_) {
                float rotation;
                MP_RETURN_IF_ERROR(ComputeRotation(detections[i], detection_spec, &rotation));
    ...
    if (cc->Outputs().HasTag(kNormRectsTag)) {
        auto output_rects = absl::make_unique<std::vector<NormalizedRect>>(detections.size());
        for (int i = 0; i < (int) detections.size(); ++i) {
            MP_RETURN_IF_ERROR(DetectionToNormalizedRect(detections[i], detection_spec, &(output_rects->at(i))));
            if (rotate_) {
                float rotation;
                MP_RETURN_IF_ERROR(ComputeRotation(detections[i], detection_spec, &rotation));
                output_rects->at(i).set_rotation(rotation);
...
absl::Status DetectionsToRectsCalculator::ComputeRotation(const Detection& detection, const DetectionSpec& detection_spec,
                                                          float* rotation) {
    ...
    const float x0 = location_data.relative_keypoints(start_keypoint_index_).x() * (float) image_size->first;
    const float y0 = location_data.relative_keypoints(start_keypoint_index_).y() * (float) image_size->second;
    const float x1 = location_data.relative_keypoints(end_keypoint_index_).x() * (float) image_size->first;
    const float y1 = location_data.relative_keypoints(end_keypoint_index_).y() * (float) image_size->second;
...
DetectionSpec DetectionsToRectsCalculator::GetDetectionSpec(const CalculatorContext* cc) {

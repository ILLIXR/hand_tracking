...
public:
    ...
    DetectionsToRenderDataCalculator(const DetectionsToRenderDataCalculator&)            = delete;
    DetectionsToRenderDataCalculator& operator=(const DetectionsToRenderDataCalculator&) = delete;
...
private:
    ...
    static void SetRenderAnnotationColorThickness(const DetectionsToRenderDataCalculatorOptions& options,
                                                  RenderAnnotation*                              render_annotation);

    static void SetTextCoordinate(bool normalized, double left, double baseline, RenderAnnotation::Text* text);

    static void SetRectCoordinate(bool normalized, double xmin, double ymin, double width, double height,
                                  RenderAnnotation::Rectangle* rect);

    static void AddLabels(const Detection& detection, const DetectionsToRenderDataCalculatorOptions& options,
                          float text_line_height, RenderData* render_data);
    static void AddFeatureTag(const Detection& detection, const DetectionsToRenderDataCalculatorOptions& options,
                              float text_line_height, RenderData* render_data);
    static void AddLocationData(const Detection& detection, const DetectionsToRenderDataCalculatorOptions& options,
                                RenderData* render_data);
    static void AddDetectionToRenderData(const Detection& detection, const DetectionsToRenderDataCalculatorOptions& options,
                                         RenderData* render_data);
};

REGISTER_CALCULATOR(DetectionsToRenderDataCalculator);

absl::Status DetectionsToRenderDataCalculator::GetContract(CalculatorContract* cc) {
    RET_CHECK(cc->Inputs().HasTag(kDetectionListTag) || cc->Inputs().HasTag(kDetectionsTag) ||
...
absl::Status DetectionsToRenderDataCalculator::Process(CalculatorContext* cc) {
    ...
    const bool  has_detection_from_list =
        cc->Inputs().HasTag(kDetectionListTag) && !cc->Inputs().Tag(kDetectionListTag).Get<DetectionList>().detection().empty();
    const bool has_detection_from_vector =
        cc->Inputs().HasTag(kDetectionsTag) && !cc->Inputs().Tag(kDetectionsTag).Get<std::vector<Detection>>().empty();
    const bool has_single_detection = cc->Inputs().HasTag(kDetectionTag) && !cc->Inputs().Tag(kDetectionTag).IsEmpty();
    if (!options.produce_empty_packet() && !has_detection_from_list && !has_detection_from_vector && !has_single_detection) {
        return absl::OkStatus();
    }
    ...
    if (has_detection_from_list) {
        for (const auto& detection : cc->Inputs().Tag(kDetectionListTag).Get<DetectionList>().detection()) {
            AddDetectionToRenderData(detection, options, render_data.get());
        }
    }
    if (has_detection_from_vector) {
        for (const auto& detection : cc->Inputs().Tag(kDetectionsTag).Get<std::vector<Detection>>()) {
            AddDetectionToRenderData(detection, options, render_data.get());
        }
    }
    if (has_single_detection) {
        AddDetectionToRenderData(cc->Inputs().Tag(kDetectionTag).Get<Detection>(), options, render_data.get());
    }
...
void DetectionsToRenderDataCalculator::SetTextCoordinate(bool normalized, double left, double baseline,
...
void DetectionsToRenderDataCalculator::SetRectCoordinate(bool normalized, double xmin, double ymin, double width, double height,
                                                         RenderAnnotation::Rectangle* rect) {
...
void DetectionsToRenderDataCalculator::AddLabels(const Detection&                               detection,
                                                 const DetectionsToRenderDataCalculatorOptions& options, float text_line_height,
                                                 RenderData* render_data) {
    ABSL_CHECK(detection.label().empty() || detection.label_id().empty() || detection.label_size() == detection.label_id_size())
    ...
    ABSL_CHECK_EQ(detection.score_size(), num_labels) << "Number of scores and labels should match for detection.";

    // Extracts all "label(_id),score" for the detection.
    std::vector<std::string> label_and_scores = {};
    for (int i = 0; i < num_labels; ++i) {
        std::string label_str = detection.label().empty() ? absl::StrCat(detection.label_id(i)) : detection.label(i);
        const float rounded_score =
            std::round(detection.score(i) * kNumScoreDecimalDigitsMultipler) / kNumScoreDecimalDigitsMultipler;
        std::string label_and_score =
            absl::StrCat(label_str, options.text_delimiter(), rounded_score, options.text_delimiter());
        label_and_scores.push_back(label_and_score);
    }
    std::vector<std::string> labels;
    if (options.render_detection_id()) {
        const std::string detection_id_str = absl::StrCat("Id: ", detection.detection_id());
        labels.push_back(detection_id_str);
    }
    if (options.one_label_per_line()) {
        labels.insert(labels.end(), label_and_scores.begin(), label_and_scores.end());
    } else {
        ...
        *text      = options.text();
        ...
        if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
            SetTextCoordinate(false, detection.location_data().bounding_box().xmin(),
                              detection.location_data().bounding_box().ymin() + (i + 1) * text_line_height, text);
        } else {
            text->set_font_height(text_line_height * 0.9);
            SetTextCoordinate(true, detection.location_data().relative_bounding_box().xmin(),
                              detection.location_data().relative_bounding_box().ymin() + (i + 1) * text_line_height, text);
        }
    }
}

void DetectionsToRenderDataCalculator::AddFeatureTag(const Detection&                               detection,
    ...
    if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
        SetTextCoordinate(false, detection.location_data().bounding_box().xmin(),
                          detection.location_data().bounding_box().ymin() + detection.location_data().bounding_box().height(),
                          feature_tag_text);
    } else {
...
void DetectionsToRenderDataCalculator::AddLocationData(const Detection&                               detection,
                                                       const DetectionsToRenderDataCalculatorOptions& options,
                                                       RenderData*                                    render_data) {
    ...
    if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
        SetRectCoordinate(false, detection.location_data().bounding_box().xmin(),
                          detection.location_data().bounding_box().ymin(), detection.location_data().bounding_box().width(),
                          detection.location_data().bounding_box().height(), location_data_rect);
    } else {
        SetRectCoordinate(true, detection.location_data().relative_bounding_box().xmin(),
                          detection.location_data().relative_bounding_box().ymin(),
                          detection.location_data().relative_bounding_box().width(),
                          detection.location_data().relative_bounding_box().height(), location_data_rect);
        ...
        if (detection.location_data().relative_keypoints_size()) {
            for (int i = 0; i < detection.location_data().relative_keypoints_size(); ++i) {
                ...
                keypoint_data->set_x(detection.location_data().relative_keypoints(i).x());
                keypoint_data->set_y(detection.location_data().relative_keypoints(i).y());
            }
        }
    }
}

void DetectionsToRenderDataCalculator::AddDetectionToRenderData(const Detection&                               detection,
                                                                const DetectionsToRenderDataCalculatorOptions& options,
                                                                RenderData*                                    render_data) {
    ABSL_CHECK(detection.location_data().format() == LocationData::BOUNDING_BOX ||
               detection.location_data().format() == LocationData::RELATIVE_BOUNDING_BOX)
    ...
    if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
        ...
        text_line_height = detection.location_data().relative_bounding_box().height() *
            std::min(kLabelToBoundingBoxRatio,
                     1 / (double) (std::max(detection.label_size(), detection.label_id_size()) + 1 /* for feature_tag */));
    }

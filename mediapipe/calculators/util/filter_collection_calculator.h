...
public:
    ...
    absl::Status Process(CalculatorContext* cc) override {
        ...
        const std::vector<bool>& filter_by = cc->Inputs().Tag("CONDITION").Get<std::vector<bool>>();

        return FilterCollection<IterableT>(std::is_copy_constructible<typename IterableT::value_type>(), cc, filter_by);
    }

    template<typename IterableU>
    absl::Status FilterCollection(std::true_type, CalculatorContext* cc, const std::vector<bool>& filter_by) {
        const IterableU& input = cc->Inputs().Tag("ITERABLE").Get<IterableU>();
        if (input.size() != filter_by.size()) {
            return absl::InternalError(
                absl::StrCat("Input vector size: ", input.size(), " doesn't mach condition vector size: ", filter_by.size()));
        }
    ...
    template<typename IterableU>
    absl::Status FilterCollection(std::false_type, CalculatorContext* cc, const std::vector<bool>& filter_by) {
        UNUSED(cc);
        UNUSED(filter_by);

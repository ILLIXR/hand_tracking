...
absl::Status RectTransformationCalculator::GetContract(CalculatorContract* cc) {
    RET_CHECK_EQ((cc->Inputs().HasTag(kNormRectTag) ? 1 : 0) + (cc->Inputs().HasTag(kNormRectsTag) ? 1 : 0) +
                     (cc->Inputs().HasTag(kRectTag) ? 1 : 0) + (cc->Inputs().HasTag(kRectsTag) ? 1 : 0),
                 1);
...
absl::Status RectTransformationCalculator::Process(CalculatorContext* cc) {
    if (cc->Inputs().HasTag(kRectTag) && !cc->Inputs().Tag(kRectTag).IsEmpty()) {
        ...
        TransformRect(&rect);
        cc->Outputs().Index(0).AddPacket(MakePacket<Rect>(rect).At(cc->InputTimestamp()));
    }
    if (cc->Inputs().HasTag(kRectsTag) && !cc->Inputs().Tag(kRectsTag).IsEmpty()) {
    ...
    if (HasTagValue(cc->Inputs(), kNormRectTag) && HasTagValue(cc->Inputs(), kImageSizeTag)) {
        ...
        cc->Outputs().Index(0).AddPacket(MakePacket<NormalizedRect>(rect).At(cc->InputTimestamp()));
    }
    if (HasTagValue(cc->Inputs(), kNormRectsTag) && HasTagValue(cc->Inputs(), kImageSizeTag)) {
        auto        rects        = cc->Inputs().Tag(kNormRectsTag).Get<std::vector<NormalizedRect>>();
        const auto& image_size   = cc->Inputs().Tag(kImageSizeTag).Get<std::pair<int, int>>();
        auto        output_rects = absl::make_unique<std::vector<NormalizedRect>>(rects.size());
        for (int i = 0; i < (int) rects.size(); ++i) {
            output_rects->at(i) = rects[i];
            auto it             = output_rects->begin() + i;
...
void RectTransformationCalculator::TransformRect(Rect* rect) {
    ...
    if (rotation == 0.f) {
        ...
        const float x_shift =
            width * options_.shift_x() * std::cos(rotation) - height * options_.shift_y() * std::sin(rotation);
        const float y_shift =
            width * options_.shift_x() * std::sin(rotation) + height * options_.shift_y() * std::cos(rotation);
...
void RectTransformationCalculator::TransformNormalizedRect(NormalizedRect* rect, int image_width, int image_height) {
    ...
    if (rotation == 0.f) {
        ...
        const float x_shift = ((float) image_width * width * options_.shift_x() * std::cos(rotation) -
                               (float) image_height * height * options_.shift_y() * std::sin(rotation)) /
            (float) image_width;
        const float y_shift = ((float) image_width * width * options_.shift_x() * std::sin(rotation) +
    ...
    if (options_.square_long()) {
        const float long_side = std::max(width * (float) image_width, height * (float) image_height);
        width                 = long_side / (float) image_width;
        height                = long_side / (float) image_height;
    } else if (options_.square_short()) {
        const float short_side = std::min(width * (float) image_width, height * (float) image_height);

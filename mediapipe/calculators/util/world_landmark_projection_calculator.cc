...
public:
    ...
    absl::Status Process(CalculatorContext* cc) override {
        ...
        if (cc->Inputs().Tag(kLandmarksTag).IsEmpty() ||
            (cc->Inputs().HasTag(kRectTag) && cc->Inputs().Tag(kRectTag).IsEmpty())) {
            return absl::OkStatus();
        }

        const auto&                                     in_landmarks = cc->Inputs().Tag(kLandmarksTag).Get<LandmarkList>();
        ...
        if (cc->Inputs().HasTag(kRectTag)) {
            ...
            rotate_fn           = [cosa, sina](const Landmark& in_landmark, Landmark* out_landmark) {
        ...
        cc->Outputs().Tag(kLandmarksTag).Add(out_landmarks.release(), cc->InputTimestamp());

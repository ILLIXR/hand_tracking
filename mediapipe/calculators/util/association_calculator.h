...
public:
    ...
    absl::Status Process(CalculatorContext* cc) override {
        ...
        if (has_prev_input_stream_ && !cc->Inputs().Get(prev_input_stream_id_).IsEmpty()) {
            ...
            const std::vector<T>& prev_input_vec = cc->Inputs().Get(prev_input_stream_id_).template Get<std::vector<T>>();

            MP_RETURN_IF_ERROR(PropagateIdsFromPreviousToCurrent(prev_input_vec, &result));
        }
...
protected:
    ...
    virtual std::pair<bool, int> GetId(const T& input) {
        UNUSED(input);
        return {false, -1};
    }
...
private:
    ...
    absl::StatusOr<std::list<T>> GetNonOverlappingElements(CalculatorContext* cc) {
        std::list<T> result;
        ...
        for (CollectionItemId id = non_empty_id + 1; id < cc->Inputs().EndId(); ++id) {
            if (id == prev_input_stream_id_ || cc->Inputs().Get(id).IsEmpty()) {
                continue;
            }
            const std::vector<T>& input_vec = cc->Inputs().Get(id).Get<std::vector<T>>();
    ...
    absl::Status PropagateIdsFromPreviousToCurrent(const std::vector<T>& prev_input_vec, std::list<T>* current) {

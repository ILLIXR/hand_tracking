...
namespace {
    ...
    inline Color MixColors(const Color& color1, const Color& color2, float color1_weight) {
        Color color;
        color.set_r(static_cast<int>(color1.r() * color1_weight + color2.r() * (1.f - color1_weight)));
        color.set_g(static_cast<int>(color1.g() * color1_weight + color2.g() * (1.f - color1_weight)));
        color.set_b(static_cast<int>(color1.b() * color1_weight + color2.b() * (1.f - color1_weight)));
        return color;
    }
    ...
    bool IsLandmarkVisibleAndPresent(const LandmarkType& landmark, bool utilize_visibility, float visibility_threshold,
                                     bool utilize_presence, float presence_threshold) {
        if (utilize_visibility && landmark.has_visibility() && landmark.visibility() < visibility_threshold) {
            return false;
        }
        if (utilize_presence && landmark.has_presence() && landmark.presence() < presence_threshold) {
            return false;
        }
        return true;
    }

    void SetColorSizeValueFromZ(float z, float z_min, float z_max, RenderAnnotation* render_annotation,
                                float min_depth_circle_thickness, float max_depth_circle_thickness) {
        ...
        const int   thickness = static_cast<int>(min_depth_circle_thickness + (1.f - Remap(z, z_min, z_max, 1)) * scale);
        render_annotation->set_thickness(thickness);
    }

    template<class LandmarkType>
    void AddConnectionToRenderData(const LandmarkType& start, const LandmarkType& end, const Color& color_start,
                                   const Color& color_end, float thickness, bool normalized, RenderData* render_data) {
        auto*                           connection_annotation = render_data->add_render_annotations();
        RenderAnnotation::GradientLine* line                  = connection_annotation->mutable_gradient_line();
    ...
    template<class LandmarkListType, class LandmarkType>
    void AddConnectionsWithDepth(const LandmarkListType& landmarks, const std::vector<int>& landmark_connections,
                                 bool utilize_visibility, float visibility_threshold, bool utilize_presence,
                                 float presence_threshold, float thickness, bool normalized, float min_z, float max_z,
                                 const Color& min_depth_line_color, const Color& max_depth_line_color,
        ...
        for (int i = 0; i < (int) landmark_connections.size(); i += 2) {
            ...
            if (!IsLandmarkVisibleAndPresent<LandmarkType>(ld0, utilize_visibility, visibility_threshold, utilize_presence,
                                                           presence_threshold) ||
                !IsLandmarkVisibleAndPresent<LandmarkType>(ld1, utilize_visibility, visibility_threshold, utilize_presence,
                                                           presence_threshold)) {
                continue;
            }
            const Color color0 = MixColors(min_depth_line_color, max_depth_line_color, Remap(ld0.z(), min_z, max_z, 1.f));
            const Color color1 = MixColors(min_depth_line_color, max_depth_line_color, Remap(ld1.z(), min_z, max_z, 1.f));
            AddConnectionToRenderData<LandmarkType>(ld0, ld1, color0, color1, thickness, normalized, render_data);
        }
    }

    template<class LandmarkType>
    void AddConnectionToRenderData(const LandmarkType& start, const LandmarkType& end, const Color& connection_color,
                                   float thickness, bool normalized, RenderData* render_data) {
        auto*                   connection_annotation = render_data->add_render_annotations();
    ...
    template<class LandmarkListType, class LandmarkType>
    void AddConnections(const LandmarkListType& landmarks, const std::vector<int>& landmark_connections,
                        bool utilize_visibility, float visibility_threshold, bool utilize_presence, float presence_threshold,
                        const Color& connection_color, float thickness, bool normalized, RenderData* render_data) {
        for (int i = 0; i < (int) landmark_connections.size(); i += 2) {
            if (landmark_connections[i] >= landmarks.landmark_size() ||
                landmark_connections[i + 1] >= landmarks.landmark_size()) {
                continue;
            ...
            AddConnectionToRenderData<LandmarkType>(ld0, ld1, connection_color, thickness, normalized, render_data);
        }
    }

    RenderAnnotation* AddPointRenderData(const Color& landmark_color, float thickness, RenderData* render_data) {
...
absl::Status LandmarksToRenderDataCalculator::GetContract(CalculatorContract* cc) {
    RET_CHECK(cc->Inputs().HasTag(kLandmarksTag) || cc->Inputs().HasTag(kNormLandmarksTag))
...
absl::Status LandmarksToRenderDataCalculator::Process(CalculatorContext* cc) {
    ...
    const Color min_depth_line_color =
        options_.has_min_depth_line_color() ? options_.min_depth_line_color() : DefaultMinDepthLineColor();
    const Color max_depth_line_color =
        options_.has_max_depth_line_color() ? options_.max_depth_line_color() : DefaultMaxDepthLineColor();
    ...
    if (cc->Inputs().HasTag(kLandmarksTag)) {
        ...
        if (visualize_depth) {
            AddConnectionsWithDepth<LandmarkList, Landmark>(
                landmarks, landmark_connections_, options_.utilize_visibility(), options_.visibility_threshold(),
                options_.utilize_presence(), options_.presence_threshold(), thickness, /*normalized=*/false, z_min, z_max,
                min_depth_line_color, max_depth_line_color, render_data);
        } else {
            AddConnections<LandmarkList, Landmark>(landmarks, landmark_connections_, options_.utilize_visibility(),
        ...
        if (options_.render_landmarks()) {
            for (int i = 0; i < landmarks.landmark_size(); ++i) {
                ...
                if (!IsLandmarkVisibleAndPresent<Landmark>(landmark, options_.utilize_visibility(),
                                                           options_.visibility_threshold(), options_.utilize_presence(),
                                                           options_.presence_threshold())) {
                    continue;
                }

                auto* landmark_data_render = AddPointRenderData(options_.landmark_color(), thickness, render_data);
                if (visualize_depth) {
                    SetColorSizeValueFromZ(landmark.z(), z_min, z_max, landmark_data_render,
                                           options_.min_depth_circle_thickness(), options_.max_depth_circle_thickness());
                }
    ...
    if (cc->Inputs().HasTag(kNormLandmarksTag)) {
        const NormalizedLandmarkList& landmarks = cc->Inputs().Tag(kNormLandmarksTag).Get<NormalizedLandmarkList>();
        ...
        if (visualize_depth) {
            AddConnectionsWithDepth<NormalizedLandmarkList, NormalizedLandmark>(
                landmarks, landmark_connections_, options_.utilize_visibility(), options_.visibility_threshold(),
                options_.utilize_presence(), options_.presence_threshold(), thickness, /*normalized=*/true, z_min, z_max,
                min_depth_line_color, max_depth_line_color, render_data);
        } else {
            AddConnections<NormalizedLandmarkList, NormalizedLandmark>(
                landmarks, landmark_connections_, options_.utilize_visibility(), options_.visibility_threshold(),
                options_.utilize_presence(), options_.presence_threshold(), options_.connection_color(), thickness,
        ...
        if (options_.render_landmarks()) {
            for (int i = 0; i < landmarks.landmark_size(); ++i) {
                ...
                if (!IsLandmarkVisibleAndPresent<NormalizedLandmark>(
                        landmark, options_.utilize_visibility(), options_.visibility_threshold(), options_.utilize_presence(),
                        options_.presence_threshold())) {
                    continue;
                }
    ...
    cc->Outputs().Tag(kRenderDataTag).Add(render_point_data.release(), cc->InputTimestamp());
    return absl::OkStatus();
}

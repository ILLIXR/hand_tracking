...
namespace {
    ...
    bool RetainMaxScoringLabelOnly(Detection* detection) {
        ...
        ABSL_CHECK(detection->label_id_size() == detection->score_size() || detection->label_size() == detection->score_size())
    ...
    float OverlapSimilarity(const NonMaxSuppressionCalculatorOptions::OverlapType overlap_type, const Rectangle_f& rect1,
    ...
    float OverlapSimilarity(const int frame_width, const int frame_height,
                            const NonMaxSuppressionCalculatorOptions::OverlapType overlap_type, const Location& location1,
    ...
    float OverlapSimilarity(const NonMaxSuppressionCalculatorOptions::OverlapType overlap_type, const Location& location1,
                            const Location& location2) {
...
public:
    ...
    absl::Status Open(CalculatorContext* cc) override {
        ...
        ABSL_CHECK_GT(options_.num_detection_streams(), 0) << "At least one detection stream need to be specified.";
        ABSL_CHECK_NE(options_.max_num_detections(), 0) << "max_num_detections=0 is not a valid value. Please choose a "
    ...
    absl::Status Process(CalculatorContext* cc) override {
        ...
        const int max_num_detections =
            (options_.max_num_detections() > -1) ? options_.max_num_detections() : static_cast<int>(indexed_scores.size());
        ...
        if (options_.algorithm() == NonMaxSuppressionCalculatorOptions::WEIGHTED) {
            WeightedNonMaxSuppression(indexed_scores, pruned_detections, max_num_detections, cc, retained_detections);
        } else {
            NonMaxSuppression(indexed_scores, pruned_detections, max_num_detections, cc, retained_detections);
        }
...
private:
    void NonMaxSuppression(const IndexedScores& indexed_scores, const Detections& detections, int max_num_detections,
        ...
        for (const auto& indexed_score : indexed_scores) {
            ...
            if (options_.min_score_threshold() > 0 && detection.score(0) < options_.min_score_threshold()) {
                break;
            }
            ...
            for (const auto& retained_location : retained_locations) {
                ...
                if (cc->Inputs().HasTag(kImageTag)) {
                    ...
                    similarity =
                        OverlapSimilarity(frame.Width(), frame.Height(), options_.overlap_type(), retained_location, location);
                } else {
                    similarity = OverlapSimilarity(options_.overlap_type(), retained_location, location);
                }
    ...
    void WeightedNonMaxSuppression(const IndexedScores& indexed_scores, const Detections& detections, int max_num_detections,
                                   CalculatorContext* cc, Detections* output_detections) {
        UNUSED(cc);
        ...
        while (!remained_indexed_scores.empty()) {
            ...
            for (const auto& indexed_score : remained_indexed_scores) {
                ...
                float    similarity = OverlapSimilarity(options_.overlap_type(), rest_location, location);
                if (similarity > options_.min_suppression_threshold()) {
            ...
            if (!candidates.empty()) {
                const int          num_keypoints = detection.location_data().relative_keypoints_size();
                ...
                for (const auto& candidate : candidates) {
                    ...
                    const auto& location_data = detections[candidate.first].location_data();
                    ...
                    for (int i = 0; i < num_keypoints; ++i) {
                        keypoints[i * 2] += location_data.relative_keypoints(i).x() * candidate.second;
                        keypoints[i * 2 + 1] += location_data.relative_keypoints(i).y() * candidate.second;
                    }
                }
                auto* weighted_location = weighted_detection.mutable_location_data()->mutable_relative_bounding_box();
                weighted_location->set_xmin(w_xmin / total_score);
                weighted_location->set_ymin(w_ymin / total_score);
                weighted_location->set_width((w_xmax / total_score) - weighted_location->xmin());
                weighted_location->set_height((w_ymax / total_score) - weighted_location->ymin());
                for (int i = 0; i < num_keypoints; ++i) {
                    auto* keypoint = weighted_detection.mutable_location_data()->mutable_relative_keypoints(i);

...
absl::Status LabelsToRenderDataCalculator::GetContract(CalculatorContract* cc) {
    if (cc->Inputs().HasTag(kClassificationsTag)) {
        ...
        RET_CHECK(cc->Inputs().HasTag(kLabelsTag)) << "Must provide input stream \"LABELS\"";
...
absl::Status LabelsToRenderDataCalculator::Process(CalculatorContext* cc) {
    if (cc->Inputs().HasTag(kVideoPrestreamTag) && cc->InputTimestamp() == Timestamp::PreStream()) {
    ...
    if (cc->Inputs().HasTag(kClassificationsTag)) {
        ...
        const std::vector<std::string>& label_vector = cc->Inputs().Tag(kLabelsTag).Get<std::vector<std::string>>();
        ...
        if (cc->Inputs().HasTag(kScoresTag)) {
            std::vector<float> score_vector = cc->Inputs().Tag(kScoresTag).Get<std::vector<float>>();
    ...
    } else if (options_.location() == LabelsToRenderDataCalculatorOptions::BOTTOM_LEFT) {
    ...
    for (int i = 0; i < num_label; ++i) {
        ...
        if (cc->Inputs().HasTag(kScoresTag) || options_.display_classification_score()) {
        ...
        if (options_.outline_thickness() > 0) {
            ...
            if (options_.outline_color_size() > 0) {
                *(text->mutable_outline_color()) = options_.outline_color(i % options_.outline_color_size());
            } else {

...
namespace {
    ...
    void SetRect(bool normalized, double xmin, double ymin, double width, double height, double rotation,
...
absl::Status RectToRenderDataCalculator::GetContract(CalculatorContract* cc) {
    RET_CHECK_EQ((cc->Inputs().HasTag(kNormRectTag) ? 1 : 0) + (cc->Inputs().HasTag(kRectTag) ? 1 : 0) +
                     (cc->Inputs().HasTag(kNormRectsTag) ? 1 : 0) + (cc->Inputs().HasTag(kRectsTag) ? 1 : 0),
                 1)
...
absl::Status RectToRenderDataCalculator::Process(CalculatorContext* cc) {
    ...
    if (cc->Inputs().HasTag(kNormRectTag) && !cc->Inputs().Tag(kNormRectTag).IsEmpty()) {
        ...
        SetRect(/*normalized=*/true, rect.x_center() - rect.width() / 2.f, rect.y_center() - rect.height() / 2.f, rect.width(),
    ...
    if (cc->Inputs().HasTag(kRectTag) && !cc->Inputs().Tag(kRectTag).IsEmpty()) {
        ...
        auto*       rectangle = NewRect(options_, render_data.get());
        SetRect(/*normalized=*/false, rect.x_center() - rect.width() / 2.f, rect.y_center() - rect.height() / 2.f, rect.width(),
                rect.height(), rect.rotation(), rectangle);
    }
    if (cc->Inputs().HasTag(kNormRectsTag) && !cc->Inputs().Tag(kNormRectsTag).IsEmpty()) {
        const auto& rects = cc->Inputs().Tag(kNormRectsTag).Get<std::vector<NormalizedRect>>();
        for (auto& rect : rects) {
            auto* rectangle = NewRect(options_, render_data.get());
            SetRect(/*normalized=*/true, rect.x_center() - rect.width() / 2.f, rect.y_center() - rect.height() / 2.f,
                    rect.width(), rect.height(), rect.rotation(), rectangle);
        }
    }
    if (cc->Inputs().HasTag(kRectsTag) && !cc->Inputs().Tag(kRectsTag).IsEmpty()) {
        ...
        for (auto& rect : rects) {
            auto* rectangle = NewRect(options_, render_data.get());
            SetRect(/*normalized=*/false, rect.x_center() - rect.width() / 2.f, rect.y_center() - rect.height() / 2.f,
                    rect.width(), rect.height(), rect.rotation(), rectangle);
        }
    }

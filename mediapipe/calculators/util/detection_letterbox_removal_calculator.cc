...
public:
    static absl::Status GetContract(CalculatorContract* cc) {
        RET_CHECK(cc->Inputs().HasTag(kDetectionsTag) && cc->Inputs().HasTag(kLetterboxPaddingTag))
    ...
    absl::Status Process(CalculatorContext* cc) override {
        ...
        const auto& input_detections  = cc->Inputs().Tag(kDetectionsTag).Get<std::vector<Detection>>();
        const auto& letterbox_padding = cc->Inputs().Tag(kLetterboxPaddingTag).Get<std::array<float, 4>>();
        ...
        for (const auto& detection : input_detections) {
            ...
            LocationData::RelativeBoundingBox* relative_bbox =
                new_detection.mutable_location_data()->mutable_relative_bounding_box();

            relative_bbox->set_xmin((detection.location_data().relative_bounding_box().xmin() - left) /
                                    (1.0f - left_and_right));
            relative_bbox->set_ymin((detection.location_data().relative_bounding_box().ymin() - top) / (1.0f - top_and_bottom));
            // The size of the bounding box will change as well.
            relative_bbox->set_width(detection.location_data().relative_bounding_box().width() / (1.0f - left_and_right));
            relative_bbox->set_height(detection.location_data().relative_bounding_box().height() / (1.0f - top_and_bottom));
        ...
        cc->Outputs().Tag(kDetectionsTag).Add(output_detections.release(), cc->InputTimestamp());
        return absl::OkStatus();
    }
};

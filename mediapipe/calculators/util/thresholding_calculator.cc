...
absl::Status ThresholdingCalculator::GetContract(CalculatorContract* cc) {
    ...
    if (cc->InputSidePackets().HasTag(kThresholdTag)) {
        ...
        RET_CHECK(!cc->Inputs().HasTag(kThresholdTag)) << "Using both the threshold input side packet and input stream is not "
                                                          "supported.";
    }
...
absl::Status ThresholdingCalculator::Open(CalculatorContext* cc) {
    ...
    const auto& options = cc->Options<::mediapipe::ThresholdingCalculatorOptions>();
    if (options.has_threshold()) {
        RET_CHECK(!cc->Inputs().HasTag(kThresholdTag)) << "Using both the threshold option and input stream is not supported.";
...
absl::Status ThresholdingCalculator::Process(CalculatorContext* cc) {
    if (cc->Inputs().HasTag(kThresholdTag) && !cc->Inputs().Tag(kThresholdTag).IsEmpty()) {
        threshold_ = cc->Inputs().Tag(kThresholdTag).Get<double>();
    }

    bool accept = false;
    RET_CHECK(!cc->Inputs().Tag(kFloatTag).IsEmpty());
    accept = static_cast<double>(cc->Inputs().Tag(kFloatTag).Get<float>()) > threshold_;

    if (cc->Outputs().HasTag(kFlagTag)) {
        cc->Outputs().Tag(kFlagTag).AddPacket(MakePacket<bool>(accept).At(cc->InputTimestamp()));
    }

    if (accept && cc->Outputs().HasTag(kAcceptTag)) {
        cc->Outputs().Tag(kAcceptTag).AddPacket(MakePacket<bool>(true).At(cc->InputTimestamp()));
    }
    if (!accept && cc->Outputs().HasTag(kRejectTag)) {
        cc->Outputs().Tag(kRejectTag).AddPacket(MakePacket<bool>(false).At(cc->InputTimestamp()));
    }

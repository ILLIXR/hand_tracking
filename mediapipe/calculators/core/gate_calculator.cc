...
public:
    ...
    static absl::Status CheckAndInitAllowDisallowInputs(CalculatorContract* cc) {
        bool input_via_side_packet = cc->InputSidePackets().HasTag(kAllowTag) || cc->InputSidePackets().HasTag(kDisallowTag);
        bool input_via_stream      = cc->Inputs().HasTag(kAllowTag) || cc->Inputs().HasTag(kDisallowTag);
        ...
        if (input_via_side_packet) {
            ...
            RET_CHECK(cc->InputSidePackets().HasTag(kAllowTag) ^ cc->InputSidePackets().HasTag(kDisallowTag));
        ...
        if (input_via_stream) {
            ...
            RET_CHECK(cc->Inputs().HasTag(kAllowTag) ^ cc->Inputs().HasTag(kDisallowTag));
    ...
    absl::Status Open(CalculatorContext* cc) final {
        if (cc->InputSidePackets().HasTag(kAllowTag)) {
            ...
            allow_by_side_packet_decision_      = cc->InputSidePackets().Tag(kAllowTag).Get<bool>();
        } else if (cc->InputSidePackets().HasTag(kDisallowTag)) {
            use_side_packet_for_allow_disallow_ = true;
            allow_by_side_packet_decision_      = !cc->InputSidePackets().Tag(kDisallowTag).Get<bool>();
        }
        ...
        if (!use_side_packet_for_allow_disallow_ && !cc->Inputs().HasTag(kAllowTag) && !cc->Inputs().HasTag(kDisallowTag)) {
    ...
    absl::Status Process(CalculatorContext* cc) final {
        ...
        if (use_option_for_allow_disallow_) {
            ...
            if (cc->Inputs().HasTag(kAllowTag) && !cc->Inputs().Tag(kAllowTag).IsEmpty()) {
                allow = cc->Inputs().Tag(kAllowTag).Get<bool>();
            }
            if (cc->Inputs().HasTag(kDisallowTag) && !cc->Inputs().Tag(kDisallowTag).IsEmpty()) {
                allow = !cc->Inputs().Tag(kDisallowTag).Get<bool>();
            }
        }
        const GateCalculatorOptions::GateState new_gate_state =
            allow ? GateCalculatorOptions::GATE_ALLOW : GateCalculatorOptions::GATE_DISALLOW;

        if (cc->Outputs().HasTag(kStateChangeTag)) {
            if (last_gate_state_ != GateCalculatorOptions::GATE_UNINITIALIZED && last_gate_state_ != new_gate_state) {
                VLOG(2) << "State transition in " << cc->NodeName() << " @ " << cc->InputTimestamp().Value() << " from "
                        << ToString(last_gate_state_) << " to " << ToString(new_gate_state);
                cc->Outputs().Tag(kStateChangeTag).AddPacket(MakePacket<bool>(allow).At(cc->InputTimestamp()));
            }
        }
        ...
        if (!allow) {
            ...
            for (int i = 0; i < num_data_streams_; ++i) {
                if (!cc->Outputs().Get("", i).IsClosed() && use_side_packet_for_allow_disallow_) {
...
private:
    GateCalculatorOptions::GateState last_gate_state_ = GateCalculatorOptions::GATE_UNINITIALIZED;

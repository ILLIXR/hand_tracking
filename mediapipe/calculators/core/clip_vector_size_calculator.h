...
public:
    static absl::Status GetContract(CalculatorContract* cc) {
        ...
        if (cc->Options<::mediapipe::ClipVectorSizeCalculatorOptions>().max_vec_size() < 1) {
            return absl::InternalError("max_vec_size should be greater than or equal to 1.");
        }
    ...
    absl::Status Open(CalculatorContext* cc) override {
        ...
        max_vec_size_ = cc->Options<::mediapipe::ClipVectorSizeCalculatorOptions>().max_vec_size();
        // Override `max_vec_size` if passed as side packet.
        if (cc->InputSidePackets().NumEntries() > 0 && !cc->InputSidePackets().Index(0).IsEmpty()) {
    ...
    absl::Status ClipVectorSize(std::true_type, CalculatorContext* cc) {
        ...
        const std::vector<U>& input_vector = cc->Inputs().Index(0).Get<std::vector<U>>();
    ...
    absl::Status ConsumeAndClipVectorSize(std::true_type, CalculatorContext* cc) {
        ...
        if (input_status.ok()) {
            ...
            output->insert(output->end(), std::make_move_iterator(begin_it), std::make_move_iterator(end_it));
        } else {
    ...
    template<typename U>
    absl::Status ConsumeAndClipVectorSize(std::false_type, CalculatorContext* cc) {
        return absl::InternalError("Cannot copy or move input vectors and clip their size.");
    }

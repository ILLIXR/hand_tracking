...
template<typename T>
using IsNotCopyable = std::enable_if_t<!std::is_copy_constructible<T>::value, bool>;

template<typename T>
using IsMovable = std::enable_if_t<std::is_move_constructible<T>::value, bool>;

template<typename T>
using IsNotMovable = std::enable_if_t<!std::is_move_constructible<T>::value, bool>;
...
public:
    static absl::Status GetContract(CalculatorContract* cc) {
        ...
        const auto& options = cc->Options<::mediapipe::SplitVectorCalculatorOptions>();
        ...
        if (options.combine_outputs()) {
            ...
            if (cc->Outputs().NumEntries() != options.ranges_size()) {
                return absl::InvalidArgumentError("The number of output streams should match the number of ranges "
            ...
            for (int i = 0; i < cc->Outputs().NumEntries(); ++i) {
                if (options.ranges(i).begin() < 0 || options.ranges(i).end() < 0 ||
                    ...
                    return absl::InvalidArgumentError("Indices should be non-negative and begin index should be less "
                                                      "than the end index.");
                }
                if (options.element_only()) {
                    if (options.ranges(i).end() - options.ranges(i).begin() != 1) {
                        return absl::InvalidArgumentError("Since element_only is true, all ranges should be of size 1.");
                    }
    ...
    absl::Status ProcessCopyableElements(CalculatorContext* cc) {
        ...
        if (combine_outputs_) {
            ...
            for (int i = 0; i < (int) ranges_.size(); ++i) {
                auto elements =
                    absl::make_unique<std::vector<U>>(input.begin() + ranges_[i].first, input.begin() + ranges_[i].second);
            ...
            if (element_only_) {
                for (int i = 0; i < (int) ranges_.size(); ++i) {
                    cc->Outputs().Index(i).AddPacket(MakePacket<U>(input[ranges_[i].first]).At(cc->InputTimestamp()));
                }
            } else {
                for (int i = 0; i < (int) ranges_.size(); ++i) {
                    auto output =
                        absl::make_unique<std::vector<T>>(input.begin() + ranges_[i].first, input.begin() + ranges_[i].second);
    ...
    absl::Status ProcessMovableElements(CalculatorContext* cc) {
        absl::StatusOr<std::unique_ptr<std::vector<U>>> input_status = cc->Inputs().Index(0).Value().Consume<std::vector<U>>();
        if (!input_status.ok())
            return input_status.status();
        std::unique_ptr<std::vector<U>> input_vector = std::move(input_status).value();
        ...
        if (combine_outputs_) {
            ...
            output->reserve(total_elements_);
            for (int i = 0; i < (int) ranges_.size(); ++i) {
                output->insert(output->end(), std::make_move_iterator(input_vector->begin() + ranges_[i].first),
            ...
            if (element_only_) {
                for (int i = 0; i < (int) ranges_.size(); ++i) {
                    cc->Outputs().Index(i).AddPacket(
                        MakePacket<U>(std::move(input_vector->at(ranges_[i].first))).At(cc->InputTimestamp()));
                }
            } else {
                for (int i = 0; i < (int) ranges_.size(); ++i) {
...
private:
    static absl::Status checkRangesDontOverlap(const ::mediapipe::SplitVectorCalculatorOptions& options) {
        for (int i = 0; i < options.ranges_size() - 1; ++i) {
            for (int j = i + 1; j < options.ranges_size(); ++j) {
                ...
                if ((range_0.begin() >= range_1.begin() && range_0.begin() < range_1.end()) ||
                    (range_1.begin() >= range_0.begin() && range_1.begin() < range_0.end())) {
                    return absl::InvalidArgumentError("Ranges must be non-overlapping when using combine_outputs "
                                                      "option.");
                }
            }
        }

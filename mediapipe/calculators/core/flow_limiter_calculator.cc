...
public:
    static absl::Status GetContract(CalculatorContract* cc) {
        ...
        cc->Inputs().Tag(kOptionsTag).Set<FlowLimiterCalculatorOptions>().Optional();
    ...
    absl::Status Open(CalculatorContext* cc) final {
        ...
        if (cc->InputSidePackets().HasTag(kMaxInFlightTag)) {
            options_.set_max_in_flight(cc->InputSidePackets().Tag(kMaxInFlightTag).Get<int>());
        }
    ...
    absl::Status Process(CalculatorContext* cc) final {
        ...
        if (finished_packet.Timestamp() == cc->InputTimestamp()) {
            while (!frames_in_flight_.empty() && frames_in_flight_.front() <= finished_packet.Timestamp()) {
        ...
        if (timeout > 0 && latest_ts == cc->InputTimestamp() && latest_ts < Timestamp::Max()) {
            while (!frames_in_flight_.empty() && (latest_ts - frames_in_flight_.front()) > timeout) {
                frames_in_flight_.pop_front();
            }
        }
        ...
        if (!input_queue.empty()) {
            ...
            Timestamp bound = cc->Inputs().Get("", 0).Value().Timestamp().NextAllowedInStream();
            SetNextTimestampBound(bound, &cc->Outputs().Get("", 0));
...
private:
    ...
    Timestamp InputTimestampBound(CalculatorContext* cc) {
        ...
        for (int i = 0; i < (int) input_queues_.size(); ++i) {
            ...
            Timestamp bound  = queue.empty() ? stream.Value().Timestamp().NextAllowedInStream() : queue.front().Timestamp();
    ...
    void ProcessAuxiliaryInputs(CalculatorContext* cc) {
        ...
        for (int i = 1; i < cc->Inputs().NumEntries(""); ++i) {
            ...
            while (!input_queues_[i].empty() && input_queues_[i].front().Timestamp() < settled_bound) {
            ...
            if (!input_queues_[i].empty()) {
                ...
                Timestamp bound = cc->Inputs().Get("", i).Value().Timestamp().NextAllowedInStream();
                SetNextTimestampBound(bound, &cc->Outputs().Get("", i));
            }
        }
    }

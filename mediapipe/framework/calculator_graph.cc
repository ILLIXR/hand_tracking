...
namespace {
    ...
    constexpr absl::StatusToStringMode kStatusLogFlags =
        absl::StatusToStringMode::kWithEverything & (~absl::StatusToStringMode::kWithPayload);
...
void CalculatorGraph::GraphInputStream::SetNextTimestampBound(Timestamp timestamp) {
...
absl::Status CalculatorGraph::InitializePacketGeneratorGraph(const std::map<std::string, Packet>& side_packets) {
    // Create and initialize the output side packets.
    if (!validated_graph_->OutputSidePacketInfos().empty()) {
        output_side_packets_ = absl::make_unique<OutputSidePacketImpl[]>(validated_graph_->OutputSidePacketInfos().size());
    }
    for (int index = 0; index < (int) validated_graph_->OutputSidePacketInfos().size(); ++index) {
        const EdgeInfo& edge_info = validated_graph_->OutputSidePacketInfos()[index];
        MP_RETURN_IF_ERROR(output_side_packets_[index].Initialize(edge_info.name, edge_info.packet_type));
    }
    ...
    return packet_generator_graph_.Initialize(validated_graph_.get(), default_executor, side_packets);
}
...
absl::Status CalculatorGraph::InitializeStreams() {
    ...
    input_stream_managers_ = absl::make_unique<InputStreamManager[]>(validated_graph_->InputStreamInfos().size());
    for (int index = 0; index < (int) validated_graph_->InputStreamInfos().size(); ++index) {
        const EdgeInfo& edge_info = validated_graph_->InputStreamInfos()[index];
        MP_RETURN_IF_ERROR(
            input_stream_managers_[index].Initialize(edge_info.name, edge_info.packet_type, edge_info.back_edge));
        input_stream_to_index_[&input_stream_managers_[index]] = index;
    }

    // Create and initialize the output streams.
    output_stream_managers_ = absl::make_unique<OutputStreamManager[]>(validated_graph_->OutputStreamInfos().size());
    for (int index = 0; index < (int) validated_graph_->OutputStreamInfos().size(); ++index) {
        const EdgeInfo& edge_info = validated_graph_->OutputStreamInfos()[index];
        MP_RETURN_IF_ERROR(output_stream_managers_[index].Initialize(edge_info.name, edge_info.packet_type));
    }

    // Initialize GraphInputStreams.
    int graph_input_stream_count = 0;
    MP_ASSIGN_OR_RETURN(auto input_tag_map, tool::TagMap::Create(validated_graph_->Config().input_stream()));
    for (const auto& stream_name : input_tag_map->Names()) {
        RET_CHECK(!graph_input_streams_.contains(stream_name)).SetNoLogging()
            << "CalculatorGraph Initialization failed, graph input stream \"" << stream_name << "\" was specified twice.";
        int output_stream_index = validated_graph_->OutputStreamIndex(stream_name);
        RET_CHECK_LE(0, output_stream_index).SetNoLogging();
        const EdgeInfo& edge_info = validated_graph_->OutputStreamInfos()[output_stream_index];
        RET_CHECK(NodeTypeInfo::NodeType::GRAPH_INPUT_STREAM == edge_info.parent_node.type).SetNoLogging();

        graph_input_streams_[stream_name] = absl::make_unique<GraphInputStream>(&output_stream_managers_[output_stream_index]);

        // Assign a virtual node ID to each graph input stream so we can treat
        // these as regular nodes for throttling.
        graph_input_stream_node_ids_[stream_name] = (int) validated_graph_->CalculatorInfos().size() + graph_input_stream_count;
...
absl::Status CalculatorGraph::InitializeCalculatorNodes() {
    ...
    for (int node_id = 0; node_id < (int) validated_graph_->CalculatorInfos().size(); ++node_id) {
        ...
        const absl::Status result =
            nodes_.back()->Initialize(validated_graph_.get(), node_ref, input_stream_managers_.get(),
                                      output_stream_managers_.get(), output_side_packets_.get(), &buffer_size_hint, profiler_);
    ...
    if (!errors.empty()) {
        return tool::CombinedStatus("CalculatorGraph::InitializeCalculatorNodes failed: ", errors);
    }

    VLOG(2) << "Maximum input stream queue size based on graph config: " << max_queue_size_;
    return absl::OkStatus();
}

absl::Status CalculatorGraph::InitializePacketGeneratorNodes(const std::vector<int>& non_scheduled_generators) {
    // Do not add wrapper nodes again if we are running the graph multiple times.
    if (packet_generator_nodes_added_)
        return absl::OkStatus();
    ...
    for (int index : non_scheduled_generators) {
        ...
        int                   buffer_size_hint = 0;
        NodeTypeInfo::NodeRef node_ref(NodeTypeInfo::NodeType::PACKET_GENERATOR, index);
        nodes_.push_back(absl::make_unique<CalculatorNode>());
        const absl::Status result =
            nodes_.back()->Initialize(validated_graph_.get(), node_ref, input_stream_managers_.get(),
                                      output_stream_managers_.get(), output_side_packets_.get(), &buffer_size_hint, profiler_);
    ...
    if (!errors.empty()) {
        return tool::CombinedStatus("CalculatorGraph::InitializePacketGeneratorNodes failed: ", errors);
    }
...
absl::Status CalculatorGraph::InitializeExecutors() {
    ...
    for (const ExecutorConfig& executor_config : validated_graph_->Config().executor()) {
        ...
        if (executor_config.name().empty()) {
            ...
            if (executor_config.type().empty()) {
                ...
                default_executor_options = &executor_config.options().GetExtension(ThreadPoolExecutorOptions::ext);
                continue;
            }
        ...
        if (executor_config.type().empty()) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                << "ExecutorConfig for \"" << executor_config.name() << "\" does not have a \"type\" field. The executor \""
    ...
    MP_ASSIGN_OR_RETURN(Executor* executor,
        ...
        // clang-format on
        MEDIAPIPE_CHECK_OK(SetExecutorInternal(executor_config.name(), std::shared_ptr<Executor>(executor)));
    }

    if (!mediapipe::ContainsKey(executors_, "")) {
        MP_RETURN_IF_ERROR(InitializeDefaultExecutor(default_executor_options, use_application_thread));
    }

    return absl::OkStatus();
}

absl::Status CalculatorGraph::InitializeDefaultExecutor(const ThreadPoolExecutorOptions* default_executor_options,
    ...
    if (use_application_thread) {
        ...
        MEDIAPIPE_CHECK_OK(
            SetExecutorInternal("",
                                std::make_shared<internal::DelegatingExecutor>(std::bind(
                                    &internal::Scheduler::AddApplicationThreadTask, &scheduler_, std::placeholders::_1))));
        VLOG(1) << "Using default executor and application thread.";
        return absl::OkStatus();
    }

    // Check the number of threads specified in the proto.
    int num_threads = default_executor_options == nullptr ? 0 : default_executor_options->num_threads();
    ...
    if (num_threads == 0 || num_threads == -1) {
        num_threads = std::min(
            ...
            std::max({validated_graph_->Config().node().size(), validated_graph_->Config().packet_generator().size(), 1}));
    }
    MP_RETURN_IF_ERROR(CreateDefaultThreadPool(default_executor_options, num_threads));
    VLOG(1) << absl::StrCat("Using default executor with num_threads: ", num_threads);
    return absl::OkStatus();
}

absl::Status CalculatorGraph::Initialize(std::unique_ptr<ValidatedGraphConfig> validated_graph,
                                         const std::map<std::string, Packet>&  side_packets) {
    RET_CHECK(!initialized_).SetNoLogging() << "CalculatorGraph can be initialized only once.";
    RET_CHECK(validated_graph->Initialized()).SetNoLogging() << "validated_graph is not initialized.";
...
absl::Status CalculatorGraph::Initialize(CalculatorGraphConfig                input_config,
    ...
    MP_RETURN_IF_ERROR(validated_graph->Initialize(std::move(input_config), /*graph_registry=*/nullptr,
                                                   /*graph_options=*/nullptr, &service_manager_));
    return Initialize(std::move(validated_graph), side_packets);
}

absl::Status CalculatorGraph::Initialize(const std::vector<CalculatorGraphConfig>&   input_configs,
                                         const std::vector<CalculatorGraphTemplate>& input_templates,
                                         const std::map<std::string, Packet>& side_packets, const std::string& graph_type,
                                         const Subgraph::SubgraphOptions* options) {
    auto validated_graph = absl::make_unique<ValidatedGraphConfig>();
    MP_RETURN_IF_ERROR(validated_graph->Initialize(input_configs, input_templates, graph_type, options, &service_manager_));
    return Initialize(std::move(validated_graph), side_packets);
}

absl::Status CalculatorGraph::ObserveOutputStream(const std::string&                         stream_name,
                                                  std::function<absl::Status(const Packet&)> packet_callback,
                                                  bool                                       observe_timestamp_bounds) {
    RET_CHECK(initialized_).SetNoLogging() << "CalculatorGraph is not initialized.";
    ...
    if (output_stream_index < 0) {
        return mediapipe::NotFoundErrorBuilder(MEDIAPIPE_LOC)
            << "Unable to attach observer to output stream \"" << stream_name << "\" because it doesn't exist.";
    }
    auto observer = absl::make_unique<internal::OutputStreamObserver>();
    MP_RETURN_IF_ERROR(observer->Initialize(stream_name, &any_packet_type_, std::move(packet_callback),
                                            &output_stream_managers_[output_stream_index], observe_timestamp_bounds));
    graph_output_streams_.push_back(std::move(observer));
    return absl::OkStatus();
}

absl::Status CalculatorGraph::SetErrorCallback(std::function<void(const absl::Status&)> error_callback) {
    ...
    RET_CHECK(!initialized_) << "SetErrorCallback must be called before Initialize()";
    error_callback_ = error_callback;
    return absl::OkStatus();
}

absl::StatusOr<OutputStreamPoller> CalculatorGraph::AddOutputStreamPoller(const std::string& stream_name,
                                                                          bool               observe_timestamp_bounds) {
    RET_CHECK(initialized_).SetNoLogging() << "CalculatorGraph is not initialized.";
    int output_stream_index = validated_graph_->OutputStreamIndex(stream_name);
    if (output_stream_index < 0) {
    ...
    MP_RETURN_IF_ERROR(internal_poller->Initialize(
        ...
        std::bind(&CalculatorGraph::UpdateThrottledNodes, this, std::placeholders::_1, std::placeholders::_2),
        &output_stream_managers_[output_stream_index], observe_timestamp_bounds));
...
absl::StatusOr<Packet> CalculatorGraph::GetOutputSidePacket(const std::string& packet_name) {
    int side_packet_index = validated_graph_->OutputSidePacketIndex(packet_name);
    if (side_packet_index < 0) {
        return mediapipe::NotFoundErrorBuilder(MEDIAPIPE_LOC)
            << "Unable to get the output side packet \"" << packet_name << "\" because it doesn't exist.";
    }
    Packet output_packet;
    if (!output_side_packets_[side_packet_index].GetPacket().IsEmpty() || scheduler_.IsTerminated()) {
    ...
    if (output_packet.IsEmpty()) {
        ...
        auto current_run_side_packet_iter = current_run_side_packets_.find(packet_name);
        if (base_packet_iter != base_packets.end() && !base_packet_iter->second.IsEmpty()) {
            output_packet = base_packet_iter->second;
        } else if (current_run_side_packet_iter != current_run_side_packets_.end() &&
            ...
            return mediapipe::UnavailableErrorBuilder(MEDIAPIPE_LOC)
                << "The output side packet \"" << packet_name << "\" is unavailable.";
        }
    }
    return output_packet;
}

absl::Status CalculatorGraph::Run(const std::map<std::string, Packet>& extra_side_packets) {
...
absl::Status CalculatorGraph::StartRun(const std::map<std::string, Packet>& extra_side_packets,
...
#if !MEDIAPIPE_DISABLE_GPU
absl::Status CalculatorGraph::SetGpuResources(std::shared_ptr<::mediapipe::GpuResources> resources) {
    RET_CHECK_NE(resources, nullptr);
    auto gpu_service = service_manager_.GetServiceObject(kGpuService);
    RET_CHECK_EQ(gpu_service, nullptr) << "The GPU resources have already been configured.";
    return service_manager_.SetServiceObject(kGpuService, std::move(resources));
}

std::shared_ptr<::mediapipe::GpuResources> CalculatorGraph::GetGpuResources() const {
    return service_manager_.GetServiceObject(kGpuService);
}

static Packet GetLegacyGpuSharedSidePacket(const std::map<std::string, Packet>& side_packets) {
    auto legacy_sp_iter = side_packets.find(kGpuSharedSidePacketName);
    if (legacy_sp_iter == side_packets.end())
        return {};
...
absl::Status CalculatorGraph::MaybeSetUpGpuServiceFromLegacySidePacket(Packet legacy_sp) {
    if (legacy_sp.IsEmpty())
        return absl::OkStatus();
    auto gpu_resources = service_manager_.GetServiceObject(kGpuService);
    if (gpu_resources) {
        ABSL_LOG(WARNING) << "::mediapipe::GpuSharedData provided as a side packet while the "
...
std::map<std::string, Packet> CalculatorGraph::MaybeCreateLegacyGpuSidePacket(Packet legacy_sp) {
    std::map<std::string, Packet> additional_side_packets;
    auto                          gpu_resources = service_manager_.GetServiceObject(kGpuService);
    if (gpu_resources &&
        (legacy_sp.IsEmpty() || legacy_sp.Get<::mediapipe::GpuSharedData*>()->gpu_resources != gpu_resources)) {
        legacy_gpu_shared_                                = absl::make_unique<mediapipe::GpuSharedData>(gpu_resources);
        additional_side_packets[kGpuSharedSidePacketName] = MakePacket<::mediapipe::GpuSharedData*>(legacy_gpu_shared_.get());
    }
...
absl::Status CalculatorGraph::PrepareGpu() {
    ...
    for (const auto& name_executor : gpu_resources->GetGpuExecutors()) {
        MP_RETURN_IF_ERROR(SetExecutorInternal(name_executor.first, name_executor.second));
    }
    return absl::OkStatus();
}
#endif // !MEDIAPIPE_DISABLE_GPU
    ...
    for (const auto& node : nodes_) {
        for (const auto& [key, request] : node->Contract().ServiceRequests()) {
            ...
            if (allow_service_default_initialization_) {
                ...
                packet_or = absl::FailedPreconditionError("Service default initialization is disallowed.");
            }
            if (packet_or.ok()) {
                MP_RETURN_IF_ERROR(service_manager_.SetServicePacket(request.Service(), std::move(packet_or).value()));
            } else if (request.IsOptional()) {
                continue;
            } else {
                return absl::InternalError(
                    absl::StrCat("Service \"", request.Service().key, "\", required by node ", node->DebugName(),
                                 ", was not provided and cannot be created: ", std::move(packet_or).status().message()));
            }
        }
    }
    return absl::OkStatus();
...
absl::Status CalculatorGraph::PrepareForRun(const std::map<std::string, Packet>& extra_side_packets,
                                            const std::map<std::string, Packet>& stream_headers) {
    if (VLOG_IS_ON(1)) {
    ...
    if (!additional_side_packets.empty()) {
        additional_side_packets.insert(extra_side_packets.begin(), extra_side_packets.end());
    ...
    absl::Status     generator_status =
        packet_generator_graph_.RunGraphSetup(*input_side_packets, &current_run_side_packets_, &non_scheduled_generators);
    ...
    if (VLOG_IS_ON(1)) {
        ...
        VLOG(1) << "Final input side packet names are: " << absl::StrJoin(input_side_packet_names, ",");
    }
...

    {
        ...
        full_input_streams_.resize(validated_graph_->CalculatorInfos().size() + graph_input_streams_.size());
    }

    for (auto& item : graph_input_streams_) {
        item.second->PrepareForRun(std::bind(&CalculatorGraph::RecordError, this, std::placeholders::_1));
    }
    for (int index = 0; index < (int) validated_graph_->OutputSidePacketInfos().size(); ++index) {
        output_side_packets_[index].PrepareForRun(std::bind(&CalculatorGraph::RecordError, this, std::placeholders::_1));
    }
    for (auto& node : nodes_) {
        ...
        const absl::Status result = node->PrepareForRun(
            ...
            std::bind(&internal::Scheduler::ScheduleNodeForOpen, &scheduler_, node.get()),
            std::bind(&internal::Scheduler::AddNodeToSourcesQueue, &scheduler_, node.get()),
            std::bind(&internal::Scheduler::ScheduleNodeIfNotThrottled, &scheduler_, node.get(), std::placeholders::_1),
            std::bind(&CalculatorGraph::RecordError, this, std::placeholders::_1), counter_factory_.get());
        if (!result.ok()) {
            // Collect as many errors as we can before failing.
            RecordError(result);
        }
    ...
    for (const auto& name_max : graph_input_stream_max_queue_size_) {
        std::unique_ptr<GraphInputStream>* stream = mediapipe::FindOrNull(graph_input_streams_, name_max.first);
        RET_CHECK(stream).SetNoLogging() << absl::Substitute("SetInputStreamMaxQueueSize called on \"$0\" which is not a "
                                                             "graph input stream.",
                                                             name_max.first);
...
absl::Status CalculatorGraph::WaitUntilIdle() {
    if (has_sources_) {
        ABSL_LOG_FIRST_N(WARNING, 1) << "WaitUntilIdle called on a graph with source nodes, which "
...
absl::Status CalculatorGraph::AddPacketToInputStream(absl::string_view stream_name, const Packet& packet) {
...
absl::Status CalculatorGraph::SetInputStreamTimestampBound(const std::string& stream_name, Timestamp timestamp) {
    std::unique_ptr<GraphInputStream>* stream = mediapipe::FindOrNull(graph_input_streams_, stream_name);
...
template<typename T>
absl::Status CalculatorGraph::AddPacketToInputStreamInternal(absl::string_view stream_name, T&& packet) {
    auto                               stream_it = graph_input_streams_.find(stream_name);
    std::unique_ptr<GraphInputStream>* stream    = stream_it == graph_input_streams_.end() ? nullptr : &stream_it->second;
    RET_CHECK(stream).SetNoLogging() << absl::Substitute("AddPacketToInputStream called on input stream \"$0\" which is not a "
                                                         "graph input stream.",
                                                         stream_name);
    auto node_id_it = graph_input_stream_node_ids_.find(stream_name);
    ABSL_CHECK(node_id_it != graph_input_stream_node_ids_.end()) << "Map key not found: " << stream_name;
    ...
    ABSL_CHECK_GE(node_id, validated_graph_->CalculatorInfos().size());
    {
        ...
        if (graph_input_stream_add_mode_ == GraphInputStreamAddMode::ADD_IF_NOT_FULL) {
            if (has_error_) {
                absl::Status error_status;
            ...
            if (!full_input_streams_[node_id].empty()) {
                return mediapipe::UnavailableErrorBuilder(MEDIAPIPE_LOC) << "Graph is throttled.";
            }
        } else if (graph_input_stream_add_mode_ == GraphInputStreamAddMode::WAIT_TILL_NOT_FULL) {
            ...
            while (!has_error_ && !full_input_streams_[node_id].empty()) {
                ...
                scheduler_.WaitUntilGraphInputStreamUnthrottled(&full_input_streams_mutex_);
            }
            if (has_error_) {
                absl::Status error_status;
                GetCombinedErrors("Graph has errors: ", &error_status);
...
absl::Status CalculatorGraph::SetInputStreamMaxQueueSize(const std::string& stream_name, int max_queue_size) {
...
absl::Status CalculatorGraph::CloseInputStream(const std::string& stream_name) {
    std::unique_ptr<GraphInputStream>* stream = mediapipe::FindOrNull(graph_input_streams_, stream_name);
    RET_CHECK(stream).SetNoLogging() << absl::Substitute("CloseInputStream called on input stream \"$0\" which is not a graph "
                                                         "input stream.",
                                                         stream_name);
...
void CalculatorGraph::RecordError(const absl::Status& error) {
    VLOG(2) << "RecordError called with " << error;
    {
        ...
        if (errors_.size() > kMaxNumAccumulatedErrors) {
            ...
            ABSL_LOG(FATAL) << "Forcefully aborting to prevent the framework running out "
                               "of memory.";
        }
    }
    if (error_callback_) {
...
bool CalculatorGraph::GetCombinedErrors(const std::string& error_prefix, absl::Status* error_status) {
    absl::MutexLock lock(&error_mutex_);
    if (!errors_.empty()) {
...
void CalculatorGraph::CallStatusHandlers(GraphRunState graph_run_state, const absl::Status& status) {
    for (int status_handler_index = 0; status_handler_index < validated_graph_->Config().status_handler_size();
         ++status_handler_index) {
        const auto& handler_config = validated_graph_->Config().status_handler(status_handler_index);
        const auto& handler_type   = handler_config.status_handler();

        const auto&          status_handler_info = validated_graph_->StatusHandlerInfos()[status_handler_index];
        const PacketTypeSet& packet_type_set     = status_handler_info.InputSidePacketTypes();
        absl::StatusOr<std::unique_ptr<PacketSet>> packet_set_statusor =
            tool::FillPacketSet(packet_type_set, current_run_side_packets_, nullptr);
        if (!packet_set_statusor.ok()) {
            RecordError(mediapipe::StatusBuilder(std::move(packet_set_statusor).status(), MEDIAPIPE_LOC).SetPrepend()
                        << "Skipping run of " << handler_type << ": ");
            continue;
        }
        absl::StatusOr<std::unique_ptr<internal::StaticAccessToStatusHandler>> static_access_statusor =
            internal::StaticAccessToStatusHandlerRegistry::CreateByNameInNamespace(validated_graph_->Package(), handler_type);
        ...
        if (graph_run_state == GraphRunState::PRE_RUN) {
            handler_result = static_access->HandlePreRunStatus(handler_config.options(), *packet_set_statusor.value(), status);
        } else { // POST_RUN
            handler_result = static_access->HandleStatus(handler_config.options(), *packet_set_statusor.value(), status);
        }
        if (!handler_result.ok()) {
            mediapipe::StatusBuilder builder(std::move(handler_result), MEDIAPIPE_LOC);
...
void CalculatorGraph::UpdateThrottledNodes(InputStreamManager* stream, bool* stream_was_full) {
    ...
    if (node_index >= (int) validated_graph_->CalculatorInfos().size()) {
        ...
        upstream_nodes = &validated_graph_->CalculatorInfos()[node_index].AncestorSources();
    }
...

    {
        ...
        if (*stream_was_full != stream_is_full) {
            for (int node_id : *upstream_nodes) {
                VLOG(2) << "Stream \"" << stream->Name() << "\" is " << (stream_is_full ? "throttling" : "no longer throttling")
                        << " node with node ID " << node_id;
                mediapipe::LogEvent(profiler_.get(),
                                    TraceEvent(stream_is_full ? TraceEvent::THROTTLED : TraceEvent::UNTHROTTLED)
                ...
                bool is_graph_input_stream = node_id >= (int) validated_graph_->CalculatorInfos().size();
...
bool IsGraphOutputStream(InputStreamManager*                                              stream,
...
bool CalculatorGraph::UnthrottleSources() {
    ...
    for (InputStreamManager* stream : full_streams) {
        if (Config().report_deadlock()) {
            RecordError(absl::UnavailableError(absl::StrCat("Detected a deadlock due to input throttling for input stream: \"",
        ...
        ABSL_LOG_EVERY_N(WARNING, 100) << absl::StrCat(
            "Resolved a deadlock by increasing max_queue_size of input stream: \"", stream->Name(), "\" of a node \"",
            GetParentNodeDebugName(stream), "\" to ", new_size, ". Consider increasing max_queue_size for better performance.");
    }
    return !full_streams.empty();
}

CalculatorGraph::GraphInputStreamAddMode CalculatorGraph::GetGraphInputStreamAddMode() const {
    absl::MutexLock lock(&full_input_streams_mutex_);
...
absl::Status CalculatorGraph::SetExecutorInternal(const std::string& name, std::shared_ptr<Executor> executor) {
...
absl::Status CalculatorGraph::SetExecutor(const std::string& name, std::shared_ptr<Executor> executor) {
    RET_CHECK(!initialized_) << "SetExecutor can only be called before Initialize()";
    if (IsReservedExecutorName(name)) {
        return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "\"" << name << "\" is a reserved executor name.";
    }
    return SetExecutorInternal(name, std::move(executor));
}

absl::Status CalculatorGraph::CreateDefaultThreadPool(const ThreadPoolExecutorOptions* default_executor_options,
                                                      int                              num_threads) {
    MediaPipeOptions           extendable_options;
    ThreadPoolExecutorOptions* options = extendable_options.MutableExtension(ThreadPoolExecutorOptions::ext);
...
const OutputStreamManager* CalculatorGraph::FindOutputStreamManager(const std::string& name) {
    return &output_stream_managers_.get()[validated_graph_->OutputStreamIndex(name)];
}
...
std::string CalculatorGraph::GetParentNodeDebugName(InputStreamManager* stream) const {
    ...
    const EdgeInfo&              edge_info          = validated_graph_->InputStreamInfos()[input_stream_index];
    ...
    if (node_index < 0 || node_index >= config.node_size()) {
        return absl::StrCat("Unknown (node index: ", node_index, ", with input stream: ", stream->Name(), ")");
    }
...
namespace {
    void PrintTimingToInfo(const std::string& label, int64_t timer_value) {
        ...
        ABSL_LOG(INFO) << label << " took "
                       << absl::StrFormat("%02lld days, %02lld:%02lld:%02lld.%03lld (total seconds: "
                                          "%lld.%06lld)",
...
absl::Status CalculatorGraph::GetCalculatorProfiles(std::vector<CalculatorProfile>* profiles) const {

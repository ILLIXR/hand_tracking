...
namespace packet_internal {
    ...
    absl::StatusOr<Packet>             PacketFromDynamicProto(const std::string& type_name, const std::string& serialized);
} // namespace packet_internal
...
private:
    ...
    friend Packet packet_internal::Create(packet_internal::HolderBase* holder, class Timestamp timestamp);
    friend Packet packet_internal::Create(std::shared_ptr<packet_internal::HolderBase> holder, class Timestamp timestamp);
    friend const packet_internal::HolderBase*                  packet_internal::GetHolder(const Packet& packet);
    friend const std::shared_ptr<packet_internal::HolderBase>& packet_internal::GetHolderShared(const Packet& packet);
    friend std::shared_ptr<packet_internal::HolderBase>        packet_internal::GetHolderShared(Packet&& packet);
...
Packet AdoptAsUniquePtr(T* ptr) {
    static_assert(!std::is_const<T>::value, "AdoptAsUniquePtr should not be called with a pointer-to-const.");
...
class SyncedPacket {
public:
    explicit SyncedPacket(const Packet& packet)
        : packet_(packet) { }
...
template<typename T, typename std::enable_if<std::is_array<T>::value>::type* = nullptr,
         typename... Args>
Packet MakePacket(Args&&... args) { // NOLINT(build/c++11)
    return Adopt(reinterpret_cast<T*>(new T{std::forward<typename std::remove_extent<T>::type>(args)...}));
}
...
std::shared_ptr<const T> SharedPtrWithPacket(Packet packet) {
    ...
    return std::shared_ptr<const T>(ptr, [packet = std::move(packet)](const T* ptr) mutable {
        UNUSED(ptr);
        packet = {};
    });
}
...
namespace packet_internal {
    ...
    template<typename T>
    const proto_ns::MessageLite* ConvertToProtoMessageLite(const T* data, std::false_type) {
        UNUSED(data);
        return nullptr;
    }

    template<typename T>
    const proto_ns::MessageLite* ConvertToProtoMessageLite(const T* data, std::true_type) {
        return data;
    }
    ...
    struct is_proto_vector<std::vector<ItemT, Allocator>> : public std::is_base_of<proto_ns::MessageLite, ItemT>::type { };
    ...
    ConvertToVectorOfProtoMessageLitePtrs(const T* data,
        ...
        UNUSED(data);
        return absl::InvalidArgumentError(absl::StrCat("The Packet stores \"", kTypeId<T>.name(), "\"",
    ...
    class MessageHolderRegistry : public GlobalFactoryRegistry<std::unique_ptr<HolderBase>> { };

    template<typename T>
    struct is_concrete_proto_t
        : public std::integral_constant<bool,
                                        std::is_base_of<proto_ns::MessageLite, T>{} &&
    ...
    MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(MessageRegistrator, MessageHolderRegistry, T{}.GetTypeName(), CreateMessageHolder<T>)
    ...
    // up to Holder?
    template<typename T>
    struct HolderPayloadRegistrator<T, typename std::enable_if<is_concrete_proto_t<T>{}>::type>
        : private MessageRegistrator<typename std::remove_cv<T>::type> { };

    template<typename T>
    class Holder
        : public HolderBase
        , private HolderPayloadRegistrator<T> {
    public:
        ...
        absl::StatusOr<std::unique_ptr<T>>
        Release(typename std::enable_if<!std::is_array<U>::value || std::extent<U>::value != 0>::type* = 0) {
            if (HasForeignOwner()) {
                return InternalError("Foreign holder can't release data ptr without ownership.");
            }
        ...
        int64_t DebugDataId() const final {
    ...
    protected:
        ...
        const proto_ns::MessageLite* GetProtoMessageLite() override {
            return ConvertToProtoMessageLite(ptr_, std::is_base_of<proto_ns::MessageLite, T>());
        }

        // Returns a vector<MessageLite*> for the data in the holder, if the
        // underlying object is a vector of protocol buffer objects, otherwise,
        // returns an error.
        StatusOr<std::vector<const proto_ns::MessageLite*>> GetVectorOfProtoMessageLite() const override {
    ...
    private:
        ...
        template<typename U = T>
        inline void delete_helper(typename std::enable_if<!std::is_array<U>::value>::type* = 0) {
            delete ptr_;
        }
        ...
        inline void delete_helper(typename std::enable_if<std::is_array<U>::value>::type* = 0) {
            ...
            delete[] reinterpret_cast<const typename std::remove_extent<U>::type*>(ptr_);
        }
    };
    ...
    public:
        ...
        bool HasForeignOwner() const final {
            return true;
        }
    };
...
inline absl::StatusOr<std::unique_ptr<T>> Packet::Consume() {
    ...
    if (!holder_->HasForeignOwner() && holder_.use_count() == 1) {
        ...
        absl::StatusOr<std::unique_ptr<T>> release_result = holder_->As<T>()->Release();
    ...
    // error with message.
    return absl::Status(absl::StatusCode::kFailedPrecondition, "Packet isn't the sole owner of the holder.");
}

template<typename T>
inline absl::StatusOr<std::unique_ptr<T>> Packet::ConsumeOrCopy(bool* was_copied,
...
template<typename T>
inline absl::StatusOr<std::unique_ptr<T>>
Packet::ConsumeOrCopy(bool* was_copied, typename std::enable_if<std::is_array<T>::value && std::extent<T>::value != 0>::type*) {
    MP_RETURN_IF_ERROR(ValidateAsType<T>());
    // If holder is the sole owner of the underlying data, consumes this packet.
    if (!holder_->HasForeignOwner() && holder_.use_count() == 1) {
        VLOG(2) << "Consuming the data of " << DebugString();
    ...
    std::copy(std::begin(original_array), std::end(original_array), std::begin(*data_ptr));
    VLOG(2) << "Setting " << DebugString() << " to empty.";
    holder_.reset();
    if (was_copied) {
        *was_copied = true;
...
namespace packet_internal {
    ...
    inline const std::shared_ptr<HolderBase>& GetHolderShared(const Packet& packet) {

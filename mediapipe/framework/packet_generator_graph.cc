...
namespace {
    ...
    absl::Status CreateInputsForGenerator(const ValidatedGraphConfig& validated_graph, int generator_index,
                                          const std::map<std::string, Packet>& side_packets, PacketSet* input_side_packet_set,
                                          bool* unrunnable) {
        const NodeTypeInfo& node_type_info = validated_graph.GeneratorInfos()[generator_index];
        const auto&         generator_name = validated_graph.Config().packet_generator(generator_index).packet_generator();
        ...
        *unrunnable = false;
        ...
        for (CollectionItemId id = node_type_info.InputSidePacketTypes().BeginId();
            ...
            const std::string& name = node_type_info.InputSidePacketTypes().TagMap()->Names()[id.value()];
            ...
            absl::Status status = node_type_info.InputSidePacketTypes().Get(id).Validate(input_side_packet_set->Get(id));
            if (!status.ok()) {
                statuses.push_back(tool::AddStatusPrefix(absl::StrCat("Input side packet \"", name, "\" for PacketGenerator \"",
                                                                      generator_name, "\" is not of the correct type: "),
                                                         status));
            }
        }
        if (!statuses.empty()) {
            return tool::CombinedStatus(absl::StrCat(generator_name, " had invalid configuration."), statuses);
        }
    ...
    absl::Status Generate(const ValidatedGraphConfig& validated_graph, int generator_index,
                          const PacketSet& input_side_packet_set, PacketSet* output_side_packet_set) {
        const NodeTypeInfo&          node_type_info   = validated_graph.GeneratorInfos()[generator_index];
        const PacketGeneratorConfig& generator_config = validated_graph.Config().packet_generator(generator_index);
        const auto&                  generator_name   = generator_config.packet_generator();

        MP_ASSIGN_OR_RETURN(
            auto static_access,
            internal::StaticAccessToGeneratorRegistry::CreateByNameInNamespace(validated_graph.Package(), generator_name),
            _ << generator_name << " is not a valid PacketGenerator.");
        MP_RETURN_IF_ERROR(static_access->Generate(generator_config.options(), input_side_packet_set, output_side_packet_set))
                .SetPrepend()
            << generator_name << "::Generate() failed. ";

        MP_RETURN_IF_ERROR(ValidatePacketSet(node_type_info.OutputSidePacketTypes(), *output_side_packet_set)).SetPrepend()
            << generator_name << "::Generate() output packets were of incorrect type: ";
        return absl::OkStatus();
    }
    ...
    public:
        ...
        // dependencies).
        GeneratorScheduler(const ValidatedGraphConfig* validated_graph, mediapipe::Executor* executor,
        ...
        void GenerateAndScheduleNext(int generator_index, std::map<std::string, Packet>* side_packets,
                                     std::unique_ptr<PacketSet> input_side_packet_set) ABSL_LOCKS_EXCLUDED(mutex_);

        // Iterate through all generators in the config, scheduling any that
        // are runnable (and haven't been scheduled yet).
        void ScheduleAllRunnableGenerators(std::map<std::string, Packet>* side_packets) ABSL_LOCKS_EXCLUDED(mutex_);
        ...
        absl::Status GetNonScheduledGenerators(std::vector<int>* non_scheduled_generators) const;
    ...
    private:
        ...
        std::deque<std::function<void()>> app_thread_tasks_ ABSL_GUARDED_BY(app_thread_mutex_);
        std::unique_ptr<internal::DelegatingExecutor>       delegating_executor_;
    };

    GeneratorScheduler::GeneratorScheduler(const ValidatedGraphConfig* validated_graph, mediapipe::Executor* executor,
                                           const std::vector<int>& non_base_generators, bool initial)
        : validated_graph_(validated_graph)
        , executor_(executor)
        , scheduled_generators_(validated_graph_->Config().packet_generator_size(), !initial) {
        if (!executor_) {
            // Run on the application thread.
            delegating_executor_ = absl::make_unique<internal::DelegatingExecutor>(
                std::bind(&GeneratorScheduler::AddApplicationThreadTask, this, std::placeholders::_1));
    ...
    void GeneratorScheduler::GenerateAndScheduleNext(int generator_index, std::map<std::string, Packet>* side_packets,
        ...
        PacketSet output_side_packet_set(validated_graph_->GeneratorInfos()[generator_index].OutputSidePacketTypes().TagMap());
        VLOG(1) << "Running generator " << generator_index;
        absl::Status status = Generate(*validated_graph_, generator_index, *input_side_packet_set, &output_side_packet_set);

        {
            ...
            for (CollectionItemId id = output_side_packet_set.BeginId(); id < output_side_packet_set.EndId(); ++id) {
                ...
                if (!item.second) {
                    statuses_.push_back(
                        absl::AlreadyExistsError(absl::StrCat("Side packet \"", name, "\" was defined twice.")));
                }
            }
            if (!statuses_.empty()) {
                return;
    ...
    void GeneratorScheduler::ScheduleAllRunnableGenerators(std::map<std::string, Packet>* side_packets) {
        absl::MutexLock lock(&mutex_);
        ...
        for (int index = 0; index < generators.size(); ++index) {
            ...
            auto input_side_packet_set =
                absl::make_unique<PacketSet>(validated_graph_->GeneratorInfos()[index].InputSidePacketTypes().TagMap());

            absl::Status status =
                CreateInputsForGenerator(*validated_graph_, index, *side_packets, input_side_packet_set.get(), &is_unrunnable);
            if (!status.ok()) {
                statuses_.push_back(std::move(status));
                continue;
            }
            ...
            ++num_tasks_;
            mutex_.Unlock();
            executor_->Schedule([this, index, side_packets, input_side_packet_set_ptr]() {
                GenerateAndScheduleNext(index, side_packets, std::unique_ptr<PacketSet>(input_side_packet_set_ptr));
                {
                    absl::MutexLock lock(&mutex_);
                    --num_tasks_;
                    if (num_tasks_ == 0) {
    ...
    absl::Status GeneratorScheduler::GetNonScheduledGenerators(std::vector<int>* non_scheduled_generators) const {
...
absl::Status PacketGeneratorGraph::Initialize(const ValidatedGraphConfig* validated_graph, mediapipe::Executor* executor,
                                              const std::map<std::string, Packet>& input_side_packets) {
    validated_graph_ = validated_graph;
    executor_        = executor;
    base_packets_    = input_side_packets;
    MP_RETURN_IF_ERROR(validated_graph_->CanAcceptSidePackets(input_side_packets));
    return ExecuteGenerators(&base_packets_, &non_base_generators_,
                             /*initial=*/true);
}

absl::Status PacketGeneratorGraph::RunGraphSetup(const std::map<std::string, Packet>& input_side_packets,
    ...
    *output_side_packets = base_packets_;
    for (const std::pair<const std::string, Packet>& item : input_side_packets) {
        ...
        if (iter != output_side_packets->end()) {
            return absl::AlreadyExistsError(absl::StrCat("Side packet \"", iter->first, "\" was defined twice."));
        }
    ...
    MP_RETURN_IF_ERROR(validated_graph_->ValidateRequiredSidePackets(*output_side_packets));
    MP_RETURN_IF_ERROR(ExecuteGenerators(output_side_packets, non_scheduled_generators, /*initial=*/false));
    return absl::OkStatus();
}

absl::Status PacketGeneratorGraph::ExecuteGenerators(std::map<std::string, Packet>* output_side_packets,
    ...
    GeneratorScheduler scheduler(validated_graph_, executor_, non_base_generators_, initial);

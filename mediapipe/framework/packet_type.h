...
private:
    ...
    struct SpecialType;
    using TypeSpec = absl::variant<absl::monostate, TypeId, MultiType, SameAs, SpecialType>;
        ...
        if (!missing_->initialized_errors) {
            for (const auto& entry : missing_->entries) {
                ...
                if (!entry.second.IsOptional()) {
                    ...
                    std::pair<std::string, std::string> tag_idx = absl::StrSplit(entry.first, ':');
                    missing_->errors.push_back(
                        absl::StrCat("Failed to get tag \"", tag_idx.first, "\" index ", tag_idx.second));
                }
            }
...
//
// This class is thread compatible.
using PacketTypeSet = internal::Collection<PacketType, internal::CollectionStorage::kStoreValue, PacketTypeSetErrorHandler>;

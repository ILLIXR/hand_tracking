...
absl::Status OutputSidePacketImpl::Initialize(const std::string& name, const PacketType* packet_type) {
    name_        = name;
    packet_type_ = packet_type;
    return absl::OkStatus();
}

void OutputSidePacketImpl::PrepareForRun(std::function<void(absl::Status)> error_callback) {
...
void OutputSidePacketImpl::AddMirror(InputSidePacketHandler* input_side_packet_handler, CollectionItemId id) {
...
absl::Status OutputSidePacketImpl::SetInternal(const Packet& packet) {
    if (initialized_) {
        return mediapipe::AlreadyExistsErrorBuilder(MEDIAPIPE_LOC) << "Output side packet \"" << name_ << "\" was already set.";
    }
    ...
    if (packet.Timestamp() != Timestamp::Unset()) {
        return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
            << "Output side packet \"" << name_ << "\" has a timestamp " << packet.Timestamp().DebugString() << ".";
    }

    absl::Status result = packet_type_->Validate(packet);
    if (!result.ok()) {
        return mediapipe::StatusBuilder(result, MEDIAPIPE_LOC).SetPrepend()
            << absl::StrCat("Packet type mismatch on calculator output side packet \"", name_, "\": ");
    }
...
void OutputSidePacketImpl::TriggerErrorCallback(const absl::Status& status) const {

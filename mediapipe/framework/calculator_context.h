...
public:
    CalculatorContext(CalculatorState* calculator_state, std::shared_ptr<tool::TagMap> input_tag_map,
    ...
    Timestamp InputTimestamp() const {
        return input_timestamps_.empty() ? Timestamp::Unset() : input_timestamps_.front();
    }
...
private:
    ...
    bool HasInputTimestamp() const {
        return !input_timestamps_.empty();
    }
    ...
    void SetGraphStatus(const absl::Status& status) {

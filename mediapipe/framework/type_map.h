...
using SerializeFn = std::function<absl::Status(const packet_internal::HolderBase& holder_base, std::string* output)>;
using DeserializeFn =
    std::function<absl::Status(const std::string& encoding, std::unique_ptr<packet_internal::HolderBase>* holder_base)>;
...
namespace type_map_internal {
    ...
    public:
        ...
        static const MediaPipeTypeData* GetValue(const KeyType& key) {
            ...
            return (value_iter == internal_map.end()) ? nullptr : &(value_iter->second.second);
        }
        ...
        static void GetKeys(std::vector<KeyType>* keys) {
            ...
            keys->clear();
            const MapType& internal_map = GetMap()->internal_map_;
            for (typename MapType::const_iterator i = internal_map.begin(); i != internal_map.end(); ++i) {
        ...
        class ValueInserter {
        public:
            ValueInserter(const char* file_and_line, const KeyType& key, const MediaPipeTypeData& value) {
                ...
                if (it == static_map->internal_map_.end()) {
                    static_map->internal_map_.emplace(key, std::make_pair(file_and_line, value));
                    return;
                }
                ...
                ABSL_CHECK_EQ(existing_data.type_id, value.type_id)
                    << "Found inconsistent type ids (" << existing_data.type_id << " vs " << value.type_id
                    << ") during mediapipe type registration. Previous definition at " << it->second.first
                    << " and current definition at " << file_and_line;
                ABSL_CHECK_EQ(existing_data.type_string, value.type_string)
                    << "Found inconsistent type strings (" << existing_data.type_string << " vs " << value.type_string
                    << ") during mediapipe type registration. Previous registration at " << it->second.first
                    << " and current registration at " << file_and_line;
                if (value.serialize_fn && value.deserialize_fn) {
                    ...
                    ABSL_CHECK(!existing_data.serialize_fn && !existing_data.deserialize_fn)
                        << "Attempting to redefine serialization functions of type " << value.type_string
                        << ", that have been defined at " << it->second.first << ", at " << file_and_line;
                    const std::string previous_file_and_line = it->second.first;
                    it->second.first                         = file_and_line;
                    it->second.second                        = value;
                    ABSL_LOG(WARNING) << "Redo mediapipe type registration of type " << value.type_string
                                      << " with serialization function at " << file_and_line << ". It was registered at "
                    ...
                    ABSL_LOG(WARNING) << "Ignore mediapipe type registration of type " << value.type_string << " at "
                                      << file_and_line
                    ...
                    ABSL_LOG(FATAL) << "Invalid mediapipe type registration at " << file_and_line
...
#define SET_MEDIAPIPE_TYPE_MAP_VALUE(map_name, key, value) \
    static map_name::ValueInserter TYPE_MAP_TEMP_OBJECT_NAME(FILE_LINE, key, value);
...
#define MEDIAPIPE_REGISTER_TYPE(type, type_name, serialize_fn, deserialize_fn)                                            \
    SET_MEDIAPIPE_TYPE_MAP_VALUE(                                                                                         \
        ...
        mediapipe::TypeId::Of<mediapipe::type_map_internal::ReflectType<void(type*)>::Type>().hash_code(),                \
        (mediapipe::MediaPipeTypeData{                                                                                    \
            mediapipe::TypeId::Of<mediapipe::type_map_internal::ReflectType<void(type*)>::Type>().hash_code(), type_name, \
...
#define MEDIAPIPE_REGISTER_TYPE_WITH_PROXY(type, type_name, serialize_fn, deserialize_fn, to_proxy_fn, from_proxy_fn) \
    SET_MEDIAPIPE_TYPE_MAP_VALUE(                                                                                     \
        mediapipe::PacketTypeIdToMediaPipeTypeData,                                                                   \
        mediapipe::tool::GetTypeHash<mediapipe::type_map_internal::ReflectType<void(type*)>::Type>(),                 \
        (mediapipe::MediaPipeTypeData{                                                                                \
            mediapipe::tool::GetTypeHash<mediapipe::type_map_internal::ReflectType<void(type*)>::Type>(), type_name,  \
            std::bind(&serialize_fn, to_proxy_fn, std::placeholders::_1, std::placeholders::_2),                      \
            std::bind(&deserialize_fn, from_proxy_fn, std::placeholders::_1, std::placeholders::_2)}));               \
    SET_MEDIAPIPE_TYPE_MAP_VALUE(                                                                                     \
        mediapipe::PacketTypeStringToMediaPipeTypeData, type_name,                                                    \
        (mediapipe::MediaPipeTypeData{                                                                                \
            mediapipe::tool::GetTypeHash<mediapipe::type_map_internal::ReflectType<void(type*)>::Type>(), type_name,  \
...
inline const std::string* MediaPipeTypeStringFromTypeId(TypeId type_id) {
    const MediaPipeTypeData* value = PacketTypeIdToMediaPipeTypeData::GetValue(type_id.hash_code());
...
inline const size_t* MediaPipeTypeId(const std::string& type_string) {
    const MediaPipeTypeData* value = PacketTypeStringToMediaPipeTypeData::GetValue(type_string);
...
inline bool SerializeFunctionsAreRegistered(const size_t type_id) {
    const MediaPipeTypeData* mediapipe_type_data = PacketTypeIdToMediaPipeTypeData::GetValue(type_id);
    return mediapipe_type_data && mediapipe_type_data->serialize_fn && mediapipe_type_data->deserialize_fn;
}

// Returns true if serialize and deserialize functions are both registered.
inline bool SerializeFunctionsAreRegistered(const std::string& type_string) {
    const MediaPipeTypeData* mediapipe_type_data = PacketTypeStringToMediaPipeTypeData::GetValue(type_string);
    return mediapipe_type_data && mediapipe_type_data->serialize_fn && mediapipe_type_data->deserialize_fn;
}
} // namespace mediapipe

...
absl::Status OutputStreamManager::Initialize(const std::string& name, const PacketType* packet_type) {
...
void OutputStreamManager::PrepareForRun(std::function<void(absl::Status)> error_callback) {
...
void OutputStreamManager::Close() {
    ...
    for (const auto& mirror : mirrors_) {
        mirror.input_stream_handler->SetNextTimestampBound(mirror.id, Timestamp::Done());
    }
}
...
void OutputStreamManager::PropagateHeader() {
    if (output_stream_spec_.locked_intro_data) {
        output_stream_spec_.TriggerErrorCallback(mediapipe::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
    ...
    for (auto& mirror : mirrors_) {
        mirror.input_stream_handler->SetHeader(mirror.id, output_stream_spec_.header);
    }
}

void OutputStreamManager::AddMirror(InputStreamHandler* input_stream_handler, CollectionItemId id) {
...
// OutputStreamHandler.
Timestamp OutputStreamManager::ComputeOutputTimestampBound(const OutputStreamShard& output_stream_shard,
    ...
    if (input_timestamp != Timestamp::Unstarted() && !input_timestamp.IsAllowedInStream()) {
        output_stream_spec_.TriggerErrorCallback(mediapipe::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
    ...
    if (output_stream_spec_.offset_enabled && input_timestamp != Timestamp::Unstarted()) {
        if (input_timestamp == Timestamp::PreStream()) {
            ...
            if (output_stream_spec_.offset >= 0) {
                ...
                input_bound = (input_timestamp + output_stream_spec_.offset).NextAllowedInStream();
            }
            ...
            input_bound = input_timestamp.NextAllowedInStream() + output_stream_spec_.offset;
        }
    }
    Timestamp new_bound;
    ...
    new_bound = std::max(new_bound, output_stream_shard.updated_next_timestamp_bound_);
    // The bound defined by added packets.
    if (!output_stream_shard.IsEmpty()) {
        new_bound = std::max(new_bound, output_stream_shard.LastAddedPacketTimestamp().NextAllowedInStream());
    }
    return new_bound;
}

// TODO Consider moving the propagation logic to OutputStreamHandler.
void OutputStreamManager::PropagateUpdatesToMirrors(Timestamp next_timestamp_bound, OutputStreamShard* output_stream_shard) {
    ABSL_CHECK(output_stream_shard);
    {
        if (next_timestamp_bound != Timestamp::Unset()) {
            absl::MutexLock lock(&stream_mutex_);
            next_timestamp_bound_ = next_timestamp_bound;
            VLOG(3) << "Next timestamp bound for output " << output_stream_spec_.name << " is " << next_timestamp_bound_;
        }
    }
    std::list<Packet>* packets_to_propagate = output_stream_shard->OutputQueue();
    VLOG(3) << "Output stream: " << Name() << " queue size: " << packets_to_propagate->size();
    VLOG(3) << "Output stream: " << Name() << " next timestamp: " << next_timestamp_bound;
    bool add_packets = !packets_to_propagate->empty();
    bool set_bound   = (next_timestamp_bound != Timestamp::Unset()) &&
        (!add_packets || packets_to_propagate->back().Timestamp().NextAllowedInStream() != next_timestamp_bound);
    ...
    for (int idx = 0; idx < mirror_count; ++idx) {
        ...
        if (add_packets) {
            ...
            if (idx == mirror_count - 1) {
                mirror.input_stream_handler->MovePackets(mirror.id, packets_to_propagate);
            } else {
                mirror.input_stream_handler->AddPackets(mirror.id, *packets_to_propagate);
            }
        }
        if (set_bound) {
            mirror.input_stream_handler->SetNextTimestampBound(mirror.id, next_timestamp_bound);
        }
    }

...
public:
    ...
    absl::Status AddPackets(const std::list<Packet>& container, bool* notify) ABSL_LOCKS_EXCLUDED(stream_mutex_);
    ...
    absl::Status MovePackets(std::list<Packet>* container, bool* notify) ABSL_LOCKS_EXCLUDED(stream_mutex_);
    ...
    // closed.
    absl::Status SetNextTimestampBound(Timestamp bound, bool* notify) ABSL_LOCKS_EXCLUDED(stream_mutex_);
    ...
    Timestamp MinTimestampOrBound(bool* is_empty) const ABSL_LOCKS_EXCLUDED(stream_mutex_);
    ...
    Timestamp GetMinTimestampAmongNLatest(int n) const ABSL_LOCKS_EXCLUDED(stream_mutex_);

    // pop_front()s packets that are earlier than the given timestamp.
    // NOTE: This is a public API intended for FixedSizeInputStreamHandler only.
    void ErasePacketsEarlierThan(Timestamp timestamp) ABSL_LOCKS_EXCLUDED(stream_mutex_);
...
private:
    ...
    absl::Status AddOrMovePacketsInternal(Container container, bool* notify) ABSL_LOCKS_EXCLUDED(stream_mutex_);

...
public:
    ...
    void Initialize(CalculatorState* calculator_state, std::shared_ptr<tool::TagMap> input_tag_map,
    ...
    CalculatorContext* GetFrontCalculatorContext(Timestamp* context_input_timestamp) ABSL_LOCKS_EXCLUDED(contexts_mutex_);
    ...
    // caller.
    CalculatorContext* PrepareCalculatorContext(Timestamp input_timestamp) ABSL_LOCKS_EXCLUDED(contexts_mutex_);
...
private:
    ...
    std::map<Timestamp, std::unique_ptr<CalculatorContext>> active_contexts_ ABSL_GUARDED_BY(contexts_mutex_);
    // Idle calculator contexts that are ready for reuse.
    std::deque<std::unique_ptr<CalculatorContext>> idle_contexts_ ABSL_GUARDED_BY(contexts_mutex_);
};

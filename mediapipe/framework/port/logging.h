...
template<typename... Ts, typename = std::enable_if_t<absl::HasOstreamOperator<typename std::vector<Ts...>::value_type>::value>>
...
private:
    virtual void send(LogSeverity severity, const char* full_filename, const char* base_filename, int line,
        ...
        UNUSED(line);
        LogEntry log_entry(severity, tm_time, absl::string_view(message, message_len));
        Send(log_entry);
    }
};

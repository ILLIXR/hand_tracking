...
public:
    InputStreamHandler(std::shared_ptr<tool::TagMap> tag_map, CalculatorContextManager* calculator_context_manager,
    ...
    virtual void PrepareForRun(std::function<void()> headers_ready_callback, std::function<void()> notification_callback,
                               std::function<void(CalculatorContext*)> schedule_callback,
                               std::function<void(absl::Status)>       error_callback);

    int NumInputStreams() const {
        return input_stream_managers_.NumEntries();
    }
    ...
    public:
        ...
        SyncSet(InputStreamHandler* input_stream_handler, std::vector<CollectionItemId> stream_ids);
        ...
        void FillInputSet(Timestamp input_timestamp, InputStreamShardSet* input_set);
...
protected:
    ...
    // input timestamp.
    virtual void FillInputSet(Timestamp input_timestamp, InputStreamShardSet* input_set) = 0;
...
using InputStreamHandlerRegistry = GlobalFactoryRegistry<std::unique_ptr<InputStreamHandler>, std::shared_ptr<tool::TagMap>,

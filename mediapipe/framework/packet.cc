...
namespace packet_internal {
    ...
    absl::StatusOr<Packet> PacketFromDynamicProto(const std::string& type_name, const std::string& serialized) {
        MP_ASSIGN_OR_RETURN(auto message_holder, packet_internal::MessageHolderRegistry::CreateByName(type_name));
        auto* message = const_cast<proto_ns::MessageLite*>(message_holder->GetProtoMessageLite());
...
std::string Packet::DebugString() const {
    std::string result = absl::StrCat("mediapipe::Packet with timestamp: ", timestamp_.DebugString());
    if (IsEmpty()) {
...
absl::Status Packet::ValidateAsType(TypeId type_id) const {
    if (ABSL_PREDICT_FALSE(IsEmpty())) {
        return absl::InternalError(absl::StrCat("Expected a Packet of type: ", MediaPipeTypeStringOrDemangled(type_id),
    ...
    if (ABSL_PREDICT_FALSE(!holder_is_right_type)) {
        return absl::InvalidArgumentError(absl::StrCat("The Packet stores \"", holder_->DebugTypeName(), "\", but \"",
...
absl::Status Packet::ValidateAsProtoMessageLite() const {
    ...
    if (ABSL_PREDICT_FALSE(holder_->GetProtoMessageLite() == nullptr)) {
        return absl::InvalidArgumentError(absl::StrCat("The Packet stores \"", holder_->DebugTypeName(), "\"",
...
StatusOr<std::vector<const proto_ns::MessageLite*>> Packet::GetVectorOfProtoMessageLitePtrs() const {
    if (holder_ == nullptr) {
        return absl::InternalError("Packet is empty.");
    }

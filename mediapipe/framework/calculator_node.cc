...
namespace {
    ...
    std::unique_ptr<PacketTypeSet> RemoveOmittedPacketTypes(const PacketTypeSet&                 packet_types,
        ...
        for (auto id = packet_types.BeginId(); id != packet_types.EndId(); ++id) {
            ...
            if (packet_types.Get(id).IsOptional() && validated_graph->IsExternalSidePacket(name) &&
...
Timestamp CalculatorNode::SourceProcessOrder(const CalculatorContext* cc) const {
    return calculator_->SourceProcessOrder(cc);
}

absl::Status CalculatorNode::Initialize(const ValidatedGraphConfig* validated_graph, NodeTypeInfo::NodeRef node_ref,
                                        InputStreamManager* input_stream_managers, OutputStreamManager* output_stream_managers,
    ...
    if (node_ref.type == NodeTypeInfo::NodeType::CALCULATOR) {
        ...
        const PacketGeneratorConfig& pg_config = validated_graph_->Config().packet_generator(node_ref.index);
        name_                                  = absl::StrCat("__pg_", node_ref.index, "_", pg_config.packet_generator());
        node_type_info_                        = &validated_graph_->GeneratorInfos()[node_ref.index];
        node_config                            = &node_type_info_->Contract().GetWrapperConfig();
    } else {
        return absl::InvalidArgumentError("node_ref is not a calculator or packet generator");
    }
    ...
    MP_RETURN_IF_ERROR(InitializeOutputSidePackets(node_type_info_->OutputSidePacketTypes(), output_side_packets));

    MP_RETURN_IF_ERROR(InitializeInputSidePackets(output_side_packets));

    MP_RETURN_IF_ERROR(
        InitializeOutputStreamHandler(node_config->output_stream_handler(), node_type_info_->OutputStreamTypes()));
    MP_RETURN_IF_ERROR(InitializeOutputStreams(output_stream_managers));

    calculator_state_ =
        absl::make_unique<CalculatorState>(name_, node_ref.index, node_config->calculator(), *node_config, profiling_context_);
    ...
    *buffer_size_hint = node_config->buffer_size_hint();
    ...
    calculator_context_manager_.Initialize(calculator_state_.get(), node_type_info_->InputStreamTypes().TagMap(),
    ...
    const bool graph_specified = node_config->input_stream_handler().has_input_stream_handler();
    const bool calc_specified  = !(node_type_info_->GetInputStreamHandler().empty());
    ...
    if (use_calc_specified) {
        *(handler_config.mutable_input_stream_handler()) = node_type_info_->GetInputStreamHandler();
        *(handler_config.mutable_options())              = node_type_info_->GetInputStreamHandlerOptions();
    }

    // Use calculator or graph specified InputStreamHandler, or the default ISH
    // already set from graph.
    MP_RETURN_IF_ERROR(InitializeInputStreamHandler(use_calc_specified ? handler_config : node_config->input_stream_handler(),
                                                    node_type_info_->InputStreamTypes()));

    for (auto& stream : output_stream_handler_->OutputStreams()) {
        stream->Spec()->offset_enabled = (contract.GetTimestampOffset() != TimestampDiff::Unset());
        stream->Spec()->offset         = contract.GetTimestampOffset();
    }
    input_stream_handler_->SetProcessTimestampBounds(contract.GetProcessTimestampBounds());

    return InitializeInputStreams(input_stream_managers, output_stream_managers);
}

absl::Status CalculatorNode::InitializeOutputSidePackets(const PacketTypeSet&  output_side_packet_types,
                                                         OutputSidePacketImpl* output_side_packets) {
    output_side_packets_ = absl::make_unique<OutputSidePacketSet>(output_side_packet_types.TagMap());
    int base_index       = node_type_info_->OutputSidePacketBaseIndex();
    RET_CHECK_LE(0, base_index);
    for (CollectionItemId id = output_side_packets_->BeginId(); id < output_side_packets_->EndId(); ++id) {
        output_side_packets_->GetPtr(id) = &output_side_packets[base_index + id.value()];
    }
    return absl::OkStatus();
}

absl::Status CalculatorNode::InitializeInputSidePackets(OutputSidePacketImpl* output_side_packets) {
    ...
    for (CollectionItemId id = node_type_info_->InputSidePacketTypes().BeginId();
        ...
        int output_side_packet_index = validated_graph_->InputSidePacketInfos()[base_index + id.value()].upstream;
        if (output_side_packet_index < 0) {
            // Not generated by a graph node. Comes from an extra side packet
            // provided to the graph.
            continue;
        }
        OutputSidePacketImpl* origin_output_side_packet = &output_side_packets[output_side_packet_index];
        VLOG(2) << "Adding mirror for input side packet with id " << id.value() << " and flat index " << base_index + id.value()
                << " which will be connected to output side packet with flat index " << output_side_packet_index;
        origin_output_side_packet->AddMirror(&input_side_packet_handler_, id);
    }
    return absl::OkStatus();
}

absl::Status CalculatorNode::InitializeOutputStreams(OutputStreamManager* output_stream_managers) {
    RET_CHECK(output_stream_managers) << "output_stream_managers is NULL";
    RET_CHECK_LE(0, node_type_info_->OutputStreamBaseIndex());
    OutputStreamManager* current_output_stream_managers = &output_stream_managers[node_type_info_->OutputStreamBaseIndex()];
    return output_stream_handler_->InitializeOutputStreamManagers(current_output_stream_managers);
}

absl::Status CalculatorNode::InitializeInputStreams(InputStreamManager*  input_stream_managers,
    ...
    InputStreamManager* current_input_stream_managers = &input_stream_managers[node_type_info_->InputStreamBaseIndex()];
    MP_RETURN_IF_ERROR(input_stream_handler_->InitializeInputStreamManagers(current_input_stream_managers));

    // Set all the mirrors.
    for (CollectionItemId id = node_type_info_->InputStreamTypes().BeginId(); id < node_type_info_->InputStreamTypes().EndId();
         ++id) {
        int output_stream_index =
            validated_graph_->InputStreamInfos()[node_type_info_->InputStreamBaseIndex() + id.value()].upstream;
        ...
        VLOG(2) << "Adding mirror for input stream with id " << id.value() << " and flat index "
                << node_type_info_->InputStreamBaseIndex() + id.value()
                << " which will be connected to output stream with flat index " << output_stream_index;
        origin_output_stream_manager->AddMirror(input_stream_handler_.get(), id);
    }
    return absl::OkStatus();
}

absl::Status CalculatorNode::InitializeInputStreamHandler(const InputStreamHandlerConfig& handler_config,
                                                          const PacketTypeSet&            input_stream_types) {
    const ProtoString& input_stream_handler_name = handler_config.input_stream_handler();
    RET_CHECK(!input_stream_handler_name.empty());
    MP_ASSIGN_OR_RETURN(input_stream_handler_,
                        InputStreamHandlerRegistry::CreateByNameInNamespace(
                            validated_graph_->Package(), input_stream_handler_name, input_stream_types.TagMap(),
                            &calculator_context_manager_, handler_config.options(),
                            /*calculator_run_in_parallel=*/max_in_flight_ > 1),
                        _ << "\"" << input_stream_handler_name << "\" is not a registered input stream handler.");

    return absl::OkStatus();
}

absl::Status CalculatorNode::InitializeOutputStreamHandler(const OutputStreamHandlerConfig& handler_config,
    ...
    MP_ASSIGN_OR_RETURN(output_stream_handler_,
                        OutputStreamHandlerRegistry::CreateByNameInNamespace(
                            validated_graph_->Package(), output_stream_handler_name, output_stream_types.TagMap(),
                            &calculator_context_manager_, handler_config.options(),
                            /*calculator_run_in_parallel=*/max_in_flight_ > 1),
                        _ << "\"" << output_stream_handler_name << "\" is not a registered output stream handler.");
    return absl::OkStatus();
}

absl::Status CalculatorNode::ConnectShardsToStreams(CalculatorContext* calculator_context) {
    RET_CHECK(calculator_context);
    MP_RETURN_IF_ERROR(input_stream_handler_->SetupInputShards(&calculator_context->Inputs()));
    return output_stream_handler_->SetupOutputShards(&calculator_context->Outputs());
}
...
absl::Status CalculatorNode::PrepareForRun(const std::map<std::string, Packet>&    all_side_packets,
                                           ...
                                           std::function<void(absl::Status)> error_callback, CounterFactory* counter_factory) {
    ...
    input_stream_handler_->PrepareForRun(
        [this]() {
            CalculatorNode::InputStreamHeadersReady();
        },
        [this]() {
            CalculatorNode::CheckIfBecameReady();
        },
        std::move(schedule_callback), error_callback);
    output_stream_handler_->PrepareForRun(error_callback);

    const auto& contract     = Contract();
    input_side_packet_types_ = RemoveOmittedPacketTypes(contract.InputSidePackets(), all_side_packets, validated_graph_);
    MP_RETURN_IF_ERROR(input_side_packet_handler_.PrepareForRun(
        input_side_packet_types_.get(), all_side_packets,
        [this]() {
            CalculatorNode::InputSidePacketsReady();
        },
        std::move(error_callback)));
    calculator_state_->SetInputSidePackets(&input_side_packet_handler_.InputSidePackets());
    ...
    for (const auto& svc_req : contract.ServiceRequests()) {
        ...
        if (it == service_packets.end()) {
            RET_CHECK(req.IsOptional()) << "required service '" << req.Service().key << "' was not provided";
        } else {
            MP_RETURN_IF_ERROR(calculator_state_->SetServicePacket(req.Service(), it->second));
        }
    }

    MP_RETURN_IF_ERROR(calculator_context_manager_.PrepareForRun(
        std::bind(&CalculatorNode::ConnectShardsToStreams, this, std::placeholders::_1)));

    MP_ASSIGN_OR_RETURN(
        auto calculator_factory,
        CalculatorBaseRegistry::CreateByNameInNamespace(validated_graph_->Package(), calculator_state_->CalculatorType()));
    calculator_ = calculator_factory->CreateCalculator(calculator_context_manager_.GetDefaultCalculatorContext());
...

    {
        ...
        input_stream_headers_ready_        = (input_stream_handler_->UnsetHeaderCount() == 0);
        input_side_packets_ready_          = (input_side_packet_handler_.MissingInputSidePacketCount() == 0);
    }
    return absl::OkStatus();
}
...
absl::Status CalculatorNode::OpenNode() {
    ...
    CalculatorContext*   default_context = calculator_context_manager_.GetDefaultCalculatorContext();
    ...
    calculator_context_manager_.PushInputTimestampToContext(default_context, Timestamp::Unstarted());
    ...
    if (IsSource()) {
        ...
        calculator_context_manager_.PushInputTimestampToContext(default_context, Timestamp(0));
    }

    ABSL_LOG_IF(FATAL, result == tool::StatusStop())
        << absl::Substitute("Open() on node \"$0\" returned tool::StatusStop() which should only be "
                            "used to signal that a source node is done producing data.",
                            DebugName());
    MP_RETURN_IF_ERROR(result).SetPrepend() << absl::Substitute("Calculator::Open() for node \"$0\" failed: ", DebugName());
    ...
    if (offset_enabled && input_stream_handler_->SyncSetCount() > 1) {
        ABSL_LOG(WARNING) << absl::Substitute("Calculator node \"$0\" is configured with multiple input sync-sets "
...
absl::Status CalculatorNode::CloseNode(const absl::Status& graph_status, bool graph_run_ended) {
    {
        absl::MutexLock status_lock(&status_mutex_);
        RET_CHECK_NE(status_, kStateClosed) << "CloseNode() must only be called once.";
    }
    ...
    if (IsSource()) {
        calculator_context_manager_.PopInputTimestampFromContext(default_context);
        calculator_context_manager_.PushInputTimestampToContext(default_context, Timestamp::Done());
    }
    calculator_context_manager_.SetGraphStatusInContext(default_context, graph_status);
    ...
    ABSL_LOG_IF(FATAL, result == tool::StatusStop())
        << absl::Substitute("Close() on node \"$0\" returned tool::StatusStop() which should only be "
                            "used to signal that a source node is done producing data.",
                            DebugName());
    ...
    MP_RETURN_IF_ERROR(result).SetPrepend() << absl::Substitute("Calculator::Close() for node \"$0\" failed: ", DebugName());
...
void CalculatorNode::CleanupAfterRun(const absl::Status& graph_status) {
    if (needs_to_close_) {
        calculator_context_manager_.PushInputTimestampToContext(calculator_context_manager_.GetDefaultCalculatorContext(),
                                                                Timestamp::Done());
...
void CalculatorNode::SchedulingLoop() {
    ...
    while (true) {
        ...
        {
            absl::MutexLock lock(&status_mutex_);
            if (scheduling_state_ == kSchedulingPending && current_in_flight_ < max_in_flight_) {
                max_allowance     = max_in_flight_ - current_in_flight_;
...
absl::Status CalculatorNode::ProcessNode(CalculatorContext* calculator_context) {
    if (IsSource()) {
        ...
        if (!result.ok()) {
            if (result == tool::StatusStop()) {
                ...
                return mediapipe::StatusBuilder(result, MEDIAPIPE_LOC).SetPrepend()
                    << absl::Substitute("Calculator::Process() for node \"$0\" failed: ", DebugName());
            }
        }
        output_stream_handler_->PostProcess(input_timestamp);
        if (node_stopped) {
            MP_RETURN_IF_ERROR(CloseNode(absl::OkStatus(), /*graph_run_ended=*/false));
        }
        return absl::OkStatus();
    } else {
        ...
        absl::Status                result  = absl::InternalError("Calculator context has no input packets.");

        int num_invocations = calculator_context_manager_.NumberOfContextTimestamps(*calculator_context);
        RET_CHECK(num_invocations <= 1 || max_in_flight_ <= 1)
            << "num_invocations:" << num_invocations << ", max_in_flight_:" << max_in_flight_;
        for (int i = 0; i < num_invocations; ++i) {
            ...
            if (input_timestamp.IsAllowedInStream()) {
                ...
                VLOG(2) << "Calling Calculator::Process() for node: " << DebugName() << " timestamp: " << input_timestamp;
                ...
                VLOG(2) << "Called Calculator::Process() for node: " << DebugName() << " timestamp: " << input_timestamp;
                ...
                ABSL_CHECK_EQ(calculator_context_manager_.NumberOfContextTimestamps(*calculator_context), 1);
                return CloseNode(absl::OkStatus(), /*graph_run_ended=*/false);
            } else {
                RET_CHECK_FAIL() << "Invalid input timestamp in ProcessNode(). timestamp: " << input_timestamp;
            }
        }
        return result;
    }
...
void CalculatorNode::SetQueueSizeCallbacks(InputStreamManager::QueueSizeCallback becomes_full_callback,
                                           InputStreamManager::QueueSizeCallback becomes_not_full_callback) {
    ABSL_CHECK(input_stream_handler_);
    input_stream_handler_->SetQueueSizeCallbacks(std::move(becomes_full_callback), std::move(becomes_not_full_callback));
}

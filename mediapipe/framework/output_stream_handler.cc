...
absl::Status OutputStreamHandler::InitializeOutputStreamManagers(OutputStreamManager* flat_output_stream_managers) {
    for (CollectionItemId id = output_stream_managers_.BeginId(); id < output_stream_managers_.EndId(); ++id) {
        output_stream_managers_.Get(id) = &flat_output_stream_managers[id.value()];
    }
    return absl::OkStatus();
}

absl::Status OutputStreamHandler::SetupOutputShards(OutputStreamShardSet* output_shards) {
...
void OutputStreamHandler::PrepareForRun(const std::function<void(absl::Status)>& error_callback) {
...
void OutputStreamHandler::PrepareOutputs(Timestamp input_timestamp, OutputStreamShardSet* output_shards) {
...
void OutputStreamHandler::TryPropagateTimestampBound(Timestamp input_bound) {
    ...
    for (OutputStreamManager* manager : output_stream_managers_) {
        if (manager->OffsetEnabled() && !manager->IsClosed() &&
            ...
            manager->PropagateUpdatesToMirrors(input_bound + manager->Offset(), &empty_output);
        }
    }
}
...
void OutputStreamHandler::Close(OutputStreamShardSet* output_shards) {
    for (CollectionItemId id = output_stream_managers_.BeginId(); id < output_stream_managers_.EndId(); ++id) {
        if (output_shards) {
            output_stream_managers_.Get(id)->PropagateUpdatesToMirrors(Timestamp::Done(), &output_shards->Get(id));
        }
        output_stream_managers_.Get(id)->Close();
    }
}

void OutputStreamHandler::PropagateOutputPackets(Timestamp input_timestamp, OutputStreamShardSet* output_shards) {
    ABSL_CHECK(output_shards);
    for (CollectionItemId id = output_stream_managers_.BeginId(); id < output_stream_managers_.EndId(); ++id) {
        OutputStreamManager* manager = output_stream_managers_.Get(id);
        if (manager->IsClosed()) {
        ...
        const Timestamp    output_bound = manager->ComputeOutputTimestampBound(*output, input_timestamp);

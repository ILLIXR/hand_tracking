...
public:
    ...
    static Timestamp FromSeconds(double seconds) {
        return Timestamp(TimestampBaseType{std::round(seconds * kTimestampUnitsPerSecond)});
    }
...
inline Timestamp::Timestamp(int64_t timestamp)
    : timestamp_(timestamp) {
    ABSL_CHECK(!IsSpecialValue()) << "Cannot directly create a Timestamp with a special value: "

...
void MatrixDataProtoFromMatrix(const Matrix& matrix, MatrixData* matrix_data) {
    ...
    proto_ns::RepeatedField<float>(matrix.data(), matrix.data() + rows * cols).Swap(matrix_data->mutable_packed_data());
}

void MatrixFromMatrixDataProto(const MatrixData& matrix_data, Matrix* matrix) {
    ABSL_CHECK_EQ(matrix_data.rows() * matrix_data.cols(), matrix_data.packed_data_size());

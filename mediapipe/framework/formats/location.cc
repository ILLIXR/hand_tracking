...
Location Location::CreateBBoxLocation(int xmin, int ymin, int width, int height) {
    LocationData location_data;
...
Location Location::CreateBBoxLocation(const Rectangle_i& rect) {
    return CreateBBoxLocation(rect.xmin(), rect.ymin(), rect.Width(), rect.Height());
}

Location Location::CreateBBoxLocation(const ::mediapipe::BoundingBox& bbox) {
    return CreateBBoxLocation(bbox.left_x(), bbox.upper_y(), bbox.right_x() - bbox.left_x(), bbox.lower_y() - bbox.upper_y());
}

Location Location::CreateRelativeBBoxLocation(float relative_xmin, float relative_ymin, float relative_width,
...
Location Location::CreateRelativeBBoxLocation(const Rectangle_f& rect) {
    return CreateRelativeBBoxLocation(rect.xmin(), rect.ymin(), rect.Width(), rect.Height());
}
...
bool Location::IsValidLocationData(const LocationData& location_data) {
    ...
    case LocationData::BOUNDING_BOX: {
        return location_data.has_bounding_box() && location_data.bounding_box().has_xmin() &&
            location_data.bounding_box().has_ymin() && location_data.bounding_box().has_width() &&
            location_data.bounding_box().has_height();
    }
    case LocationData::RELATIVE_BOUNDING_BOX: {
        return location_data.has_relative_bounding_box() && location_data.relative_bounding_box().has_xmin() &&
            location_data.relative_bounding_box().has_ymin() && location_data.relative_bounding_box().has_width() &&
            location_data.relative_bounding_box().has_height();
    }
    case LocationData::MASK: {
        return location_data.has_mask() && location_data.mask().has_width() && location_data.mask().has_height() &&
...
Location& Location::Scale(const float scale) {
    ABSL_CHECK(!location_data_.has_mask()) << "Location mask scaling is not implemented.";
    ...
    case LocationData::MASK: {
        ABSL_LOG(FATAL) << "Scaling for location data of type MASK is not supported.";
        break;
    }
    }
    return *this;
...
Location& Location::Square(int image_width, int image_height) {
    ...
    case LocationData::RELATIVE_BOUNDING_BOX: {
        ...
        if (max_dimension > absolute_width) {
            box->set_xmin((absolute_xmin + absolute_width / 2 - max_dimension / 2) / image_width);
            box->set_width(max_dimension / image_width);
        } else if (max_dimension > absolute_height) {
            box->set_ymin((absolute_ymin + absolute_height / 2 - max_dimension / 2) / image_height);
            box->set_height(max_dimension / image_height);
        }
        break;
    }
    case LocationData::MASK: {
        ABSL_LOG(FATAL) << "Squaring for location data of type MASK is not supported.";
        break;
    }
    }
    return *this;
...
Location& Location::ShiftToFitBestIntoImage(int image_width, int image_height) {
    ...
    case LocationData::BOUNDING_BOX: {
        auto* box = location_data_.mutable_bounding_box();
        box->set_xmin(
            static_cast<int>(std::round(box->xmin() + BestShift(box->xmin(), box->xmin() + box->width(), image_width))));
        box->set_ymin(
            static_cast<int>(std::round(box->ymin() + BestShift(box->ymin(), box->ymin() + box->height(), image_height))));
        break;
    }
    case LocationData::RELATIVE_BOUNDING_BOX: {
        auto* box = location_data_.mutable_relative_bounding_box();
        box->set_xmin(box->xmin() + BestShift(box->xmin(), box->xmin() + box->width(), 1.0f));
        box->set_ymin(box->ymin() + BestShift(box->ymin(), box->ymin() + box->height(), 1.0f));
        break;
    }
    case LocationData::MASK: {
        auto        mask_bounding_box = MaskToRectangle(location_data_);
        const float x_shift           = BestShift(mask_bounding_box.xmin(), mask_bounding_box.xmax(), image_width);
        const float y_shift           = BestShift(mask_bounding_box.xmin(), mask_bounding_box.xmax(), image_height);
        auto*       mask              = location_data_.mutable_mask();
        ABSL_CHECK_EQ(image_width, mask->width());
        ABSL_CHECK_EQ(image_height, mask->height());
        for (auto& interval : *mask->mutable_rasterization()->mutable_interval()) {
...
Location& Location::Crop(const Rectangle_i& crop_box) {
    ...
    case LocationData::RELATIVE_BOUNDING_BOX:
        ABSL_LOG(FATAL) << "Can't crop a relative bounding box using absolute coordinates. "
    ...
    case LocationData::MASK: {
        ...
        for (const auto& interval : location_data_.mask().rasterization().interval()) {
            if (interval.y() >= crop_box.ymin() && interval.y() < crop_box.ymax() && interval.left_x() < crop_box.xmax() &&
                interval.right_x() > crop_box.xmin()) {
                auto* new_interval = rasterization->add_interval();
                new_interval->set_y(interval.y() - crop_box.ymin());
                new_interval->set_left_x(std::max(interval.left_x() - crop_box.xmin(), 0));
                new_interval->set_right_x(std::min(interval.right_x() - crop_box.xmin(), crop_box.Width()));
            }
        }
...
Location& Location::Crop(const Rectangle_f& crop_box) {
    ...
    case LocationData::GLOBAL:
        ...
        break;
    case LocationData::BOUNDING_BOX:
        ABSL_LOG(FATAL) << "Can't crop an absolute bounding box using relative coordinates. "
    ...
    case LocationData::MASK:
        ABSL_LOG(FATAL) << "Can't crop a mask using relative coordinates. Use the "
...
template<>
Rectangle_i Location::ConvertToBBox<Rectangle_i>(int image_width, int image_height) const {
    switch (location_data_.format()) {
    case LocationData::GLOBAL: {
    ...
    case LocationData::RELATIVE_BOUNDING_BOX: {
        ...
        return Rectangle_i(static_cast<int>(std::round(image_width * box.xmin())),
                           static_cast<int>(std::round(image_height * box.ymin())), static_cast<int>(image_width * box.width()),
...
Rectangle_f Location::ConvertToRelativeBBox(int image_width, int image_height) const {
    switch (location_data_.format()) {
    case LocationData::GLOBAL: {
    ...
    case LocationData::BOUNDING_BOX: {
        const auto& box = location_data_.bounding_box();
        return Rectangle_f(static_cast<float>(box.xmin()) / image_width, static_cast<float>(box.ymin()) / image_height,
                           static_cast<float>(box.width()) / image_width, static_cast<float>(box.height()) / image_height);
    }
    case LocationData::RELATIVE_BOUNDING_BOX: {
        const auto& box = location_data_.relative_bounding_box();
        return Rectangle_f(box.xmin(), box.ymin(), box.width(), box.height());
    ...
    case LocationData::MASK: {
        ...
        return Rectangle_f(static_cast<float>(rect.xmin()) / image_width, static_cast<float>(rect.ymin()) / image_height,
                           static_cast<float>(rect.Width()) / image_width, static_cast<float>(rect.Height()) / image_height);
    }
    default: {
        // Note that this shouldn't happen, but a default clause is required by
        // the Android compiler.
...
template<>
::mediapipe::BoundingBox Location::ConvertToBBox<::mediapipe::BoundingBox>(int image_width, int image_height) const {
...
std::vector<Point2_i> Location::ConvertToKeypoints(int image_width, int image_height) const {
    std::vector<Point2_i> keypoints;
    for (const auto& keypoint : location_data_.relative_keypoints()) {
        keypoints.emplace_back(Point2_i(static_cast<int>(std::round(image_width * keypoint.x())),

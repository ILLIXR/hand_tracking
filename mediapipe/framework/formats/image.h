...
public:
    ...
    explicit Image(ImageFrameSharedPtr image_frame)
        : gpu_buffer_(std::make_shared<GpuBufferStorageImageFrame>(std::move(image_frame))) {
        use_gpu_ = false;
    }
...
public:
    ...
    uint8_t* Pixels() {
        if (frame_)
            return frame_->MutablePixelData();
        return nullptr;
    }

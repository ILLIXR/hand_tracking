...
#if (defined(__i386__) || defined(__x86_64__)) && defined(__POPCNT__) && defined(__GNUC__)
...
const ImageFrame::Deleter ImageFrame::PixelDataDeleter::kAlignedFree = aligned_free;
...
ImageFrame::ImageFrame(ImageFormat::Format format, int width, int height, uint32_t alignment_boundary)
    : format_(format)
    , width_(width)
    , height_(height) {
...
ImageFrame::ImageFrame(ImageFormat::Format format, int width, int height, int width_step, uint8_t* pixel_data,
                       ImageFrame::Deleter deleter) {
    AdoptPixelData(format, width, height, width_step, pixel_data, deleter);
}

ImageFrame::ImageFrame(ImageFrame&& move_from) {
...
void ImageFrame::Reset(ImageFormat::Format format, int width, int height, uint32_t alignment_boundary) {
    format_ = format;
    width_  = width;
    height_ = height;
    ...
    if (alignment_boundary == 1) {
        pixel_data_ = {new uint8_t[height * width_step_], PixelDataDeleter::kArrayDelete};
    } else {
        ...
        pixel_data_ = {reinterpret_cast<uint8_t*>(aligned_malloc(height * width_step_, (int) alignment_boundary)),
                       PixelDataDeleter::kAlignedFree};
    }
}

void ImageFrame::AdoptPixelData(ImageFormat::Format format, int width, int height, int width_step, uint8_t* pixel_data,
                                ImageFrame::Deleter deleter) {
    format_     = format;
    width_      = width;
    height_     = height;
...
void ImageFrame::InternalCopyFrom(int width, int height, int width_step, int channel_size, const uint8_t* pixel_data) {
...
void ImageFrame::SetAlignmentPaddingAreas() {
    ...
    for (int row = 0; row < height_; ++row) {
        ...
        while (padding_index + pixel_size - 1 < padding_size) {
            ...
            for (int pixel_byte_index = 0; pixel_byte_index < pixel_size; ++pixel_byte_index) {
...
bool ImageFrame::IsAligned(uint32_t alignment_boundary) const {
    ...
    if ((reinterpret_cast<uintptr_t>(pixel_data_.get()) % alignment_boundary) != 0) {
        return false;
    }
...
#else
    const proto_ns::EnumValueDescriptor* enum_value_descriptor = ImageFormat::Format_descriptor()->FindValueByNumber(format);
    if (enum_value_descriptor == nullptr) {
        return absl::StrCat("Format with number ", format, " is not a valid format.");
    } else {
        return absl::StrCat("Format ", enum_value_descriptor->DebugString(), " is not valid in this situation.");
    }
#endif
}
...
int ImageFrame::ChannelSize() const {
    return ChannelSizeForFormat(format_);
}
...
int ImageFrame::ByteDepth() const {
    return ChannelSizeForFormat(format_);
}
...
void ImageFrame::CopyFrom(const ImageFrame& image_frame, uint32_t alignment_boundary) {
    // Reset the current image.
    Reset(image_frame.Format(), image_frame.Width(), image_frame.Height(), alignment_boundary);

    ABSL_CHECK_EQ(format_, image_frame.Format());
    InternalCopyFrom(image_frame.Width(), image_frame.Height(), image_frame.WidthStep(), image_frame.ChannelSize(),
                     image_frame.PixelData());
}

void ImageFrame::CopyPixelData(ImageFormat::Format format, int width, int height, const uint8_t* pixel_data,
                               uint32_t alignment_boundary) {
    CopyPixelData(format, width, height, 0 /* contiguous storage */, pixel_data, alignment_boundary);
}

void ImageFrame::CopyPixelData(ImageFormat::Format format, int width, int height, int width_step, const uint8_t* pixel_data,
                               uint32_t alignment_boundary) {
    Reset(format, width, height, alignment_boundary);
    InternalCopyFrom(width, height, width_step, ChannelSizeForFormat(format), pixel_data);
}
...
void ImageFrame::CopyToBuffer(uint8_t* buffer, int buffer_size) const {
    ...
    if (IsContiguous()) {
        ...
        InternalCopyToBuffer(0 /* contiguous storage */, reinterpret_cast<char*>(buffer));
    }
}

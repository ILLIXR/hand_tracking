...
#define TENSOR_UNIQUE_VIEW_TYPE_ID() \
    static inline uint64_t kId =     \
        tensor_internal::FnvHash64(__FILE__, tensor_internal::FnvHash64(TENSOR_INT_TO_STRING(__LINE__)))

// Generates unique view id at compile-time using FILE and LINE and Type of the
// template view's argument.
#define TENSOR_UNIQUE_VIEW_TYPE_ID_T(T) static inline uint64_t kId = tool::GetTypeHash<T>();
...
#define TENSOR_INT_TO_STRING(x)  TENSOR_INT_TO_STRING2(x)
    ...
    struct TypeInList<T, TypeList<T, Ts...>> : std::integral_constant<std::size_t, 0> { };

    template<typename T, typename TOther, typename... Ts>
    struct TypeInList<T, TypeList<TOther, Ts...>>
        : std::integral_constant<std::size_t, 1 + TypeInList<T, TypeList<Ts...>>::value> { };

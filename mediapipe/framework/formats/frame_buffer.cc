...
namespace {
    ...
    bool IsSupportedYuvFormat(FrameBuffer::Format format) {
        return format == FrameBuffer::Format::kNV21 || format == FrameBuffer::Format::kNV12 ||
            format == FrameBuffer::Format::kYV12 || format == FrameBuffer::Format::kYV21;
    }

    // Returns supported 1-plane FrameBuffer in YuvData structure.
    absl::StatusOr<FrameBuffer::YuvData> GetYuvDataFromOnePlaneFrameBuffer(const FrameBuffer& source) {
        if (!IsSupportedYuvFormat(source.format())) {
            return absl::InvalidArgumentError("The source FrameBuffer format is not part of YUV420 family.");
        }

        FrameBuffer::YuvData result;
        const int            y_buffer_size = source.plane(0).stride().row_stride_bytes * source.dimension().height;
        const int            uv_buffer_size =
            ((source.plane(0).stride().row_stride_bytes + 1) / 2) * ((source.dimension().height + 1) / 2);
        ...
        if (source.format() == FrameBuffer::Format::kNV21) {
            ...
            if (result.y_row_stride == source.dimension().width && result.y_row_stride % 2 == 1) {
    ...
    absl::StatusOr<FrameBuffer::YuvData> GetYuvDataFromTwoPlaneFrameBuffer(const FrameBuffer& source) {
        if (source.format() != FrameBuffer::Format::kNV12 && source.format() != FrameBuffer::Format::kNV21) {
    ...
    absl::StatusOr<FrameBuffer::YuvData> GetYuvDataFromThreePlaneFrameBuffer(const FrameBuffer& source) {
        if (!IsSupportedYuvFormat(source.format())) {
            return absl::InvalidArgumentError("The source FrameBuffer format is not part of YUV420 family.");
        }

        if (source.plane(1).stride().row_stride_bytes != source.plane(2).stride().row_stride_bytes ||
            source.plane(1).stride().pixel_stride_bytes != source.plane(2).stride().pixel_stride_bytes) {
            return absl::InternalError("Unsupported YUV planar format.");
        }
        FrameBuffer::YuvData result;
        if (source.format() == FrameBuffer::Format::kNV21 || source.format() == FrameBuffer::Format::kYV12) {
...
absl::StatusOr<FrameBuffer::YuvData> FrameBuffer::GetYuvDataFromFrameBuffer(const FrameBuffer& source) {
    if (!IsSupportedYuvFormat(source.format())) {
        return absl::InvalidArgumentError("The source FrameBuffer format is not part of YUV420 family.");
    }
    ...
    return absl::InvalidArgumentError("The source FrameBuffer must be consisted by 1, 2, or 3 planes");
}

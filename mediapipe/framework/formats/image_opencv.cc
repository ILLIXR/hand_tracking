...
namespace formats {
    ...
    std::shared_ptr<cv::Mat> MatView(const mediapipe::Image* image) {
        ...
        struct MatWithPixelLock {
            ...
            // std::make_shared.
            MatWithPixelLock(mediapipe::Image* image)
                : lock(image) { }
        ...
        const int    type     = CV_MAKETYPE(GetMatType(image->image_format()), image->channels());
        const size_t steps[]  = {static_cast<size_t>(image->step()),
                                 static_cast<size_t>(ImageFrame::ByteDepthForFormat(image->image_format()))};
        auto         owner    = std::make_shared<MatWithPixelLock>(const_cast<mediapipe::Image*>(image));
        uint8_t*     data_ptr = owner->lock.Pixels();
        ABSL_CHECK(data_ptr != nullptr);
        // Use Image to initialize in-place. Image still owns memory.
        if (steps[0] == (size_t) sizes[1] * image->channels() * ImageFrame::ByteDepthForFormat(image->image_format())) {

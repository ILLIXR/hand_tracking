...
public:
    ...
    struct Stride {
        ...
        bool operator==(const Stride& other) const {
            return row_stride_bytes == other.row_stride_bytes && pixel_stride_bytes == other.pixel_stride_bytes;
        }
    ...
    FrameBuffer(const std::vector<Plane>& planes, Dimension dimension, Format format)
        : planes_(planes)
    ...
    static absl::StatusOr<YuvData> GetYuvDataFromFrameBuffer(const FrameBuffer& source);

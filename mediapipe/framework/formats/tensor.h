...
public:
    ...
    enum class ElementType { kNone, kFloat16, kFloat32, kUInt8, kInt8, kInt32, kInt64, kChar, kBool };
    ...
    Tensor(ElementType element_type, const Shape& shape, const QuantizationParameters& quantization_parameters,
    ...
    public:
        ...
        auto buffer() const {
            ...
            return static_cast<typename std::tuple_element<std::is_const<T>::value, std::tuple<P*, const P*>>::type>(buffer_);
        }
    ...
    CpuWriteView GetCpuWriteView(uint64_t source_location_hash = tensor_internal::FnvHash64(builtin_FILE(),
                                                                                            builtin_LINE())) const;
    ...
    public:
        ...
        AHardwareBufferView(AHardwareBufferView&& src)
            ...
            ahwb_release_callbacks_ = std::exchange(src.ahwb_release_callbacks_, nullptr);
        }
        ...
        void SetReadingFinishedFunc(FinishingFunc&& func) {
            ABSL_CHECK(ahwb_written_) << "AHWB write view can't accept 'reading finished callback'";
            *ahwb_written_ = std::move(func);
        }

        // TODO: verify if multiple functions can be specified.
        void SetWritingFinishedFD(int fd, FinishingFunc func = nullptr) {
            ABSL_CHECK(fence_fd_) << "AHWB read view can't accept 'writing finished file descriptor'";
            *fence_fd_     = fd;
            *ahwb_written_ = std::move(func);
        }
    ...
    protected:
        ...
        AHardwareBufferView(HardwareBuffer* hardware_buffer, int file_descriptor, int* fence_fd, FinishingFunc* ahwb_written,
    ...
    public:
        ...
        static Layout GetLayoutDimensions(const Tensor::Shape& shape, int* width, int* height);
    ...
    protected:
        ...
        OpenGlBufferView(GLuint name, std::unique_ptr<absl::MutexLock>&& lock, GLsync* ssbo_read)
            : View(std::move(lock))
            , name_(name)
    ...
    OpenGlBufferView GetOpenGlBufferWriteView(uint64_t source_location_hash = tensor_internal::FnvHash64(builtin_FILE(),
                                                                                                         builtin_LINE())) const;
    ...
    int bytes() const {
        return shape_.num_elements() * element_size();
    }
    ...
    bool ready_on_gpu() const {
        return valid_ & (kValidMetalBuffer | kValidOpenGlBuffer | kValidAHardwareBuffer | kValidOpenGlTexture2d);
    }

    bool ready_as_metal_buffer() const {
        return valid_ & kValidMetalBuffer;
    }

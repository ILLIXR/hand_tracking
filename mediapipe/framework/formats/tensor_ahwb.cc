...
namespace {
    ...
    bool IsGlSupported() {
        static const bool extensions_allowed = [] {
            ...
            glBufferStorageExternalEXT =
                reinterpret_cast<PFNGLBUFFERSTORAGEEXTERNALEXTPROC>(eglGetProcAddress("glBufferStorageExternalEXT"));
            ...
            return eglClientWaitSyncKHR && eglWaitSyncKHR && eglGetNativeClientBufferANDROID && glBufferStorageExternalEXT &&
    ...
    absl::Status MapAHardwareBufferToGlBuffer(AHardwareBuffer* handle, size_t size) {
        if (!IsGlSupported()) {
            return absl::UnknownError("No GL extension functions found to bind AHardwareBuffer and "
                                      "OpenGL buffer");
        }
    ...
    public:
        ...
        static void Add(std::shared_ptr<HardwareBuffer> ahwb, GLuint opengl_buffer, EGLSyncKHR ssbo_sync, GLsync ssbo_read,
                        Tensor::FinishingFunc&& ahwb_written, std::shared_ptr<mediapipe::GlContext> gl_context,
            ...
            to_release_local.emplace_back(
                absl::WrapUnique(new DelayedReleaser(std::move(ahwb), opengl_buffer, ssbo_sync, ssbo_read,
            ...
            {
                absl::MutexLock lock(&mutex);
                to_release_->insert(to_release_->end(), std::make_move_iterator(to_release_local.begin()),
    ...
    protected:
        ...
        static inline NoDestructor<std::deque<std::unique_ptr<DelayedReleaser>>> to_release_;

        DelayedReleaser(std::shared_ptr<HardwareBuffer> ahwb, GLuint opengl_buffer, EGLSyncKHR fence_sync, GLsync ssbo_read,
                        Tensor::FinishingFunc&& ahwb_written, std::shared_ptr<mediapipe::GlContext> gl_context,
...
Tensor::AHardwareBufferView Tensor::GetAHardwareBufferReadView() const {
    ...
    ABSL_CHECK(!(valid_ & kValidOpenGlTexture2d)) << "Tensor conversion between OpenGL texture and AHardwareBuffer is not "
                                                     "supported.";
    bool transfer = ahwb_ == nullptr;
    ABSL_CHECK_OK(AllocateAHardwareBuffer()) << "AHardwareBuffer is not supported on the target system.";
...
void Tensor::CreateEglSyncAndFd() const {
    gl_context_->Run([this]() {
        if (IsGlSupported()) {
            auto egl_display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
            if (egl_display != EGL_NO_DISPLAY) {
                fence_sync_ = eglCreateSyncKHR(egl_display, EGL_SYNC_NATIVE_FENCE_ANDROID, nullptr);
...
void Tensor::MoveCpuOrSsboToAhwb() const {
    auto dest = ahwb_->Lock(HardwareBufferSpec::AHARDWAREBUFFER_USAGE_CPU_WRITE_RARELY);
    ...
    if (valid_ & kValidCpu) {
        ...
        gl_context_->Run([this, dest]() {
            ...
            const void* src = glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, bytes(), GL_MAP_READ_BIT);
        ...
        ABSL_LOG(FATAL) << "Can't convert tensor with mask " << valid_ << " into AHWB.";
    }
...
bool Tensor::InsertAhwbToSsboFence() const {
    ...
    if (fence_fd_ != -1) {
        ...
        EGLint sync_attribs[] = {EGL_SYNC_NATIVE_FENCE_FD_ANDROID, (EGLint) fd_for_egl, EGL_NONE};
        fence_sync_           = eglCreateSyncKHR(egl_display, EGL_SYNC_NATIVE_FENCE_ANDROID, sync_attribs);
        if (fence_sync_ != EGL_NO_SYNC_KHR) {
    ...
    if (__builtin_available(android 26, *)) {
        if (ahwb_) {
            if (ssbo_read_ != 0 || fence_sync_ != EGL_NO_SYNC_KHR || ahwb_written_) {
                if (ssbo_written_ != -1)
                    close(ssbo_written_);
                DelayedReleaser::Add(std::move(ahwb_), opengl_buffer_, fence_sync_, ssbo_read_, std::move(ahwb_written_),
                                     gl_context_, std::move(ahwb_release_callbacks_));
                opengl_buffer_ = GL_INVALID_INDEX;
            } else {
    ...
    if (ahwb_ != nullptr) {
        if (!(valid_ & kValidCpu)) {
            if ((valid_ & kValidOpenGlBuffer) && ssbo_written_ == -1) {
                ...
                ABSL_CHECK(ahwb_written_(true)) << "An error oqcured while waiting for the buffer to be written";
            }
        }
        auto ptr = ahwb_->Lock(HardwareBufferSpec::AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN, ssbo_written_);
...
void* Tensor::MapAhwbToCpuWrite() const {
    if (ahwb_ != nullptr) {
        ...
        auto locked_ptr = ahwb_->Lock(HardwareBufferSpec::AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN);
...
void Tensor::TrackAhwbUsage(uint64_t source_location_hash) const {
    if (ahwb_tracking_key_ == 0) {
        ...
        ahwb_tracking_key_ = tensor_internal::FnvHash64(ahwb_tracking_key_, memory_alignment_);
    }

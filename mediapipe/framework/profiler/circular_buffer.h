...
public:
    ...
    T operator*() const {
        return buffer_->GetAbsolute(index_);
    }

    T* operator->() const {
        &buffer_->GetAbsolute(index_);
    }
    ...
    iterator& operator+=(const int64_t& num) {
        return index_ += num, *this;
    }

    int64_t operator-(const iterator& it) const {

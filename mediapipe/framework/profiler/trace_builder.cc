...
struct TaskId {
    ...
    inline size_t hash() const {
        return id + ts.Value() + (event_type << 10);
    }
};
} // namespace mediapipe
...
namespace {
    void BasicTraceEventTypes(TraceEventRegistry* result) {
        ...
        std::vector<TraceEventType> basic_types = {
            ...
            {TraceEvent::CPU_TASK_SYSTEM, "System-time processing packets.", true, true},
            ...
            {TraceEvent::GPU_CALIBRATION, "A time measured by GPU clock and by CPU clock.", true, false},
            {TraceEvent::PACKET_QUEUED, "An input queue size when a packet arrives.", true, true, false},
            ...
            {TraceEvent::GPU_TASK_INVOKE_ADVANCED,
             ...
             "interpreter."},
            {TraceEvent::TPU_TASK_INVOKE_ASYNC, "CPU timing for async initiation of a TPU task."},
        };
    ...
    public:
        ...
        void clear() {
            pointer_id_map_.clear(), string_id_map_.clear();
        }
    ...
    public:
        ...
        void clear() {
            pointer_id_map_.clear();
        }

        const absl::node_hash_map<int64_t, int32_t>& map() {
...
public:
    ...
    TraceEventRegistry* trace_event_registry() {
        return &trace_event_registry_;
    }

    static Timestamp TimestampAfter(const TraceBuffer& buffer, absl::Time begin_time) {
        Timestamp max_ts = Timestamp::Min();
        for (auto iter = buffer.begin(); iter < buffer.end(); ++iter) {
            TraceEvent event = *iter;
            if (event.event_time >= begin_time)
                break;
    ...
    void CreateTrace(const TraceBuffer& buffer, absl::Time begin_time, absl::Time end_time, GraphTrace* result) {
        ...
        for (const TraceEvent& event : snapshot) {
            ...
            TaskId hop_id{stream_id_map_[event.stream_id], event.packet_ts, event.event_type};
        ...
        for (const TraceEvent& event : snapshot) {
            ...
            if (task_ids.count(task_id) == 0) {
                ...
                BuildCalculatorTrace(task_events_[task_id], result->add_calculator_trace());
            }
        }
    ...
    void CreateLog(const TraceBuffer& buffer, absl::Time begin_time, absl::Time end_time, GraphTrace* result) {
        // Snapshot recent TraceEvents
        std::vector<TraceEvent> snapshot;
        snapshot.reserve(10000);
        TraceBuffer::iterator buffer_end = buffer.end();
...
private:
    ...
    int64_t LogTimestamp(Timestamp ts) {
        return ts.Value() - base_ts_;
    }

    // Return a time in micros relative to the base time.
    int64_t LogTime(absl::Time time) {
        return ToUnixMicros(time) - base_time_;
    }
    ...
    void BuildStreamTrace(const TraceEvent& event, GraphTrace::StreamTrace* result) {
    ...
    void BuildCalculatorTrace(const EventList& task_events, GraphTrace::CalculatorTrace* result) {
        ...
        for (const TraceEvent* event : task_events) {
            ...
            if (trace_event_registry_[event->event_type].is_stream_event()) {
                auto stream_trace = event->is_finish ? result->add_output_trace() : result->add_input_trace();
    ...
    void BuildEventLog(const TraceEvent& event, GraphTrace::CalculatorTrace* result) {
        if (event.is_finish) {
...
Timestamp TraceBuilder::TimestampAfter(const TraceBuffer& buffer, absl::Time begin_time) {
    return Impl::TimestampAfter(buffer, begin_time);
}

void TraceBuilder::CreateTrace(const TraceBuffer& buffer, absl::Time begin_time, absl::Time end_time, GraphTrace* result) {
    impl_->CreateTrace(buffer, begin_time, end_time, result);
}

void TraceBuilder::CreateLog(const TraceBuffer& buffer, absl::Time begin_time, absl::Time end_time, GraphTrace* result) {

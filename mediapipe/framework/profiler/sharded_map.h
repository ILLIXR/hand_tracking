...
public:
    ...
    using const_iterator = Iterator<const ShardedMap*, typename Map::const_iterator>;
    ...
    inline std::pair<iterator, bool> insert(const value_type& val) ABSL_NO_THREAD_SAFETY_ANALYSIS {
    ...
    public:
        ...
        inline typename std::iterator_traits<map_iterator>::reference operator*() const {
            return *iter_;
        }

        inline typename std::iterator_traits<map_iterator>::pointer operator->() const {
            return &(operator*());
        }
    ...
    private:
        ...
        inline void Clear() ABSL_NO_THREAD_SAFETY_ANALYSIS {
            if (!map_)
                return;
            bool is_end = (shard_ == map_->maps_.size() - 1 && iter_ == map_->maps_[shard_].end());
            if (!is_end) {

...
namespace {
    ...
    std::string PacketIdToString(const PacketId& packet_id) {
        return absl::Substitute("stream_name: $0, timestamp_usec: $1", packet_id.stream_name, packet_id.timestamp_usec);
    }

    int GetLogIntervalCount(const ProfilerConfig& profiler_config) {
        return profiler_config.trace_log_interval_count() ? profiler_config.trace_log_interval_count()
                                                          : kDefaultLogIntervalCount;
    }

    int GetLogFileCount(const ProfilerConfig& profiler_config) {
        return profiler_config.trace_log_count() ? profiler_config.trace_log_count() : kDefaultLogFileCount;
    }
    ...
    bool IsTraceLogEnabled(const ProfilerConfig& profiler_config) {
        return IsTracerEnabled(profiler_config) && !profiler_config.trace_log_disabled();
    }
    ...
    bool IsTraceIntervalEnabled(const ProfilerConfig& profiler_config, GraphTracer* tracer) {
        return IsTraceLogEnabled(profiler_config) && tracer && absl::ToInt64Microseconds(tracer->GetTraceLogInterval()) != -1;
    }

    using PacketInfoMap = ShardedMap<std::string, std::list<std::pair<int64_t, PacketInfo>>>;
...
GraphProfiler::GraphProfiler()
    ...
    clock_ = std::shared_ptr<mediapipe::Clock>(mediapipe::MonotonicClock::CreateSynchronizedMonotonicClock());
}

GraphProfiler::~GraphProfiler() { }

void GraphProfiler::Initialize(const ValidatedGraphConfig& validated_graph_config) {
    absl::WriterMutexLock lock(&profiler_mutex_);
    validated_graph_ = &validated_graph_config;
    ABSL_CHECK(!is_initialized_) << "Cannot initialize the profiler for the same graph multiple times.";
    ...
    for (int node_id = 0; node_id < validated_graph_config.CalculatorInfos().size(); ++node_id) {
        std::string       node_name = tool::CanonicalNodeName(validated_graph_config.Config(), node_id);
        CalculatorProfile profile;
        profile.set_name(node_name);
        InitializeTimeHistogram(interval_size_usec, num_intervals, profile.mutable_process_runtime());
        if (profiler_config_.enable_stream_latency()) {
            InitializeTimeHistogram(interval_size_usec, num_intervals, profile.mutable_process_input_latency());
            InitializeTimeHistogram(interval_size_usec, num_intervals, profile.mutable_process_output_latency());

            const CalculatorGraphConfig::Node& node_config = validated_graph_config.Config().node(node_id);
            InitializeOutputStreams(node_config);
            InitializeInputStreams(node_config, interval_size_usec, num_intervals, &profile);
        }

        auto iter = calculator_profiles_.insert({node_name, profile});
        ABSL_CHECK(iter.second) << absl::Substitute("Calculator \"$0\" has already been added.", node_name);
    }
...
void GraphProfiler::Reset() {
    absl::WriterMutexLock lock(&profiler_mutex_);
    for (auto iter = calculator_profiles_.begin(); iter != calculator_profiles_.end(); ++iter) {
        ...
        for (auto& input_stream_profile : *(calculator_profile->mutable_input_stream_profiles())) {
...
absl::Status GraphProfiler::Start(mediapipe::Executor* executor) {
    ...
    Resume();
    if (is_tracing_ && IsTraceIntervalEnabled(profiler_config_, tracer()) && executor != nullptr) {
        ...
        auto status =
            file::SetContents(absl::StrCat(trace_log_path, "trace_writing_check"), "can write trace logs to this location");
        if (status.ok()) {
            ABSL_LOG(INFO) << "trace_log_path: " << trace_log_path;
        } else {
            ABSL_LOG(ERROR) << "cannot write to trace_log_path: " << trace_log_path << ": " << status;
        }
...
void GraphProfiler::LogEvent(const TraceEvent& event) {
    ...
    if (packet_tracer_) {
        if (event.event_type == GraphTrace::GPU_TASK || event.event_type == GraphTrace::GPU_CALIBRATION) {
...
void GraphProfiler::AddPacketInfo(const TraceEvent& packet_info) {
    ...
    if (!packet_timestamp.IsRangeValue()) {
        ABSL_LOG(WARNING) << absl::Substitute("Skipped adding packet info because the timestamp $0 for stream "
                                              "\"$1\" is not valid.",
    ...
    int64_t production_time_usec =
        profiler_config_.use_packet_timestamp_for_added_packet() ? packet_timestamp.Value() : TimeNowUsec();
    AddPacketInfoInternal(PacketId({stream_name, packet_timestamp.Value()}), production_time_usec, production_time_usec);
}

absl::Status GraphProfiler::GetCalculatorProfiles(std::vector<CalculatorProfile>* profiles) const {
    absl::ReaderMutexLock lock(&profiler_mutex_);
    RET_CHECK(is_initialized_) << "GetCalculatorProfiles can only be called after Initialize()";
...
void GraphProfiler::InitializeTimeHistogram(int64_t interval_size_usec, int64_t num_intervals, TimeHistogram* histogram) {
...
void GraphProfiler::InitializeOutputStreams(const CalculatorGraphConfig::Node& node_config) { }

void GraphProfiler::InitializeInputStreams(const CalculatorGraphConfig::Node& node_config, int64_t interval_size_usec,
                                           int64_t num_intervals, CalculatorProfile* calculator_profile) {
    std::shared_ptr<tool::TagMap> input_tag_map       = TagMap::Create(node_config.input_stream()).value();
    ...
    for (int i = 0; i < input_tag_map_names.size(); ++i) {
        ...
        StreamProfile* input_stream_profile = calculator_profile->add_input_stream_profiles();
        input_stream_profile->set_name(input_stream_name);
        input_stream_profile->set_back_edge(back_edge_ids.find(i) != back_edge_ids.end());
        InitializeTimeHistogram(interval_size_usec, num_intervals, input_stream_profile->mutable_latency());
    }
}

std::set<int> GraphProfiler::GetBackEdgeIds(const CalculatorGraphConfig::Node& node_config, const TagMap& input_tag_map) {
    ...
    for (const auto& input_stream_info : node_config.input_stream_info()) {
        ...
        std::string tag;
        int         index;
        MEDIAPIPE_CHECK_OK(tool::ParseTagIndex(input_stream_info.tag_index(), &tag, &index))
            << absl::Substitute("Cannot parse TAG or index for the backedge \"$0\"", input_stream_info.tag_index());
        ABSL_CHECK(0 <= index && index < input_tag_map.NumEntries(tag))
            << absl::Substitute("The input_stream_info for tag \"$0\" (index "
...
void GraphProfiler::AddPacketInfoInternal(const PacketId& packet_id, int64_t production_time_usec,
...
void GraphProfiler::AddPacketInfoForOutputPackets(const OutputStreamShardSet& output_stream_shard_set,
    ...
    for (const OutputStreamShard& output_stream_shard : output_stream_shard_set) {
        for (const Packet& output_packet : *output_stream_shard.OutputQueue()) {
            AddPacketInfoInternal(PacketId({output_stream_shard.Name(), output_packet.Timestamp().Value()}),
                                  production_time_usec, source_process_start_usec);
        }
    }
}

int64_t GraphProfiler::AddStreamLatencies(const CalculatorContext& calculator_context, int64_t start_time_usec,
                                          int64_t end_time_usec, CalculatorProfile* calculator_profile) {
    // Update input streams profiles.
    int64_t min_source_process_start_usec = AddInputStreamTimeSamples(calculator_context, start_time_usec, calculator_profile);

    // Update output production times.
    AddPacketInfoForOutputPackets(calculator_context.Outputs(), end_time_usec, min_source_process_start_usec);
    return min_source_process_start_usec;
}

void GraphProfiler::SetOpenRuntime(const CalculatorContext& calculator_context, int64_t start_time_usec,
    ...
    ABSL_CHECK(profile_iter != calculator_profiles_.end())
        << absl::Substitute("Calculator \"$0\" has not been added during initialization.", calculator_context.NodeName());
    CalculatorProfile* calculator_profile = &profile_iter->second;
    calculator_profile->set_open_runtime(time_usec);

    if (profiler_config_.enable_stream_latency()) {
        AddStreamLatencies(calculator_context, start_time_usec, end_time_usec, calculator_profile);
    }
}

void GraphProfiler::SetCloseRuntime(const CalculatorContext& calculator_context, int64_t start_time_usec,
                                    int64_t end_time_usec) {
    absl::ReaderMutexLock lock(&profiler_mutex_);
    if (!is_profiling_) {
        return;
...
void GraphProfiler::AddTimeSample(int64_t start_time_usec, int64_t end_time_usec, TimeHistogram* histogram) {
    if (end_time_usec < start_time_usec) {
        ABSL_LOG(ERROR) << absl::Substitute("end_time_usec ($0) is < start_time_usec ($1)", end_time_usec, start_time_usec);
        return;
    }
...
int64_t GraphProfiler::AddInputStreamTimeSamples(const CalculatorContext& calculator_context, int64_t start_time_usec,
    ...
    for (CollectionItemId id = calculator_context.Inputs().BeginId(); id < calculator_context.Inputs().EndId(); ++id) {
        ++input_stream_counter;
        if (calculator_context.Inputs().Get(id).Value().IsEmpty() ||
            calculator_profile->input_stream_profiles(input_stream_counter).back_edge()) {
            continue;
        }

        PacketId    packet_id   = {calculator_context.Inputs().Get(id).Name(), input_timestamp_usec};
        ...
        if (packet_info == nullptr) {
            ...
            ABSL_LOG_FIRST_N(WARNING, 10) << "Expected packet info is missing for: " << PacketIdToString(packet_id);
            continue;
        }
        AddTimeSample(packet_info->production_time_usec, start_time_usec,
                      calculator_profile->mutable_input_stream_profiles(input_stream_counter)->mutable_latency());

        min_source_process_start_usec = std::min(min_source_process_start_usec, packet_info->source_process_start_usec);
    }

    return min_source_process_start_usec;
}

void GraphProfiler::AddProcessSample(const CalculatorContext& calculator_context, int64_t start_time_usec,
                                     int64_t end_time_usec) {
    absl::ReaderMutexLock lock(&profiler_mutex_);
    if (!is_profiling_) {
        return;
    ...
    AddTimeSample(start_time_usec, end_time_usec, calculator_profile->mutable_process_runtime());

    if (profiler_config_.enable_stream_latency()) {
        int64_t min_source_process_start_usec =
            AddStreamLatencies(calculator_context, start_time_usec, end_time_usec, calculator_profile);
        // Update input and output trace latencies.
        AddTimeSample(min_source_process_start_usec, start_time_usec, calculator_profile->mutable_process_input_latency());
        AddTimeSample(min_source_process_start_usec, end_time_usec, calculator_profile->mutable_process_output_latency());
    }
}
...
void AssignNodeNames(GraphProfile* profile) {
    ...
    GraphTrace*            graph_trace  = profile->graph_trace_size() > 0 ? profile->mutable_graph_trace(0) : nullptr;
    if (graph_trace) {
    ...
    if (graph_trace) {
        graph_trace->mutable_calculator_name()->Assign(canonical_names.begin(), canonical_names.end());
    }
}
...
absl::StatusOr<std::string> GraphProfiler::GetTraceLogPath() {
    if (!IsTraceLogEnabled(profiler_config_)) {
        return absl::InternalError("Trace log writing is disabled, unable to get trace_log_path.");
    }
    if (profiler_config_.trace_log_path().empty()) {
        MP_ASSIGN_OR_RETURN(std::string directory_path, GetDefaultTraceLogDirectory());
        std::string trace_log_path = absl::StrCat(directory_path, "/", kDefaultLogFilePrefix);
...
absl::Status GraphProfiler::CaptureProfile(GraphProfile* result, PopulateGraphConfig populate_config) {
    ...
    absl::Time end_time = clock_->TimeNow() - absl::Microseconds(profiler_config_.trace_log_margin_usec());
    if (tracer()) {
...
absl::Status GraphProfiler::WriteProfile() {
    ...
    RET_CHECK(profile.SerializeToZeroCopyStream(&out)) << "Could not write binary GraphProfile to: " << log_path;
    return absl::OkStatus();
}

...
struct TraceEvent {
    ...
    static constexpr EventType GPU_TASK_INVOKE_ADVANCED = GraphTrace::GPU_TASK_INVOKE_ADVANCED;
    static constexpr EventType TPU_TASK_INVOKE_ASYNC    = GraphTrace::TPU_TASK_INVOKE_ASYNC;
};
...
public:
    ...
    TraceEventType(EventType event_type, std::string description, bool is_packet_event = false, bool is_stream_event = false,
    ...
    inline EventType event_type() const {
        return event_type_;
    }
    ...
    inline bool is_packet_event() const {
        return is_packet_event_;
    }

    // True if stream details are logged with this type of event.
    inline bool is_stream_event() const {
        return is_stream_event_;
    }

    // True if event_data values are assigned compact id's.
    inline bool id_event_data() const {
        return id_event_data_;
    }
...
using TraceEventRegistry = std::unordered_map<TraceEvent::EventType, TraceEventType, EventTypeHash>;

...
public:
    explicit WepPerformanceTraceScope(TraceEvent::EventType event_type, const char* event_type_str, CalculatorContext* cc)
        : event_type_str_(event_type_str)
        , cc_(cc) {
        const auto& calculator_name = cc->NodeName();
        std::string start_name      = absl::StrCat(calculator_name, "::", event_type_str_, "_start");
    ...
    ~WepPerformanceTraceScope() {
        ...
        std::string start_name      = absl::StrCat(calculator_name, "::", event_type_str_, "_start");
        std::string end_name        = absl::StrCat(calculator_name, "::", event_type_str_, "_end");
        std::string measure_name    = absl::StrCat(calculator_name, "::", event_type_str_);
        EM_ASM(
            {
                const startName   = UTF8ToString($0);
...
private:
    ...
    #define MEDIAPIPE_WEB_PERFORMANCE_SCOPE(event_type, calculator_context) \
        mediapipe::WepPerformanceTraceScope web_trace_scope(mediapipe::TraceEvent::event_type, #event_type, calculator_context)

...
absl::Duration GraphTracer::GetTraceLogInterval() {
    return profiler_config_.trace_log_interval_usec() ? absl::Microseconds(profiler_config_.trace_log_interval_usec())
                                                      : kDefaultTraceLogInterval;
}

int64_t GraphTracer::GetTraceLogCapacity() {
    return profiler_config_.trace_log_capacity() ? profiler_config_.trace_log_capacity() : 20000;
}
...
void GraphTracer::LogInputEvents(GraphTrace::EventType event_type, const CalculatorContext* context, absl::Time event_time) {
...
void GraphTracer::LogOutputEvents(GraphTrace::EventType event_type, const CalculatorContext* context, absl::Time event_time) {
    // For source nodes, the first output timestamp is used as the input_ts.
    Timestamp input_ts = (context->Inputs().NumEntries() > 0) ? context->InputTimestamp() : GetOutputTimestamp(context);
...
void GraphTracer::GetTrace(absl::Time begin_time, absl::Time end_time, GraphTrace* result) {
...
void GraphTracer::GetLog(absl::Time begin_time, absl::Time end_time, GraphTrace* result) {
    absl::MutexLock lock(trace_builder_mutex());
    trace_builder_.CreateLog(trace_buffer_, begin_time, end_time, result);
    trace_builder_.Clear();
}

const TraceBuffer& GraphTracer::GetTraceBuffer() {

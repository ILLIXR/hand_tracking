...
// static
absl::StatusOr<Executor*> ThreadPoolExecutor::Create(const MediaPipeOptions& extendable_options) {
    auto& options = extendable_options.GetExtension(ThreadPoolExecutorOptions::ext);
    if (!options.has_num_threads()) {
        return absl::InvalidArgumentError("num_threads is not specified in ThreadPoolExecutorOptions.");
    }
...
ThreadPoolExecutor::ThreadPoolExecutor(const ThreadOptions& thread_options, int num_threads)
    : thread_pool_(thread_options, thread_options.name_prefix().empty() ? "mediapipe" : thread_options.name_prefix(),
                   num_threads) {
    Start();
}
...
void ThreadPoolExecutor::Start() {
    ...
    VLOG(2) << "Started thread pool with " << thread_pool_.num_threads() << " threads.";
}

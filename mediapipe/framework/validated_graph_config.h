...
public:
    ...
    absl::Status Initialize(const ValidatedGraphConfig& validated_graph, const CalculatorGraphConfig::Node& node,
                            int node_index);
    ...
    const PacketTypeSet& InputStreamTypes() const {
        return contract_.Inputs();
    }

    const PacketTypeSet& OutputStreamTypes() const {
        return contract_.Outputs();
    }
    ...
    int InputSidePacketBaseIndex() const {
        return input_side_packet_base_index_;
    }
    ...
    int InputStreamBaseIndex() const {
        return input_stream_base_index_;
    }

    int OutputStreamBaseIndex() const {
        return output_stream_base_index_;
    }
    ...
    void SetInputStreamBaseIndex(int index) {
        input_stream_base_index_ = index;
    }
    ...
    bool AddSource(int index) {
        return ancestor_sources_.insert(index).second;
    }
...
public:
    ...
    absl::Status Initialize(CalculatorGraphConfig input_config, const GraphRegistry* graph_registry = nullptr,
    ...
    absl::Status Initialize(const std::string& graph_type, const GraphRegistry* graph_registry = nullptr,
                            const Subgraph::SubgraphOptions* graph_options   = nullptr,
                            const GraphServiceManager*       service_manager = nullptr);
    ...
    absl::Status Initialize(const std::vector<CalculatorGraphConfig>&   input_configs,
                            const std::vector<CalculatorGraphTemplate>& input_templates, const std::string& graph_type = "",
                            const Subgraph::SubgraphOptions* graph_options   = nullptr,
                            const GraphServiceManager*       service_manager = nullptr);
...
private:
    ...
    absl::Status PerformBasicTransforms(const GraphRegistry* graph_registry, const Subgraph::SubgraphOptions* graph_options,
    ...
    absl::Status AddOutputSidePacketsForNode(NodeTypeInfo* node_type_info, bool* need_sorting_ptr);
    ...
    absl::Status AddInputStreamsForNode(NodeTypeInfo* node_type_info, bool* need_sorting_ptr);
    // A helper function for adding a single output stream EdgeInfo.
    absl::Status AddOutputStream(NodeTypeInfo::NodeRef node, const std::string& name, PacketType* packet_type);
    ...
    absl::Status ResolveAnyTypes(std::vector<EdgeInfo>* input_edges, std::vector<EdgeInfo>* output_edges);
    // Narrow down OneOf types if they other end is a single type.
    absl::Status ResolveOneOfTypes(std::vector<EdgeInfo>* input_edges, std::vector<EdgeInfo>* output_edges);
...
template<typename T>
absl::Status ValidatedGraphConfig::CanAcceptSidePackets(const std::map<std::string, T>& side_packets) const {
    for (const auto& output_side_packet : output_side_packets_) {
        if (ContainsKey(side_packets, output_side_packet.name)) {
            return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
                << "Side packet \"" << output_side_packet.name << "\" is both provided and generated by a PacketGenerator.";
        }
    }

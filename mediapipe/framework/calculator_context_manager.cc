...
void CalculatorContextManager::Initialize(CalculatorState* calculator_state, std::shared_ptr<tool::TagMap> input_tag_map,
                                          std::shared_ptr<tool::TagMap> output_tag_map, bool calculator_run_in_parallel) {
...
absl::Status CalculatorContextManager::PrepareForRun(std::function<absl::Status(CalculatorContext*)> setup_shards_callback) {
    setup_shards_callback_ = std::move(setup_shards_callback);
    default_context_       = absl::make_unique<CalculatorContext>(calculator_state_, input_tag_map_, output_tag_map_);
...
CalculatorContext* CalculatorContextManager::GetDefaultCalculatorContext() const {
    ABSL_CHECK(default_context_.get());
    return default_context_.get();
}

CalculatorContext* CalculatorContextManager::GetFrontCalculatorContext(Timestamp* context_input_timestamp) {
...
CalculatorContext* CalculatorContextManager::PrepareCalculatorContext(Timestamp input_timestamp) {
    ...
    if (idle_contexts_.empty()) {
        auto new_context = absl::make_unique<CalculatorContext>(calculator_state_, input_tag_map_, output_tag_map_);
        ...
        active_contexts_.emplace(input_timestamp, std::move(idle_contexts_.front()));

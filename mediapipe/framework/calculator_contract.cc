...
namespace {
    ...
    CalculatorGraphConfig::Node MakePacketGeneratorWrapperConfig(const PacketGeneratorConfig& node,
        ...
        *wrapper_node.mutable_output_side_packet() = node.output_side_packet();
        ...
        auto* wrapper_options =
            wrapper_node.mutable_options()->MutableExtension(mediapipe::PacketGeneratorWrapperCalculatorOptions::ext);
...
absl::Status CalculatorContract::Initialize(const CalculatorGraphConfig::Node& node) {
    ...
    auto input_side_packet_statusor = tool::TagMap::Create(node.input_side_packet());
    if (!input_side_packet_statusor.ok()) {
        statuses.push_back(std::move(input_side_packet_statusor).status());
    }
    auto output_side_packet_statusor = tool::TagMap::Create(node.output_side_packet());
    if (!output_side_packet_statusor.ok()) {
        statuses.push_back(std::move(output_side_packet_statusor).status());
    }

    if (!statuses.empty()) {
        auto builder = mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Unable to initialize TagMaps for node.";
    ...
    inputs_              = absl::make_unique<PacketTypeSet>(std::move(input_stream_statusor).value());
    outputs_             = absl::make_unique<PacketTypeSet>(std::move(output_stream_statusor).value());
    input_side_packets_  = absl::make_unique<PacketTypeSet>(std::move(input_side_packet_statusor).value());
    output_side_packets_ = absl::make_unique<PacketTypeSet>(std::move(output_side_packet_statusor).value());
    return absl::OkStatus();
}

absl::Status CalculatorContract::Initialize(const PacketGeneratorConfig& node, const std::string& package) {
    std::vector<absl::Status> statuses;
    ...
    if (!statuses.empty()) {
        auto builder = UnknownErrorBuilder(MEDIAPIPE_LOC) << "NodeTypeInfo Initialization failed.";
        for (const auto& status : statuses) {
            builder << "\n" << status.message();
        }
#if !(defined(MEDIAPIPE_LITE) || defined(MEDIAPIPE_MOBILE))
    ...
    wrapper_config_ = std::make_unique<CalculatorGraphConfig::Node>(MakePacketGeneratorWrapperConfig(node, package));
...
absl::Status CalculatorContract::Initialize(const StatusHandlerConfig& node) {
    ...
    if (!statuses.empty()) {
        auto builder = mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "NodeTypeInfo Initialization failed.";
        for (const auto& status : statuses) {
            builder << "\n" << status.message();
        }
#if !(defined(MEDIAPIPE_LITE) || defined(MEDIAPIPE_MOBILE))

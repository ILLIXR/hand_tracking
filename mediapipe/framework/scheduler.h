...
namespace internal {
    ...
    public:
        ...
        // Runs application thread tasks while waiting.
        void WaitUntilGraphInputStreamUnthrottled(absl::Mutex* secondary_mutex) ABSL_LOCKS_EXCLUDED(state_mutex_)
        ...
        void AddNodeToSourcesQueue(CalculatorNode* node) ABSL_LOCKS_EXCLUDED(state_mutex_);
    ...
    private:
        ...
        bool TryToScheduleNextSourceLayer() ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mutex_);
        ...
        std::priority_queue<SchedulerQueue::Item> sources_queue_ ABSL_GUARDED_BY(state_mutex_);
        ...
        std::deque<std::function<void()>> app_thread_tasks_ ABSL_GUARDED_BY(state_mutex_);

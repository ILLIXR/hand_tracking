...
absl::Status InputStreamManager::Initialize(const std::string& name, const PacketType* packet_type, bool back_edge) {
    name_        = name;
...
void InputStreamManager::SetQueueSizeCallbacks(QueueSizeCallback becomes_full_callback,
...
absl::Status InputStreamManager::AddPackets(const std::list<Packet>& container, bool* notify) {
    return AddOrMovePacketsInternal<const std::list<Packet>&>(container, notify);
}

absl::Status InputStreamManager::MovePackets(std::list<Packet>* container, bool* notify) {
    return AddOrMovePacketsInternal<std::list<Packet>&>(*container, notify);
}

template<typename Container>
absl::Status InputStreamManager::AddOrMovePacketsInternal(Container container, bool* notify) {
    *notify                     = false;
    ...
    bool queue_became_full      = false;
    {
        ...
        bool was_queue_full = (max_queue_size_ != -1 && queue_.size() >= (size_t) max_queue_size_);
        ...
        for (auto& packet : container) {
            ...
            if (!result.ok()) {
                return tool::AddStatusPrefix(
                    absl::StrCat("Packet type mismatch on a calculator receiving from stream \"", name_, "\": "), result);
            }
            ...
            if (!timestamp.IsAllowedInStream()) {
                return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                    << "In stream \"" << name_
                    << "\", timestamp not specified or set to illegal value: " << timestamp.DebugString();
            }
                ...
                if (timestamp < next_timestamp_bound_) {
                    return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                        << "Packet timestamp mismatch on a calculator receiving from "
                           "stream \""
                        << name_ << "\". Current minimum expected timestamp is " << next_timestamp_bound_.DebugString()
                        << " but received " << timestamp.DebugString()
            ...
            ++num_packets_added_;
            VLOG(3) << "Input stream:" << name_ << " has added packet at time: " << packet.Timestamp();
            if (std::is_const<typename std::remove_reference<Container>::type>::value) {
                queue_.emplace_back(packet);
            } else {
                queue_.emplace_back(std::move(packet));
            }
        }
        queue_became_full = (!was_queue_full && max_queue_size_ != -1 && queue_.size() >= (size_t) max_queue_size_);
        if (queue_.size() > 1) {
            VLOG(3) << "Queue size greater than 1: stream name: " << name_ << " queue_size: " << queue_.size();
        }
        VLOG(3) << "Input stream:" << name_ << " becomes non-empty status:" << queue_became_non_empty
                << " Size: " << queue_.size();
    }
...
absl::Status InputStreamManager::SetNextTimestampBound(const Timestamp bound, bool* notify) {
    *notify = false;
    {
        // Scope to prevent locking the stream when notification is called.
        absl::MutexLock stream_lock(&stream_mutex_);
        ...
        if (enable_timestamps_ && bound < next_timestamp_bound_) {
            return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
                ...
                << name_ << "\". Current minimum expected timestamp is " << next_timestamp_bound_.DebugString()
                << " but received " << bound.DebugString();
        }
        ...
        if (bound > next_timestamp_bound_) {
            ...
            VLOG(3) << "Next timestamp bound for input " << name_ << " is " << next_timestamp_bound_;
            if (queue_.empty()) {
...
Timestamp InputStreamManager::MinTimestampOrBoundHelper() const ABSL_EXCLUSIVE_LOCKS_REQUIRED(stream_mutex_) {
    return queue_.empty() ? next_timestamp_bound_ : queue_.front().Timestamp();
}

Packet InputStreamManager::PopPacketAtTimestamp(Timestamp timestamp, int* num_packets_dropped, bool* stream_is_done) {
    ...
    *stream_is_done              = false;
    ...
    Packet packet;
    {
        ...
        VLOG(3) << "Input stream " << name_ << " selecting at timestamp:" << timestamp.Value()
        ...
        VLOG(3) << "Input stream removed packets:" << name_ << " Size:" << queue_.size();

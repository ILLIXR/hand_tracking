...
namespace internal {
    ...
    bool SchedulerQueue::IsIdle() {
        VLOG(3) << "Scheduler queue empty: " << queue_.empty() << ", # of pending tasks: " << num_pending_tasks_;
    ...
    void SchedulerQueue::RunNextTask() {
        ...
        bool               is_open_node;
        {
            ...
            ABSL_CHECK(!node->Closed()) << "Scheduled a node that was closed. This should not happen.";
        }
    ...
    void SchedulerQueue::RunCalculatorNode(CalculatorNode* node, CalculatorContext* cc) {
        ...
        if (shared_->stopping && node->IsSource()) {
            ...
            const absl::Status result = node->CloseNode(absl::OkStatus(), /*graph_run_ended=*/false);
            shared_->timer.EndNode(start_time);
            if (!result.ok()) {
                VLOG(3) << node->DebugName() << " had an error while closing due to StatusStop()!";

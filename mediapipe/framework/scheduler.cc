...
namespace internal {
    ...
    Scheduler::Scheduler(CalculatorGraph* graph)
        : graph_(graph)
        , shared_()
        , default_queue_(&shared_) {
        shared_.error_callback = std::bind(&CalculatorGraph::RecordError, graph_, std::placeholders::_1);
        default_queue_.SetIdleCallback(std::bind(&Scheduler::QueueIdleStateChanged, this, std::placeholders::_1));
    ...
    void Scheduler::SetExecutor(Executor* executor) {
        ABSL_CHECK_EQ(state_, STATE_NOT_STARTED) << "SetExecutor must not be called after the scheduler has started";
        default_queue_.SetExecutor(executor);
    }

    // TODO: Consider renaming this method CreateNonDefaultQueue.
    absl::Status Scheduler::SetNonDefaultExecutor(const std::string& name, Executor* executor) {
        RET_CHECK_EQ(state_, STATE_NOT_STARTED) << "SetNonDefaultExecutor must not "
                                                   "be called after the scheduler "
                                                   "has started";
        auto inserted = non_default_queues_.emplace(name, absl::make_unique<SchedulerQueue>(&shared_));
        RET_CHECK(inserted.second) << "SetNonDefaultExecutor must be called only once for the executor \"" << name << "\"";

        SchedulerQueue* queue = inserted.first->second.get();
        queue->SetIdleCallback(std::bind(&Scheduler::QueueIdleStateChanged, this, std::placeholders::_1));
    ...
    void Scheduler::HandleIdle() {
        ...
        while (IsIdle() && (state_ == STATE_RUNNING || state_ == STATE_CANCELLING)) {
            ...
            if (shared_.has_error || (active_sources_.empty() && sources_queue_.empty() && graph_input_streams_closed_)) {
    ...
    void Scheduler::WaitUntilGraphInputStreamUnthrottled(absl::Mutex* secondary_mutex) {
        ...
        ApplicationThreadAwait([this, seq_num]() ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mutex_) {
    ...
    absl::Status Scheduler::WaitForObservedOutput() {
        bool observed = false;
        ApplicationThreadAwait([this, &observed]() ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mutex_) {
    ...
    void Scheduler::ApplicationThreadAwait(const std::function<bool()>& stop_condition) {
        absl::MutexLock lock(&state_mutex_);
    ...
    bool Scheduler::IsIdle() {
        return non_idle_queue_count_ == 0;
    }
    ...
    // container.
    void Scheduler::ScheduleNodeIfNotThrottled(CalculatorNode* node, CalculatorContext* calculator_context) {
        ABSL_DCHECK(node);
    ...
    void Scheduler::ScheduleUnthrottledReadyNodes(const std::vector<CalculatorNode*>& nodes_to_schedule) {
    ...
    bool Scheduler::TryToScheduleNextSourceLayer() {
        ...
        if (!unopened_sources_.empty() &&
            (*unopened_sources_.begin())->source_layer() < sources_queue_.top().Node()->source_layer()) {
            ...
            if (graph_input_streams_closed_) {
                graph_->RecordError(absl::UnknownError("Detected a deadlock because source nodes cannot be activated when a "
        ...
        while (!sources_queue_.empty()) {
            ...
            if (activated_any && (node->source_layer() != active_sources_.back()->source_layer())) {
                break;
            }

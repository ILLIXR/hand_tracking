...
std::string DebugEdgeNames(const std::string& edge_type, const proto_ns::RepeatedPtrField<ProtoString>& edges) {
    if (edges.empty()) {
...
std::string DebugName(const CalculatorGraphConfig::Node& node_config) {
    ...
    if (name.empty()) {
        return absl::StrCat("[", node_config.calculator(), ", ", DebugEdgeNames("input stream", node_config.input_stream()),
                            ", and ", DebugEdgeNames("output stream", node_config.output_stream()), "]");
    }
    return name;
}
...
namespace {
    ...
    std::string DebugName(const PacketGeneratorConfig& node_config) {
        return absl::StrCat("[", node_config.packet_generator(), ", ",
                            DebugEdgeNames("input side packet", node_config.input_side_packet()), ", and ",
    ...
    std::string DebugName(const StatusHandlerConfig& node_config) {
        return absl::StrCat("[", node_config.status_handler(), ", ",
                            DebugEdgeNames("input side packet", node_config.input_side_packet()), "]");
    }

    std::string DebugName(const CalculatorGraphConfig& config, NodeTypeInfo::NodeType node_type, int node_index) {
        switch (node_type) {
        ...
        ABSL_LOG(FATAL) << "Unknown NodeTypeInfo::NodeType: " << NodeTypeInfo::NodeTypeToString(node_type);
    }
    ...
    absl::Status AddPredefinedExecutorConfigs(CalculatorGraphConfig* graph_config) {
        ...
        for (ExecutorConfig& executor_config : *graph_config->mutable_executor()) {
            if (executor_config.name().empty()) {
                if (graph_config->num_threads()) {
                    return absl::InvalidArgumentError("ExecutorConfig for the default executor and the graph-level "
        ...
        if (!has_default_executor_config) {
            ...
            if (graph_config->num_threads()) {
                ...
                options->MutableExtension(ThreadPoolExecutorOptions::ext)->set_num_threads(graph_config->num_threads());
...
std::string NodeTypeInfo::NodeTypeToString(NodeType node_type) {
    ...
    ABSL_LOG(FATAL) << "Unknown NodeTypeInfo::NodeType: " << static_cast<int>(node_type);
}

absl::Status NodeTypeInfo::Initialize(const ValidatedGraphConfig& validated_graph, const CalculatorGraphConfig::Node& node,
                                      int node_index) {
    node_.type  = NodeType::CALCULATOR;
    node_.index = node_index;
    MP_RETURN_IF_ERROR(contract_.Initialize(node));
    contract_.SetNodeName(CanonicalNodeName(validated_graph.Config(), node_index));
    ...
    if (!node.input_stream_info().empty()) {
        ...
        for (const auto& input_stream_info : node.input_stream_info()) {
            std::string tag;
            int         index;
            MP_RETURN_IF_ERROR(tool::ParseTagIndex(input_stream_info.tag_index(), &tag, &index));
            CollectionItemId id = contract_.Inputs().GetId(tag, index);
            if (!id.IsValid()) {
                return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                    << "Input stream with tag_index \"" << input_stream_info.tag_index()
    ...
    for (const mediapipe::protobuf::Any& options : node.node_options()) {
        RET_CHECK(type_urls.insert(options.type_url()).second)
            << "Options type: '" << options.type_url() << "' specified more than once for a single calculator node config.";
    }
#endif
    ...
    MP_RETURN_IF_ERROR(calculator_factory->GetContract(&contract_)).SetPrepend() << node_class << ": ";

    // Validate result of FillExpectations or GetContract.
    std::vector<absl::Status> statuses;
    absl::Status              status = ValidatePacketTypeSet(contract_.Inputs());
    if (!status.ok()) {
        statuses.push_back(mediapipe::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend() << "For input streams ");
    }
    status = ValidatePacketTypeSet(contract_.Outputs());
    if (!status.ok()) {
        statuses.push_back(mediapipe::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend() << "For output streams ");
    }
    status = ValidatePacketTypeSet(contract_.InputSidePackets());
    if (!status.ok()) {
        statuses.push_back(mediapipe::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend()
                           << "For input side packets ");
    }
    if (!statuses.empty()) {
        return tool::CombinedStatus(
            absl::StrCat(node_class, "::", calculator_factory->ContractMethodName(), " failed to validate: "), statuses);
    }
    return absl::OkStatus();
}

absl::Status NodeTypeInfo::Initialize(const ValidatedGraphConfig& validated_graph, const PacketGeneratorConfig& node,
                                      int node_index) {
    ...
    MP_ASSIGN_OR_RETURN(
        auto static_access,
        internal::StaticAccessToGeneratorRegistry::CreateByNameInNamespace(validated_graph.Package(), node_class),
        _ << "Unable to find PacketGenerator \"" << node_class << "\"");
    {
        LegacyCalculatorSupport::Scoped<CalculatorContract> s(&contract_);
        MP_RETURN_IF_ERROR(
            static_access->FillExpectations(node.options(), &contract_.InputSidePackets(), &contract_.OutputSidePackets()))
                .SetPrepend()
            << node_class << ": ";
    }
    ...
    if (!statuses.empty()) {
        return tool::CombinedStatus(absl::StrCat(node_class, "::FillExpectations failed to validate: "), statuses);
    }
    return absl::OkStatus();
}

absl::Status NodeTypeInfo::Initialize(const ValidatedGraphConfig& validated_graph, const StatusHandlerConfig& node,
                                      int node_index) {
    ...
    MP_ASSIGN_OR_RETURN(
        auto static_access,
        internal::StaticAccessToStatusHandlerRegistry::CreateByNameInNamespace(validated_graph.Package(), node_class),
        _ << "Unable to find StatusHandler \"" << node_class << "\"");
    {
        LegacyCalculatorSupport::Scoped<CalculatorContract> s(&contract_);
        MP_RETURN_IF_ERROR(static_access->FillExpectations(node.options(), &contract_.InputSidePackets())).SetPrepend()
            << node_class << ": ";
    }

    // Validate result of FillExpectations.
    MP_RETURN_IF_ERROR(ValidatePacketTypeSet(contract_.InputSidePackets())).SetPrepend()
        << node_class << "::FillExpectations failed to validate: ";
    return absl::OkStatus();
}

absl::Status ValidatedGraphConfig::Initialize(CalculatorGraphConfig input_config, const GraphRegistry* graph_registry,
                                              const Subgraph::SubgraphOptions* graph_options,
                                              const GraphServiceManager*       service_manager) {
    RET_CHECK(!initialized_) << "ValidatedGraphConfig can be initialized only once.";
    ...
    MP_RETURN_IF_ERROR(PerformBasicTransforms(graph_registry, graph_options, service_manager));
    ...
    for (int index = 0; index < (int) generators_.size(); ++index) {
        ...
        RET_CHECK(node_type_info->Node().type == NodeTypeInfo::NodeType::PACKET_GENERATOR);
    ...
    MP_RETURN_IF_ERROR(ResolveAnyTypes(&input_side_packets_, &output_side_packets_));
    MP_RETURN_IF_ERROR(ResolveOneOfTypes(&input_side_packets_, &output_side_packets_));
...
absl::Status ValidatedGraphConfig::Initialize(const std::string& graph_type, const GraphRegistry* graph_registry,
                                              const Subgraph::SubgraphOptions* graph_options,
                                              const GraphServiceManager*       service_manager) {
    graph_registry = graph_registry ? graph_registry : &GraphRegistry::global_graph_registry;
    Subgraph::SubgraphOptions local_graph_options;
    if (graph_options) {
        local_graph_options = *graph_options;
    }
    SubgraphContext subgraph_context = SubgraphContext(&local_graph_options, service_manager);
    auto            status_or_config = graph_registry->CreateByName("", graph_type, &subgraph_context);
    MP_RETURN_IF_ERROR(status_or_config.status());
    return Initialize(status_or_config.value(), graph_registry, graph_options, service_manager);
}

absl::Status ValidatedGraphConfig::Initialize(const std::vector<CalculatorGraphConfig>&   input_configs,
                                              const std::vector<CalculatorGraphTemplate>& input_templates,
                                              const std::string& graph_type, const Subgraph::SubgraphOptions* graph_options,
                                              const GraphServiceManager* service_manager) {
    ...
    return Initialize(graph_type, &graph_registry, graph_options, service_manager);
}

absl::Status ValidatedGraphConfig::PerformBasicTransforms(const GraphRegistry*             graph_registry,
                                                          const Subgraph::SubgraphOptions* graph_options,
                                                          const GraphServiceManager*       service_manager) {
    MP_RETURN_IF_ERROR(tool::ExpandSubgraphs(&config_, graph_registry, graph_options, service_manager));
    ...
    if (config_.has_input_stream_handler()) {
        const auto& graph_level_input_stream_handler = config_.input_stream_handler();
...
absl::Status ValidatedGraphConfig::InitializeCalculatorInfo() {
    ...
    for (const auto& node : config_.node()) {
        ...
        absl::Status status = calculators_.back().Initialize(*this, node, (int) calculators_.size() - 1);
        if (!status.ok()) {
            statuses.push_back(status);
        }
    }
    return tool::CombinedStatus("ValidatedGraphConfig Initialization failed.", statuses);
}
...
absl::Status ValidatedGraphConfig::InitializeGeneratorInfo() {
    ...
    for (const auto& node : config_.packet_generator()) {
        ...
        absl::Status status = generators_.back().Initialize(*this, node, (int) generators_.size() - 1);
        if (!status.ok()) {
            statuses.push_back(status);
        }
    }
...
absl::Status ValidatedGraphConfig::InitializeStatusHandlerInfo() {
    ...
    for (const auto& node : config_.status_handler()) {
        ...
        absl::Status status = status_handlers_.back().Initialize(*this, node, (int) status_handlers_.size() - 1);
        if (!status.ok()) {
            statuses.push_back(status);
        }
    }
...
absl::Status ValidatedGraphConfig::InitializeSidePacketInfo(bool* need_sorting_ptr) {
    for (NodeTypeInfo* node_type_info : sorted_nodes_) {
        MP_RETURN_IF_ERROR(AddInputSidePacketsForNode(node_type_info));
        MP_RETURN_IF_ERROR(AddOutputSidePacketsForNode(node_type_info, need_sorting_ptr));
    }
    ...
    for (int index = 0; index < config_.status_handler_size(); ++index) {
        ...
        RET_CHECK(node_type_info->Node().type == NodeTypeInfo::NodeType::STATUS_HANDLER);
        RET_CHECK_EQ(node_type_info->Node().index, index);
        MP_RETURN_IF_ERROR(AddInputSidePacketsForNode(node_type_info));
    }
    return absl::OkStatus();
...
absl::Status ValidatedGraphConfig::AddInputSidePacketsForNode(NodeTypeInfo* node_type_info) {
    ...
    const tool::TagMap& tag_map = *node_type_info->InputSidePacketTypes().TagMap();
...
absl::Status ValidatedGraphConfig::AddOutputSidePacketsForNode(NodeTypeInfo* node_type_info, bool* need_sorting_ptr) {
    node_type_info->SetOutputSidePacketBaseIndex((int) output_side_packets_.size());
    const tool::TagMap& tag_map = *node_type_info->OutputSidePacketTypes().TagMap();
    for (CollectionItemId id = tag_map.BeginId(); id < tag_map.EndId(); ++id) {
        const std::string& name = tag_map.Names()[id.value()];
        ...
        if (!mediapipe::InsertIfNotPresent(&side_packet_to_producer_, name, (int) output_side_packets_.size() - 1)) {
            return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Output Side Packet \"" << name << "\" defined twice.";
        }
            ...
            if (need_sorting_ptr) {
                *need_sorting_ptr = true;
                ...
                return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
                    << "Side packet \"" << name << "\" was produced after it was used.";
            }
        }
    }
    return absl::OkStatus();
...
absl::Status ValidatedGraphConfig::InitializeStreamInfo(bool* need_sorting_ptr) {
    // Define output streams for graph input streams.
    MP_ASSIGN_OR_RETURN(std::shared_ptr<tool::TagMap> graph_input_streams, tool::TagMap::Create(config_.input_stream()));
    for (int index = 0; index < (int) graph_input_streams->Names().size(); ++index) {
        ...
        NodeTypeInfo::NodeRef virtual_node{NodeTypeInfo::NodeType::GRAPH_INPUT_STREAM, index + config_.node_size()};
        MP_RETURN_IF_ERROR(AddOutputStream(virtual_node, name, owned_packet_types_.back().get()));
    }
    ...
    for (NodeTypeInfo& node_type_info : calculators_) {
        ...
        MP_RETURN_IF_ERROR(AddInputStreamsForNode(&node_type_info, need_sorting_ptr));
...
absl::Status ValidatedGraphConfig::AddOutputStreamsForNode(NodeTypeInfo* node_type_info) {
    ...
    for (CollectionItemId id = tag_map.BeginId(); id < tag_map.EndId(); ++id) {
        MP_RETURN_IF_ERROR(
            AddOutputStream(node_type_info->Node(), tag_map.Names()[id.value()], &node_type_info->OutputStreamTypes().Get(id)));
    }
    return absl::OkStatus();
}

absl::Status ValidatedGraphConfig::AddOutputStream(NodeTypeInfo::NodeRef node, const std::string& name,
    ...
    if (!mediapipe::InsertIfNotPresent(&stream_to_producer_, name, (int) output_streams_.size() - 1)) {
        return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Output Stream \"" << name << "\" defined twice.";
    }
    return absl::OkStatus();
}

absl::Status ValidatedGraphConfig::AddInputStreamsForNode(NodeTypeInfo* node_type_info, bool* need_sorting_ptr) {
    ...
    if (!config_.node(node_index).input_stream_info().empty()) {
        ...
        for (const auto& input_stream_info : config_.node(node_index).input_stream_info()) {
    ...
    for (CollectionItemId id = tag_map.BeginId(); id < tag_map.EndId(); ++id) {
        ...
        if (iter != stream_to_producer_.end()) {
            if (edge_info.back_edge) {
                ...
                if (!need_sorting_ptr) {
                    ABSL_LOG(WARNING) << "Input Stream \"" << name << "\" for node with sorted index " << node_index << " name "
            ...
            if (edge_info.back_edge) {
                VLOG(1) << "Encountered expected behavior: the back edge \"" << name
                        << "\" for node with (possibly sorted) index " << node_index << " name "
                        << node_type_info->Contract().GetNodeName() << " has an output stream which we have not yet seen.";
                ...
                *need_sorting_ptr = true;
                ...
                return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
                    << "Input Stream \"" << name << "\" for node with sorted index " << node_index << " name "
                    << node_type_info->Contract().GetNodeName() << " does not have a corresponding output stream.";
            }
        }
        ...
        if (edge_info.upstream > -1) {
            ...
            if (parent_node.type == NodeTypeInfo::NodeType::CALCULATOR) {
                ...
                output_streams_to_consumer_nodes_[edge_info.upstream].push_back(this_idx);
            }
        }
...
NodeTypeInfo::NodeRef ValidatedGraphConfig::NodeForSorterIndex(int index) const {
    if (index < (int) generators_.size()) {
        return {NodeTypeInfo::NodeType::PACKET_GENERATOR, index};
    } else {
        return {NodeTypeInfo::NodeType::CALCULATOR, index - static_cast<int>(generators_.size())};
    }
}
...
absl::Status ValidatedGraphConfig::TopologicalSortNodes() {
    ...
    for (int index = 0; index < (int) input_streams_.size(); ++index) {
        ...
        if (iter != stream_to_producer_.end()) {
            ...
            if (output_streams_[upstream].parent_node.type != NodeTypeInfo::NodeType::GRAPH_INPUT_STREAM &&
                !input_streams_[index].back_edge) {
                VLOG(3) << "Adding an edge for stream \"" << name << "\" from " << output_streams_[upstream].parent_node.index
                        << " to " << input_streams_[index].parent_node.index;
                sorter.AddEdge(SorterIndexForNode(output_streams_[upstream].parent_node),
                               SorterIndexForNode(input_streams_[index].parent_node));
            }
        }
    }
    for (int index = 0; index < (int) input_side_packets_.size(); ++index) {
        if (input_side_packets_[index].parent_node.type != NodeTypeInfo::NodeType::PACKET_GENERATOR &&
            input_side_packets_[index].parent_node.type != NodeTypeInfo::NodeType::CALCULATOR) {
            continue;
        }
        ...
        if (iter != side_packet_to_producer_.end()) {
            ...
            sorter.AddEdge(SorterIndexForNode(output_side_packets_[upstream].parent_node),
    ...
    while (sorter.GetNext(&index, &cyclic, &cycle_indexes)) {
        ...
        if (node.type == NodeTypeInfo::NodeType::PACKET_GENERATOR) {
            VLOG(3) << "Taking generator with index " << node.index << " in the original order";
            tmp_generators.emplace_back(std::move(generators_[node.index]));
            tmp_generators.back().SetNodeIndex((int) tmp_generators.size() - 1);
            generator_configs.Add()->Swap(config_.mutable_packet_generator(node.index));
            sorted_nodes_.push_back(&tmp_generators.back());
        } else {
            VLOG(3) << "Taking calculator with index " << node.index << " in the original order";
    ...
    if (cyclic) {
        ...
        auto node_name_formatter = [this](std::string* out, int i) {
            ...
            absl::StrAppend(out,
                            n.type == NodeTypeInfo::NodeType::CALCULATOR ? tool::CanonicalNodeName(Config(), n.index)
    ...
    for (int index = 0; index < (int) input_streams_.size(); ++index) {
        ...
        if (input_stream.back_edge) {
            RET_CHECK_EQ(-1, input_stream.upstream)
                << "Shouldn't have been able to know the upstream index for back edge" << input_stream.name << ".";
            auto iter = stream_to_producer_.find(input_stream.name);
            RET_CHECK(iter != stream_to_producer_.end()) << "Unable to find upstream edge for back edge \"" << input_stream.name
...
absl::Status ValidatedGraphConfig::ValidateSidePacketTypes() {
    for (const auto& side_packet : input_side_packets_) {
        ...
        if (side_packet.upstream != -1 &&
            !side_packet.packet_type->IsConsistentWith(*output_side_packets_[side_packet.upstream].packet_type)) {
            return absl::UnknownError(
                absl::Substitute("Input side packet \"$0\" of $1 \"$2\" expected a packet of type "
                                 "\"$3\" but the connected output side packet will be of type \"$4\"",
                                 side_packet.name, NodeTypeInfo::NodeTypeToString(side_packet.parent_node.type),
                                 DebugName(config_, side_packet.parent_node.type, side_packet.parent_node.index),
                                 side_packet.packet_type->DebugTypeName(),
                                 output_side_packets_[side_packet.upstream].packet_type->DebugTypeName()));
        }
    }
    return absl::OkStatus();
}

absl::Status ValidatedGraphConfig::ResolveAnyTypes(std::vector<EdgeInfo>* input_edges, std::vector<EdgeInfo>* output_edges) {
...
absl::Status ValidatedGraphConfig::ResolveOneOfTypes(std::vector<EdgeInfo>* input_edges, std::vector<EdgeInfo>* output_edges) {
    for (EdgeInfo& input_edge : *input_edges) {
        if (input_edge.upstream == -1) {
            continue;
        }
        ...
        PacketType* input_root  = input_edge.packet_type->GetSameAs();
        PacketType* output_root = output_edge.packet_type->GetSameAs();
        if (!input_root->IsConsistentWith(*output_root))
            continue;
...
absl::Status ValidatedGraphConfig::ValidateStreamTypes() {
    for (const EdgeInfo& stream : input_streams_) {
        ...
        if (!stream.packet_type->IsConsistentWith(*output_streams_[stream.upstream].packet_type)) {
            return absl::UnknownError(absl::Substitute(
                ...
                "\"$3\"",
                stream.name, mediapipe::DebugName(config_.node(stream.parent_node.index)), stream.packet_type->DebugTypeName(),
...
absl::Status ValidatedGraphConfig::ValidateExecutors() {
    ...
    for (const ExecutorConfig& executor_config : config_.executor()) {
        if (IsReservedExecutorName(executor_config.name())) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                << "\"" << executor_config.name() << "\" is a reserved executor name.";
        }
        if (!declared_names.emplace(executor_config.name()).second) {
            if (executor_config.name().empty()) {
                return absl::InvalidArgumentError("ExecutorConfig for the default executor is duplicate.");
            } else {
                return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                    << "ExecutorConfig for \"" << executor_config.name() << "\" is duplicate.";
            }
        }
    }
    for (const CalculatorGraphConfig::Node& node_config : config_.node()) {
        ...
        if (!declared_names.contains(executor_name)) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                << "The executor \"" << executor_name << "\" is not declared in an ExecutorConfig.";
        }
    }
    return absl::OkStatus();
}
...
absl::Status ValidatedGraphConfig::ValidateRequiredSidePackets(const std::map<std::string, Packet>& side_packets) const {
    std::vector<absl::Status> statuses;
    for (const auto& required_item : required_side_packets_) {
        ...
        if (iter == side_packets.end()) {
            ...
            statuses.push_back(mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                               << "Side packet \"" << required_item.first << "\" is required but was not provided.");
            continue;
        }
        for (int index : required_item.second) {
            absl::Status status = input_side_packets_[index].packet_type->Validate(iter->second);
            if (!status.ok()) {
                statuses.push_back(mediapipe::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend()
                                   << "Side packet \"" << required_item.first << "\" failed validation: ");
            }
        }
    }
    if (!statuses.empty()) {
        return tool::CombinedStatus("ValidateRequiredSidePackets failed to validate: ", statuses);
    }
    return absl::OkStatus();
}

absl::Status
ValidatedGraphConfig::ValidateRequiredSidePacketTypes(const std::map<std::string, PacketType>& side_packet_types) const {
    std::vector<absl::Status> statuses;
    for (const auto& required_item : required_side_packets_) {
        ...
        for (int index : required_item.second) {
            if (!input_side_packets_[index].packet_type->IsConsistentWith(iter->second)) {
                return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
                    << "Side packet \"" << required_item.first << "\" has incorrect type.";
            }
        }
    }
    if (!statuses.empty()) {
    ...
    for (int node_index = 0; node_index < (int) calculators_.size(); ++node_index) {
        ...
        if (node_type_info.InputStreamTypes().NumEntries() == 0) {
            ...
            for (int stream_index = node_type_info.InputStreamBaseIndex();
                 stream_index < node_type_info.InputStreamBaseIndex() + node_type_info.InputStreamTypes().NumEntries();
                 ++stream_index) {
                // Get all the sources of the upstream node.
                RET_CHECK(stream_index >= 0 && stream_index < (int) input_streams_.size())
                    << "Unable to find input streams for non-source node with index " << node_index << " tried to use "
                    << stream_index;
                const EdgeInfo& input_edge_info = input_streams_[stream_index];
                RET_CHECK_LE(0, input_edge_info.upstream)
                    << "input stream \"" << input_edge_info.name << "\" is not connected to an output stream.";
                const EdgeInfo& output_edge_info = output_streams_[input_edge_info.upstream];
                RET_CHECK_LE(0, output_edge_info.parent_node.index)
                    << "output stream \"" << output_edge_info.name << "\" does not have a valid node which owns it.";
                RET_CHECK_LE((size_t) output_edge_info.parent_node.index, calculators_.size() + config_.input_stream_size())
                    << "output stream \"" << output_edge_info.name << "\" does not have a valid node which owns it.";
                if (output_edge_info.parent_node.type == NodeTypeInfo::NodeType::GRAPH_INPUT_STREAM) {
                    // Add the virtual node for the graph input stream.
                    node_type_info.AddSource(output_edge_info.parent_node.index);
                    continue;
                }
                for (int source : calculators_[output_edge_info.parent_node.index].AncestorSources()) {
...
absl::StatusOr<std::string> ValidatedGraphConfig::RegisteredSidePacketTypeName(const std::string& name) {
    auto iter    = side_packet_to_producer_.find(name);
    bool defined = false;
    if (iter != side_packet_to_producer_.end()) {
        defined                     = true;
        ...
        if (output_edge.packet_type) {
            const std::string* registered_type = output_edge.packet_type->RegisteredTypeName();
            if (registered_type) {
    ...
    for (const EdgeInfo& input_edge : input_side_packets_) {
        if (input_edge.name == name) {
            ...
            if (input_edge.packet_type) {
                const std::string* registered_type = input_edge.packet_type->RegisteredTypeName();
                if (registered_type) {
                    return *registered_type;
                }
            }
    ...
    return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Unable to find the type for side packet \"" << name
                                                         << "\".  It may be set to AnyType or something else that isn't "
                                                            "determinable, or the type may be defined but not registered.";
}

absl::StatusOr<std::string> ValidatedGraphConfig::RegisteredStreamTypeName(const std::string& name) {
    auto iter = stream_to_producer_.find(name);
    if (iter == stream_to_producer_.end()) {
        return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
    ...
    return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Unable to find the type for stream \"" << name
                                                         << "\".  It may be set to AnyType or something else that isn't "
                                                            "determinable, or the type may be defined but not registered.";
}

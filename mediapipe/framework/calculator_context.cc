...
const InputStreamSet& CalculatorContext::InputStreams() const {
    if (!input_streams_) {
        ...
        for (CollectionItemId id = input_streams_->BeginId(); id < input_streams_->EndId(); ++id) {
...
const OutputStreamSet& CalculatorContext::OutputStreams() const {
    if (!output_streams_) {
        ...
        for (CollectionItemId id = output_streams_->BeginId(); id < output_streams_->EndId(); ++id) {
            output_streams_->Get(id) = const_cast<OutputStreamShard*>(&outputs_.Get(id));
        }
    }

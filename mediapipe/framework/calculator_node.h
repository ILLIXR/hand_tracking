...
public:
    ...
    // validated_graph.
    absl::Status Initialize(const ValidatedGraphConfig* validated_graph, NodeTypeInfo::NodeRef node_ref,
    ...
    absl::Status PrepareForRun(const std::map<std::string, Packet>& all_side_packets,
    ...
    // the graph run.
    void CleanupAfterRun(const absl::Status& graph_status) ABSL_LOCKS_EXCLUDED(status_mutex_);
    ...
    bool IsSource() const {
        return input_stream_handler_->NumInputStreams() == 0 && output_stream_handler_->NumOutputStreams() != 0;
    }
    ...
    absl::Status CloseNode(const absl::Status& graph_status, bool graph_run_ended) ABSL_LOCKS_EXCLUDED(status_mutex_);
...
private:
    ...
    absl::Status InitializeOutputSidePackets(const PacketTypeSet&  output_side_packet_types,
    ...
    // provided.
    absl::Status InitializeInputSidePackets(OutputSidePacketImpl* output_side_packets);
    // Sets up the output streams from the main flat array.
    absl::Status InitializeOutputStreams(OutputStreamManager* output_stream_managers);
    ...
    absl::Status InitializeInputStreams(InputStreamManager* input_stream_managers, OutputStreamManager* output_stream_managers);

    absl::Status InitializeInputStreamHandler(const InputStreamHandlerConfig& handler_config,
                                              const PacketTypeSet&            input_stream_types);
    absl::Status InitializeOutputStreamHandler(const OutputStreamHandlerConfig& handler_config,
    ...
    void CloseOutputStreams(OutputStreamShardSet* outputs) ABSL_LOCKS_EXCLUDED(status_mutex_);
    ...
    enum NodeStatus { kStateUninitialized = 0, kStatePrepared = 1, kStateOpened = 2, kStateActive = 3, kStateClosed = 4 };
    ...
    bool input_stream_headers_ready_called_ ABSL_GUARDED_BY(status_mutex_) = false;

...
public:
    ...
    // of the calcualtor;
    OutputStreamHandler(std::shared_ptr<tool::TagMap> tag_map, CalculatorContextManager* calculator_context_manager,
    ...
    int NumOutputStreams() const {
        return output_stream_managers_.NumEntries();
    }
    ...
    void PrepareForRun(const std::function<void(absl::Status)>& error_callback) ABSL_LOCKS_EXCLUDED(timestamp_mutex_);
    ...
    void UpdateTaskTimestampBound(Timestamp timestamp) ABSL_LOCKS_EXCLUDED(timestamp_mutex_);

    // Invoked after a call to Calculator::Process() function.
    void PostProcess(Timestamp input_timestamp) ABSL_LOCKS_EXCLUDED(timestamp_mutex_);
...
protected:
    ...
    virtual void PropagationLoop() ABSL_EXCLUSIVE_LOCKS_REQUIRED(timestamp_mutex_) = 0;
...
using OutputStreamHandlerRegistry = GlobalFactoryRegistry<std::unique_ptr<OutputStreamHandler>, std::shared_ptr<tool::TagMap>,
...
#define REGISTER_OUTPUT_STREAM_HANDLER(name)                                       \
    REGISTER_FACTORY_FUNCTION_QUALIFIED(                                           \
        mediapipe::OutputStreamHandlerRegistry, output_handler_registration, name, \
        absl::make_unique<name, std::shared_ptr<tool::TagMap>, CalculatorContextManager*, const MediaPipeOptions&, bool>)

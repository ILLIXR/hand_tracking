...
namespace internal {
    ...
    absl::Status GraphOutputStream::Initialize(const std::string& stream_name, const PacketType* packet_type,
        ...
        std::shared_ptr<tool::TagMap> tag_map = tool::TagMap::Create(input_stream_field).value();
        input_stream_handler_ = absl::make_unique<GraphOutputStreamHandler>(tag_map, /*cc_manager=*/nullptr, MediaPipeOptions(),
        ...
        MP_RETURN_IF_ERROR(input_stream_->Initialize(stream_name, packet_type, /*back_edge=*/false));
        MP_RETURN_IF_ERROR(input_stream_handler_->InitializeInputStreamManagers(input_stream_.get()));
        output_stream_manager->AddMirror(input_stream_handler_.get(), id);
        return absl::OkStatus();
    }

    void GraphOutputStream::PrepareForRun(std::function<void()>             notification_callback,
    ...
    absl::Status OutputStreamObserver::Initialize(const std::string& stream_name, const PacketType* packet_type,
        ...
        return GraphOutputStream::Initialize(stream_name, packet_type, output_stream_manager, observe_timestamp_bounds);
    }
        ...
        while (true) {
            ...
            if (empty) {
                ...
                if (observe_timestamp_bounds_ && min_timestamp < Timestamp::Done()) {
                    Timestamp settled = (min_timestamp == Timestamp::PostStream() ? Timestamp::PostStream()
                ...
                {
                    absl::MutexLock l(&mutex_);
                    Timestamp       new_min_timestamp = input_stream_->MinTimestampOrBound(&empty);
            ...
            Packet packet = input_stream_->PopPacketAtTimestamp(min_timestamp, &num_packets_dropped, &stream_is_done);
            RET_CHECK_EQ(num_packets_dropped, 0).SetNoLogging()
                << absl::Substitute("Dropped $0 packet(s) on input stream \"$1\".", num_packets_dropped, input_stream_->Name());
    ...
    absl::Status OutputStreamPollerImpl::Initialize(const std::string& stream_name, const PacketType* packet_type,
                                                    std::function<void(InputStreamManager*, bool*)> queue_size_callback,
                                                    OutputStreamManager* output_stream_manager, bool observe_timestamp_bounds) {
        MP_RETURN_IF_ERROR(
            GraphOutputStream::Initialize(stream_name, packet_type, output_stream_manager, observe_timestamp_bounds));
        input_stream_handler_->SetQueueSizeCallbacks(queue_size_callback, queue_size_callback);
        return absl::OkStatus();
    }

    void OutputStreamPollerImpl::PrepareForRun(std::function<void()>             notification_callback,
                                               std::function<void(absl::Status)> error_callback) {
        input_stream_handler_->PrepareForRun(
            /*headers_ready_callback=*/[] { }, std::move(notification_callback),
            /*schedule_callback=*/nullptr, std::move(error_callback));
    ...
    void OutputStreamPollerImpl::SetMaxQueueSize(int queue_size) {
        ABSL_CHECK(queue_size >= -1) << "Max queue size must be either -1 or non-negative.";
        input_stream_handler_->SetMaxQueueSize(queue_size);
    }

    int OutputStreamPollerImpl::QueueSize() {
        return input_stream_->QueueSize();
    }
    ...
    bool OutputStreamPollerImpl::Next(Packet* packet) {
        ...
        while (true) {
            ...
            if (empty_queue) {
                timestamp_bound_changed = input_stream_handler_->ProcessTimestampBounds() &&
                    output_timestamp_ < min_timestamp.PreviousAllowedInStream();
            }
            if (graph_has_error_ || !empty_queue || timestamp_bound_changed || min_timestamp == Timestamp::Done()) {
                break;
            } else {
        ...
        if (!empty_queue) {
            int  num_packets_dropped = 0;
            bool stream_is_done      = false;
            *packet = input_stream_->PopPacketAtTimestamp(min_timestamp, &num_packets_dropped, &stream_is_done);

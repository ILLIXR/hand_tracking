...
ImmediateInputStreamHandler::ImmediateInputStreamHandler(std::shared_ptr<tool::TagMap>      tag_map,
    ...
    : InputStreamHandler(tag_map, calculator_context_manager, options, calculator_run_in_parallel) {
...
void ImmediateInputStreamHandler::PrepareForRun(std::function<void()>                   headers_ready_callback,
                                                std::function<void()>                   notification_callback,
                                                std::function<void(CalculatorContext*)> schedule_callback,
                                                std::function<void(absl::Status)>       error_callback) {
    {
        absl::MutexLock lock(&mutex_);
        for (int i = 0; i < (int) sync_sets_.size(); ++i) {
...
NodeReadiness ImmediateInputStreamHandler::GetNodeReadiness(Timestamp* min_stream_timestamp) {
    absl::MutexLock lock(&mutex_);
    ...
    for (int i = 0; i < (int) sync_sets_.size(); ++i) {
        ...
        if (readiness == NodeReadiness::kReadyForProcess) {
            ...
            if (ProcessTimestampBounds()) {
                ...
                static const Timestamp kDonePrecedingTimestamp = Timestamp::Done().PreviousAllowedInStream();
...
void ImmediateInputStreamHandler::FillInputSet(Timestamp input_timestamp, InputStreamShardSet* input_set) {
    absl::MutexLock lock(&mutex_);
    for (int i = 0; i < (int) sync_sets_.size(); ++i) {

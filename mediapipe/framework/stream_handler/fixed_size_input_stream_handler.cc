...
FixedSizeInputStreamHandler::FixedSizeInputStreamHandler(std::shared_ptr<tool::TagMap>      tag_map,
                                                         CalculatorContextManager*          cc_manager,
                                                         const mediapipe::MediaPipeOptions& options,
                                                         bool                               calculator_run_in_parallel)
    : DefaultInputStreamHandler(std::move(tag_map), cc_manager, options, calculator_run_in_parallel) {
    const auto& ext     = options.GetExtension(mediapipe::FixedSizeInputStreamHandlerOptions::ext);
...
void FixedSizeInputStreamHandler::EraseAllSurplus() {
    ...
    for (const auto& stream : input_stream_managers_) {
        ...
        Timestamp min_timestamp = stream->GetMinTimestampAmongNLatest(target_queue_size_);

        // Record the min timestamp among the newest target_queue_size_ packets
        // across all InputStreamImpls.
        min_timestamp_all_streams = std::min(min_timestamp_all_streams, min_timestamp);
    }
...
Timestamp FixedSizeInputStreamHandler::PreviousAllowedInStream(Timestamp bound) {
...
void FixedSizeInputStreamHandler::EraseAnySurplus(bool keep_one) {
    ...
    for (const auto& stream : input_stream_managers_) {
        int32_t queue_size = (stream->QueueSize() >= trigger_queue_size_) ? target_queue_size_ : trigger_queue_size_ - 1;
        if (stream->QueueSize() > queue_size) {
            kept_timestamp_ =
                std::max(kept_timestamp_, stream->GetMinTimestampAmongNLatest(queue_size + 1).NextAllowedInStream());
        }
    }
    if (keep_one) {
        // In order to preserve one viable timestamp, do not truncate past
        // the timestamp bound of the least current stream.
        kept_timestamp_ = std::min(kept_timestamp_, PreviousAllowedInStream(MinStreamBound()));
    }
    for (auto& stream : input_stream_managers_) {
...
NodeReadiness FixedSizeInputStreamHandler::GetNodeReadiness(Timestamp* min_stream_timestamp) {
    ...
    NodeReadiness result = DefaultInputStreamHandler::GetNodeReadiness(min_stream_timestamp);

    // If a packet has arrived below kept_timestamp_, recalculate.
    while (*min_stream_timestamp < kept_timestamp_ && result == NodeReadiness::kReadyForProcess) {
        EraseSurplusPackets(false);
...
void FixedSizeInputStreamHandler::AddPackets(CollectionItemId id, const std::list<Packet>& packets) {
...
void FixedSizeInputStreamHandler::MovePackets(CollectionItemId id, std::list<Packet>* packets) {
...
void FixedSizeInputStreamHandler::FillInputSet(Timestamp input_timestamp, InputStreamShardSet* input_set) {

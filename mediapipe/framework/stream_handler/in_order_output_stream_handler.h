...
public:
    InOrderOutputStreamHandler(std::shared_ptr<tool::TagMap> tag_map, CalculatorContextManager* calculator_context_manager,
...
private:
    ...
    void PropagatePackets(CalculatorContext** calculator_context, Timestamp* context_timestamp)
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(timestamp_mutex_);

    void PropagationBound(CalculatorContext** calculator_context, Timestamp* context_timestamp)
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(timestamp_mutex_);
};
} // namespace mediapipe

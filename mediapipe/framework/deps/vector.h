...
namespace deps {
    namespace internal_vector {
        ...
        protected:
            ...
            // integer.
            typedef typename std::conditional<std::is_integral<T>::value, double, T>::type FloatType;
        ...
        public:
            ...
            bool operator<(const D& b) const {
                const T* ap = static_cast<const D&>(*this).Data();
                const T* bp = b.Data();
                return std::lexicographical_compare(ap, ap + this->Size(), bp, bp + b.Size());
            }
            ...
            VecTemplate<int> IRound() const {
                using std::lrint;
                return Generate<VecTemplate<int>>(
                    [](const T& x) {
                        return lrint(x);
                    },
                    AsD());
...
// ======================================================================
template<typename T>
class Vector2 : public mediapipe::deps::internal_vector::BasicVector<Vector2, T, 2> {
private:
...
public:
    ...
    explicit Vector2(const Vector3<T>& b)
        : Vector2(b.x(), b.y()) { }

    explicit Vector2(const Vector4<T>& b)
        : Vector2(b.x(), b.y()) { }
...
template<typename T>
class Vector3 : public mediapipe::deps::internal_vector::BasicVector<Vector3, T, 3> {
private:
...
public:
    ...
    explicit Vector3(const Vector4<T>& b)
        : Vector3(b.x(), b.y(), b.z()) { }
    ...
    bool aequal(const Vector3& vb, FloatType margin) const {
        using std::abs;
        return (abs(c_[0] - vb.c_[0]) < margin) && (abs(c_[1] - vb.c_[1]) < margin) && (abs(c_[2] - vb.c_[2]) < margin);
    }
    ...
    Vector3 CrossProd(const Vector3& vb) const {
        return Vector3(c_[1] * vb.c_[2] - c_[2] * vb.c_[1], c_[2] * vb.c_[0] - c_[0] * vb.c_[2],
...
template<typename T>
class Vector4 : public mediapipe::deps::internal_vector::BasicVector<Vector4, T, 4> {
private:
...
public:
    ...
    bool aequal(const Vector4& vb, FloatType margin) const {
        using std::fabs;
        return (fabs(c_[0] - vb.c_[0]) < margin) && (fabs(c_[1] - vb.c_[1]) < margin) && (fabs(c_[2] - vb.c_[2]) < margin) &&

...
void* ThreadPool::WorkerThread::ThreadBody(void* arg) {
    ...
    int                 nice_priority_level = thread->pool_->thread_options().nice_priority_level();
    const std::set<int> selected_cpus       = thread->pool_->thread_options().cpu_set();
#if defined(__linux__)
    const std::string name = internal::CreateThreadName(thread->name_prefix_, syscall(SYS_gettid));
    if (nice_priority_level != 0) {
        if (nice(nice_priority_level) != -1 || errno == 0) {
            ...
            ABSL_LOG(ERROR) << "Error : " << strerror(errno) << std::endl
                            << "Could not change the nice priority level by " << nice_priority_level;
        }
    }
    if (!selected_cpus.empty()) {
        ...
        if (sched_setaffinity(syscall(SYS_gettid), sizeof(cpu_set_t), &cpu_set) != -1 || errno == 0) {
            VLOG(1) << "Pinned the thread pool executor to processor " << absl::StrJoin(selected_cpus, ", processor ") << ".";
        } else {
            ABSL_LOG(ERROR) << "Error : " << strerror(errno) << std::endl
    ...
    if (error != 0) {
        ABSL_LOG(ERROR) << "Error : " << strerror(error) << std::endl << "Failed to set name for thread: " << name;
    }
#else
...
ThreadPool::ThreadPool(const ThreadOptions& thread_options, const std::string& name_prefix, int num_threads)
    : name_prefix_(name_prefix)

...
TopologicalSorter::TopologicalSorter(int num_nodes)
...
void TopologicalSorter::AddEdge(int from, int to) {
    ABSL_CHECK(!traversal_started_ && from < num_nodes_ && to < num_nodes_ && from >= 0 && to >= 0);
    adjacency_lists_[from].push_back(to);
}

bool TopologicalSorter::GetNext(int* node_index, bool* cyclic, std::vector<int>* output_cycle_nodes) {
    if (!traversal_started_) {
        ...
        for (int from = 0; from < num_nodes_; ++from) {
            ...
            adjacency_list.erase(std::unique(adjacency_list.begin(), adjacency_list.end()), adjacency_list.end());
...
void TopologicalSorter::FindCycle(std::vector<int>* cycle_nodes) {
    ...
    for (int start_node = 0; start_node < num_nodes_; ++start_node) {
        ...
        while (!dfs_stack.empty()) {
            ...
            if ((size_t) cur_state->adjacency_list_index >= adjacency_lists_[cur_state->node].size()) {
            ...
            const int child = adjacency_lists_[cur_state->node][cur_state->adjacency_list_index];

    ...
    private:
        ...
        static void SanityCheck() {
            ...
            if (std::numeric_limits<T>::is_signed) {
                ABSL_CHECK_EQ(-1, std::numeric_limits<T>::min() + std::numeric_limits<T>::max())
    ...
    public:
        ...
        static void ValidateInit(U arg) {
            ...
            if (std::is_floating_point<U>::value) {
                ...
                if ((static_cast<U>(static_cast<T>(arg)) != arg) || (!std::numeric_limits<T>::is_signed && arg < 0) ||
...
#define MEDIAPIPE_DEFINE_SAFE_INT_TYPE(type_name, value_type, policy_type)                          \
    ...
    typedef mediapipe::intops::StrongInt<type_name##_safe_tag_, value_type,                         \

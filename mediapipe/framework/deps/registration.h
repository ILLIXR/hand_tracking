...
public:
    ...
    RegistrationToken Register(absl::string_view name, Function func) ABSL_LOCKS_EXCLUDED(lock_) {
    ...
    template<typename... Args2,
             absl::enable_if_t<std::is_convertible<std::tuple<Args2...>, std::tuple<Args...>>::value, int> = 0>
    ReturnType Invoke(absl::string_view name, Args2&&... args) ABSL_LOCKS_EXCLUDED(lock_) {
        Function function;
        {
            absl::ReaderMutexLock lock(&lock_);
            auto                  it = functions_.find(name);
            if (it == functions_.end()) {
                return absl::NotFoundError(absl::StrCat("No registered object with name: ", name));
            }
            function = it->second;
        }
    ...
    template<typename... Args2>
    ReturnType Invoke(absl::string_view ns, absl::string_view name, Args2&&... args) ABSL_LOCKS_EXCLUDED(lock_) {
    ...
    // Namespaces in |name| and |ns| are separated by kNameSep.
    bool IsRegistered(absl::string_view ns, absl::string_view name) const ABSL_LOCKS_EXCLUDED(lock_) {
    ...
    // unregistered, though this will never happen with registrations made via
    // MEDIAPIPE_REGISTER_FACTORY_FUNCTION.
    std::unordered_set<std::string> GetRegisteredNames() const ABSL_LOCKS_EXCLUDED(lock_) {
        absl::ReaderMutexLock           lock(&lock_);
        std::unordered_set<std::string> names;
        std::for_each(functions_.cbegin(), functions_.cend(), [&names](const std::pair<const std::string, Function>& pair) {
    ...
    std::string GetNormalizedName(absl::string_view name) {
        ...
        if (names[0].empty()) {
            ...
            ABSL_CHECK_EQ(1u, names.size()) << "A registered class name must be either fully qualified "
    ...
    static std::string GetLookupName(const absl::string_view cxx_type_name) {
        ...
        std::vector<absl::string_view> names    = absl::StrSplit(cxx_type_name, kCxxSep);
        if (names[0].empty()) {
            names.erase(names.begin());
        }
...
public:
    static RegistrationToken Register(absl::string_view name, typename Functions::Function func) {
    ...
    template<typename... Args2>
    static typename Functions::ReturnType CreateByName(absl::string_view name, Args2&&... args) {
    ...
    template<typename... Args2,
             absl::enable_if_t<std::is_convertible<std::tuple<Args2...>, std::tuple<Args...>>::value, int> = 0>
    static typename Functions::ReturnType CreateByNameInNamespace(absl::string_view ns, absl::string_view name,
                                                                  Args2&&... args) {
...
#define REGISTRY_STATIC_VAR(var_name, line)       REGISTRY_STATIC_VAR_INNER(var_name, line)
...
#if MEDIAPIPE_DISABLE_STATIC_REGISTRATION && MEDIAPIPE_ENABLE_STATIC_REGISTRATION_DRY_RUN
static_assert(false,
    ...
    #define MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, var_name, name, ...)
    #define MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(RegistratorName, RegistryType, name, ...) \
        template<typename T>                                                                \
    ...
    #define INTERNAL_MEDIAPIPE_REGISTER_FACTORY_STRINGIFY(x)        INTERNAL_MEDIAPIPE_REGISTER_FACTORY_STRINGIFY_HELPER(x)

    #define MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, var_name, name, ...)                        \
        static mediapipe::RegistrationToken* REGISTRY_STATIC_VAR(var_name, __LINE__) = []() {                       \
            ABSL_RAW_LOG(WARNING, "Registration Dry Run: %s",                                                       \
                         INTERNAL_MEDIAPIPE_REGISTER_FACTORY_STRINGIFY(RegistryType::Register(name, __VA_ARGS__))); \
            return nullptr;                                                                                         \
        }();

    #define MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(RegistratorName, RegistryType, names, ...)                             \
        template<typename T>                                                                                             \
        struct Internal##RegistratorName {                                                                               \
            ...
            static mediapipe::RegistrationToken Make() {                                                                 \
                ABSL_RAW_LOG(WARNING, "Registration Dry Run: %s",                                                        \
                             INTERNAL_MEDIAPIPE_REGISTER_FACTORY_STRINGIFY(RegistryType::Register(names, __VA_ARGS__))); \
                ABSL_RAW_LOG(WARNING, "Where typeid(T).name() is: %s", typeid(T).name());                                \
                return {};                                                                                               \
            }                                                                                                            \
                                                                                                                         \
            using RequireStatics = registration_internal::ForceStaticInstantiation<&registration>;                       \
        };                                                                                                               \
        /* Static members of template classes can be defined in the header. */                                           \
        template<typename T>                                                                                             \
        NoDestructor<mediapipe::RegistrationToken> Internal##RegistratorName<T>::registration(                           \
    ...
    #define MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, var_name, name, ...) \
        static mediapipe::RegistrationToken* REGISTRY_STATIC_VAR(var_name, __LINE__) =       \
            new mediapipe::RegistrationToken(RegistryType::Register(name, __VA_ARGS__));
...
#define MEDIAPIPE_REGISTER_FACTORY_FUNCTION(RegistryType, name, ...) \
    MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, registration_##name, #name, __VA_ARGS__)
...
#define REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, var_name, name, ...) \
    MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(RegistryType, var_name, #name, __VA_ARGS__)

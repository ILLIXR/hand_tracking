...
mediapipe::StatusBuilder RetCheckFailSlowPath(mediapipe::source_location location);

// Returns a StatusBuilder that corresponds to a `RET_CHECK` failure.
mediapipe::StatusBuilder RetCheckFailSlowPath(mediapipe::source_location location, const char* condition);

// Returns a StatusBuilder that corresponds to a `RET_CHECK` failure.
mediapipe::StatusBuilder RetCheckFailSlowPath(mediapipe::source_location location, const char* condition,
                                              const absl::Status& status);

inline StatusBuilder RetCheckImpl(const absl::Status& status, const char* condition, mediapipe::source_location location) {
...
#define RET_CHECK_OK(status) MP_RETURN_IF_ERROR(mediapipe::RetCheckImpl((status), #status, MEDIAPIPE_LOC))

#define RET_CHECK_FAIL() return mediapipe::RetCheckFailSlowPath(MEDIAPIPE_LOC)

#define MEDIAPIPE_INTERNAL_RET_CHECK_OP(name, op, lhs, rhs) RET_CHECK((lhs) op(rhs))

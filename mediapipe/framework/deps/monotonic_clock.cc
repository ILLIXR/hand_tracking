    ...
    virtual absl::Time TimeNow() {
        ...
        // As there are several early exits from this function, use absl::MutexLock.
        {
            ...
            ABSL_CHECK_LE(last_raw_time_, state_->max_time)
                << "non-monotonic behavior: last_raw_time_=" << last_raw_time_ << ", max_time=" << state_->max_time;
...
private:
    ...
    virtual void GetCorrectionMetrics(int* correction_count, double* max_correction) {
        absl::MutexLock l(&state_->lock);
        if (correction_count != nullptr)
            *correction_count = correction_count_;

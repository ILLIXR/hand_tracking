...
// }
        ...
        public:
            ...
            bool HasNextEntry() {
                return dir_ != nullptr && next_entry_ != nullptr;
            }
        ...
        private:
            void ReadNextEntry() {
                ...
                while (next_entry_ && (std::string(next_entry_->d_name) == "." || std::string(next_entry_->d_name) == "..")) {
                    next_entry_ = readdir(dir_);
                }
            }
        ...
        public:
            ...
            std::string NextEntry() {
                if (HasNextEntry()) {
                    PathString result = directory_ + Utf8ToNative("\\") + PathString(find_data_.cFileName);
                    ReadNextEntry();
    ...
    absl::Status GetContents(absl::string_view file_name, std::string* output, bool read_as_binary) {
        ...
        if (fp == NULL) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "Can't find file: " << file_name;
        }
        ...
        while (!feof(fp)) {
            ...
            if (ret == 0 && ferror(fp)) {
                return mediapipe::InternalErrorBuilder(MEDIAPIPE_LOC) << "Error while reading file: " << file_name;
            }
    ...
    absl::Status SetContents(absl::string_view file_name, absl::string_view content) {
        ...
        if (fp == NULL) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "Can't open file: " << file_name;
        }
        ...
        if (fclose(fp) != 0 || write_error) {
            return mediapipe::InternalErrorBuilder(MEDIAPIPE_LOC)
                << "Error while writing file: " << file_name << ". Error message: " << strerror(write_error);
        }
        return absl::OkStatus();
    }
    ...
    absl::Status MatchInTopSubdirectories(const std::string& parent_directory, const std::string& file_name,
        ...
        while (parent_listing.HasNextEntry()) {
            std::string      subdirectory = JoinPath(parent_directory, parent_listing.NextEntry());
    ...
    absl::Status Exists(absl::string_view file_name) {
        ...
        default:
            return absl::NotFoundError(absl::StrCat("The path does not exist: ", file_name));
        }
    }

...
namespace tool {
    ...
    namespace {
        ...
        absl::StatusOr<std::vector<FieldData>> GetFieldValues(const FieldData& message_data, std::string field_name) {
            std::string       type_name  = ProtoUtilLite::ParseTypeUrl(message_data.message_value().type_url());
            const Descriptor* descriptor = OptionsRegistry::GetProtobufDescriptor(type_name);
            ...
            int                       count;
            MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldCount(mesage_bytes, proto_path, field->type(), &count));
            std::vector<std::string> field_values;
            MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldRange(mesage_bytes, proto_path, count, field->type(), &field_values));
            std::vector<FieldData> result;
            for (int i = 0; i < (int) field_values.size(); ++i) {
                FieldData   r;
                std::string message_type = field->message_type() ? field->message_type()->full_name() : "";
                MP_RETURN_IF_ERROR(ProtoUtilLite::ReadValue(field_values[i], field->type(), message_type, &r));
        ...
        std::string GetFieldString(const FieldData& message_data, std::string field_name) {
        ...
        void RegisterDescriptorProtos(absl::flat_hash_map<std::string, Descriptor>& result) {
            std::vector<Descriptor> descriptors = {
                {"google::protobuf.FileDescriptorSet",
                 {
                     {"file", 1, FieldType::TYPE_MESSAGE, "google::protobuf.FileDescriptorProto"},
                 }},
                {"google::protobuf.FileDescriptorProto",
                 {
                     {"package", 2, FieldType::TYPE_STRING, ""},
                     {"message_type", 4, FieldType::TYPE_MESSAGE, "google::protobuf.DescriptorProto"},
                 }},
                {"google::protobuf.DescriptorProto",
                 {
                     {"name", 1, FieldType::TYPE_STRING, ""},
                     {"field", 2, FieldType::TYPE_MESSAGE, "google::protobuf.FieldDescriptorProto"},
                     {"extension", 6, FieldType::TYPE_MESSAGE, "google::protobuf.FieldDescriptorProto"},
                     {"nested_type", 3, FieldType::TYPE_MESSAGE, "google::protobuf.DescriptorProto"},
                 }},
    ...
    RegistrationToken OptionsRegistry::Register(const FieldData& file_descriptor_set) {
    ...
    void OptionsRegistry::Register(const FieldData& message_type, const std::string& parent_name) {
    ...
    void OptionsRegistry::FindAllExtensions(absl::string_view extendee, std::vector<const FieldDescriptor*>* result) {
        absl::ReaderMutexLock lock(&mutex());
        result->clear();
    ...
    absl::flat_hash_map<std::string, std::vector<FieldDescriptor>>& OptionsRegistry::extensions() {
        static auto* extensions = new absl::flat_hash_map<std::string, std::vector<FieldDescriptor>>();
        return *extensions;
    }
    ...
    Descriptor::Descriptor(const std::string& full_name, const FieldData& descriptor_proto)
    ...
    Descriptor::Descriptor(const std::string& full_name, const std::vector<FieldDescriptor>& fields)
        : full_name_(full_name) {
        for (const auto& field : fields) {
    ...
    FieldDescriptor::FieldDescriptor(std::string name, int number, FieldType type, std::string message_type)
        : name_(name)
        , number_(number)
        , type_(type)

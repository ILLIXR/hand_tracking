...
namespace tool {
    ...
    public:
        ...
        OptionsSyntaxHelper()
            : syntax_{"OPTIONS", "options", "/"} { }

        // Returns the tag name for an option protobuf field.
        std::string OptionFieldTag(const std::string& name) {
            return name;
        }

        // Returns the packet name for an option protobuf field.
        absl::string_view OptionFieldPacket(absl::string_view name) {
            return name;
        }

        // Returns the option protobuf field name for a tag or packet name.
        absl::string_view OptionFieldName(absl::string_view name) {
            return name;
        }
        ...
        FieldPath OptionFieldPath(absl::string_view tag, const Descriptor* descriptor) {
        ...
        std::string GraphOptionName(const std::string& option_name) {
            std::string packet_prefix = syntax_.packet_name + absl::AsciiStrToLower(syntax_.separator);
        ...
        std::string OptionTagName(const std::string& option_name) {
            return absl::StrCat(syntax_.tag_name, syntax_.separator, OptionFieldTag(option_name));
        }
    ...
    OptionsSyntaxUtil::OptionsSyntaxUtil(const std::string& tag_name, const std::string& packet_name,
    ...
    OptionsSyntaxUtil::FieldPath OptionsSyntaxUtil::OptionFieldPath(absl::string_view tag, const Descriptor* descriptor) {
        return syntax_helper_->OptionFieldPath(tag, descriptor);
    }

    std::vector<absl::string_view> OptionsSyntaxUtil::StrSplitTags(absl::string_view tag_and_name) {

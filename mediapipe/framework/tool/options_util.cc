...
namespace tool {
    ...
    std::string ExtensionType(const std::string& option_fields_tag) {
        ...
        options_field_util::FieldPath field_path = syntax_util.OptionFieldPath(option_fields_tag, nullptr);
    ...
    FieldPath GetPath(const std::string& path_tag, const std::string& message_type) {
        OptionsSyntaxUtil syntax_util;
        const Descriptor* descriptor = OptionsRegistry::GetProtobufDescriptor(message_type);
    ...
    std::string MessageType(FieldData message) {
        return options_field_util::ParseTypeUrl(std::string(message.message_value().type_url()));
    }
    ...
    absl::Status CopyLiteralOptions(CalculatorGraphConfig::Node parent_node, CalculatorGraphConfig* config) {
        ...
        for (auto& node : *config->mutable_node()) {
            for (const std::string& option_def : node.option_value()) {
                ...
                std::vector<absl::string_view> tag_and_name         = syntax_util.StrSplitTags(option_def);
                ...
                ASSIGN_IF_OK(graph_options, GetGraphOptions(graph_data, graph_extension_type));
                FieldData parent_options;
                ASSIGN_IF_OK(parent_options, GetNodeOptions(parent_data, graph_extension_type));
                MP_ASSIGN_OR_RETURN(graph_options, MergeMessages(graph_options, parent_options));
                FieldData node_options;
                MP_ASSIGN_OR_RETURN(node_options, GetNodeOptions(node_data, node_extension_type));
                if (!node_options.has_message_value() || !graph_options.has_message_value()) {
                    continue;
                }
                ...
                MP_ASSIGN_OR_RETURN(packet_data, GetFieldValues(graph_options, graph_path));
                MP_RETURN_IF_ERROR(MergeFieldValues(node_options, node_path, packet_data));

...
namespace tool {
    ...
    void TagMap::InitializeNames(const std::map<std::string, std::vector<std::string>>& tag_to_names) {
    ...
    absl::Status TagMap::Initialize(const proto_ns::RepeatedPtrField<ProtoString>& tag_index_names) {
        ...
        for (const auto& tag_index_name : tag_index_names) {
            ...
            if (!names[index].empty()) {
                return mediapipe::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
                    << "tag \"" << tag << "\" index " << index << " already had a name \"" << names[index]
        ...
        for (auto& item : mapping_) {
            ...
            if (tag_data.count != (int) names.size()) {
                auto builder = mediapipe::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
                    << "Not all indexes were assigned names.  Tag \"" << item.first << "\" has the following:\n";
    ...
    absl::Status TagMap::Initialize(const TagAndNameInfo& info) {
        if (info.tags.empty()) {
            if (!info.names.empty()) {
                mapping_.emplace(std::piecewise_construct, std::forward_as_tuple(""),
            ...
            if (info.tags.size() != info.names.size()) {
                return absl::FailedPreconditionError("Expected info.tags.size() == info.names.size()");
            }

            // Add the tags (unsorted).
            for (int i = 0; i < (int) info.tags.size(); ++i) {
                auto item =
                    mapping_.emplace(std::piecewise_construct, std::forward_as_tuple(info.tags[i]), std::forward_as_tuple());
        ...
        for (const auto& item : mapping_) {
            ...
            if (tag.empty()) {
                ...
                *fields.Add() = absl::StrCat(tag, ":", names_[tag_data.id.value()]);
                ...
                for (int i = 0; i < tag_data.count; ++i) {
                    *fields.Add() = absl::StrCat(tag, ":", i, ":", names_[tag_data.id.value() + i]);
                }
            }
        }
        return fields;
    ...
    std::pair<std::string, int> TagMap::TagAndIndexFromId(CollectionItemId id) const {
        for (const auto& item : mapping_) {

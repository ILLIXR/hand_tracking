...
namespace tool {
    ...
    public:
        ...
        static void FindAllExtensions(absl::string_view extendee, std::vector<const FieldDescriptor*>* result);

    private:
        // Registers protobuf descriptors for a message type and nested types.
        static void Register(const FieldData& message_type, const std::string& parent_name);

        static absl::flat_hash_map<std::string, Descriptor>&                   descriptors();
        static absl::flat_hash_map<std::string, std::vector<FieldDescriptor>>& extensions();

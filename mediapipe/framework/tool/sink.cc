...
namespace tool {
    ...
    absl::Status MediaPipeInternalSidePacketToPacketStreamCalculator::GetContract(CalculatorContract* cc) {
        cc->InputSidePackets().Index(0).SetAny();
        cc->Outputs().Index(0).SetSameAs(&cc->InputSidePackets().Index(0));
        return absl::OkStatus();
    }

    absl::Status MediaPipeInternalSidePacketToPacketStreamCalculator::Open(CalculatorContext* cc) {
        cc->Outputs().Index(0).AddPacket(cc->InputSidePackets().Index(0).At(Timestamp::PostStream()));
        cc->Outputs().Index(0).Close();
        return absl::OkStatus();
    }

    absl::Status MediaPipeInternalSidePacketToPacketStreamCalculator::Process(CalculatorContext* cc) {
        UNUSED(cc);
    ...
    void AddVectorSink(const std::string&     stream_name, //
        ...
        node->set_name(
            GetUnusedNodeName(*config, absl::StrCat("callback_packet_calculator_that_generators_", input_side_packet_name)));
        ...
        CallbackPacketCalculatorOptions* options =
            node->mutable_options()->MutableExtension(CallbackPacketCalculatorOptions::ext);
    ...
    void AddSidePacketSink(const std::string& side_packet_name, CalculatorGraphConfig* config, Packet* dumped_packet) {
        ...
        const std::string            node_name =
            GetUnusedNodeName(*config, absl::StrCat("calculator_converts_side_packet_", side_packet_name));
        conversion_node->set_name(node_name);
        conversion_node->set_calculator("MediaPipeInternalSidePacketToPacketStreamCalculator");
        conversion_node->add_input_side_packet(GetUnusedSidePacketName(*config, side_packet_name));

        const std::string output_stream_name = absl::StrCat(node_name, "_output_stream");
        conversion_node->add_output_stream(output_stream_name);
        AddPostStreamPacketSink(output_stream_name, config, dumped_packet);
    }

    void AddCallbackCalculator(const std::string& stream_name, CalculatorGraphConfig* config,
                               std::string* callback_side_packet_name, bool use_std_function) {
        ABSL_CHECK(config);
        ABSL_CHECK(callback_side_packet_name);
        CalculatorGraphConfig::Node* sink_node = config->add_node();
        sink_node->set_name(GetUnusedNodeName(*config, absl::StrCat("callback_calculator_that_collects_stream_", stream_name)));
        sink_node->set_calculator("CallbackCalculator");
        sink_node->add_input_stream(stream_name);

        const std::string input_side_packet_name = GetUnusedSidePacketName(*config, absl::StrCat(stream_name, "_callback"));
        *callback_side_packet_name               = input_side_packet_name;
        if (use_std_function) {
            // Uses tag "CALLBACK" if the input side packet contains a std::function.
            sink_node->add_input_side_packet(absl::StrCat("CALLBACK:", input_side_packet_name));
        } else {
    ...
    void AddMultiStreamCallback(const std::vector<std::string>&                 streams,
                                std::function<void(const std::vector<Packet>&)> callback, CalculatorGraphConfig* config,
    ...
    void AddMultiStreamCallback(const std::vector<std::string>&                 streams,
                                std::function<void(const std::vector<Packet>&)> callback, CalculatorGraphConfig* config,
                                std::map<std::string, Packet>* side_packets, bool observe_timestamp_bounds) {
        ABSL_CHECK(config);
        ABSL_CHECK(side_packets);
        CalculatorGraphConfig::Node* sink_node = config->add_node();
        const std::string name = GetUnusedNodeName(*config, absl::StrCat("multi_callback_", absl::StrJoin(streams, "_")));
        ...
        if (observe_timestamp_bounds) {
            const std::string observe_ts_bounds_packet_name =
                GetUnusedSidePacketName(*config, absl::StrCat(name, "_observe_ts_bounds"));
            sink_node->add_input_side_packet(absl::StrCat("OBSERVE_TIMESTAMP_BOUNDS:", observe_ts_bounds_packet_name));
            InsertIfNotPresent(side_packets, observe_ts_bounds_packet_name, MakePacket<bool>(true));
        }
        const std::string input_side_packet_name = GetUnusedSidePacketName(*config, absl::StrCat(name, "_callback"));
        sink_node->add_input_side_packet(absl::StrCat("VECTOR_CALLBACK:", input_side_packet_name));

        InsertIfNotPresent(side_packets, input_side_packet_name,
                           MakePacket<std::function<void(const std::vector<Packet>&)>>(std::move(callback)));
    }

    void AddCallbackWithHeaderCalculator(const std::string& stream_name, const std::string& stream_header,
                                         CalculatorGraphConfig* config, std::string* callback_side_packet_name,
                                         bool use_std_function) {
        ABSL_CHECK(config);
        ABSL_CHECK(callback_side_packet_name);
        CalculatorGraphConfig::Node* sink_node = config->add_node();
        sink_node->set_name(GetUnusedNodeName(
            *config, absl::StrCat("callback_calculator_that_collects_stream_and_header_", stream_name, "_", stream_header)));
        ...
        const std::string input_side_packet_name =
            GetUnusedSidePacketName(*config, absl::StrCat(stream_name, "_", stream_header, "_callback"));
        *callback_side_packet_name = input_side_packet_name;
    ...
    absl::Status CallbackCalculator::GetContract(CalculatorContract* cc) {
        ...
        if (cc->InputSidePackets().HasTag("CALLBACK")) {
            cc->InputSidePackets().Tag("CALLBACK").Set<std::function<void(const Packet&)>>();
        } else if (cc->InputSidePackets().HasTag("VECTOR_CALLBACK")) {
            cc->InputSidePackets().Tag("VECTOR_CALLBACK").Set<std::function<void(const std::vector<Packet>&)>>();
            allow_multiple_streams = true;
        } else {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "InputSidePackets must use tags.";
        }
    ...
    absl::Status CallbackCalculator::Open(CalculatorContext* cc) {
        if (cc->InputSidePackets().HasTag("CALLBACK")) {
            callback_ = cc->InputSidePackets().Tag("CALLBACK").Get<std::function<void(const Packet&)>>();
        } else if (cc->InputSidePackets().HasTag("VECTOR_CALLBACK")) {
            vector_callback_ =
                cc->InputSidePackets().Tag("VECTOR_CALLBACK").Get<std::function<void(const std::vector<Packet>&)>>();
        } else {
            ABSL_LOG(FATAL) << "InputSidePackets must use tags.";
        }
        if (callback_ == nullptr && vector_callback_ == nullptr) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "missing callback.";
        }
        if (cc->InputSidePackets().HasTag("OBSERVE_TIMESTAMP_BOUNDS") &&
    ...
    absl::Status CallbackWithHeaderCalculator::GetContract(CalculatorContract* cc) {
        ...
        if (cc->InputSidePackets().UsesTags()) {
            ...
            cc->InputSidePackets().Tag("CALLBACK").Set<std::function<void(const Packet&, const Packet&)>>();
        } else {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "InputSidePackets must use tags.";
        }
        return absl::OkStatus();
    ...
    absl::Status CallbackWithHeaderCalculator::Open(CalculatorContext* cc) {
        if (cc->InputSidePackets().UsesTags()) {
            callback_ = cc->InputSidePackets().Tag("CALLBACK").Get<std::function<void(const Packet&, const Packet&)>>();
        } else {
            ABSL_LOG(FATAL) << "InputSidePackets must use tags.";
        }
        if (callback_ == nullptr) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "callback is nullptr.";
        }
        if (!cc->Inputs().HasTag("INPUT")) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "No input stream connected.";
        }
        if (!cc->Inputs().HasTag("HEADER")) {
            ...
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "No header stream connected.";
        }
    ...
    absl::Status CallbackWithHeaderCalculator::Process(CalculatorContext* cc) {
        if (!cc->Inputs().Tag("INPUT").Value().IsEmpty() && header_packet_.IsEmpty()) {
            // Header packet should be available before we receive any normal input
            // stream packet.
            return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "Header not available!";
        }
        if (header_packet_.IsEmpty() && !cc->Inputs().Tag("HEADER").Value().IsEmpty()) {

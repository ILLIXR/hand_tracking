...
absl::Status ReadProto(proto_ns::io::ZeroCopyInputStream* in, bool read_text, const std::string& source,
                       proto_ns::Message* result) {
    if (read_text) {
        RET_CHECK(proto_ns::TextFormat::Parse(in, result)) << "could not parse text proto: " << source;
    } else {
        RET_CHECK(result->ParseFromZeroCopyStream(in)) << "could not parse binary proto: " << source;
    }
    return absl::OkStatus();
}

absl::Status WriteProto(const proto_ns::Message& message, bool write_text, const std::string& dest,
                        proto_ns::io::ZeroCopyOutputStream* out) {
    if (write_text) {
        RET_CHECK(proto_ns::TextFormat::Print(message, out)) << "could not write text proto to: " << dest;
    } else {
        RET_CHECK(message.SerializeToZeroCopyStream(out)) << "could not write binary proto to: " << dest;
    }
    return absl::OkStatus();
}
...
int main(int argc, char** argv) {
    ...
    if (absl::GetFlag(FLAGS_proto_source).empty()) {
        status.Update(absl::InvalidArgumentError("--proto_source must be specified"));
    }
    if (absl::GetFlag(FLAGS_proto_output).empty()) {
        status.Update(absl::InvalidArgumentError("--proto_output must be specified"));
    }
    if (!status.ok()) {
        return EXIT_FAILURE;
    }
    mediapipe::CalculatorGraphConfig config;
    EXIT_IF_ERROR(mediapipe::ReadFile(absl::GetFlag(FLAGS_proto_source), true, &config));
    EXIT_IF_ERROR(mediapipe::WriteFile(absl::GetFlag(FLAGS_proto_output), false, config));
    return EXIT_SUCCESS;
}

...
namespace tool {
    ...
    namespace {
        ...
        absl::Status ParseEntry(absl::string_view& path, ProtoPath* result) {
            ...
            return ok ? absl::OkStatus() : absl::InvalidArgumentError(absl::StrCat("Failed to parse ProtoPath entry: ", path));
        }
        ...
        absl::Status SetMapKeyTypes(const std::vector<FieldType>& key_types, ProtoPath* result) {
            ...
            for (ProtoPathEntry& entry : *result) {
                if (entry.map_id >= 0) {
                    ...
                    MP_RETURN_IF_ERROR(ProtoUtilLite::Serialize({entry.key_value}, key_type, &key_value));
        ...
        int FieldCount(const FieldValue& base, ProtoPath field_path, FieldType field_type) {
            int result = 0;
            ABSL_CHECK_OK(ProtoUtilLite::GetFieldCount(base, field_path, field_type, &result));
            return result;
        }
    ...
    public:
        ...
        bool ExpandTemplates(const TemplateDict& args, const CalculatorGraphTemplate& templ, CalculatorGraphConfig* output) {
    ...
    private:
        ...
        bool ExpandTemplateRule(int base_index, const FieldValue& base_message, std::vector<FieldValue>* result) {
        ...
        bool ExpandPeerRules(int base_index, const FieldValue& base_message, std::vector<FieldValue>* result) {
            ...
            return ExpandNestedRules(next_index, base_rule.path(), base_message, result);
        }

        // Return the field values addressed by a template rule.
        absl::Status GetBaseValue(const std::string& base_path, const TemplateExpression& rule, const FieldValue& output,
            ...
            return ProtoUtilLite::GetFieldRange(output, field_path, 1, GetFieldType(rule), base);
        }

        // Replace the field values addressed by a template rule.
        absl::Status ReplaceBaseValue(const std::string& base_path, const TemplateExpression& rule,
            ...
            if (rule.has_field_value()) {
                ...
                if (!field_values.empty() && FieldCount(*output, field_path, GetFieldType(rule)) > 0) {
                    return absl::InvalidArgumentError(
                        absl::StrCat("Multiple values specified for non-repeated field: ", rule.path()));
                }
                // For a non-repeated field, the field value is stored only in the rule.
                field_path[field_path.size() - 1].index = 0;
                field_count                             = 0;
            }
            return ProtoUtilLite::ReplaceFieldRange(output, field_path, field_count, GetFieldType(rule), field_values);
        }

        // Replaces nested fields by following nested template rules.
        bool ExpandNestedRules(int base_index, const std::string& base_path, const FieldValue& base_message,
                               std::vector<FieldValue>* result) {
            absl::Status status;
        ...
        std::vector<int> GetNestedRules(int rule_index, const std::string& rule_path) {
        ...
        bool ExpandIterationRule(int base_index, const FieldValue& base_message, std::vector<FieldValue>* result) {
        ...
        bool ExpandDeclaration(int base_index, const FieldValue& base_message, std::vector<FieldValue>* result) {
            // Retrieve the var param and the range expression.
            const TemplateExpression& rule = template_rules_.rule().Get(base_index);
            if (rule.arg().empty() || rule.arg().size() > 2) {
                RecordError(absl::InvalidArgumentError("Param declaration must specify a parameter name and "
        ...
        bool ExpandConditionalRule(int base_index, const FieldValue& base_message, std::vector<FieldValue>* result) {
        ...
        bool ExpandExpressionRule(int base_index, std::vector<FieldValue>* result) {
            ...
            std::vector<FieldValue>   values;
            absl::Status              status = AsFieldValues(std::vector<TemplateArgument>{item}, GetFieldType(rule), &values);
            if (!status.ok()) {
                RecordError(status);
                return false;
            }
        ...
        TemplateArgument EvalDot(const TemplateExpression& expr) {
            ...
            if (result == nullptr) {
                RecordError(absl::NotFoundError(absl::StrCat("param field: ", expr.arg(1).param())));
                return AsArgument(0.0);
            }
            return *result;
        }
        ...
        TemplateArgument AsDict(const std::vector<TemplateArgument>& args) {
            ...
            if (args.size() % 2 != 0) {
                RecordError(
                    absl::InvalidArgumentError(absl::StrCat("Dict requires an even number of arguments, got: ", args.size())));
                return result;
            }
        ...
        void EvalNestedExpressions(const TemplateExpression& expr, std::vector<TemplateArgument>* result) {
        ...
        TemplateArgument EvalExpression(const TemplateExpression& expr) {
            ...
            if (expr.op() == "paren") {
                ...
                return AsArgument(static_cast<double>(args[0].has_dict() ? args[0].mutable_dict()->arg_size()
        ...
        absl::Status AsFieldValues(const std::vector<TemplateArgument>& args, FieldType field_type,
                                   std::vector<FieldValue>* result) {
            for (int i = 0; i < (int) args.size(); ++i) {
                if (args[i].has_dict()) {
                    ...
                    std::string text_value = args[i].has_num() ? mediapipe::SimpleDtoa(args[i].num()) : args[i].str();
                    std::vector<FieldValue> r;
                    MP_RETURN_IF_ERROR(ProtoUtilLite::Serialize({text_value}, field_type, &r));
    ...
    absl::Status TemplateExpander::ExpandTemplates(const TemplateDict& args, const CalculatorGraphTemplate& templ,

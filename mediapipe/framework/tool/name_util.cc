...
namespace tool {
    ...
    std::string GetUnusedNodeName(const CalculatorGraphConfig& config, const std::string& node_name_base) {
        ...
        while (ContainsKey(node_names, candidate)) {
            candidate = absl::StrCat(node_name_base, "_", absl::StrFormat("%02d", iter));
            ++iter;
        }
        return candidate;
    }
    ...
    std::string GetUnusedSidePacketName(const CalculatorGraphConfig& config, const std::string& input_side_packet_name_base) {
        ...
        while (input_side_packets.contains(candidate)) {
            candidate = absl::StrCat(input_side_packet_name_base, "_", absl::StrFormat("%02d", iter));
            ++iter;
        }
        return candidate;
    }
    ...
    std::string CanonicalNodeName(const CalculatorGraphConfig& graph_config, int node_id) {
        ...
        std::string node_name   = node_config.name().empty() ? node_config.calculator() : node_config.name();
        int         count       = 0;
        int         sequence    = 0;
        for (int i = 0; i < graph_config.node_size(); i++) {
            const auto& current_node_config = graph_config.node(i);
            std::string current_node_name =
                current_node_config.name().empty() ? current_node_config.calculator() : current_node_config.name();
    ...
    std::string CatTag(const std::string& tag, int index) {
        std::string colon_index = (index <= 0 || tag.empty()) ? "" : absl::StrCat(":", index);

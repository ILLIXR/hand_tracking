...
namespace tool {
    ...
    template<class T, typename std::enable_if<IsExtension<T>::value, int>::type = 0>
    ...
    void SetNodeOptions(CalculatorGraphConfig::Node& node_config, const T& value) {
            ...
            // protobuf::Any is unavailable with third_party/protobuf:protobuf-lite.
#else
        for (mediapipe::protobuf::Any& options : *node_config.mutable_node_options()) {
            if (options.Is<T>()) {
    ...
    public:
        ...
        const T& Get() const {
            ...
            if (node_config_->has_options() && HasExtension<T>(node_config_->options())) {
        ...
        bool Has() const {
            ...
            if (node_config_->has_options() && HasExtension<T>(node_config_->options())) {
                ...
                // protobuf::Any is unavailable with third_party/protobuf:protobuf-lite.
#else
            for (const mediapipe::protobuf::Any& options : node_config_->node_options()) {
                if (options.Is<T>()) {
                    return true;
                }
            }

...
absl::Status SwitchMuxCalculator::GetContract(CalculatorContract* cc) {
        ...
        for (int index = 0; index < cc->Outputs().NumEntries(tag); ++index) {
            ...
            if (output_id.IsValid()) {
                for (int channel = 0; channel < channel_count; ++channel) {
                    auto input_id = cc->Inputs().GetId(tool::ChannelTag(tag, channel), index);
                    if (input_id.IsValid()) {
    ...
    for (const std::string& tag : channel_tags) {
        ...
        for (int index = 0; index < num_entries; ++index) {
            ...
            if (output_id.IsValid()) {
                for (int channel = 0; channel < channel_count; ++channel) {
                    auto input_id = cc->InputSidePackets().GetId(tool::ChannelTag(tag, channel), index);
                    if (input_id.IsValid()) {
                        cc->InputSidePackets().Get(input_id).SetSameAs(&cc->OutputSidePackets().Get(output_id));
                    }
                }
            }
        }
...
void SwitchMuxCalculator::RecordChannel(CalculatorContext* cc) {
    ...
    if (channel_settled == cc->InputTimestamp() && new_channel_index != channel_index_) {
...
void SwitchMuxCalculator::SendActivePackets(CalculatorContext* cc) {
    ...
    for (auto it = channel_history_.begin(); it != channel_history_.end(); ++it) {
        ...
        Timestamp channel_end    = (std::next(it) == channel_history_.end()) ? ChannelSettledTimestamp(cc).NextAllowedInStream()
                                                                             : std::next(it)->first;
        ...
        for (const std::string& tag : channel_tags_) {
            ...
            for (int index = 0; index < cc->Inputs().NumEntries(input_tag); ++index) {
                ...
                stream_settled = std::min(stream_settled, SettledTimestamp(cc->Inputs().Get(input_id)));
            }
        }

        // A history entry is expired only if all streams have advanced past it.
        if (stream_settled.NextAllowedInStream() < channel_end || std::next(it) == channel_history_.end()) {
            break;
        }
    ...
    if (expired_history != Timestamp::Unset()) {
        channel_history_.erase(channel_history_.begin(), std::next(channel_history_.find(expired_history)));
    }
}

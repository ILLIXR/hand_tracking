...
#define MEDIAPIPE_TAG_INDEX_NAME_REGEX "(" MEDIAPIPE_TAG_REGEX ":(" MEDIAPIPE_NUMBER_REGEX ":)?)?" MEDIAPIPE_NAME_REGEX
#define MEDIAPIPE_TAG_INDEX_REGEX      "(" MEDIAPIPE_TAG_REGEX ")?(:" MEDIAPIPE_NUMBER_REGEX ")?"
    ...
    absl::Status GetTagAndNameInfo(const proto_ns::RepeatedPtrField<ProtoString>& tags_and_names, TagAndNameInfo* info) {
        ...
        if (!info->tags.empty() && info->names.size() != info->tags.size()) {
            info->tags.clear();
            info->names.clear();
            return absl::InvalidArgumentError(absl::StrCat("Each set of names must use exclusively either tags or indexes.  "
                                                           "Encountered: \"",
        ...
        if (info.tags.empty()) {
            ...
            if (info.names.size() != info.tags.size()) {
                return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                    << "Number of tags " << info.names.size() << " does not match the number of tags " << info.tags.size();
            }
    ...
    absl::Status ValidateName(const std::string& name) {
        return name.length() > 0 && (name[0] == '_' || islower(name[0])) &&
            ...
            : absl::InvalidArgumentError(
                  absl::StrCat("Name \"", absl::CEscape(name), "\" does not match \"" MEDIAPIPE_NAME_REGEX "\"."));
    }

    absl::Status ValidateNumber(const std::string& number) {
        return (number.length() == 1 && isdigit(number[0])) ||
                (number.length() > 1 && isdigit(number[0]) && number[0] != '0' &&
            ...
            ? absl::OkStatus()
            : absl::InvalidArgumentError(
                  absl::StrCat("Number \"", absl::CEscape(number), "\" does not match \"" MEDIAPIPE_NUMBER_REGEX "\"."));
    }
    ...
    absl::Status ValidateTag(const std::string& tag) {
        return tag.length() > 0 && (tag[0] == '_' || isupper(tag[0])) &&
            ...
            ? absl::OkStatus()
            : absl::InvalidArgumentError(
                  absl::StrCat("Tag \"", absl::CEscape(tag), "\" does not match \"" MEDIAPIPE_TAG_REGEX "\"."));
    }
    ...
    absl::Status ParseTagAndName(absl::string_view tag_and_name, std::string* tag, std::string* name) {
        ...
        if (name_index == -1 || tag_status != absl::OkStatus() || name_status != absl::OkStatus()) {
            tag->clear();
            name->clear();
            return absl::InvalidArgumentError(absl::StrCat("\"tag and name\" is invalid, \"", tag_and_name,
                                                           "\" does not match "
    ...
    absl::Status ParseTagIndexName(const std::string& tag_index_name, std::string* tag, int* index, std::string* name) {
        ...
        if (name_index == -1 || !tag_status.ok() || !number_status.ok() || !name_status.ok()) {
            return absl::InvalidArgumentError(absl::StrCat("TAG:index:name is invalid, \"", tag_index_name,
                                                           "\" does not match "
    ...
    absl::Status ParseTagIndex(const std::string& tag_index, std::string* tag, int* index) {
        ...
        if (the_index == -1 || !tag_status.ok() || !number_status.ok()) {
            return absl::InvalidArgumentError(absl::StrCat("TAG:index is invalid, \"", tag_index,
                                                           "\" does not match "
                                                           "\"" MEDIAPIPE_TAG_INDEX_REGEX
                                                           "\" (examples: \"TAG\" \"VIDEO:2\")."));
        }
        *tag   = v[0];
        *index = the_index;
        return absl::OkStatus();

...
namespace tool {
    ...
    public:
        ...
        absl::StatusOr<CalculatorGraphConfig> GetConfig(const Subgraph::SubgraphOptions& options) override;
    };
    ...
    CalculatorGraphConfig::Node* BuildDemuxNode(const std::map<TagIndex, std::string>& input_tags,
    ...
    CalculatorGraphConfig::Node* BuildMuxNode(const std::map<TagIndex, std::string>& output_tags,
                                              CalculatorGraphConfig*                 config) {
        UNUSED(output_tags);
        UNUSED(config);
        CalculatorGraphConfig::Node* result = config->add_node();
    ...
    CalculatorGraphConfig::Node* BuildTimestampNode(CalculatorGraphConfig* config, bool async_selection) {
        ...
        *result->mutable_calculator()       = "PacketSequencerCalculator";
        if (!async_selection) {
            *result->mutable_input_stream_handler()->mutable_input_stream_handler() = "DefaultInputStreamHandler";
        }
        return result;
    }
    ...
    void ClearContainerOptions(CalculatorGraphConfig::Node* dest) {
        if (dest->has_options() && dest->mutable_options()->HasExtension(SwitchContainerOptions::ext)) {
            ClearContainerOptions(dest->mutable_options()->MutableExtension(SwitchContainerOptions::ext));
        }
    ...
    absl::Status ValidateContract(const CalculatorGraphConfig::Node& subgraph_node,
                                  const Subgraph::SubgraphOptions&   subgraph_options) {
        auto                            options = Subgraph::GetOptions<mediapipe::SwitchContainerOptions>(subgraph_options);
        ...
        if (options.has_select() && options.has_enable()) {
            return absl::InvalidArgumentError("Only one of SwitchContainer options 'enable' and 'select' can be "
                                              "specified");
        }
        if (side_tags.count({"SELECT", 0}) + side_tags.count({"ENABLE", 0}) > 1 ||
            input_tags.count({"SELECT", 0}) + input_tags.count({"ENABLE", 0}) > 1) {
            return absl::InvalidArgumentError("Only one of SwitchContainer inputs 'ENABLE' and 'SELECT' can be "
                                              "specified");
        }
    ...
    bool ContainsTag(const proto_ns::RepeatedPtrField<std::string>& tags, TagIndex item) {
        for (const std::string& t : tags) {
            if (ParseTagIndex(t) == item)
                return true;
        }
        return false;
    }

    absl::StatusOr<CalculatorGraphConfig> SwitchContainer::GetConfig(const Subgraph::SubgraphOptions& options) {
        CalculatorGraphConfig                     config;
        ...
        const auto& switch_options  = Subgraph::GetOptions<mediapipe::SwitchContainerOptions>(options);
        ...
        for (const auto& p : input_tags) {
            if (!tick_streams.empty() && !ContainsTag(tick_streams, p.first))
                continue;
        ...
        auto nodes = Subgraph::GetOptions<mediapipe::SwitchContainerOptions>(options).contained_node();
        std::vector<CalculatorGraphConfig::Node> contained_nodes(nodes.begin(), nodes.end());

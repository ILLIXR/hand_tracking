        ...
        for (int index = 0; index < cc->Inputs().NumEntries(tag); ++index) {
            ...
            if (input_id.IsValid()) {
                ...
                for (int channel = 0; channel < channel_count; ++channel) {
                    auto output_id = cc->Outputs().GetId(tool::ChannelTag(tag, channel), index);
    ...
    for (const std::string& tag : channel_tags) {
        ...
        for (int index = 0; index < num_entries; ++index) {
            ...
            if (input_id.IsValid()) {
                ...
                for (int channel = 0; channel < channel_count; ++channel) {
                    auto output_id = cc->OutputSidePackets().GetId(tool::ChannelTag(tag, channel), index);
                    if (output_id.IsValid()) {
                        cc->OutputSidePackets().Get(output_id).SetSameAs(&cc->InputSidePackets().Get(input_id));
                    }
                }
            }
        }
    ...
    for (const std::string& tag : channel_tags_) {
        for (int index = 0; index < cc->Inputs().NumEntries(tag); ++index) {
            ...
            if (!input_queue_[input_id].empty()) {
                ...
                stream_settled         = std::min(stream_settled, stream_bound.PreviousAllowedInStream());
            }
        }
    }

...
namespace tool {
    ...
    absl::Status GetFieldValues(uint32_t field_id, CodedInputStream* in, CodedOutputStream* out,
                                std::vector<std::string>* field_values) {
        uint32_t tag;
        while ((tag = in->ReadTag()) != 0) {
            ...
            if (field_number == field_id) {
                if (!IsLengthDelimited(wire_type) && IsLengthDelimited(WireFormatLite::GetTagWireType(tag))) {
    ...
    void SetFieldValues(uint32_t field_id, WireFormatLite::WireType wire_type, const std::vector<std::string>& field_values,
    ...
    void FieldAccess::GetMessage(std::string* result) {
        *result = message_;
        ...
        CodedOutputStream        out(&sos);
        WireFormatLite::WireType wire_type = WireFormatLite::WireTypeForFieldType(field_type_);
    ...
    namespace {
        ...
        absl::StatusOr<std::pair<FieldAccess, int>> AccessField(const ProtoPathEntry& entry, FieldType field_type,
            ...
            return absl::InvalidArgumentError(absl::StrCat("ProtoPath field missing, field-id: ", entry.field_id,
                                                           ", map-id: ", entry.map_id, ", key: ", entry.key_value,
    ...
    absl::Status ProtoUtilLite::ReplaceFieldRange(FieldValue* message, ProtoPath proto_path, int length, FieldType field_type,
                                                  const std::vector<FieldValue>& field_values) {
        ProtoPathEntry entry = proto_path.front();
        proto_path.erase(proto_path.begin());
        FieldType type = !proto_path.empty() ? WireFormatLite::TYPE_MESSAGE : field_type;
    ...
    absl::Status ProtoUtilLite::GetFieldRange(const FieldValue& message, ProtoPath proto_path, int length, FieldType field_type,
        ...
        if (!proto_path.empty()) {
            RET_CHECK_NO_LOG(index >= 0 && index < (int) v.size());
            MP_RETURN_IF_ERROR(GetFieldRange(v[index], proto_path, length, field_type, field_values));
        } else {
            if (length == -1) {
            ...
            RET_CHECK_NO_LOG(index >= 0 && index <= (int) v.size());
            RET_CHECK_NO_LOG(index + length >= 0 && index + length <= (int) v.size());
            field_values->insert(field_values->begin(), v.begin() + index, v.begin() + index + length);
        }
        return absl::OkStatus();
    }

    // Returns the number of field values in a repeated protobuf field.
    absl::Status ProtoUtilLite::GetFieldCount(const FieldValue& message, ProtoPath proto_path, FieldType field_type,
                                              int* field_count) {
        ProtoPathEntry entry = proto_path.front();
        proto_path.erase(proto_path.begin());
        FieldType type = !proto_path.empty() ? WireFormatLite::TYPE_MESSAGE : field_type;
        ...
        if (!proto_path.empty()) {
            RET_CHECK_NO_LOG(index >= 0 && index < (int) v.size());
            MP_RETURN_IF_ERROR(GetFieldCount(v[index], proto_path, field_type, field_count));
        } else {
    ...
    absl::Status SyntaxStatus(bool ok, const std::string& text, T* result) {
        ...
        return ok ? absl::OkStatus()
                  : absl::InvalidArgumentError(
                        absl::StrCat("Syntax error: \"", text, "\"", " for type: ", MediaPipeTypeStringOrDemangled<T>(), "."));
    }
    ...
    template<typename T>
    absl::Status WritePrimitive(void (*writer)(T, proto_ns::io::CodedOutputStream*), const std::string& text,
                                CodedOutputStream* out) {
        T value;
    ...
    static absl::Status SerializeValue(const std::string& text, FieldType field_type, FieldValue* field_value) {
        absl::Status       status;
        ...
        case W::TYPE_GROUP:
        case W::TYPE_MESSAGE:
            return absl::UnimplementedError("SerializeValue cannot serialize a Message.");
        case W::TYPE_UINT32:
    ...
    static absl::Status ReadPrimitive(CodedInputStream* input, std::string* result) {
        CType value;
        if (!WireFormatLite::ReadPrimitive<CType, DeclaredType>(input, &value)) {
            return absl::InvalidArgumentError(
                absl::StrCat("Bad serialized value: ", MediaPipeTypeStringOrDemangled<CType>(), "."));
        }
    ...
    static absl::Status DeserializeValue(const FieldValue& bytes, FieldType field_type, std::string* result) {
    ...
    absl::Status ProtoUtilLite::Serialize(const std::vector<std::string>& text_values, FieldType field_type,
    ...
    absl::Status ProtoUtilLite::Deserialize(const std::vector<FieldValue>& field_values, FieldType field_type,
    ...
    absl::Status ProtoUtilLite::WriteValue(const FieldData& value, FieldType field_type, std::string* field_bytes) {
        ...
        case WireFormatLite::TYPE_MESSAGE:
            ...
            break;
        default:
            return absl::UnimplementedError(absl::StrCat("Cannot write type: ", field_type));
        }
        return absl::OkStatus();
    }
    ...
    static ValueT ReadValue(absl::string_view field_bytes, absl::Status* status) {
        ...
        if (!WireFormatLite::ReadPrimitive<ValueT, kFieldType>(&input, &result)) {
            status->Update(absl::InvalidArgumentError(
                absl::StrCat("Bad serialized value: ", MediaPipeTypeStringOrDemangled<ValueT>(), ".")));
        }
        return result;
    }

    absl::Status ReadValue(absl::string_view field_bytes, FieldType field_type, absl::string_view message_type,
                           FieldData* result) {
        absl::Status status;
        result->Clear();
        switch (field_type) {
        case WireFormatLite::TYPE_INT32:
            result->set_int32_value(ReadValue<int32_t, WireFormatLite::TYPE_INT32>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_SINT32:
            result->set_int32_value(ReadValue<int32_t, WireFormatLite::TYPE_SINT32>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_INT64:
            result->set_int64_value(ReadValue<int64_t, WireFormatLite::TYPE_INT64>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_SINT64:
            result->set_int64_value(ReadValue<int64_t, WireFormatLite::TYPE_SINT64>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_UINT32:
            result->set_uint32_value(ReadValue<uint32_t, WireFormatLite::TYPE_UINT32>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_UINT64:
            result->set_uint64_value(ReadValue<uint32_t, WireFormatLite::TYPE_UINT32>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_DOUBLE:
            result->set_double_value(ReadValue<double, WireFormatLite::TYPE_DOUBLE>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_FLOAT:
            result->set_float_value(ReadValue<float, WireFormatLite::TYPE_FLOAT>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_BOOL:
            result->set_bool_value(ReadValue<bool, WireFormatLite::TYPE_BOOL>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_ENUM:
            result->set_enum_value(ReadValue<int32_t, WireFormatLite::TYPE_ENUM>(field_bytes, &status));
            break;
        case WireFormatLite::TYPE_STRING:
        ...
        case WireFormatLite::TYPE_MESSAGE:
            ...
            result->mutable_message_value()->set_type_url(ProtoUtilLite::TypeUrl(message_type));
            break;
        default:
            status = absl::UnimplementedError(absl::StrCat("Cannot read type: ", field_type));
            break;
        }
        return status;
    }

    absl::Status ProtoUtilLite::ReadValue(absl::string_view field_bytes, FieldType field_type, absl::string_view message_type,
                                          FieldData* result) {
        return mediapipe::tool::ReadValue(field_bytes, field_type, message_type, result);
    }

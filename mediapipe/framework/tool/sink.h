...
namespace tool {
    ...
    void AddCallbackCalculator(const std::string& stream_name, CalculatorGraphConfig* config,
                               std::string* callback_side_packet_name, bool use_std_function = false);
    ...
    void AddMultiStreamCallback(const std::vector<std::string>&                 streams,
                                std::function<void(const std::vector<Packet>&)> callback, CalculatorGraphConfig* config,
                                std::map<std::string, Packet>* side_packets, bool observe_timestamp_bounds = false);
    ...
    public:
        ...
        CallbackWithHeaderCalculator& operator=(const CallbackWithHeaderCalculator&) = delete;
    ...
    // input side packet.
    class MediaPipeInternalSidePacketToPacketStreamCalculator : public CalculatorBase {
    public:
        static absl::Status GetContract(CalculatorContract* cc);

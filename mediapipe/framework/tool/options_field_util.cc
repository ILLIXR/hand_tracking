...
namespace tool {
    namespace options_field_util {
        ...
        namespace {
            ...
            absl::Status WriteField(const FieldData& packet, const FieldDescriptor* field, std::string* result) {
                return ProtoUtilLite::WriteValue(packet, field->type(), result);
            }

            // Deserializes a packet from a protobuf field.
            absl::Status ReadField(absl::string_view bytes, const FieldDescriptor& field, FieldData* result) {
                std::string message_type =
                    (field.type() == WireFormatLite::TYPE_MESSAGE) ? field.message_type()->full_name() : "";
                return ProtoUtilLite::ReadValue(bytes, field.type(), message_type, result);
            }

            // Reads all values from a repeated field.
            absl::StatusOr<std::vector<FieldData>> GetFieldValues(const FieldData& message_data, const FieldDescriptor& field) {
                ...
                int                      count;
                MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldCount(message_bytes, proto_path, field.type(), &count));
                std::vector<std::string> field_values;
                MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldRange(message_bytes, proto_path, count, field.type(), &field_values));
            ...
            absl::Status GetFieldValue(const FieldData& message_data, const FieldPathEntry& entry, FieldData* result) {
                ...
                std::vector<std::string> field_values;
                MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldRange(message_bytes, proto_path, 1, field_type, &field_values));
                MP_RETURN_IF_ERROR(ReadField(field_values[0], *entry.field, result));
                return absl::OkStatus();
            }

            // Writes one value to a field.
            absl::Status SetFieldValue(FieldData& result, const FieldPathEntry& entry, const FieldData& value) {
                int                      index         = std::max(0, entry.index);
                ProtoUtilLite::ProtoPath proto_path    = {{entry.field->number(), index}};
                std::string*             message_bytes = result.mutable_message_value()->mutable_value();
                int                      field_count;
                MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldCount(*message_bytes, proto_path, entry.field->type(), &field_count));
                if (index > field_count) {
                    return absl::OutOfRangeError(absl::StrCat("Option field index out of range: ", index));
                }
                int         replace_length = index < field_count ? 1 : 0;
                std::string field_value;
                MP_RETURN_IF_ERROR(WriteField(value, entry.field, &field_value));
                MP_RETURN_IF_ERROR(ProtoUtilLite::ReplaceFieldRange(message_bytes, proto_path, replace_length,
                                                                    entry.field->type(), {field_value}));
                return absl::OkStatus();
            }
            ...
            absl::Status SetFieldValues(FieldData& result, const FieldPathEntry& entry, const std::vector<FieldData>& values) {
                ...
                std::string*             message_bytes = result.mutable_message_value()->mutable_value();
                int                      field_count;
                MP_RETURN_IF_ERROR(ProtoUtilLite::GetFieldCount(*message_bytes, proto_path, field_type, &field_count));
                ...
                MP_RETURN_IF_ERROR(
                    ProtoUtilLite::ReplaceFieldRange(message_bytes, proto_path, replace_length, field_type, field_values));
                return absl::OkStatus();
            }

            // Returns true for a field of type "google.protobuf.Any".
            bool IsProtobufAny(const FieldDescriptor* field) {
                return field->type() == FieldType::TYPE_MESSAGE && field->message_type()->full_name() == kGoogleProtobufAny;
            }
            ...
            StatusOr<int> FindExtensionIndex(const FieldData& message_data, FieldPathEntry* entry) {
                ...
                MP_ASSIGN_OR_RETURN(field_values, GetFieldValues(message_data, *entry->field));
                for (int i = 0; i < (int) field_values.size(); ++i) {
                    FieldData extension = ParseProtobufAny(field_values[i]);
                    if (extension_type == "*" || ParseTypeUrl(extension.message_value().type_url()) == extension_type) {
                        return i;
                    }
                }
                return -1;
            ...
            bool HasField(const FieldPath& field_path, const FieldData& message_data) {
                ...
                return value.ok() && value->value_case() != mediapipe::FieldData::VALUE_NOT_SET;
            }

            // Returns the extension field containing the specified extension-type.
            const FieldDescriptor* FindExtensionField(const FieldData& message_data, absl::string_view extension_type) {
                std::string                         message_type = ParseTypeUrl(message_data.message_value().type_url());
        ...
        absl::StatusOr<Packet> ReadMessage(const std::string& value, const std::string& type_name) {
            return packet_internal::PacketFromDynamicProto(type_name, value);
        }

        // Merge two options FieldData values.
        absl::StatusOr<FieldData> MergeMessages(const FieldData& base, const FieldData& over) {
            FieldData    result;
            absl::Status status;
            ...
            if (over.value_case() != base.value_case()) {
                return absl::InvalidArgumentError(
                    absl::StrCat("Cannot merge field data with data types: ", base.value_case(), ", ", over.value_case()));
            }
            if (over.message_value().type_url() != base.message_value().type_url()) {
                return absl::InvalidArgumentError(
                    absl::StrCat("Cannot merge field data with message types: ", base.message_value().type_url(), ", ",
        ...
        absl::Status FindExtension(const FieldData& message_data, FieldPathEntry* entry) {
            ...
            // Returns the extension field containing the specified extension-type.
            std::string&           extension_type = entry->extension_type;
            const FieldDescriptor* field          = FindExtensionField(message_data, extension_type);
            if (field != nullptr) {
                entry->field = field;
                entry->index = 0;
                return absl::OkStatus();
            }
            return absl::NotFoundError(absl::StrCat("Option extension not found: ", extension_type));
        }

        // Return the FieldPath referencing an extension message.
        FieldPath GetExtensionPath(const std::string& parent_type, const std::string& extension_type,
                                   const std::string& field_name, bool is_protobuf_any) {
            FieldPath               result;
            const tool::Descriptor* parent_descriptor = tool::OptionsRegistry::GetProtobufDescriptor(parent_type);
        ...
        absl::StatusOr<FieldData> GetNodeOptions(const FieldData& message_data, const std::string& extension_type) {
            constexpr char kOptionsName[]     = "options";
            constexpr char kNodeOptionsName[] = "node_options";
            std::string    parent_type = options_field_util::ParseTypeUrl(std::string(message_data.message_value().type_url()));
            FieldPath      path;
            absl::Status   status;
        ...
        absl::StatusOr<FieldData> GetGraphOptions(const FieldData& message_data, const std::string& extension_type) {
            constexpr char kOptionsName[]      = "options";
        ...
        absl::StatusOr<std::vector<FieldData>> GetFieldValues(const FieldData& message_data, const FieldPath& field_path) {
            ...
            if ((int) results.size() <= index) {
                return absl::OutOfRangeError(
                    absl::StrCat("Missing field value: ", head.field ? head.field->name() : "#", " at index: ", index));
            }
            if (!tail.empty()) {
        ...
        absl::StatusOr<FieldData> GetField(const FieldData& message_data, const FieldPath& field_path) {
            std::vector<FieldData> results;
            MP_ASSIGN_OR_RETURN(results, GetFieldValues(message_data, field_path));
            if (results.empty()) {
                FieldPathEntry tail = field_path.back();
                return absl::OutOfRangeError(
                    absl::StrCat("Missing field value: ", tail.field ? tail.field->name() : "##", " at index: ", tail.index));
            }
            return results[0];
        }

        // Writes FieldData values into protobuf field.
        absl::Status SetFieldValues(FieldData& message_data, const FieldPath& field_path,
        ...
        absl::Status SetField(FieldData& message_data, const FieldPath& field_path, const FieldData& value) {
        ...
        absl::Status MergeFieldValues(FieldData& message_data, const FieldPath& field_path,
                                      const std::vector<FieldData>& values) {
            absl::Status           status;
            FieldType              field_type = field_path.empty() ? FieldType::TYPE_MESSAGE : field_path.back().field->type();
        ...
        void SetOptionsMessage(const FieldData& node_options, CalculatorGraphConfig::Node* node) {
        ...
        FieldData AsFieldData(const proto_ns::MessageLite& message) {
            FieldData result;
            *result.mutable_message_value()->mutable_value()    = message.SerializePartialAsString();
            *result.mutable_message_value()->mutable_type_url() = TypeUrl(message.GetTypeName());
            return result;
        }
        ...
        absl::StatusOr<Packet> AsPacket(const FieldData& data) {
            ...
            case FieldData::ValueCase::kMessageValue: {
                auto r = packet_internal::PacketFromDynamicProto(ParseTypeUrl(std::string(data.message_value().type_url())),
        ...
        absl::StatusOr<FieldData> AsFieldData(Packet packet) {
            static const auto* kTypeIds = new std::map<TypeId, int32_t>{
                {kTypeId<int32_t>, WireFormatLite::CPPTYPE_INT32},      {kTypeId<int64_t>, WireFormatLite::CPPTYPE_INT64},
                {kTypeId<uint32_t>, WireFormatLite::CPPTYPE_UINT32},    {kTypeId<uint64_t>, WireFormatLite::CPPTYPE_UINT64},
                {kTypeId<double>, WireFormatLite::CPPTYPE_DOUBLE},      {kTypeId<float>, WireFormatLite::CPPTYPE_FLOAT},
                {kTypeId<bool>, WireFormatLite::CPPTYPE_BOOL},          {kTypeId<ProtoEnum>, WireFormatLite::CPPTYPE_ENUM},
                {kTypeId<std::string>, WireFormatLite::CPPTYPE_STRING},
            };

            FieldData result;
            if (packet.ValidateAsProtoMessageLite().ok()) {
                result.mutable_message_value()->set_value(packet.GetProtoMessageLite().SerializeAsString());
                result.mutable_message_value()->set_type_url(TypeUrl(packet.GetProtoMessageLite().GetTypeName()));
                return absl::OkStatus();
            }

            if (kTypeIds->count(packet.GetTypeId()) == 0) {
                return absl::UnimplementedError(absl::StrCat("Cannot construct FieldData for: ", packet.DebugTypeName()));
            }

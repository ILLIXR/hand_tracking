...
public:
    SubgraphContext()
        : SubgraphContext(nullptr, nullptr) { }
    ...
    SubgraphContext(CalculatorGraphConfig::Node* node, const GraphServiceManager* service_manager)
        : default_node_(node ? absl::nullopt : absl::optional<CalculatorGraphConfig::Node>(CalculatorGraphConfig::Node()))
        , original_node_(node ? *node : default_node_.value())
        , default_service_manager_(service_manager ? absl::nullopt : absl::optional<GraphServiceManager>(GraphServiceManager()))
        , service_manager_(service_manager ? *service_manager : default_service_manager_.value())
        , options_map_(std::move(tool::MutableOptionsMap().Initialize(original_node_))) { }
...
public:
    ...
    virtual absl::StatusOr<CalculatorGraphConfig> GetConfig(const SubgraphOptions& options) {
        UNUSED(options);
    ...
    static CalculatorGraphConfig::Node GetNode(const Subgraph::SubgraphOptions& supgraph_options) {
...
#define REGISTER_MEDIAPIPE_GRAPH(name) \
    REGISTER_FACTORY_FUNCTION_QUALIFIED(mediapipe::SubgraphRegistry, subgraph_registration, name, absl::make_unique<name>)
...
public:
    ...
    virtual absl::StatusOr<CalculatorGraphConfig> GetConfig(const Subgraph::SubgraphOptions& options);
...
public:
    ...
    absl::StatusOr<CalculatorGraphConfig> CreateByName(absl::string_view ns, absl::string_view type_name,

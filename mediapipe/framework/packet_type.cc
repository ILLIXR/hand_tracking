...
absl::Status PacketType::AcceptNone(const TypeSpec& type) {
    ...
    if (special && (special->accept_fn_ == AcceptNone || special->accept_fn_ == AcceptAny))
        return absl::OkStatus();
    return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC) << "No packets are allowed for type: [No Type]";
}
...
const std::string* PacketType::RegisteredTypeName() const {
    if (auto* same_as = SameAsPtr())
        return same_as->RegisteredTypeName();
...
namespace internal {
    ...
    public:
        explicit QuoteFormatter(Formatter&& f)
...
std::string PacketType::TypeNameForOneOf(TypeIdSpan types) {
    return absl::StrCat("OneOf<", absl::StrJoin(types, ", ", internal::TypeIdFormatter()), ">");
}
...
std::string PacketType::DebugTypeName() const {
    if (auto* same_as = absl::get_if<SameAs>(&type_spec_)) {
        ...
        return absl::StrCat("[Same Type As ", same_as->other->GetSameAs()->DebugTypeName(), "]");
    }
...
static bool HaveCommonType(absl::Span<const TypeId> types1, absl::Span<const TypeId> types2) {
...
absl::Status PacketType::Validate(const Packet& packet) const {
    if (!IsInitialized()) {
        return absl::InvalidArgumentError("Uninitialized PacketType was used for validation.");
    }
    if (SameAsPtr()) {
    ...
    if (auto* multi_type = absl::get_if<MultiType>(&type_spec_)) {
        ...
        if (HaveCommonType(multi_type->types, absl::MakeSpan(&packet_type, 1))) {
            ...
            return absl::InvalidArgumentError(
                absl::StrCat("The Packet stores \"", packet.DebugTypeName(), "\", but one of ",
                             absl::StrJoin(multi_type->types, ", ", internal::QuoteFormatter(internal::TypeIdFormatter())),
                             " was requested."));
        }
    }
    if (auto* special = absl::get_if<SpecialType>(&type_spec_)) {
...
absl::Status ValidatePacketTypeSet(const PacketTypeSet& packet_type_set) {
    ...
    for (CollectionItemId id = packet_type_set.BeginId(); id < packet_type_set.EndId(); ++id) {
        if (!packet_type_set.Get(id).IsInitialized()) {
            auto item = packet_type_set.TagAndIndexFromId(id);
            errors.push_back(absl::StrCat("Tag \"", item.first, "\" index ", item.second, " was not expected."));
        }
    }
    if (!errors.empty()) {
        return absl::InvalidArgumentError(absl::StrCat("ValidatePacketTypeSet failed:\n", absl::StrJoin(errors, "\n")));
    }
    return absl::OkStatus();
}

absl::Status ValidatePacketSet(const PacketTypeSet& packet_type_set, const PacketSet& packet_set) {
    std::vector<absl::Status> errors;
    if (!packet_type_set.TagMap()->SameAs(*packet_set.TagMap())) {
        return absl::InvalidArgumentError(absl::StrCat("TagMaps do not match.  PacketTypeSet TagMap:\n",
    ...
    for (CollectionItemId id = packet_type_set.BeginId(); id < packet_type_set.EndId(); ++id) {
        ...
        if (!status.ok()) {
            std::pair<std::string, int> tag_index = packet_type_set.TagAndIndexFromId(id);
            errors.push_back(mediapipe::StatusBuilder(status, MEDIAPIPE_LOC).SetPrepend()
                             << "Packet \"" << packet_type_set.TagMap()->Names()[id.value()] << "\" with tag \""
                             << tag_index.first << "\" and index " << tag_index.second << " failed validation.  ");
        }
    }
    if (!errors.empty()) {

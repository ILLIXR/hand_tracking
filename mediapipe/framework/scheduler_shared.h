...
namespace internal {
    ...
    struct SchedulerTimes {
        ...
        double overhead() const {
            return 1.0 - (static_cast<double>(node_time) / static_cast<double>(total_time));
        }
    };
    ...
    public:
        SchedulerTimer() {
            clock_ = std::unique_ptr<mediapipe::Clock>(mediapipe::MonotonicClock::CreateSynchronizedMonotonicClock());
        }
        ...
        int64_t StartNode() {
            return absl::ToUnixMicros(clock_->TimeNow());
        }

        // Called immediately after invoking ProcessNode or CloseNode.
        void EndNode(int64_t node_start_time) {
            total_node_time_.fetch_add(absl::ToUnixMicros(clock_->TimeNow()) - node_start_time, std::memory_order_relaxed);
        }

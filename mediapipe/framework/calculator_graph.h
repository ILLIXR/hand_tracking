...
public:
    ...
    absl::Status Initialize(const std::vector<CalculatorGraphConfig>&   configs,
    ...
    // async use cases.
    absl::Status ObserveOutputStream(const std::string& stream_name, std::function<absl::Status(const Packet&)> packet_callback,
    ...
    ABSL_DEPRECATED("Prefer using a Calculator to get information of all sorts out of the "
                    "graph.")
    ...
    ABSL_DEPRECATED("CalculatorGraph will not allow external callers to explictly pause and "
                    "resume a graph.")
    void Pause();
    ...
    bool IsNodeThrottled(int node_id) ABSL_LOCKS_EXCLUDED(full_input_streams_mutex_);
...
private:
    ...
    public:
        ...
        void AddPacket(const Packet& packet) {
            shard_.AddPacket(packet);
        }

        void AddPacket(Packet&& packet) {
            shard_.AddPacket(std::move(packet));
        }
    ...
    template<typename T>
    absl::Status AddPacketToInputStreamInternal(absl::string_view stream_name, T&& packet);
    ...
    absl::Status InitializeDefaultExecutor(const ThreadPoolExecutorOptions* default_executor_options,
    ...
    absl::Status CreateDefaultThreadPool(const ThreadPoolExecutorOptions* default_executor_options, int num_threads);
    ...
    absl::Status InitializePacketGeneratorGraph(const std::map<std::string, Packet>& side_packets);
    absl::Status InitializeStreams();
    absl::Status InitializeProfiler();
    absl::Status InitializeCalculatorNodes();
    absl::Status InitializePacketGeneratorNodes(const std::vector<int>& non_scheduled_generators);
    ...
    absl::Status PrepareForRun(const std::map<std::string, Packet>& extra_side_packets,
                               const std::map<std::string, Packet>& stream_headers);
    ...
    std::map<std::string, Packet> MaybeCreateLegacyGpuSidePacket(Packet legacy_sp);
    ...
    void CallStatusHandlers(GraphRunState graph_run_state, const absl::Status& status);
    ...
    std::vector<std::shared_ptr<internal::GraphOutputStream>> graph_output_streams_;
    ...
    GraphInputStreamAddMode graph_input_stream_add_mode_ ABSL_GUARDED_BY(full_input_streams_mutex_);
    ...
    std::vector<absl::flat_hash_set<InputStreamManager*>> full_input_streams_ ABSL_GUARDED_BY(full_input_streams_mutex_);
    ...
    absl::CondVar wait_to_add_packet_cond_var_ ABSL_GUARDED_BY(full_input_streams_mutex_);

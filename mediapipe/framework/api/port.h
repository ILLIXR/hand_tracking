...
namespace api2 {
    ...
    namespace internal {
        ...
        template<class CC>
        auto GetCollection(CC* cc, const SideInputBase& port) -> decltype(cc->InputSidePackets()) {
            UNUSED(port);
        ...
        template<class CC>
        auto GetCollection(CC* cc, const SideOutputBase& port) -> decltype(cc->OutputSidePackets()) {
            UNUSED(port);
        ...
        template<typename T,
                 typename std::enable_if<!std::is_same<T, AnyType>{} && !IsOneOf<T>{} && !IsSameType<T>{}, int>::type = 0>
        ...
        template<typename T, typename std::enable_if<std::is_same<T, AnyType>{}, int>::type = 0>
        inline void SetType(CalculatorContract* cc, PacketType& pt) {
            UNUSED(cc);
            pt.SetAny();
        }
        ...
        template<typename ValueT>
        InputShardOrSideAccess<ValueT> SinglePortAccess(mediapipe::CalculatorContext* cc, InputStreamShard* stream,
        ...
        auto AccessPort(std::false_type, const PortT& port, CC* cc) {
            ...
            return SinglePortAccess<ValueT>(cc, internal::GetOrNull(collection, port.Tag(), 0));
        }

        template<typename ValueT, typename X, class CC>
        class MultiplePortAccess {
        public:
            using AccessT = decltype(SinglePortAccess<ValueT>(std::declval<CC*>(), std::declval<X*>()));
            ...
            public:
                ...
                AccessT operator*() const {
                    return SinglePortAccess<ValueT>(cc_, p_);
                }
            ...
            Iterator begin() {
                return Iterator(cc_, first_);
            }

            Iterator end() {
                return Iterator(cc_, first_ + count_);
            }
        ...
        auto AccessPort(std::true_type, const PortT& port, CC* cc) {
            ...
            return MultiplePortAccess<ValueT, EntryT, CC>(cc, first, collection.NumEntries(port.Tag()));
        }
        ...
        struct ActualPayloadType<T, std::enable_if_t<IsSameType<T>{}, void>> {
            using type = typename ActualPayloadType<typename std::decay_t<decltype(T::kPort)>::value_t>::type;
        };
    ...
    template<typename Base, typename ValueT, bool IsOptional = false, bool IsMultiple = false>
    class SideFallbackT;
    ...
    template<typename Base, typename ValueT, bool IsOptionalV = false, bool IsMultipleV = false>
    ...
    public:
        ...
        auto operator()(CalculatorContext* cc) const {
            return internal::AccessPort<PayloadT>(std::integral_constant<bool, IsMultipleV>{}, *this, cc);
        }
    ...
    public:
        ...
        auto operator()(CalculatorContext* cc) const {
            auto& stream_collection = internal::GetCollection(cc, stream_port);
            auto& side_collection   = internal::GetCollection(cc, side_port);
            return internal::SinglePortAccess<PayloadT>(cc, internal::GetOrNull(stream_collection, Tag(), 0),
    ...
    protected:
        absl::Status AddToContract(CalculatorContract* cc) const {
            ...
            int connected_count = stream_port(cc).IsConnected() + side_port(cc).IsConnected();
            if (connected_count > 1)
                return absl::InvalidArgumentError(
                    absl::StrCat(Tag(), " can be connected as a stream or as a side packet, but not both"));
            if (!IsOptionalV && connected_count == 0)
                return absl::InvalidArgumentError(absl::StrCat(Tag(), " must be connected"));
            return {};
        }

        using StreamPort = PortCommon<Base, ValueT, true, IsMultipleV>;
        using SidePort   = PortCommon<typename internal::SideBase<Base>::type, ValueT, true, IsMultipleV>;
    ...
    public:
        ...
        void SetNextTimestampBound(Timestamp timestamp) {
            if (output_)
                output_->SetNextTimestampBound(timestamp);
        }

        bool IsClosed() const {
            return output_ ? output_->IsClosed() : true;
        }
    ...
    public:
        void Send(Packet<T>&& packet) {
            if (output_)
                output_->AddPacket(ToOldPacket(std::move(packet)));
        }

        void Send(const Packet<T>& packet) {
            if (output_)
                output_->AddPacket(ToOldPacket(packet));
        }
        ...
        void Send(const T& payload) {
            Send(payload, context_.InputTimestamp());
        }
        ...
        void SetHeader(const PacketBase& header) {
            if (output_)
                output_->SetHeader(ToOldPacket(header));
        }
    ...
    private:
        ...
        friend OutputShardAccess<T> internal::SinglePortAccess<T>(mediapipe::CalculatorContext*, OutputStreamShard*);
    };
    ...
    private:
        ...
        friend OutputShardAccess<internal::Generic> internal::SinglePortAccess<internal::Generic>(mediapipe::CalculatorContext*,
                                                                                                  OutputStreamShard*);
    };
    ...
    public:
        void Set(Packet<T> packet) {
            if (output_)
                output_->Set(ToOldPacket(std::move(packet)));
        }

        void Set(const T& payload) {
            Set(api2::MakePacket<T>(payload));
        }

        void Set(T&& payload) {
            Set(api2::MakePacket<T>(std::move(payload)));
        }
    ...
    private:
        OutputSidePacketAccess(OutputSidePacket* output)
            : output_(output) { }

        OutputSidePacket* output_;

        friend OutputSidePacketAccess<T> internal::SinglePortAccess<T>(mediapipe::CalculatorContext*, OutputSidePacket*);
    };

    template<typename T>
    class InputShardAccess : public Packet<T> {
    public:
        const PacketBase& packet() const& {
            return *this;
        }

        // Since InputShardAccess is currently created as a temporary, this avoids
        // easy mistakes with dangling references.
        PacketBase packet() const&& {
            return *this;
        }

        bool IsDone() const {
            return stream_->IsDone();
        }
        ...
        PacketBase Header() const {
            return FromOldPacket(stream_->Header());
        }
        ...
        template<class U = T, class = std::enable_if_t<std::is_same<U, T>{}, decltype(&Packet<U>::Consume)>>
        absl::StatusOr<std::unique_ptr<U>> Consume() {
            return WrapConsumeCall(&Packet<T>::Consume);
        }

        template<class V, class U = T, std::enable_if_t<internal::IsCompatibleType<V, U>{}, int> = 0>
    ...
    private:
        InputShardAccess(const CalculatorContext&, InputStreamShard* stream)
            : Packet<T>(stream ? FromOldPacket(stream->Value()).template As<T>() : Packet<T>())
            , stream_(stream) { }
        ...
        friend InputShardAccess<T> internal::SinglePortAccess<T>(mediapipe::CalculatorContext*, InputStreamShard*);
    };
    ...
    private:
        InputSidePacketAccess(const mediapipe::Packet* packet)
            : Packet<T>(packet ? FromOldPacket(*packet).template As<T>() : Packet<T>())
            , connected_(packet != nullptr) { }

        const bool connected_;

        friend InputSidePacketAccess<T> internal::SinglePortAccess<T>(mediapipe::CalculatorContext*, const mediapipe::Packet*);
    };
    ...
    private:
        InputShardOrSideAccess(const CalculatorContext&, InputStreamShard* stream, const mediapipe::Packet* packet)
        ...
        friend InputShardOrSideAccess<T> internal::SinglePortAccess<T>(mediapipe::CalculatorContext*, InputStreamShard*,
    ...
    public:
        bool IsConnected() const {
            return packet_type_ != nullptr;
        }

    protected:
        PacketTypeAccess(PacketType* pt)
            : packet_type_(pt) { }

        PacketType* packet_type_;

        template<typename T>
        friend PacketTypeAccess internal::SinglePortAccess(mediapipe::CalculatorContract*, PacketType*);
    };
    ...
    private:
        ...
        template<typename T>
        friend PacketTypeAccessFallback internal::SinglePortAccess(mediapipe::CalculatorContract*, PacketType*, bool);
    };

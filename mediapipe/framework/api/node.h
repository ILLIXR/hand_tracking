...
namespace internal {
    ...
    template<class T>
    class CalculatorBaseFactoryFor<T, typename std::enable_if<std::is_base_of<mediapipe::api2::Node, T>{}>::type>
    ...
    public:
        ...
        std::unique_ptr<CalculatorBase> CreateCalculator(CalculatorContext* calculator_context) final {
    ...
    private:
        template<typename U>
        auto UpdateContract(CalculatorContract* cc) -> decltype(U::UpdateContract(cc)) {
...
namespace api2 {
    namespace internal {
        ...
        MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(NodeRegistrator, mediapipe::CalculatorBaseRegistry, T::kCalculatorName,
                                              absl::make_unique<mediapipe::internal::CalculatorBaseFactoryFor<T>>)

        MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(SubgraphRegistrator, mediapipe::SubgraphRegistry, T::kCalculatorName,
                                              absl::make_unique<T>)
    ...
    class RegisteredNode
        : public Node
        , private internal::NodeRegistrator<Impl> { };
...
#define MEDIAPIPE_NODE_CONTRACT(...)                                                        \
    static constexpr auto kContract = mediapipe::api2::internal::MakeContract(__VA_ARGS__); \
    using Contract                  = typename mediapipe::api2::internal::TaggedContract<decltype(kContract), kContract>;
...
#define MEDIAPIPE_NODE_IMPLEMENTATION(Impl)                                                                   \
    MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(mediapipe::CalculatorBaseRegistry, calculator_registration, \
...
#define MEDIAPIPE_REGISTER_NODE(name)                                                                                \
    MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(mediapipe::CalculatorBaseRegistry, calculator_registration, #name, \
...
#define MEDIAPIPE_SUBGRAPH_IMPLEMENTATION(Impl)                                                                              \
    MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(mediapipe::SubgraphRegistry, subgraph_registration, Impl::kCalculatorName, \
                                                  absl::make_unique<Impl>)

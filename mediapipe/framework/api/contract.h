...
namespace api2 {
    ...
    public:
        ...
        explicit constexpr StreamHandler(const char (&name)[N])
            : name_(N, name) { }

        const const_str& name() {
            return name_;
        }
    ...
    public:
        ...
        absl::Status AddToContract(CalculatorContract* cc) const {
            if (offset_ != kUnset)
                cc->SetTimestampOffset(offset_);
            return {};
        }

    private:
        constexpr TimestampChange(int64_t offset)
            : offset_(offset) { }
    ...
    namespace internal {
        ...
        template<class T>
        struct HasNestedItems<T, std::void_t<decltype(std::declval<std::decay_t<T>>().nested_items())>> : std::true_type { };
        ...
        struct TaggedAccess {
            ...
            static constexpr auto& get(Tag tag) {
                constexpr auto i = internal::tuple_find(
                    [tag](auto x) {
                        return x == tag;
                    },
                    kTagTuple);
        ...
        public:
            ...
            constexpr auto timestamp_change_count() const {
                return internal::filtered_tuple_indices<IsSubclass<TimestampChange>::pred>(all_items).size();
            }
        ...
        public:
            ...
            using TaggedInputs      = TaggedAccess<GetMember<decltype(c2.inputs()), &C2T::inputs>>;
            using TaggedOutputs     = TaggedAccess<GetMember<decltype(c2.outputs()), &C2T::outputs>>;
            using TaggedSideInputs  = TaggedAccess<GetMember<decltype(c2.side_inputs()), &C2T::side_inputs>>;
            using TaggedSideOutputs = TaggedAccess<GetMember<decltype(c2.side_outputs()), &C2T::side_outputs>>;
        };
        ...
        public:
            ...
            auto operator()(CalculatorContext* cc) const {
                ...
                // types don't match.
                return output_sender(cc,
                                     internal::tuple_apply(f_,
                                                           internal::map_tuple(
                                                               [cc](const auto& port) {
            ...
            auto inputs() const {
                return internal::filter_tuple<IsInputPort>(args_);
            }

            auto outputs() const {
                return internal::filter_tuple<IsOutputPort>(args_);
            }
    ...
    constexpr auto ProcessFn(F&& f, P&&... args) {
        return internal::FunCaller<F, P...>(std::forward<F>(f), std::forward<P>(args)...);
    }

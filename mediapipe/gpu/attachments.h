...
namespace internal {
    ...
    static std::enable_if_t<!std::is_array<T>::value, AttachmentPtr<T>> MakeAttachmentPtr(Args&&... args) {
        return {new T(std::forward<Args>(args)...), [](void* ptr) {
    ...
    public:
        ...
        explicit constexpr Attachment(FactoryT factory)
            : factory_(factory) { }
        ...
        T& Get(Context& ctx) const {
            return ctx.GetCachedAttachment(*this);
        }
        ...
        template<class... Args>
        inline static std::enable_if_t<!std::is_array<T>::value, AttachmentPtr<T>> MakePtr(Args&&... args) {

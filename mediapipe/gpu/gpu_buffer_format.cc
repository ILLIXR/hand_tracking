...
const GlTextureInfo& GlTextureInfoForGpuBufferFormat(GpuBufferFormat format, int plane, GlVersion gl_version) {
    // TODO: check/add more cases using info from
    // CVPixelFormatDescriptionCreateWithPixelFormatType.
    static const mediapipe::NoDestructor<absl::flat_hash_map<GpuBufferFormat, std::vector<GlTextureInfo>>> gles3_format_info{{
    ...
    static const auto* gles2_format_info = ([] {
        auto formats = new absl::flat_hash_map<GpuBufferFormat, std::vector<GlTextureInfo>>(*gles3_format_info);
    #ifdef GL_ES_VERSION_2_0
    ...
    ABSL_CHECK(iter != format_info->end()) << "unsupported format: "
                                           << static_cast<std::underlying_type_t<decltype(format)>>(format);
    const auto& planes = iter->second;
    #ifndef __APPLE__
    ABSL_CHECK_EQ(planes.size(), 1) << "multiplanar formats are not supported on this platform";
    #endif

...
public:
    using ItemFactory = absl::AnyInvocable<absl::StatusOr<std::unique_ptr<Item>>() const>;
    ...
    static std::shared_ptr<ReusablePool<Item>> Create(ItemFactory item_factory, const MultiPoolOptions& options) {
        return std::shared_ptr<ReusablePool<Item>>(new ReusablePool<Item>(std::move(item_factory), options));
    }
...
private:
    ...
    void TrimAvailable(std::vector<std::unique_ptr<Item>>* trimmed) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
...
template<class Item>
void ReusablePool<Item>::TrimAvailable(std::vector<std::unique_ptr<Item>>* trimmed) {
    int keep = std::max(keep_count_ - in_use_count_, 0);
    if (available_.size() > keep) {
        ABSL_LOG_FIRST_N(WARNING, 1) << "Maximum number of pooled buffers reached. Consider increasing "

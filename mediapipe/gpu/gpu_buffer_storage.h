...
namespace internal {
    ...
    public:
        ...
        using StorageFactory   = std::function<std::shared_ptr<GpuBufferStorage>(int, int, GpuBufferFormat)>;
        using StorageConverter = std::function<std::shared_ptr<GpuBufferStorage>(std::shared_ptr<GpuBufferStorage>)>;
        ...
        RegistryToken Register() {
            return RegisterFactory<Storage>([](int width, int height, GpuBufferFormat format) -> std::shared_ptr<Storage> {
                return CreateStorage<Storage>(overload_priority<10>{}, width, height, format);
            });
        }
        ...
        RegistryToken RegisterConverter(F&& converter) {
            ...
            return Register(
                [converter](std::shared_ptr<GpuBufferStorage> source) -> std::shared_ptr<GpuBufferStorage> {
    ...
    private:
        ...
        static auto CreateStorage(overload_priority<1>, Args... args) -> decltype(Storage::Create(args...)) {
        ...
        RegistryToken Register(StorageFactory factory, std::vector<TypeId> provider_hashes);
        RegistryToken Register(StorageConverter converter, std::vector<TypeId> provider_hashes, TypeId source_storage);

        absl::flat_hash_map<TypeId, StorageFactory>                      factory_for_view_provider_;
        absl::flat_hash_map<std::pair<TypeId, TypeId>, StorageConverter> converter_for_view_provider_and_existing_storage_;
    };
    ...
    constexpr bool GpuBufferStorageRegistry::kDisableRegistration<
        Storage, std::void_t<decltype(&Storage::kDisableGpuBufferRegistration)>> = Storage::kDisableGpuBufferRegistration;
    ...
    class GpuBufferStorageImpl
        : public GpuBufferStorage
        , public U... {
    public:
    ...
    private:
        ...
        TypeId storage_type() const final {
            return kTypeId<T>;
        }

        const void* down_cast_impl(TypeId to, types<>) const {
            UNUSED(to);
            return nullptr;
        }

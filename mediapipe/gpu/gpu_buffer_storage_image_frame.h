...
class GpuBufferStorageImageFrame
    : public internal::GpuBufferStorageImpl<GpuBufferStorageImageFrame, internal::ViewProvider<ImageFrame>,
...
public:
    ...
    GpuBufferStorageImageFrame(int width, int height, GpuBufferFormat format) {
        image_frame_ = std::make_shared<ImageFrame>(ImageFormatForGpuBufferFormat(format), width, height);
    }
    ...
    std::shared_ptr<const ImageFrame> GetReadView(internal::types<ImageFrame>) const override {
        return image_frame_;
    }
    ...
    std::shared_ptr<const FrameBuffer> GetReadView(internal::types<FrameBuffer>) const override;

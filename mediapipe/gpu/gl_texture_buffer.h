...
class GlTextureBuffer
    : public internal::GpuBufferStorageImpl<GlTextureBuffer, internal::ViewProvider<GlTextureView>>
...
public:
    ...
    using DeletionCallback = std::function<void(std::shared_ptr<GlSyncPoint> sync_token)>;
    ...
    static std::unique_ptr<GlTextureBuffer> Wrap(GLenum target, GLuint name, int width, int height, GpuBufferFormat format,
    ...
    static std::unique_ptr<GlTextureBuffer> Create(int width, int height, GpuBufferFormat format, const void* data = nullptr,
                                                   int alignment = 4);
    ...
    static std::unique_ptr<GlTextureBuffer> Create(const internal::GpuBufferSpec& spec) {
    ...
    // The commands producing the texture are assumed to be completed at the
    // time of this call. If not, call Updated on the result.
    GlTextureBuffer(GLenum target, GLuint name, int width, int height, GpuBufferFormat format,
                    DeletionCallback deletion_callback, std::shared_ptr<GlContext> producer_context = nullptr);
    ~GlTextureBuffer();
    ...
    GlTextureView GetReadView(internal::types<GlTextureView>, int plane) const override;
...
private:
    ...
    mutable std::unique_ptr<GlMultiSyncPoint>
        consumer_multi_sync_   ABSL_GUARDED_BY(consumer_sync_mutex_) = absl::make_unique<GlMultiSyncPoint>();

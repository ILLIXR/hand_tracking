...
public:
    explicit GlContextExecutor(GlContext* gl_context)
...
GpuResources::StatusOrGpuResources GpuResources::Create(PlatformGlContext external_context) {
    MP_ASSIGN_OR_RETURN(std::shared_ptr<GlContext> context, GlContext::Create(external_context, kGlContextUseDedicatedThread));
    std::shared_ptr<GpuResources> gpu_resources(new GpuResources(std::move(context)));
    ...
    : gl_key_context_(new GlContextMapType(),
                      [](auto* map) {
                          ...
                          for (auto& [key, context] : *map) {
                              ...
                              ABSL_LOG_IF(ERROR, !status.ok()) << "Failed to flush GlContext jobs: " << status;
                          }
                          delete map;
                      })
#if MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
    , texture_caches_(std::make_shared<CvTextureCacheManager>())
    , gpu_buffer_pool_([tc = texture_caches_](const internal::GpuBufferSpec& spec, const MultiPoolOptions& options) {
        return CvPixelBufferPoolWrapper::Create(spec, options, tc.get());
    })
#endif // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
{
    gl_key_context_->insert({SharedContextKey(), gl_context});
    named_executors_[kGpuExecutorName] = std::make_shared<GlContextExecutor>(gl_context.get());
#if __APPLE__
    #if MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
...
absl::Status GpuResources::PrepareGpuNode(CalculatorNode* node) {
    ...
    bool gets_own_context = (node_type == "ImageFrameToGpuBufferCalculator") ||
        (node_type == "GpuBufferToImageFrameCalculator") || (node_type == "GlSurfaceSinkCalculator");

    const auto& options = node->GetCalculatorState().Options<mediapipe::GlContextOptions>();
    ...
    MP_ASSIGN_OR_RETURN(std::shared_ptr<GlContext> context, GetOrCreateGlContext(context_key));

    if (kGlContextUseDedicatedThread) {
        std::string executor_name = absl::StrCat(kGpuExecutorName, "_", context_key);
        node->SetExecutor(executor_name);
        if (!ContainsKey(named_executors_, executor_name)) {
            named_executors_.emplace(executor_name, std::make_shared<GlContextExecutor>(context.get()));
        }
    }
    context->SetProfilingContext(node->GetCalculatorState().GetSharedProfilingContext());
...
// canonicalized name.
const std::shared_ptr<GlContext>& GpuResources::gl_context(CalculatorContext* cc) {
    if (cc) {
...
GlContext::StatusOrGlContext GpuResources::GetOrCreateGlContext(const std::string& key) {
    auto it = gl_key_context_->find(key);
    if (it == gl_key_context_->end()) {
        MP_ASSIGN_OR_RETURN(std::shared_ptr<GlContext> new_context,
                            GlContext::Create(*gl_key_context_->at(SharedContextKey()), kGlContextUseDedicatedThread));
...
GpuSharedData::GpuSharedData()
    : GpuSharedData(kPlatformGlContextNone) { }

#if !MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
static std::shared_ptr<GlTextureBuffer> GetGlTextureBufferFromPool(int width, int height, GpuBufferFormat format) {
    ...
    if (cc && cc->Service(kGpuService).IsAvailable()) {
        GpuBufferMultiPool* pool = &cc->Service(kGpuService).GetObject().gpu_buffer_pool();
        ...
        texture_buffer = texture_buffer_from_pool->internal_storage<GlTextureBuffer>();
    } else {
...
static auto kGlTextureBufferPoolRegistration = [] {
    ...
    return internal::GpuBufferStorageRegistry::Get().RegisterFactory<GlTextureBuffer>(GetGlTextureBufferFromPool);
}();
#endif // !MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER

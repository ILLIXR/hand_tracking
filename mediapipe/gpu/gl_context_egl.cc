...
namespace {
    ...
    static void EglThreadExitCallback(void* key_value) {
        ...
        if (current_display != EGL_NO_DISPLAY) {
            ...
            eglMakeCurrent(current_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
        }
        eglReleaseThread();
    }
    ...
    static void EnsureEglThreadRelease() {
        ...
        pthread_setspecific(egl_release_thread_key, reinterpret_cast<void*>(0xDEADBEEF));
    }

    static absl::StatusOr<EGLDisplay> GetInitializedDefaultEglDisplay() {
        EGLDisplay display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
        RET_CHECK(display != EGL_NO_DISPLAY) << "eglGetDisplay() returned error " << std::showbase << std::hex << eglGetError();

        EGLint     major           = 0;
        EGLint     minor           = 0;
        EGLBoolean egl_initialized = eglInitialize(display, &major, &minor);
        RET_CHECK(egl_initialized) << "Unable to initialize EGL";
        ABSL_LOG(INFO) << "Successfully initialized EGL. Major : " << major << " Minor: " << minor;
...
GlContext::StatusOrGlContext GlContext::Create(std::nullptr_t nullp, bool create_thread) {
    return Create(EGL_NO_CONTEXT, create_thread);
}

GlContext::StatusOrGlContext GlContext::Create(const GlContext& share_context, bool create_thread) {
    return Create(share_context.context_, create_thread);
}

GlContext::StatusOrGlContext GlContext::Create(EGLContext share_context, bool create_thread) {
...
absl::Status GlContext::CreateContextInternal(EGLContext share_context, int gl_version) {
    ...
    EGLint     num_configs;
    EGLBoolean success = eglChooseConfig(display_, config_attr, &config_, 1, &num_configs);
    if (!success) {
        return ::mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC)
            << "eglChooseConfig() returned error " << std::showbase << std::hex << eglGetError();
    }
    if (!num_configs) {
        return mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "eglChooseConfig() returned no matching EGL configuration for "
    ...
    RET_CHECK(context_ != EGL_NO_CONTEXT) << "Could not create GLES " << gl_version << " context; "
                                          << "eglCreateContext() returned error " << std::showbase << std::hex << error
                                          << (error == EGL_BAD_CONTEXT ? ": external context uses a different version of OpenGL"
                                                                       : "");
...
absl::Status GlContext::CreateContext(EGLContext share_context) {
    ...
    if (!status.ok()) {
        ABSL_LOG(WARNING) << "Creating a context with OpenGL ES 3 failed: " << status;
    ...
    RET_CHECK(surface_ != EGL_NO_SURFACE) << "eglCreatePbufferSurface() returned error " << std::showbase << std::hex
                                          << eglGetError();
...
void GlContext::DestroyContext() {
    ...
    if (HasContext()) {
        ...
        auto detach_program = [this] {
            ...
            if (eglMakeCurrent(display_, surface_, surface_, context_)) {
                ...
                ABSL_LOG(ERROR) << "eglMakeCurrent() returned error " << std::showbase << std::hex << eglGetError();
            }
    ...
    if (surface_ != EGL_NO_SURFACE) {
        if (!eglDestroySurface(display_, surface_)) {
            ABSL_LOG(ERROR) << "eglDestroySurface() returned error " << std::showbase << std::hex << eglGetError();
        }
        surface_ = EGL_NO_SURFACE;
    }
    if (context_ != EGL_NO_CONTEXT) {
        if (!eglDestroyContext(display_, context_)) {
            ABSL_LOG(ERROR) << "eglDestroyContext() returned error " << std::showbase << std::hex << eglGetError();
        }
...
absl::Status GlContext::SetCurrentContextBinding(const ContextBinding& new_binding) {
    ...
    EGLBoolean success = eglMakeCurrent(display, new_binding.draw_surface, new_binding.read_surface, new_binding.context);
    RET_CHECK(success) << "eglMakeCurrent() returned error " << std::showbase << std::hex << eglGetError();
    return absl::OkStatus();
}

bool GlContext::HasContext() const {

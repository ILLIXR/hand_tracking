...
public:
    ...
    // this header.
    template<class T, class = std::void_t<decltype(internal::AsGpuBufferStorage(std::declval<T>()))>>
    ...
    int width() const {
        return holder_ ? holder_->width() : 0;
    }

    int height() const {
        return holder_ ? holder_->height() : 0;
    }
    ...
    decltype(auto) GetReadView(Args... args) const {
        return GetViewProviderOrDie<View>(false).GetReadView(internal::types<View>{}, std::forward<Args>(args)...);
    }
    ...
    decltype(auto) GetWriteView(Args... args) {
        return GetViewProviderOrDie<View>(true).GetWriteView(internal::types<View>{}, std::forward<Args>(args)...);
    }
...
private:
    internal::GpuBufferStorage* GetStorageForView(TypeId view_provider_type, bool for_writing) const {
        return holder_ ? holder_->GetStorageForView(view_provider_type, for_writing) : nullptr;
    }

    internal::GpuBufferStorage& GetStorageForViewOrDie(TypeId view_provider_type, bool for_writing) const;

    template<class View>
    internal::ViewProvider<View>& GetViewProviderOrDie(bool for_writing) const {
        using VP = internal::ViewProvider<View>;
        return *GetStorageForViewOrDie(kTypeId<VP>, for_writing).template down_cast<VP>();
    }
    ...
    public:
        explicit StorageHolder(std::shared_ptr<internal::GpuBufferStorage> storage)
            : StorageHolder(storage->width(), storage->height(), storage->format()) {
    ...
    private:
        ...
        mutable std::vector<std::shared_ptr<internal::GpuBufferStorage>> storages_ ABSL_GUARDED_BY(mutex_);
    };

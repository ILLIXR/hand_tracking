...
namespace {
    ...
    struct StorageTypeFormatter {
        void operator()(std::string* out, const std::shared_ptr<internal::GpuBufferStorage>& s) const {
...
std::string GpuBuffer::DebugString() const {
    return holder_ ? absl::StrCat("GpuBuffer[", width(), "x", height(), " ", format(), " as ", holder_->DebugString(), "]")
                   : "GpuBuffer[invalid]";
}
...
internal::GpuBufferStorage* GpuBuffer::StorageHolder::GetStorageForView(TypeId view_provider_type, bool for_writing) const {
...

    {
        ...
        if (!chosen_storage) {
            for (const auto& s : storages_) {
                if (auto converter = internal::GpuBufferStorageRegistry::Get().StorageConverterForViewProvider(
    ...
    if (for_writing) {
        ...
        if (chosen_storage) {
            ...
            if (auto factory = internal::GpuBufferStorageRegistry::Get().StorageFactoryForViewProvider(view_provider_type)) {
...
internal::GpuBufferStorage& GpuBuffer::GetStorageForViewOrDie(TypeId view_provider_type, bool for_writing) const {
    auto* chosen_storage = GpuBuffer::GetStorageForView(view_provider_type, for_writing);
    ABSL_CHECK(chosen_storage) << "no view provider found for requested view " << view_provider_type.name()
                               << "; storages available: " << (holder_ ? holder_->DebugString() : "invalid");
...
CVPixelBufferRef GetCVPixelBufferRef(const GpuBuffer& buffer) {
    if (buffer.GetStorageForView(kTypeId<internal::ViewProvider<CVPixelBufferRef>>,

...
namespace {
    ...
    std::string AddLineNumbers(const GLchar* source) {
        ...
        std::string              format        = absl::StrFormat("%%%ii %%s", static_cast<int>(ceilf(log10(1 + lines.size()))));
...
GLint GlhCompileShader(GLenum target, const GLchar* source, GLuint* shader, bool force_log_errors) {
...
GLint GlhLinkProgram(GLuint program, bool force_log_errors) {
    ...
    ABSL_LOG_IF(ERROR, status == GL_FALSE) << "Failed to link program " << program;
...
GLint GlhValidateProgram(GLuint program) {
    ...
    ABSL_LOG_IF(ERROR, status == GL_FALSE) << "Failed to validate program " << program;

    return status;
}

GLint GlhCreateProgram(const GLchar* vert_src, const GLchar* frag_src, GLsizei attr_count, const GLchar* const* attr_names,
                       const GLint* attr_locations, GLuint* program, bool force_log_errors) {
    ...
    *program = glCreateProgram();
    ...
    ok = ok && GlhCompileShader(GL_VERTEX_SHADER, vert_src, &vert_shader, force_log_errors);
    ok = ok && GlhCompileShader(GL_FRAGMENT_SHADER, frag_src, &frag_shader, force_log_errors);
...
bool CreateShaderProgram(GLuint vertex_shader, GLuint fragment_shader,
                         const std::unordered_map<GLuint, std::string>& attributes, GLuint* shader_program) {

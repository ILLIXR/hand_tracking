...
public:
    ...
    GpuBufferMultiPool& gpu_buffer_pool() {
        return gpu_buffer_pool_;
    }

#ifdef __APPLE__
    MetalSharedResources& metal_shared() {
        return *metal_shared_;
    }
#endif // defined(__APPLE__)§
...
private:
    static std::shared_ptr<GpuResources> CreateGpuResourcesOrDie(PlatformGlContext external_context) {
        auto status_or_resources = GpuResources::Create(external_context);
        MEDIAPIPE_CHECK_OK(status_or_resources.status()) << ": could not create GpuResources";

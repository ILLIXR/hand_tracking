...
namespace internal_gl_context {
    ...
    bool IsOpenGlVersionSameOrAbove(const OpenGlVersion& version, const OpenGlVersion& expected_version) {
        return (version.major == expected_version.major && version.minor >= expected_version.minor) ||
...
static void SetThreadName(const char* name) {
    ...
    if (res != 0) {
        ABSL_LOG_FIRST_N(INFO, 1) << "Can't set pthread names: name: \"" << name << "\"; error: " << res;
    }
#elif __APPLE__
...
bool GlContext::ParseGlVersion(absl::string_view version_string, GLint* major, GLint* minor) {
    ...
    size_t start = pos - 1;
    while (start > 0 && isdigit(version_string[start - 1]))
        --start;
    ...
    if (pos == absl::string_view::npos || (pos2 != absl::string_view::npos && pos2 < pos)) {
        pos = pos2;
    }
...
absl::Status GlContext::GetGlExtensions() {
    ...
    if (!SymbolAvailable(&glGetStringi)) {
        ABSL_LOG(ERROR) << "GL major version > 3.0 indicated, but glGetStringi not "
...
absl::Status GlContext::FinishInitialization(bool create_thread) {
    if (create_thread) {
        ...
        MP_RETURN_IF_ERROR(thread_->Run([this] {
            return EnterContext(nullptr);
        }));
    }
    ...
    return Run([this]() -> absl::Status {
        ...
        if (err == GL_NO_ERROR) {
            ...
            if (!ParseGlVersion(version_string, &gl_major_version_, &gl_minor_version_)) {
                ABSL_LOG(WARNING) << "invalid GL_VERSION format: '" << version_string << "'; assuming 2.0";
                gl_major_version_ = 2;
                gl_minor_version_ = 0;
            }
        }
        ...
        if (gl_major_version_from_context_creation > 0 && gl_major_version_ != gl_major_version_from_context_creation) {
            ABSL_LOG(WARNING) << "Requested a context with major GL version " << gl_major_version_from_context_creation
                              << " but context reports major version " << gl_major_version_ << ". Setting to "
        ...
        can_linear_filter_float_textures_ =
            HasGlExtension("OES_texture_float_linear") || HasGlExtension("GL_OES_texture_float_linear");
#else
...
GlContext::~GlContext() {
    ...
    if (thread_) {
        ...
        ABSL_LOG_IF(ERROR, !status.ok()) << "Failed to deactivate context on thread: " << status;
...
absl::Status GlContext::Run(GlStatusFunction gl_func, int node_id, Timestamp input_timestamp) {
    absl::Status status;
    if (profiling_helper_) {
        gl_func = [=] {
...
absl::Status GlContext::SwitchContext(ContextBinding*       saved_context,
                                      const ContextBinding& new_context) ABSL_NO_THREAD_SAFETY_ANALYSIS {
    std::shared_ptr<GlContext> old_context_obj = CurrentContext().lock();
    std::shared_ptr<GlContext> new_context_obj = new_context.context_object.lock();
    if (saved_context) {
...
public:
    ...
    explicit GlSyncWrapper(GLsync sync)
        : sync_(sync) { }
    ...
    GlSyncWrapper(GlSyncWrapper&& other)
        : sync_(nullptr) {
        *this = std::move(other);
    }
    ...
    void WaitOnGpu() {
        ...
        if (!GlContext::IsAnyContextCurrent()) {
            ...
            ABSL_LOG_FIRST_N(ERROR, 1) << "An attempt to wait for a sync without any context current.";
            return;
        }
        ...
        // TODO: remove after glWaitSync crashes are resolved.
        ABSL_CHECK(context->ShouldUseFenceSync())
            << absl::StrFormat("An attempt to wait for a sync when it should not be used. (OpenGL "
                               "Version "
                               "%d.%d)",
...
public:
    ...
    ~GlFenceSyncPoint() {
        if (sync_) {
            gl_context_->RunWithoutWaiting([sync = new GlSyncWrapper(std::move(sync_))] {
                delete sync;
            });
        }
    }
...
public:
    ...
    explicit GlExternalFenceSyncPoint(const std::shared_ptr<GlContext>& graph_service_gl_context)
        : GlSyncPoint(nullptr)
    ...
    ~GlExternalFenceSyncPoint() {
        if (sync_) {
            graph_service_gl_context_->RunWithoutWaiting([sync = new GlSyncWrapper(std::move(sync_))] {
                delete sync;
            });
        }
    }
...
bool GlMultiSyncPoint::IsReady() {
    syncs_.erase(std::remove_if(syncs_.begin(), syncs_.end(), std::bind(&GlSyncPoint::IsReady, std::placeholders::_1)),
                 syncs_.end());
    return syncs_.empty();
}
...
bool GlContext::ShouldUseFenceSync() const {
    ...
    return SymbolAvailable(&glWaitSync) &&
        internal_gl_context::IsOpenGlVersionSameOrAbove({.major = gl_major_version(), .minor = gl_minor_version()},
...
GlContext::CreateSyncTokenForCurrentExternalContext(const std::shared_ptr<GlContext>& delegate_graph_context) {
    ABSL_CHECK(delegate_graph_context);
    if (!IsAnyContextCurrent())
        return nullptr;
    if (delegate_graph_context->ShouldUseFenceSync()) {
        return std::shared_ptr<GlSyncPoint>(new GlExternalFenceSyncPoint(delegate_graph_context));
    } else {
        glFinish();
        return nullptr;
    }
...
std::shared_ptr<GlSyncPoint> GlContext::TestOnly_CreateSpecificSyncToken(SyncTokenTypeForTest type) {
    std::shared_ptr<GlSyncPoint> token;
    switch (type) {
...
bool GlContext::CheckForGlErrors(bool force) {
    ...
    while ((error = glGetError()) != GL_NO_ERROR) {
        ...
        case GL_INVALID_FRAMEBUFFER_OPERATION:
            ABSL_LOG(INFO) << "Found unchecked GL error: GL_INVALID_FRAMEBUFFER_OPERATION";
            break;
...
const GlTextureInfo& GlTextureInfoForGpuBufferFormat(GpuBufferFormat format, int plane) {
...
const GlContext::Attachment<GLuint> kUtilityFramebuffer([](GlContext&) -> GlContext::Attachment<GLuint>::Ptr {
    GLuint framebuffer;

...
void GlCalculatorHelper::InitializeInternal(CalculatorContext* cc, GpuResources* gpu_resources) {
...
absl::Status GlCalculatorHelper::Open(CalculatorContext* cc) {
    ...
    RET_CHECK(gpu_service.IsAvailable()) << "GPU service not available. Did you forget to call "
...
// static
absl::Status GlCalculatorHelper::UpdateContract(CalculatorContract* cc, bool request_gpu_as_optional) {
...
// static
absl::Status GlCalculatorHelper::SetupInputSidePackets(PacketTypeSet* input_side_packets) {
    ...
    ABSL_LOG(WARNING) << "CalculatorContract not available. If you're calling this "
                         ...
                         "instead.";
    auto id = input_side_packets->GetId(kGpuSharedTagName, 0);
    RET_CHECK(id.IsValid()) << "A " << mediapipe::kGpuSharedTagName << " input side packet is required here.";
    input_side_packets->Get(id).Set<GpuSharedData*>();
    return absl::OkStatus();
}

absl::Status GlCalculatorHelper::RunInGlContext(std::function<absl::Status(void)> gl_func,
                                                CalculatorContext*                calculator_context) {
    if (calculator_context) {
        return gl_context_->Run(std::move(gl_func), calculator_context->NodeId(), calculator_context->InputTimestamp());
    } else {
...
absl::Status GlCalculatorHelper::RunInGlContext(std::function<absl::Status(void)> gl_func) {
    if (!Initialized())
        return absl::InternalError("helper not initialized");
    // TODO: Remove LegacyCalculatorSupport from MediaPipe OSS.
    auto calculator_context = LegacyCalculatorSupport::Scoped<CalculatorContext>::current();
    return RunInGlContext(gl_func, calculator_context);
}

GLuint GlCalculatorHelper::framebuffer() const {
    return framebuffer_;
}
...
void GlCalculatorHelper::BindFramebuffer(const GlTexture& dst) {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, dst.target(), dst.name(), 0);
...
GlTexture GlCalculatorHelper::MapGpuBuffer(const GpuBuffer& gpu_buffer, GlTextureView view) {
    if (gpu_buffer.format() != GpuBufferFormat::kUnknown) {
        // TODO: do the params need to be reset here??
        glBindTexture(view.target(), view.name());
        GlTextureInfo info = GlTextureInfoForGpuBufferFormat(gpu_buffer.format(), view.plane(), GetGlVersion());
        gl_context_->SetStandardTextureParams(view.target(), info.gl_internal_format);
...
GlTexture GlCalculatorHelper::CreateSourceTexture(const GpuBuffer& gpu_buffer, int plane) {
    return MapGpuBuffer(gpu_buffer, gpu_buffer.GetReadView<GlTextureView>(plane));
}

GlTexture GlCalculatorHelper::CreateSourceTexture(const ImageFrame& image_frame) {
...
GpuBuffer GlCalculatorHelper::GpuBufferWithImageFrame(std::shared_ptr<ImageFrame> image_frame) {
    return GpuBuffer(std::make_shared<GpuBufferStorageImageFrame>(std::move(image_frame)));
}

GpuBuffer GlCalculatorHelper::GpuBufferCopyingImageFrame(const ImageFrame& image_frame) {
...
void GlCalculatorHelper::GetGpuBufferDimensions(const GpuBuffer& pixel_buffer, int* width, int* height) {
    ABSL_CHECK(width);
    ABSL_CHECK(height);
...
GlTexture GlCalculatorHelper::CreateDestinationTexture(int width, int height, GpuBufferFormat format) {
    if (!framebuffer_) {
        CreateFramebuffer();
    }

    auto gpu_buffer = gpu_resources_->gpu_buffer_pool().GetBuffer(width, height, format);
    ABSL_CHECK_OK(gpu_buffer);
    return MapGpuBuffer(*gpu_buffer, gpu_buffer->GetWriteView<GlTextureView>(0));
}

GlTexture GlCalculatorHelper::CreateDestinationTexture(const ImageFrame& image_frame) {
...
GlTexture GlCalculatorHelper::CreateSourceTexture(const mediapipe::Image& image) {
...
std::unique_ptr<ImageFrame> GlTexture::GetFrame<ImageFrame>() const {
    view_->DoneWriting();
    std::shared_ptr<const ImageFrame> view = gpu_buffer_.GetReadView<ImageFrame>();
...
template<>
std::unique_ptr<mediapipe::Image> GlTexture::GetFrame<mediapipe::Image>() const {

...
public:
    ...
    static StatusOrGlContext Create(const GlContext& share_context, bool create_thread);
    static StatusOrGlContext Create(PlatformGlContext share_context, bool create_thread);
#if HAS_EAGL
    static StatusOrGlContext Create(EAGLSharegroup* sharegroup, bool create_thread);
#endif // HAS_EAGL
    ...
    void SetProfilingContext(std::shared_ptr<mediapipe::ProfilingContext> profiling_context);

    // Executes a function in the GL context. Waits for the
    // function's execution to be complete before returning to the caller.
    absl::Status Run(GlStatusFunction gl_func, int node_id = -1, Timestamp input_timestamp = Timestamp::Unset());
    ...
    EMSCRIPTEN_WEBGL_CONTEXT_HANDLE webgl_context() const {
        return context_;
    }

    EmscriptenWebGLContextAttributes webgl_attributes() const {
        return attrs_;
    }
#elif HAS_EGL
...
#elif HAS_EAGL
    EAGLContext* eagl_context() const {
        return context_;
    }

    CVOpenGLESTextureCacheRef cv_texture_cache() const {
    ...
    NSOpenGLPixelFormat* nsgl_pixel_format() const {
        return pixel_format_;
    }

    CVOpenGLTextureCacheRef cv_texture_cache() const {
        return *texture_cache_;
    }
#endif // HAS_EGL
    ...
    PlatformGlContext native_context() const {
        return context_;
    }
    ...
    GLint gl_major_version() const {
        return gl_major_version_;
    }

    GLint gl_minor_version() const {
        return gl_minor_version_;
    }

    static bool ParseGlVersion(absl::string_view version_string, GLint* major, GLint* minor);
    ...
    int64_t gl_finish_count() {
        return gl_finish_count_;
    }
    ...
    template<typename T, typename = typename std::enable_if<std::is_void<typename std::result_of<T()>::type>::value>::type>
    void Run(T f) {
        Run([f] {
            f();
    ...
    static std::shared_ptr<GlSyncPoint>
    CreateSyncTokenForCurrentExternalContext(const std::shared_ptr<GlContext>& delegate_graph_context);
    ...
    std::shared_ptr<GlSyncPoint> TestOnly_CreateSpecificSyncToken(SyncTokenTypeForTest type);
...
private:
    ...
    absl::Status CreateContextInternal(EMSCRIPTEN_WEBGL_CONTEXT_HANDLE share_context, int webgl_version);
    ...
    static absl::Status SwitchContext(ContextBinding* saved_context, const ContextBinding& new_context);
    ...
    static absl::Status SetCurrentContextBinding(const ContextBinding& new_binding);
    ...
    absl::flat_hash_map<const AttachmentBase*, internal::AttachmentPtr<void>> attachments_;
...
ABSL_DEPRECATED("Prefer passing an explicit GlVersion argument (use "

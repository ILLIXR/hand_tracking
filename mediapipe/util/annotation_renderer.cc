...
namespace {
    ...
    bool NormalizedtoPixelCoordinates(double normalized_x, double normalized_y, int image_width, int image_height, int* x_px,
                                      int* y_px) {
        ...
        if (normalized_x < 0 || normalized_x > 1.0 || normalized_y < 0 || normalized_y > 1.0) {
    ...
    cv::RotatedRect RectangleToOpenCVRotatedRect(int left, int top, int right, int bottom, double rotation) {
        return cv::RotatedRect(cv::Point2f((left + right) / 2.f, (top + bottom) / 2.f), cv::Size2f(right - left, bottom - top),
                               rotation / M_PI * 180.f);
    }

    void cv_line2(cv::Mat& img, const cv::Point& start, const cv::Point& end, const cv::Scalar& color1,
                  const cv::Scalar& color2, int thickness) {
...
void AnnotationRenderer::RenderDataOnImage(const RenderData& render_data) {
    for (const auto& annotation : render_data.render_annotations()) {
        ...
        } else if (annotation.data_case() == RenderAnnotation::kFilledRoundedRectangle) {
...
int AnnotationRenderer::GetImageWidth() const {
    return mat_image_.cols;
}

int AnnotationRenderer::GetImageHeight() const {
    return mat_image_.rows;
}
...
void AnnotationRenderer::SetScaleFactor(float scale_factor) {
    if (scale_factor > 0.0f)
        scale_factor_ = std::min(scale_factor, 1.0f);
}
...
void AnnotationRenderer::DrawRectangle(const RenderAnnotation& annotation) {
    ...
    if (rectangle.normalized()) {
        ABSL_CHECK(NormalizedtoPixelCoordinates(rectangle.left(), rectangle.top(), image_width_, image_height_, &left, &top));
        ABSL_CHECK(
            NormalizedtoPixelCoordinates(rectangle.right(), rectangle.bottom(), image_width_, image_height_, &right, &bottom));
    } else {
    ...
    if (rectangle.rotation() != 0.0) {
        const auto& rect         = RectangleToOpenCVRotatedRect(left, top, right, bottom, rectangle.rotation());
        const int   kNumVertices = 4;
        cv::Point2f vertices[kNumVertices];
        rect.points(vertices);
        for (int i = 0; i < kNumVertices; i++) {
            cv::line(mat_image_, vertices[i], vertices[(i + 1) % kNumVertices], color, thickness);
        }
    } else {
    ...
    if (rectangle.has_top_left_thickness()) {
        ...
        cv::Point2f vertices[kNumVertices];
        rect.points(vertices);
        const int top_left_thickness = ClampThickness(round(rectangle.top_left_thickness() * scale_factor_));
        cv::ellipse(mat_image_, vertices[1], cv::Size(top_left_thickness, top_left_thickness), 0.0, 0, 360, color, -1);
    }
}

void AnnotationRenderer::DrawFilledRectangle(const RenderAnnotation& annotation) {
    int         left      = -1;
    int         top       = -1;
    int         right     = -1;
    int         bottom    = -1;
...
void AnnotationRenderer::DrawRoundedRectangle(const RenderAnnotation& annotation) {
    int         left      = -1;
    int         top       = -1;
    int         right     = -1;
    int         bottom    = -1;
    ...
    const cv::Scalar color         = MediapipeColorToOpenCVColor(annotation.color());
    const int        thickness     = ClampThickness(round(annotation.thickness() * scale_factor_));
    const int        corner_radius = round(annotation.rounded_rectangle().corner_radius() * scale_factor_);
    const int        line_type     = annotation.rounded_rectangle().line_type();
    DrawRoundedRectangle(mat_image_, cv::Point(left, top), cv::Point(right, bottom), color, thickness, line_type,
                         corner_radius);
}

void AnnotationRenderer::DrawFilledRoundedRectangle(const RenderAnnotation& annotation) {
    int         left      = -1;
    int         top       = -1;
    int         right     = -1;
    int         bottom    = -1;
    const auto& rectangle = annotation.filled_rounded_rectangle().rounded_rectangle().rectangle();
    if (rectangle.normalized()) {
        ABSL_CHECK(NormalizedtoPixelCoordinates(rectangle.left(), rectangle.top(), image_width_, image_height_, &left, &top));
        ABSL_CHECK(
            NormalizedtoPixelCoordinates(rectangle.right(), rectangle.bottom(), image_width_, image_height_, &right, &bottom));
    ...
    const cv::Scalar color         = MediapipeColorToOpenCVColor(annotation.color());
    const int        corner_radius = annotation.rounded_rectangle().corner_radius() * scale_factor_;
    const int        line_type     = annotation.rounded_rectangle().line_type();
    DrawRoundedRectangle(mat_image_, cv::Point(left, top), cv::Point(right, bottom), color, -1, line_type, corner_radius);
}

void AnnotationRenderer::DrawRoundedRectangle(cv::Mat src, cv::Point top_left, cv::Point bottom_right,
                                              const cv::Scalar& line_color, int thickness, int line_type, int corner_radius) {
    // Corners:
    // p1 - p2
    // |     |
    // p4 - p3
    ...
    cv::line(src, cv::Point(p1.x + corner_radius, p1.y), cv::Point(p2.x - corner_radius, p2.y), line_color, thickness,
             line_type);
    cv::line(src, cv::Point(p2.x, p2.y + corner_radius), cv::Point(p3.x, p3.y - corner_radius), line_color, thickness,
             line_type);
    cv::line(src, cv::Point(p4.x + corner_radius, p4.y), cv::Point(p3.x - corner_radius, p3.y), line_color, thickness,
             line_type);
    cv::line(src, cv::Point(p1.x, p1.y + corner_radius), cv::Point(p4.x, p4.y - corner_radius), line_color, thickness,
             line_type);

    // Draw arcs at corners.
    cv::ellipse(src, p1 + cv::Point(corner_radius, corner_radius), cv::Size(corner_radius, corner_radius), 180.0, 0, 90,
                line_color, thickness, line_type);
    cv::ellipse(src, p2 + cv::Point(-corner_radius, corner_radius), cv::Size(corner_radius, corner_radius), 270.0, 0, 90,
                line_color, thickness, line_type);
    cv::ellipse(src, p3 + cv::Point(-corner_radius, -corner_radius), cv::Size(corner_radius, corner_radius), 0.0, 0, 90,
                line_color, thickness, line_type);
    cv::ellipse(src, p4 + cv::Point(corner_radius, -corner_radius), cv::Size(corner_radius, corner_radius), 90.0, 0, 90,
                line_color, thickness, line_type);
}
...
void AnnotationRenderer::DrawOval(const RenderAnnotation& annotation) {
    ...
    if (enclosing_rectangle.normalized()) {
        ABSL_CHECK(NormalizedtoPixelCoordinates(enclosing_rectangle.left(), enclosing_rectangle.top(), image_width_,
                                                image_height_, &left, &top));
        ABSL_CHECK(NormalizedtoPixelCoordinates(enclosing_rectangle.right(), enclosing_rectangle.bottom(), image_width_,
...
void AnnotationRenderer::DrawFilledOval(const RenderAnnotation& annotation) {
    ...
    cv::Point        center((left + right) / 2, (top + bottom) / 2);
    cv::Size         size(std::max(0, (right - left) / 2), std::max(0, (bottom - top) / 2));
    const double     rotation = enclosing_rectangle.rotation() / M_PI * 180.f;
    const cv::Scalar color    = MediapipeColorToOpenCVColor(annotation.color());
...
void AnnotationRenderer::DrawArrow(const RenderAnnotation& annotation) {
    ...
    if (arrow.normalized()) {
        ABSL_CHECK(
            NormalizedtoPixelCoordinates(arrow.x_start(), arrow.y_start(), image_width_, image_height_, &x_start, &y_start));
        ABSL_CHECK(NormalizedtoPixelCoordinates(arrow.x_end(), arrow.y_end(), image_width_, image_height_, &x_end, &y_end));
    } else {
    ...
    cv::Point arrowtip_left_start(static_cast<int>(round(arrowtip_left[0])), static_cast<int>(round(arrowtip_left[1])));
    cv::Point arrowtip_right_start(static_cast<int>(round(arrowtip_right[0])), static_cast<int>(round(arrowtip_right[1])));
...
void AnnotationRenderer::DrawPoint(const RenderAnnotation::Point& point, const RenderAnnotation& annotation) {
    int x = -1;
    int y = -1;
    if (point.normalized()) {
        ABSL_CHECK(NormalizedtoPixelCoordinates(point.x(), point.y(), image_width_, image_height_, &x, &y));
    } else {
...
void AnnotationRenderer::DrawLine(const RenderAnnotation& annotation) {
    ...
    if (line.normalized()) {
        ABSL_CHECK(
            NormalizedtoPixelCoordinates(line.x_start(), line.y_start(), image_width_, image_height_, &x_start, &y_start));
        ABSL_CHECK(NormalizedtoPixelCoordinates(line.x_end(), line.y_end(), image_width_, image_height_, &x_end, &y_end));
    } else {
...
void AnnotationRenderer::DrawText(const RenderAnnotation& annotation) {
    ...
    if (text.normalized()) {
        ABSL_CHECK(NormalizedtoPixelCoordinates(text.left(), text.baseline(), image_width_, image_height_, &left, &baseline));
    ...
    int          text_baseline = 0;
    cv::Size     text_size     = cv::getTextSize(text.display_text(), font_face, font_scale, thickness, &text_baseline);
    ...
    if (text.outline_thickness() > 0.0) {
        const int background_thickness =
            ClampThickness(round((annotation.thickness() + 2.0 * text.outline_thickness()) * scale_factor_));
        const cv::Scalar outline_color = MediapipeColorToOpenCVColor(text.outline_color());
        cv::putText(mat_image_, text.display_text(), origin, font_face, font_scale, outline_color, background_thickness,
                    /*lineType=*/8,
                    /*bottomLeftOrigin=*/flip_text_vertically_);
    }
    cv::putText(mat_image_, text.display_text(), origin, font_face, font_scale, color, thickness, /*lineType=*/8,
                /*bottomLeftOrigin=*/flip_text_vertically_);
}

double AnnotationRenderer::ComputeFontScale(int font_face, int font_size, int thickness) {
    double base_line;
    double cap_line;
    ...
    return (static_cast<double>(font_size) - (thick / 2.0F)) / (cap_line + base_line);
}

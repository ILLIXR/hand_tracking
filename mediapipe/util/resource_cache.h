...
template<typename Key, typename Value, typename KeyHash = typename absl::flat_hash_map<Key, int>::hasher>
class ResourceCache {
public:
    Value Lookup(const Key& key, absl::FunctionRef<Value(const Key& key, int request_count)> create) {
        ...
        Entry* entry;
        if (map_it == map_.end()) {
            std::tie(map_it, std::ignore) = map_.try_emplace(key, std::make_unique<Entry>(key));

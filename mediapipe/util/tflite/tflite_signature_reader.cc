...
namespace {
    ...
    absl::flat_hash_map<uint32_t, std::string> FlipKVInMap(const std::map<std::string, uint32_t>& map) {
...
TfLiteSignatureReader::GetInputOutputTensorNamesFromTfliteSignature(const tflite::Interpreter& interpreter,
                                                                    const std::string*         signature_key) {
    std::vector<const std::string*> model_signature_keys = interpreter.signature_keys();
    ...
    if (signature_key == nullptr && model_signature_keys.size() > 1) {
        ...
        return absl::InvalidArgumentError(absl::StrCat("Model contains multiple signatures but no signature key "
    ...
    if (signature_key != nullptr) {
        RET_CHECK(std::find_if(model_signature_keys.begin(), model_signature_keys.end(),
    ...
    const absl::flat_hash_map<uint32_t, std::string> model_input_tensor_id_to_name_map =
        FlipKVInMap(interpreter.signature_inputs(signature_key_str->c_str()));
    const absl::flat_hash_map<uint32_t, std::string> model_output_tensor_id_to_name_map =
    ...
    for (int i = 0; i < (int) model_input_tensor_ids.size(); ++i) {
        const auto it = model_input_tensor_id_to_name_map.find(model_input_tensor_ids[i]);
        if (it == model_input_tensor_id_to_name_map.end()) {
            return absl::InternalError(absl::StrCat("Input tensor id ", model_input_tensor_ids[i], " not found in signature."));
        }
    ...
    for (int i = 0; i < (int) model_output_tensor_ids.size(); ++i) {
        const auto it = model_output_tensor_id_to_name_map.find(model_output_tensor_ids[i]);
        if (it == model_output_tensor_id_to_name_map.end()) {
            return absl::InternalError(
                absl::StrCat("Output tensor id ", model_output_tensor_ids[i], " not found in signature."));
        }
...
absl::StatusOr<absl::flat_hash_map<SignatureName, SignatureInputOutputTensorNames>>
TfLiteSignatureReader::GetInputOutputTensorNamesFromAllTfliteSignatures(const tflite::Interpreter& interpreter) {
    absl::flat_hash_map<SignatureName, SignatureInputOutputTensorNames> result;
    std::vector<const std::string*>                                     model_signature_keys = interpreter.signature_keys();
    for (const std::string* signature_key : model_signature_keys) {
        MP_ASSIGN_OR_RETURN(SignatureInputOutputTensorNames input_output_tensor_names,
                            GetInputOutputTensorNamesFromTfliteSignature(interpreter, signature_key));
        auto [unused_iter, was_inserted] = result.insert({*signature_key, std::move(input_output_tensor_names)});
        RET_CHECK(was_inserted) << "Duplicate signature key: " << *signature_key
                                << ". Available signature keys: " << absl::StrJoin(model_signature_keys, ", ");
    }
    return result;
}
} // namespace mediapipe

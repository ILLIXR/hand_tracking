...
namespace gpu {
    namespace {
        ...
        void UpdateShapes(const tflite::Interpreter& interpreter, const std::vector<int>& indices,
        ...
        absl::Status InitializeShapes(const tflite::FlatBufferModel& flatbuffer, const tflite::OpResolver& op_resolver,
        ...
        absl::Status VerifyShapes(const std::vector<TensorObjectDef>& actual, const std::vector<BHWC>& expected) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                RET_CHECK(dims.b == bhwc.b && dims.h == bhwc.h && dims.w == bhwc.w && dims.c == bhwc.c);
            }
            return absl::OkStatus();
        }
    ...
    absl::Status TFLiteGPURunner::InitializeWithModel(const tflite::FlatBufferModel& flatbuffer,
        ...
        MP_RETURN_IF_ERROR(BuildFromFlatBuffer(flatbuffer, op_resolver, graph_gl_.get(), allow_quant_ops));
        MP_RETURN_IF_ERROR(BuildFromFlatBuffer(flatbuffer, op_resolver, graph_cl_.get(), allow_quant_ops));
        ...
        MP_RETURN_IF_ERROR(InitializeShapes(flatbuffer, op_resolver, &input_shape_from_model_, &output_shape_from_model_));
        return absl::OkStatus();
    }
    ...
    absl::Status TFLiteGPURunner::Build() {
        ...
        for (int flow_index = 0; flow_index < input_shapes_.size(); ++flow_index) {
            MP_RETURN_IF_ERROR(builder->SetInputObjectDef(flow_index, GetSSBOObjectDef(input_shapes_[flow_index].c)));
        }
        for (int flow_index = 0; flow_index < output_shapes_.size(); ++flow_index) {
            MP_RETURN_IF_ERROR(builder->SetOutputObjectDef(flow_index, GetSSBOObjectDef(output_shapes_[flow_index].c)));
        }
    ...
    absl::Status TFLiteGPURunner::BindSSBOToInputTensor(GLuint ssbo_id, int input_id) {
        OpenGlBuffer buffer;
        buffer.id = ssbo_id;
        return runner_->SetInputObject(input_id, std::move(buffer));
    }

    absl::Status TFLiteGPURunner::BindSSBOToOutputTensor(GLuint ssbo_id, int output_id) {
        OpenGlBuffer buffer;
        buffer.id = ssbo_id;
        return runner_->SetOutputObject(output_id, std::move(buffer));
    }

    absl::Status TFLiteGPURunner::Invoke() {
        return runner_->Run();
    }

    absl::Status TFLiteGPURunner::InitializeOpenGL(std::unique_ptr<InferenceBuilder>* builder) {
        ...
        MP_RETURN_IF_ERROR(NewInferenceEnvironment(env_options, &gl_environment_, &properties));
        MP_RETURN_IF_ERROR(gl_environment_->NewInferenceBuilder(std::move(*graph_gl_), gl_options, builder));
        return absl::OkStatus();
    }

    absl::Status TFLiteGPURunner::InitializeOpenCL(std::unique_ptr<InferenceBuilder>* builder) {
#if defined(__ANDROID__) || defined(MEDIAPIPE_CHROMIUMOS)
        ...
        MP_RETURN_IF_ERROR(cl::NewInferenceEnvironment(env_options, &cl_environment_, &properties));

        if (serialized_model_.empty() && opencl_init_from_serialized_model_is_forced_) {
        ...
        if (!serialized_model_.empty()) {
            ...
            VLOG(2) << "Failed to init from serialized model: [" << init_status << "]. Trying to init from scratch.";
        }
        ...
        MP_RETURN_IF_ERROR(cl_environment_->NewInferenceBuilder(cl_options, std::move(graph_cl), builder));
    ...
    absl::Status TFLiteGPURunner::InitializeOpenCLFromSerializedModel(std::unique_ptr<InferenceBuilder>* builder) {
#if defined(__ANDROID__) || defined(MEDIAPIPE_CHROMIUMOS)
        RET_CHECK(cl_environment_) << "CL environment is not initialized.";
        MP_RETURN_IF_ERROR(cl_environment_->NewInferenceBuilder(serialized_model_, builder));
        MP_RETURN_IF_ERROR(VerifyShapes(builder->get()->inputs(), input_shapes_));
        return VerifyShapes(builder->get()->outputs(), output_shapes_);
#else
        return absl::UnimplementedError("OpenCL and serialized model are not supported.");
#endif // defined(__ANDROID__) || defined(MEDIAPIPE_CHROMIUMOS)
    }

    absl::StatusOr<std::vector<uint8_t>> TFLiteGPURunner::GetSerializedBinaryCache() {
#if defined(__ANDROID__) || defined(MEDIAPIPE_CHROMIUMOS)
        RET_CHECK(cl_environment_) << "CL environment is not initialized.";
    ...
    absl::StatusOr<std::vector<uint8_t>> TFLiteGPURunner::GetSerializedModel() {
        ...
        MP_RETURN_IF_ERROR(cl_environment_->BuildSerializedModel(cl_options, std::move(graph_cl), &serialized_model));
    ...
    void TFLiteGPURunner::SetSerializedModel(std::vector<uint8_t>&& serialized_model) {
#if defined(__ANDROID__) || defined(MEDIAPIPE_CHROMIUMOS)

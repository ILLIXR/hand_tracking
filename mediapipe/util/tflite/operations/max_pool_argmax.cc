...
namespace tflite_operations {
    namespace {
        ...
        inline void MaxPoolArgmax(const ::tflite::PoolParams& params, const ::tflite::RuntimeShape& input_shape,
                                  const float* input_data, const ::tflite::RuntimeShape& output_shape, float* output_data,
                                  float* indices_data) {
            // Start of copy from
            // https://github.com/tensorflow/tensorflow/blob/master/tensorflow/lite/kernels/internal/reference/reference_ops.h
                        ...
                        for (int channel = 0; channel < depth; ++channel) {
                            ...
                            for (int filter_y = filter_y_start; filter_y < filter_y_end; ++filter_y) {
                                for (int filter_x = filter_x_start; filter_x < filter_x_end; ++filter_x) {
                                    ...
                                    float     cur  = input_data[Offset(input_shape, batch, in_y, in_x, channel)];
                                    if (cur > max) {
                                        max   = cur;
                                        max_x = filter_x;
                                        max_y = filter_y;
                            ...
                            output_data[Offset(output_shape, batch, out_y, out_x, channel)] =
                                ::tflite::ActivationFunctionWithMinMax(max, params.float_activation_min,
        ...
        TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
            auto*                params       = reinterpret_cast<const TfLitePoolParams*>(node->custom_initial_data);
            TfLitePaddingValues* data_padding = reinterpret_cast<TfLitePaddingValues*>(node->user_data);
            ...
            auto compute_out_size = [padding](int image_size, int filter_size, int stride) -> int {
                return padding == kTfLitePaddingSame ? (image_size + stride - 1) / stride
                    : padding == kTfLitePaddingValid ? (image_size - filter_size + stride) / stride
                                                     : 0;
            };

            int out_width  = compute_out_size(width, params->filter_width, params->stride_width);
            int out_height = compute_out_size(height, params->filter_height, params->stride_height);

            data_padding->height =
                ::tflite::ComputePadding(params->stride_height, 1, height, params->filter_height, out_height);
            data_padding->width = ::tflite::ComputePadding(params->stride_width, 1, width, params->filter_width, out_width);
        ...
        TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
            ...
            ::tflite::CalculateActivationRange(params->activation, &activation_min, &activation_max);
            ...
            MaxPoolArgmax(op_params, ::tflite::GetTensorShape(input), ::tflite::GetTensorData<float>(input),
    ...
    TfLiteRegistration* RegisterMaxPoolingWithArgmax2D() {
        static TfLiteRegistration reg = {[](TfLiteContext*, const char*, size_t) -> void* {

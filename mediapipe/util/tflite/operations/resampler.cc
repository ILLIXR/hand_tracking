...
namespace tflite_operations {
    namespace {
        ...
        bool ResampleTensors(const float* src, int width, int height, int depth, const float* warp, float* dst) {
            auto get_data_point = [&](const int x, const int y, const int chan) {
                const bool point_is_in_range = (x >= 0 && y >= 0 && x <= width - 1 && y <= height - 1);
            ...
            for (int dst_y = 0; dst_y < height; ++dst_y) {
                for (int dst_x = 0; dst_x < width; ++dst_x) {
                    ...
                    if (x > static_cast<float>(-1.0) && y > static_cast<float>(-1.0) && x < static_cast<float>(width) &&
                        ...
                        for (int chan = 0; chan < depth; ++chan) {
                            ...
                            const float img_cxcy = (1.0f - dx) * (1.0f - dy) * get_data_point(cx, cy, chan);
                            const float img_fxcy = dx * (1.0f - dy) * get_data_point(fx, cy, chan);
                            const float img_cxfy = (1.0f - dx) * dy * get_data_point(cx, fy, chan);
        ...
        TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
            ...
            TfLiteTensor* output = ::tflite::GetOutput(context, node, kOutputTensorDestinationIndex);
            TF_LITE_ENSURE(context, output != nullptr);
            const TfLiteTensor* source = ::tflite::GetInput(context, node, kInputTensorSourceIndex);
            ...
            const TfLiteTensor* warp = ::tflite::GetInput(context, node, kInputTensorWarpIndex);
        ...
        TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
            const TfLiteTensor* src  = ::tflite::GetInput(context, node, kInputTensorSourceIndex);
            const TfLiteTensor* warp = ::tflite::GetInput(context, node, kInputTensorWarpIndex);
            TfLiteTensor*       dst  = ::tflite::GetOutput(context, node, kOutputTensorDestinationIndex);

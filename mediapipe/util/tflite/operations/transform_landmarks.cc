...
namespace tflite_operations {
    namespace {
        ...
        namespace v1 {
            ...
            inline void TransformLandmarks(const tflite::gpu::TransformLandmarksAttributes& params,
                ...
                tflite::RuntimeShape input_shape_with_batch{/*batch=*/1, input0_shape.Dims(1), input0_shape.Dims(2),
                                                            input0_shape.Dims(3)};
                tflite::RuntimeShape output_shape_with_batch{/*batch=*/1, output_shape.Dims(1), output_shape.Dims(2),
                                                             output_shape.Dims(3)};

                // Read first two rows of transformation matrix
                tflite::gpu::float4 x_transform(transform_matrix[0], transform_matrix[1], transform_matrix[2],
                                                transform_matrix[3] * params.scale);
                tflite::gpu::float4 y_transform(transform_matrix[4], transform_matrix[5], transform_matrix[6],
                                                transform_matrix[7] * params.scale);

                for (int out_y = 0; out_y < output_height; ++out_y) {
                    for (int out_x = 0; out_x < output_width; ++out_x) {
                        for (int landmark = 0; landmark < output_channels / params.dimensions; ++landmark) {
                            const int offset = Offset(output_shape_with_batch, 0, out_y, out_x, landmark * params.dimensions);

                            if (params.dimensions == 2) {
                                tflite::gpu::float4 lv(landmarks[offset], landmarks[offset + 1], static_cast<float>(0.0),
                            ...
                            if (params.dimensions == 3) {
                                tflite::gpu::float4 lv(landmarks[offset], landmarks[offset + 1], static_cast<float>(0.0),
                                                       static_cast<float>(1.0));
                                tflite::gpu::float3 transformed(DotProduct(x_transform, lv), DotProduct(y_transform, lv), lv.z);
                                output_data[offset]     = transformed.x;
                                output_data[offset + 1] = transformed.y;
            ...
            TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
                ...
                TransformLandmarks(op_params, tflite::GetTensorShape(input0), tflite::GetTensorData<float>(input0),
                                   tflite::GetTensorShape(input1), tflite::GetTensorData<float>(input1),
        ...
        namespace v2 {
            ...
            inline void TransformLandmarksV2(const tflite::gpu::TransformLandmarksAttributes& params,
                                             const tflite::RuntimeShape& input0_shape, const float* landmarks,
                ...
                tflite::RuntimeShape input_shape_with_batch{/*batch=*/1, input0_shape.Dims(0), input0_shape.Dims(1),
                                                            input0_shape.Dims(2)};
                tflite::RuntimeShape output_shape_with_batch{/*batch=*/1, output_shape.Dims(0), output_shape.Dims(1),
                                                             output_shape.Dims(2)};
            ...
            TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
                ...
                tflite::gpu::BHWC    output_shape(1, runtime_output_shape.Dims(0), runtime_output_shape.Dims(1),
                ...
                TransformLandmarksV2(op_params, tflite::GetTensorShape(input0), tflite::GetTensorData<float>(input0),
